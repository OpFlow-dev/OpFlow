...
TBBMALLOCPROXY_EXPORT void *PREFIX(malloc)(ZONE_ARG size_t size) __THROW { return scalable_malloc(size); }
...
TBBMALLOCPROXY_EXPORT int mallopt(int /*param*/, int /*value*/) __THROW { return 1; }
...
TBBMALLOCPROXY_EXPORT void *operator new(size_t sz) { return InternalOperatorNew(sz); }
TBBMALLOCPROXY_EXPORT void *operator new[](size_t sz) { return InternalOperatorNew(sz); }
...
void __TBB_malloc_safer_delete(void *ptr) { __TBB_malloc_safer_free(ptr, nullptr); }
...
void *safer_expand(void *, size_t) { return nullptr; }
...
#define __TBB_ORIG_ALLOCATOR_REPLACEMENT_CALL_ENTRY_NO_FALLBACK(CRT_VER, function_name, dbgsuffix)           \
    ReplaceFunctionWithStore(#CRT_VER #dbgsuffix ".dll", #function_name,                                     \
                             (FUNCPTR) __TBB_malloc_safer_##function_name##_##CRT_VER##dbgsuffix, 0,         \
                             nullptr);
    ...
    for (size_t j = 0; j < arrayLength(modules_to_replace); j++) {
        ...
        for (size_t i = 0; i < arrayLength(c_routines_to_replace); i++) {
            ReplaceFunctionWithStore(modules_to_replace[j].name, c_routines_to_replace[i]._func,
                                     c_routines_to_replace[i]._fptr, nullptr, nullptr,
        ...
        for (size_t i = 0; i < arrayLength(cxx_routines_to_replace); i++) {
            ...
            ReplaceFunctionWithStore(modules_to_replace[j].name, cxx_routines_to_replace[i]._func,
                                     cxx_routines_to_replace[i]._fptr, nullptr, nullptr,
