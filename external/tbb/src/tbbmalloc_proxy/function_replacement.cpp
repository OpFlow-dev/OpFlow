...
namespace Log {
    ...
    static void record(FunctionInfo functionInfo, const char *opcodeString, bool status) {
        ...
        LOG_PRINT(entry, RECORD_LENGTH, "%s: %s (%s), byte pattern: <%s>", status ? "Success" : "Fail",
                  functionInfo.funcName, functionInfo.dllName, opcodeString);
...
private:
    ...
    MemoryBuffer *CreateBuffer(UINT_PTR addr) {
        ...
        if (m_lastBuffer - m_pages == MAX_NUM_BUFFERS) return 0;

        void *newAddr = Addrint2Ptr(addr);
        // Get information for the region which the given address belongs to
        MEMORY_BASIC_INFORMATION memInfo;
        if (VirtualQuery(newAddr, &memInfo, sizeof(memInfo)) != sizeof(memInfo)) return 0;

        for (;;) {
            // The new address to check is beyond the current region and aligned to allocation size
            newAddr = Addrint2Ptr((Ptr2Addrint(memInfo.BaseAddress) + memInfo.RegionSize + m_allocSize)
                                  & ~(UINT_PTR)(m_allocSize - 1));
            ...
            if (!IsInDistance(addr, Ptr2Addrint(newAddr), MAX_DISTANCE)) break;

            if (VirtualQuery(newAddr, &memInfo, sizeof(memInfo)) != sizeof(memInfo)) break;
...
static UINT CheckOpcodes(const char **opcodes, void *inpAddr, bool abortOnError,
    ...
    if (!opcodesStringsCount || opcodes_pointer != (size_t) opcodes) {
        while (*(opcodes + opcodesStringsCount) != nullptr) {
            if ((i = strlen(*(opcodes + opcodesStringsCount))) > maxOpcodesLength) maxOpcodesLength = i;
            opcodesStringsCount++;
        }
        opcodes_pointer = (size_t) opcodes;
        __TBB_ASSERT(maxOpcodesLength / 2 <= MAX_PATTERN_SIZE,
                     "Pattern exceeded the limit of 28 opcodes/56 symbols");
    }
    ...
    for (UINT idx = 0; idx < opcodesStringsCount; ++idx) {
        ...
        if (result) {
            if (functionInfo) { Log::record(*functionInfo, opcodeString, /*status*/ true); }
            return idx + 1;// avoid 0 which indicates a failure
        }
    }
    if (functionInfo) { Log::record(*functionInfo, opcodeString, /*status*/ false); }
    if (abortOnError) {
...
static DWORD InsertTrampoline32(void *inpAddr, void *targetAddr, const char *pattern, void **storedAddr) {
    ...
    if (!IsInDistance(srcAddr, tgtAddr, MAX_DISTANCE)) return 0;
    ...
    *codePtr = 0xE9;
    ...
    for (unsigned i = SIZE_OF_RELJUMP; i < bytesToMove; i++) { *(codePtr + i) = 0x90; }
...
static DWORD InsertTrampoline64(void *inpAddr, void *targetAddr, const char *pattern, void **storedAddr) {
    ...
    *(codePtr + 1) = 0x25;
    ...
    // Fill the rest with NOPs to correctly see disassembler of old code in debugger.
    for (unsigned i = SIZE_OF_INDJUMP; i < bytesToMove; i++) { *(codePtr + i) = 0x90; }
...
static bool InsertTrampoline(void *inpAddr, void *targetAddr, const char **opcodes, void **origFunc) {
    ...
    DWORD origProt = 0;
    if (!VirtualProtect(inpAddr, MAX_PROBE_SIZE, PAGE_EXECUTE_WRITECOPY, &origProt)) return FALSE;
    ...
    if (!probeSize) probeSize = InsertTrampoline64(inpAddr, targetAddr, pattern, origFunc);

    // Restore original protection
    VirtualProtect(inpAddr, MAX_PROBE_SIZE, origProt, &origProt);

    if (!probeSize) return FALSE;
...
FRR_TYPE ReplaceFunctionA(const char *dllName, const char *funcName, FUNCPTR newFunc, const char **opcodes,
    ...
    if (!dllName || !*dllName) return FRR_NODLL;
...
extern "C" __declspec(dllexport) int TBB_malloc_replacement_log(char ***function_replacement_log_ptr) {
    if (function_replacement_log_ptr != nullptr) { *function_replacement_log_ptr = Log::records; }
