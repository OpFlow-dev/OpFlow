...
namespace tbb {
        ...
        namespace r1 {
            ...
            private:
                void topology_initialization(std::size_t groups_num) {
                    ...
                    if (hwloc_topology_init(&topology) == 0) {
                        ...
                        if (hwloc_topology_set_flags(topology, flags) != 0) { return; }
#endif
                        if (hwloc_topology_load(topology) == 0) { initialization_state = topology_loaded; }
                    }
                    if (initialization_state != topology_loaded) return;
                ...
                void core_types_topology_parsing() {
                    ...
                    __TBB_ASSERT(hwloc_get_api_version() >= 0x20400,
                                 "Hybrid CPUs support interfaces required HWLOC >= 2.4");
                    ...
                    if (!core_types_parsing_broken) {
                        ...
                        for (int core_type = 0; core_type < core_types_number; ++core_type) {
                            ...
                            if (!hwloc_cpukinds_get_info(topology, core_type, current_mask, &efficiency,
                                                         nullptr, nullptr, 0)
                                && efficiency >= 0) {
                                hwloc_bitmap_and(current_mask, current_mask, process_cpu_affinity_mask);
                                ...
                                __TBB_ASSERT(hwloc_bitmap_weight(current_mask) >= 0,
                                             "Infinivitely filled core type mask");
                            } else {
                ...
                void processor_groups_topology_parsing() {
                    ...
                    for (auto proc_group_mask : processor_groups_affinity_masks_list) {
                        __TBB_ASSERT(!hwloc_bitmap_intersects(tmp, proc_group_mask),
                                     "Masks of processor groups intersect.");
                ...
                void enforce_hwloc_2_5_runtime_linkage() {
                    ...
                    hwloc_get_obj_with_same_locality(topology, some_core, HWLOC_OBJ_CORE, nullptr, nullptr,
                                                     0);
#endif
                }

                void initialize(std::size_t groups_num) {
                    if (initialization_state != uninitialized) return;
                    ...
                    if (initialization_state == topology_loaded) initialization_state = topology_parsed;
                }
            ...
            public:
                ...
                ~system_topology() {
                    ...
                    if (initialization_state >= topology_allocated) { hwloc_topology_destroy(topology); }
                ...
                void fill_topology_information(int& _numa_nodes_count, int*& _numa_indexes_list,
                    ...
                    __TBB_ASSERT(is_topology_parsed(),
                ...
                void fill_constraints_affinity_mask(affinity_mask input_mask, int numa_node_index,
                    ...
                    __TBB_ASSERT(is_topology_parsed(),
                                 "Trying to get access to uninitialized system_topology");
                    __TBB_ASSERT(numa_node_index < (int) numa_affinity_masks_list.size(),
                                 "Wrong NUMA node id");
                    __TBB_ASSERT(core_type_index < (int) core_types_affinity_masks_list.size(),
                                 "Wrong core type id");
                    __TBB_ASSERT(max_threads_per_core == -1 || max_threads_per_core > 0,
                                 "Wrong max_threads_per_core");
                    ...
                    if (numa_node_index >= 0) {
                        hwloc_bitmap_and(constraints_mask, constraints_mask,
                                         numa_affinity_masks_list[numa_node_index]);
                    }
                    if (core_type_index >= 0) {
                        hwloc_bitmap_and(constraints_mask, constraints_mask,
                                         core_types_affinity_masks_list[core_type_index]);
                    }
                    if (max_threads_per_core > 0) {
                        ...
                        while ((current_core
                                = hwloc_get_next_obj_by_type(topology, HWLOC_OBJ_CORE, current_core))
                               != nullptr) {
                            ...
                            for (int id = hwloc_bitmap_first(core_mask); id != -1;
                                 id = hwloc_bitmap_next(core_mask, id)) {
                ...
                void fit_num_threads_per_core(affinity_mask result_mask, affinity_mask current_mask,
                                              affinity_mask constraints_mask) {
     ...
     */
                void fit_to_processor_group(affinity_mask result_mask, affinity_mask constraints_mask,
                                            std::size_t slot_num) {
                    __TBB_ASSERT(number_of_processors_groups > 1, nullptr);
                    hwloc_bitmap_zero(result_mask);
                ...
                int get_default_concurrency(int numa_node_index, int core_type_index,
                                            int max_threads_per_core) {
                    __TBB_ASSERT(is_topology_parsed(),
                                 "Trying to get access to uninitialized system_topology");

                    hwloc_cpuset_t constraints_mask = hwloc_bitmap_alloc();
                    fill_constraints_affinity_mask(constraints_mask, numa_node_index, core_type_index,
            ...
            public:
                binding_handler(std::size_t size, int numa_node_id, int core_type_id,
                                int max_threads_per_core)
                    : affinity_backup(size)
#ifdef _WIN32
                      ,
                      affinity_buffer(size), my_numa_node_id(numa_node_id), my_core_type_id(core_type_id),
                ...
                void apply_affinity(unsigned slot_num) {
                    ...
                    if (topology.number_of_processors_groups > 1 && my_max_threads_per_core != -1
                        ...
                        topology.fit_num_threads_per_core(affinity_buffer[slot_num],
                                                          affinity_backup[slot_num], handler_affinity_mask);
                        thread_affinity = affinity_buffer[slot_num];
                    }
#if __TBBBIND_HWLOC_WINDOWS_API_AVAILABLE
                    else if (topology.number_of_processors_groups > 1) {
                        topology.fit_to_processor_group(affinity_buffer[slot_num], handler_affinity_mask,
                                                        slot_num);
                        thread_affinity = affinity_buffer[slot_num];
                    }
#endif
            ...
            TBBBIND_EXPORT void __TBB_internal_initialize_system_topology(std::size_t groups_num,
                ...
                system_topology::instance().fill_topology_information(
                        numa_nodes_count, numa_indexes_list, core_types_count, core_types_indexes_list);
            }

            TBBBIND_EXPORT binding_handler*
            __TBB_internal_allocate_binding_handler(int number_of_slots, int numa_id, int core_type_id,
                                                    int max_threads_per_core) {
            ...
            TBBBIND_EXPORT int __TBB_internal_get_default_concurrency(int numa_id, int core_type_id,
                                                                      int max_threads_per_core) {
                return system_topology::instance().get_default_concurrency(numa_id, core_type_id,
                                                                           max_threads_per_core);
            }
