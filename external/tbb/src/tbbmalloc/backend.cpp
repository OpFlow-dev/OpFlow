...
/* assume MapMemory and UnmapMemory are customized */
        ...
        void *getRawMemory(size_t size, PageType pageType) { return MapMemory(size, pageType); }

        int freeRawMemory(void *object, size_t size) { return UnmapMemory(object, size); }
        ...
        void Backend::UsedAddressRange::registerAlloc(uintptr_t left, uintptr_t right) {
            ...
            MALLOC_ASSERT(leftBound.load(std::memory_order_relaxed)
                          ...
                          ASSERT_TEXT);
            MALLOC_ASSERT(leftBound.load(std::memory_order_relaxed) <= left
                                  && right <= rightBound.load(std::memory_order_relaxed),
                          ASSERT_TEXT);
        }
        ...
        void Backend::UsedAddressRange::registerFree(uintptr_t left, uintptr_t right) {
            ...
            MALLOC_ASSERT((!rightBound.load(std::memory_order_relaxed)
                           && leftBound.load(std::memory_order_relaxed) == ADDRESS_UPPER_BOUND)
        ...
        void *Backend::allocRawMem(size_t &size) {
            ...
            if (extMemPool->userPool()) {
                if (extMemPool->fixedPool
                    && bootsrapMemDone == bootsrapMemStatus.load(std::memory_order_acquire))
                    return nullptr;
                ...
                if (hugePages.isEnabled) {
                    if (hugePages.isHPAvailable) { res = getRawMemory(allocSize, PREALLOCATED_HUGE_PAGE); }
                    if (!res && hugePages.isTHPAvailable) {
                        res = getRawMemory(allocSize, TRANSPARENT_HUGE_PAGE);
                    }
                }

                if (!res) { res = getRawMemory(allocSize, REGULAR); }
            }
        ...
        public:
            ...
            void initLocked() {
                value.store(LOCKED, std::memory_order_release);
            }// TBB_REVAMP_TODO: was relaxed
            void makeCoalscing() {
            ...
            size_t tryLock(State state) {
                ...
                for (;;) {
                    if (sz <= MAX_LOCKED_VAL) { break; }
                    if (value.compare_exchange_strong(sz, state)) { break; }
                }
                return sz;
            }
            void unlock(size_t size) {
                MALLOC_ASSERT(value.load(std::memory_order_relaxed) <= MAX_LOCKED_VAL,
                              "The lock is not locked");
                MALLOC_ASSERT(size > MAX_LOCKED_VAL, ASSERT_TEXT);
                value.store(size, std::memory_order_release);
            }
            bool isLastRegionBlock() const {
                return value.load(std::memory_order_relaxed) == LAST_REGION_BLOCK;
            }
        ...
        public:
            ...
            size_t tryLockBlock() {
                ...
                if (sz <= GuardedSize::MAX_LOCKED_VAL) return false;
        ...
        inline bool BackendSync::waitTillBlockReleased(intptr_t startModifiedCnt) {
            ...
            public:
                ITT_Guard(void *p) : ptr(p) { MALLOC_ITT_SYNC_PREPARE(ptr); }
                ~ITT_Guard() { MALLOC_ITT_SYNC_ACQUIRED(ptr); }
            };
            ...
            while (true) {
                ...
                if (currCoalescQInFlyBlocks > 0 && backend->scanCoalescQ(/*forceCoalescQDrop=*/false)) break;
        ...
        void CoalRequestQ::putBlock(FreeBlock *fBlock) {
            ...
            for (;;) {
                ...
                if (blocksToFree.compare_exchange_strong(myBlToFree, fBlock)) { return; }
            }
        }
        ...
        FreeBlock *Backend::IndexedBins::getFromBin(int binIdx, BackendSync *sync, size_t size,
                                                    bool needAlignedRes, bool alignedBin, bool wait,
                                                    int *binLocked) {
            ...
            if (!b->empty()) {
                ...
                for (FreeBlock *curr = b->head.load(std::memory_order_relaxed); curr; curr = curr->next) {
                    ...
                    if (fBlock) {
                        ...
                        if (freeBins[binIdx].empty()) bitMask.set(binIdx, false);
        ...
        void Backend::Bin::removeBlock(FreeBlock *fBlock) {
            ...
            if (tail == fBlock) tail = fBlock->prev;
            if (fBlock->prev) fBlock->prev->next = fBlock->next;
            if (fBlock->next) fBlock->next->prev = fBlock->prev;
        }
        ...
        void Backend::IndexedBins::addBlock(int binIdx, FreeBlock *fBlock, size_t /* blockSz */,
            ...
            fBlock->next = fBlock->prev = nullptr;
            {
                ...
                if (addToTail) {
                    fBlock->prev = b->tail;
                    b->tail = fBlock;
                    if (fBlock->prev) fBlock->prev->next = fBlock;
                    ...
                    b->head.store(fBlock, std::memory_order_relaxed);
                    if (fBlock->next) fBlock->next->prev = fBlock;
                    if (!b->tail) b->tail = fBlock;
                }
            }
        ...
        void Backend::IndexedBins::reset() {
            for (unsigned i = 0; i < Backend::freeBinsNum; i++) freeBins[i].reset();
            bitMask.reset();
        }
        ...
        bool ExtMemoryPool::regionsAreReleaseable() const { return !keepAllMemory && !delayRegsReleasing; }

        FreeBlock *Backend::splitBlock(FreeBlock *fBlock, int num, size_t size, bool blockIsAligned,
            ...
            if (needAlignedBlock && !blockIsAligned) {
                ...
                bool markAligned = (blockIsAligned ^ needAlignedBlock) ? toAlignedBin(splitBlock, splitSize)
                                                                       : blockIsAligned;
        ...
        size_t Backend::getMaxBinnedSize() const {
            return hugePages.isEnabled && !inUserPool() ? maxBinned_HugePage : maxBinned_SmallPage;
        }
        ...
        FreeBlock *Backend::releaseMemInCaches(intptr_t startModifiedCnt, int *lockedBinsThreshold,
                                               int numOfLockedBins) {
            // something released from caches
            if (extMemPool->hardCachesCleanup(false)) return (FreeBlock *) VALID_BLOCK_IN_BIN;

            if (bkndSync.waitTillBlockReleased(startModifiedCnt)) return (FreeBlock *) VALID_BLOCK_IN_BIN;
        ...
        FreeBlock *Backend::askMemFromOS(size_t blockSize, intptr_t startModifiedCnt,
                                         int *lockedBinsThreshold, int numOfLockedBins, bool *splittableRet,
                                         bool needSlabRegion) {
            FreeBlock *block;
            ...
            if (blockSize >= quiteLarge) {
                ...
                if (!block) return releaseMemInCaches(startModifiedCnt, lockedBinsThreshold, numOfLockedBins);
                *splittableRet = false;
            } else {
                ...
                if (blockSize < quiteSmall) {
                    ...
                    if (block)
                        for (unsigned idx = 0; idx < NUM_OF_REG; idx++)
                            if (!addNewRegion(regSz_sizeBased, regType, /*addToBin=*/true)) break;
                } else {
        ...
        void Backend::releaseCachesToLimit() {
            ...
            for (int cleanLocal = 0; cleanLocal < 2; cleanLocal++)
                while (cleanLocal ? extMemPool->allLocalCaches.cleanup(/*cleanOnlyUnused=*/true)
        ...
        FreeBlock *Backend::IndexedBins::findBlock(int nativeBin, BackendSync *sync, size_t size,
                                                   bool needAlignedBlock, bool alignedBin,
                                                   int *numOfLockedBins) {
            for (int i = getMinNonemptyBin(nativeBin); i < (int) freeBinsNum; i = getMinNonemptyBin(i + 1))
                if (FreeBlock *block = getFromBin(i, sync, size, needAlignedBlock, alignedBin, /*wait=*/false,
                                                  numOfLockedBins))
                    return block;
        ...
        void Backend::requestBootstrapMem() {
            if (bootsrapMemDone == bootsrapMemStatus.load(std::memory_order_acquire)) return;
            MallocMutex::scoped_lock lock(bootsrapMemStatusMutex);
            if (bootsrapMemDone == bootsrapMemStatus) return;
        ...
        FreeBlock *Backend::genericGetBlock(int num, size_t size, bool needAlignedBlock) {
            ...
            for (;;) {
                ...
                do {
                    ...
                    numOfLockedBins = 0;
                    if (needAlignedBlock) {
                        block = freeSlabAlignedBins.findBlock(nativeBin, &bkndSync, num * size,
                                                              needAlignedBlock,
                                                              /*alignedBin=*/true, &numOfLockedBins);
                        if (!block && extMemPool->fixedPool)
                            block = freeLargeBlockBins.findBlock(nativeBin, &bkndSync, num * size,
                                                                 needAlignedBlock,
                        ...
                        if (!block && extMemPool->fixedPool)
                         ...
                         && (numOfLockedBins > lockedBinsThreshold || cleanCnt % 2 == 1
                             || cleanCnt != backendCleanCnt.load(std::memory_order_acquire)));
        ...
        LargeMemoryBlock *Backend::getLargeBlock(size_t size) {
            LargeMemoryBlock *lmb = (LargeMemoryBlock *) genericGetBlock(1, size, /*needAlignedRes=*/false);
            if (lmb) {
                lmb->unalignedSize = size;
                if (extMemPool->userPool()) extMemPool->lmbList.add(lmb);
            }
            return lmb;
        }
        ...
        void Backend::putBackRefSpace(void *b, size_t size, bool rawMemUsed) {
            if (rawMemUsed) freeRawMemory(b, size);
        ...
        void Backend::removeBlockFromBin(FreeBlock *fBlock) {
            if (fBlock->myBin != Backend::NO_BIN) {
                if (fBlock->slabAligned) freeSlabAlignedBins.lockRemoveBlock(fBlock->myBin, fBlock);
                else
        ...
        void AllLargeBlocksList::add(LargeMemoryBlock *lmb) {
            ...
            lmb->gPrev = nullptr;
            lmb->gNext = loHead;
            if (lmb->gNext) lmb->gNext->gPrev = lmb;
            loHead = lmb;
        }

        void AllLargeBlocksList::remove(LargeMemoryBlock *lmb) {
            MallocMutex::scoped_lock scoped_cs(largeObjLock);
            if (loHead == lmb) loHead = lmb->gNext;
            if (lmb->gNext) lmb->gNext->gPrev = lmb->gPrev;
            if (lmb->gPrev) lmb->gPrev->gNext = lmb->gNext;
        }

        void Backend::putLargeBlock(LargeMemoryBlock *lmb) {
            if (extMemPool->userPool()) extMemPool->lmbList.remove(lmb);
        ...
        void *Backend::remap(void *ptr, size_t oldSize, size_t newSize, size_t alignment) {
            ...
            if (inUserPool()
                || min(oldSize, newSize) < maxBinned_SmallPage
            ...
            if (!right->isLastRegionBlock()) return nullptr;
            ...
            if (oldRegion->type != MEMREG_ONE_BLOCK) return nullptr;// we are not single in the region
            ...
            const size_t requestSize = alignUp(sizeof(MemRegion) + alignedSize + sizeof(LastFreeBlock),
                                               extMemPool->granularity);
            ...
            FreeBlock *fBlock
                    = (FreeBlock *) alignUp((uintptr_t) region + sizeof(MemRegion), largeObjectAlignment);
            ...
            MALLOC_ASSERT((uintptr_t) lmb + lmb->unalignedSize >= (uintptr_t) object + lmb->objectSize,
                          "An object must fit to the block.");
        ...
        FreeBlock *Backend::doCoalesc(FreeBlock *fBlock, MemRegion **mRegion) {
            ...
            if (rightSz != GuardedSize::LOCKED) {
                ...
                if (GuardedSize::LAST_REGION_BLOCK == rightSz) {
                    ...
                    size_t rSz = right->rightNeig(rightSz)->trySetLeftUsed(GuardedSize::COAL_BLOCK);
                    if (rSz <= GuardedSize::MAX_LOCKED_VAL) {
                        ...
                        size_t nextRightSz = nextRight->trySetMeUsed(GuardedSize::COAL_BLOCK);
            ...
            if (memRegion) {
                MALLOC_ASSERT((uintptr_t) memRegion + memRegion->allocSz
                                      >= (uintptr_t) right + sizeof(LastFreeBlock),
                              ASSERT_TEXT);
        ...
        bool Backend::coalescAndPutList(FreeBlock *list, bool forceCoalescQDrop, bool reportBlocksProcessed) {
            ...
            for (FreeBlock *helper; list; list = helper,
                ...
                if (memRegion && memRegion->blockSz == toRet->sizeTmp && !extMemPool->fixedPool) {
                    if (extMemPool->regionsAreReleaseable()) {
                        // release the region, because there is no used blocks in it
                        if (toRet->blockInBin) removeBlockFromBin(toRet);
                ...
                if (toRet->blockInBin) {
                    ...
                    if (toRet->myBin == bin && toRet->slabAligned == toAligned) needAddToBin = false;
                    else {
        ...
        FreeBlock *Backend::findBlockInRegion(MemRegion *region, size_t exactBlockSize) {
            FreeBlock *fBlock;
            size_t blockSz;
            uintptr_t fBlockEnd, lastFreeBlock = (uintptr_t) region + region->allocSz - sizeof(LastFreeBlock);
            ...
            if (fBlockEnd <= (uintptr_t) fBlock) return nullptr;// allocSz is too small
            ...
            if (blockSz < numOfSlabAllocOnMiss * slabSize) return nullptr;
        ...
        FreeBlock *Backend::addNewRegion(size_t size, MemRegionType memRegType, bool addToBin) {
            ...
            const size_t requestSize = memRegType == MEMREG_SLAB_BLOCKS
                                               ? size
            ...
            if (!region) {
                MALLOC_ASSERT(rawSize == requestSize,
                              "getRawMem has not allocated memory but changed the allocated size.");
                return nullptr;
            }
            if (rawSize < sizeof(MemRegion)) {
                if (!extMemPool->fixedPool) freeRawMem(region, rawSize);
                return nullptr;
            }
        ...
        size_t Backend::Bin::countFreeBlocks() {
            size_t cnt = 0;
            {
                ...
                for (FreeBlock *fb = head; fb; fb = fb->next) cnt++;
            }
            return cnt;
        }
