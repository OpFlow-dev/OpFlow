 ...
 */
        struct BackRefBlock : public BlockI {
            ...
            BackRefBlock(const BackRefBlock *blockToUse, intptr_t num)
                : nextForUse(NULL),
                  bumpPtr((FreeObject *) ((uintptr_t) blockToUse + slabSize - sizeof(void *))),
                  freeList(NULL), nextRawMemBlock(NULL), allocatedCount(0), myNum(num), addedToForUse(false) {
 ...
 */
   ...
   taking into account VirtualAlloc allocation granularity */
        ...
        bool initBackRefMain(Backend *backend) {
            bool rawMemUsed;
            BackRefMain *main = (BackRefMain *) backend->getBackRefSpace(BackRefMain::mainSize, &rawMemUsed);
            ...
            for (int i = 0; i < BackRefMain::leaves; i++) {
                BackRefBlock *bl
                        = (BackRefBlock *) ((uintptr_t) main + BackRefMain::bytes + i * BackRefBlock::bytes);
                bl->zeroSet();
                main->initEmptyBackRefBlock(bl);
                if (i) main->addToForUseList(bl);
        ...
        void destroyBackRefMain(Backend *backend) {
            if (backRefMain.load(std::memory_order_acquire)) {// Is initBackRefMain() called?
                for (BackRefBlock *curr = backRefMain.load(std::memory_order_relaxed)->allRawMemBlocks;
                     curr;) {
        ...
        bool BackRefMain::requestNewSpace() {
            ...
            for (BackRefBlock *bl = newBl; blocksToUse > 0;
                ...
                if (active.load(std::memory_order_relaxed)->allocatedCount.load(std::memory_order_relaxed)
                    == BR_MAX_CNT) {
        ...
        void *getBackRef(BackRefIdx backRefIdx) {
            ...
            if (!(backRefMain.load(std::memory_order_acquire))
                || backRefIdx.getMain() > (backRefMain.load(std::memory_order_relaxed)
                                                   ->lastUsed.load(std::memory_order_acquire))
                || backRefIdx.getOffset() >= BR_MAX_CNT) {
                return NULL;
            }
            std::atomic<void *> &backRefEntry
                    = *(std::atomic<void *> *) ((uintptr_t) backRefMain.load(std::memory_order_relaxed)
                                                        ->backRefBl[backRefIdx.getMain()]
                                                + sizeof(BackRefBlock)
        ...
        void setBackRef(BackRefIdx backRefIdx, void *newPtr) {
            MALLOC_ASSERT(backRefIdx.getMain() <= backRefMain.load(std::memory_order_relaxed)
            ...
            ((std::atomic<void *> *) ((uintptr_t) backRefMain.load(std::memory_order_relaxed)
                    ...
                    ->store(newPtr, std::memory_order_relaxed);
        }
        ...
        BackRefIdx BackRefIdx::newBackRef(bool largeObj) {
            ...
            do {
                ...
                if (!blockToUse) return BackRefIdx();
                toUse = NULL;
                {// the block is locked to find a reference
                    ...
                    if (toUse) {
                        ...
                        blockToUse->allocatedCount.store(
                                blockToUse->allocatedCount.load(std::memory_order_relaxed) + 1,
            ...
            if (lastBlockFirstUsed) backRefMain.load(std::memory_order_relaxed)->requestNewSpace();
        ...
        void removeBackRef(BackRefIdx backRefIdx) {
            ...
            MALLOC_ASSERT(backRefIdx.getMain() <= backRefMain.load(std::memory_order_relaxed)
                                  ...
                                  && backRefIdx.getOffset() < BR_MAX_CNT,
                          ASSERT_TEXT);
            BackRefBlock *currBlock
                    = backRefMain.load(std::memory_order_relaxed)->backRefBl[backRefIdx.getMain()];
            std::atomic<void *> &backRefEntry
                    = *(std::atomic<void *> *) ((uintptr_t) currBlock + sizeof(BackRefBlock)
                                                + backRefIdx.getOffset() * sizeof(std::atomic<void *>));
            MALLOC_ASSERT(((uintptr_t) &backRefEntry > (uintptr_t) currBlock
                           && (uintptr_t) &backRefEntry < (uintptr_t) currBlock + slabSize),
                          ASSERT_TEXT);
            {
                ...
                MALLOC_ASSERT(!backRefEntryValue
                                      || (backRefEntryValue > (uintptr_t) currBlock
                                          && backRefEntryValue < (uintptr_t) currBlock + slabSize),
                              ASSERT_TEXT);
#endif
                currBlock->freeList = (FreeObject *) &backRefEntry;
                currBlock->allocatedCount.store(currBlock->allocatedCount.load(std::memory_order_relaxed) - 1,
                                                std::memory_order_relaxed);
            }
