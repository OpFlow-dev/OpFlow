...
namespace rml {
    ...
    namespace internal {
        ...
        inline bool isSolidPtr(void *ptr) { return (UNUSABLE | (intptr_t) ptr) != UNUSABLE; }
        inline bool isNotForUse(void *ptr) { return (intptr_t) ptr == UNUSABLE; }
 ...
 */
...
#if USE_WINTHREAD
                Tid_key = TlsAlloc();
                if (Tid_key == TLS_ALLOC_FAILURE) return false;
#else
...
#endif /* USE_WINTHREAD */
                    if (status) fprintf(stderr, "The memory manager cannot delete tls key\n");
                    Tid_key = 0;
                }
            }
#endif
...
#if COLLECT_STATISTICS
        int STAT_increment(ThreadId tid, int bin, int ctr) { return ::STAT_increment(tid.id, bin, ctr); }
#endif
...
#if USE_PTHREAD
            bool isCurrentThreadId() const {
                return pthread_equal(pthread_self(), tid.load(std::memory_order_relaxed));
            }
#else
            bool isCurrentThreadId() const {
                return GetCurrentThreadId() == tid.load(std::memory_order_relaxed);
            }
#endif
        ...
        bool TLSKey::init() {
            ...
            if (TLS_pointer_key == TLS_ALLOC_FAILURE) return false;
#else
 ...
 */
        ...
        class LocalBlockFields : public GlobalBlockFields,
                                 Padding<blockHeaderAlignment - sizeof(GlobalBlockFields)> {
        protected:
            Block *next;
       ...
       NULL for orphaned blocks. */
        ...
        class Block : public LocalBlockFields, Padding<2 * blockHeaderAlignment - sizeof(LocalBlockFields)> {
        public:
            bool empty() const {
 ...
 */
        ...
        public:
            ...
            bool externalCleanup(bool cleanOnlyUnused, bool cleanBins) {
                ...
                return released | lloc.externalCleanup(&memPool->extMemPool)
                       | freeSlabBlocks.externalCleanup();
            }
            bool cleanupBlockBins();
            void markUsed() {
                unused.store(false, std::memory_order_relaxed);
            }// called by owner when TLS touched
            void markUnused() {
                unused.store(true, std::memory_order_relaxed);
            }// can be called by not owner thread
        };
...
#if MALLOC_DEBUG
            for (uint32_t i = 0; i < numBlockBinLimit; i++) tls->bin[i].verifyInitState();
#endif
        ...
        void AllLocalCaches::unregisterThread(TLSRemote *tls) {
            ...
            if (head == tls) head = tls->next;
            if (tls->next) tls->next->prev = tls->prev;
            if (tls->prev) tls->prev->next = tls->next;
        ...
        bool AllLocalCaches::cleanup(bool cleanOnlyUnused) {
            bool released = false;
            {
                ...
                for (TLSRemote *curr = head; curr; curr = curr->next)
                    released |= static_cast<TLSData *>(curr)->externalCleanup(cleanOnlyUnused,
                                                                              /*cleanBins=*/false);
            }
            return released;
        }
        ...
        void AllLocalCaches::markUnused() {
            ...
            for (TLSRemote *curr = head; curr; curr = curr->next) static_cast<TLSData *>(curr)->markUnused();
        }
...
#pragma intrinsic(_BitScanReverse)
        ...
        static inline unsigned int highestBitPos(unsigned int n) {
            MALLOC_ASSERT(n >= 64 && n < 1024,
                          ASSERT_TEXT);// only needed for bsr array lookup, but always true
            unsigned int pos;
        ...
        static unsigned int getIndexOrObjectSize(unsigned int size) {
            if (size <= maxSmallObjectSize) {// selection from 8/16/24/32/40/48/56/64
                ...
                return indexRequest ? index : (index + 1) << 3;
                    ...
                    size
                    <= maxSegregatedObjectSize) {// 80/96/112/128 / 160/192/224/256 / 320/384/448/512 / 640/768/896/1024
                ...
                if (indexRequest)
                    ...
                    MALLOC_ASSERT(alignment == 16 || alignment == 32 || alignment == 64 || alignment == 128,
                                  ASSERT_TEXT);
                ...
                if (size <= fittingSize3) {
                    if (size <= fittingSize2) {
                        if (size <= fittingSize1) return indexRequest ? minFittingIndex : fittingSize1;
                        else
                    ...
                    if (size <= fittingSize5) {
                        if (size <= fittingSize4) return indexRequest ? minFittingIndex + 3 : fittingSize4;
                        else
        ...
        void *BootStrapBlocks::allocate(MemoryPool *memPool, size_t size) {
            ...
            {// Lock with acquire
                ...
                if (bootStrapObjectList) {
                    ...
                    bootStrapBlock->bumpPtr = (FreeObject *) ((uintptr_t) bootStrapBlock->bumpPtr
                                                              - bootStrapBlock->objectSize);
 ...
 * By usage convention there is no race on the initialization. */
        ...
        template <bool poolDestroy>
        void AllLargeBlocksList::releaseAll(Backend *backend) {
        ...
        TLSData *MemoryPool::getTLS(bool create) {
            ...
            if (create && !tls) tls = extMemPool.tlsPointerKey.createTLS(this, &extMemPool.backend);
            return tls;
        }
 ...
 */
        inline Bin *TLSData::getAllocationBin(size_t size) { return bin + getIndex(size); }
        ...
        Block *MemoryPool::getEmptyBlock(size_t size) {
            ...
            FreeBlockPool::ResOfGet resOfGet
                    = tls ? tls->freeSlabBlocks.getBlock() : FreeBlockPool::ResOfGet(NULL, false);
                ...
                if (!extMemPool.userPool())
                    for (int i = 0; i < num; i++) {
                        ...
                        if (backRefIdx[i].isInvalid()) {
                            ...
                            for (int j = 0; j < i; j++) removeBackRef(backRefIdx[j]);
                            Block *b = result;
        ...
        void MemoryPool::returnEmptyBlock(Block *block, bool poolTheBlock) {
            ...
            if (poolTheBlock) {
                ...
                if (!extMemPool.userPool()) removeBackRef(*(block->getBackRefIdx()));
                extMemPool.backend.putSlabBlock(block);
            }
        }

        bool ExtMemoryPool::init(intptr_t poolId, rawAllocType rawAlloc, rawFreeType rawFree,
        ...
        bool MemoryPool::reset() {
            ...
            if (!extMemPool.reset()) return false;

            if (!extMemPool.initTLS()) return false;
        ...
        void Bin::verifyTLSBin(size_t size) const {
            ...
            if (activeBlk) {
                ...
                for (Block *temp = activeBlk->next; temp; temp = temp->next) {
                    ...
                    if (temp->next) { MALLOC_ASSERT(temp->next->previous == temp, ASSERT_TEXT); }
                }
                for (Block *temp = activeBlk->previous; temp; temp = temp->previous) {
                    ...
                    if (temp->previous) { MALLOC_ASSERT(temp->previous->next == temp, ASSERT_TEXT); }
                }
 ...
 */
        void Bin::outofTLSBin(Block *block) {
            ...
            if (block == activeBlk) { activeBlk = block->previous ? block->previous : block->next; }
            /* Unlink the block */
            if (block->previous) {
        ...
        Block *Bin::getPrivatizedFreeListBlock() {
            ...
            if (!mailbox.load(std::memory_order_acquire))// hotpath is empty mailbox
                ...
                if (block) {
                    MALLOC_ASSERT(block->isOwnedByCurrentThread(), ASSERT_TEXT);
                    MALLOC_ASSERT(!isNotForUse(block->nextPrivatizable.load(std::memory_order_relaxed)),
                                  ASSERT_TEXT);
        ...
        bool Bin::cleanPublicFreeLists() {
            Block *block;
            if (!mailbox.load(std::memory_order_acquire)) return false;
            else {
...
#if COLLECT_STATISTICS
                if (isFull) STAT_increment(getThreadId(), getIndex(objectSize), examineNotEmpty);
                else
                    STAT_increment(getThreadId(), getIndex(objectSize), examineEmptyEnough);
#endif
            }
        ...
        void Block::adjustPositionInBin(Bin *bin /*=NULL*/) {
            ...
            if (isFull && !adjustFullness()) {
                if (!bin) bin = tlsPtr.load(std::memory_order_relaxed)->getAllocationBin(objectSize);
        ...
        void Block::freeOwnObject(void *object) {
            ...
            if (tlsPtr.load(std::memory_order_relaxed)->getAllocationBin(objectSize)->getActiveBlock()
                != this)
            ...
            if (empty()) {
                ...
                tlsPtr.load(std::memory_order_relaxed)
                        ->getAllocationBin(objectSize)
                        ->processEmptyBlock(this, /*poolTheBlock=*/true);
            } else {// hot path
        ...
        bool Block::readyToShare() {
            ...
            {
                MallocMutex::scoped_lock scoped_cs(publicFreeListLock);
                if ((oldVal = publicFreeList) == NULL) (intptr_t &) (publicFreeList) = UNUSABLE;
            }
#endif
            return oldVal == NULL;
        }
        ...
        void Block::initEmptyBlock(TLSData *tls, size_t size) {
            ...
            TRACEF(("[ScalableMalloc trace] Empty block %p is initialized, owner is %ld, objectSize is %d, "
                    "bumpPtr is %p\n",
        ...
        void OrphanedBlocks::reset() {
            for (uint32_t i = 0; i < numBlockBinLimit; i++) new (bins + i) LifoList();
        }
            ...
            for (uint32_t i = 0; i < numBlockBinLimit; i++) {
                ...
                while (block) {
                    ...
                    if (block->empty()) {
                        block->reset();
                        // slab blocks in user's pools do not have valid backRefIdx
                        if (!backend->inUserPool()) removeBackRef(*(block->getBackRefIdx()));
        ...
        void FreeBlockPool::returnBlock(Block *block) {
            ...
            if (!localHead) {
                ...
                for (int i = 0; i < POOL_LOW_MARK - 2; i++) headToFree = headToFree->next;
                ...
                for (Block *currBl = headToFree; currBl; currBl = helper) {
                    helper = currBl->next;
                    // slab blocks in user's pools do not have valid backRefIdx
                    if (!backend->inUserPool()) removeBackRef(currBl->backRefIdx);
        ...
        void Block::reset() {
            ...
            MALLOC_ASSERT(allocatedCount == 0, ASSERT_TEXT);
            MALLOC_ASSERT(!isSolidPtr(publicFreeList.load(std::memory_order_relaxed)), ASSERT_TEXT);
            if (!isStartupAllocObject()) STAT_increment(getThreadId(), getIndex(objectSize), freeBlockBack);
        ...
        FreeObject *Block::findObjectToFree(const void *object) const {
            ...
            if (objectSize <= maxSegregatedObjectSize) objectToFree = (FreeObject *) object;
 ...
 */
        ...
        class StartupBlock : public Block {
            size_t availableSize() const { return slabSize - ((uintptr_t) bumpPtr - (uintptr_t) this); }
        ...
        StartupBlock *StartupBlock::getBlock() {
            ...
            StartupBlock *block
                    = static_cast<StartupBlock *>(defaultMemPool->extMemPool.backend.getSlabBlock(1));
       ...
       and objects bigger than word on word's bound. */
            ...
            size_t reqSize = size + sizeof(size_t);
            {
                ...
                if (!firstStartupBlock || firstStartupBlock->availableSize() < reqSize) {
                    ...
                    if (firstStartupBlock) firstStartupBlock->previous = (Block *) newBlock;
        ...
        void StartupBlock::free(void *ptr) {
            Block *blockToRelease = NULL;
            {
                ...
                MALLOC_ASSERT(startupAllocObjSizeMark == objectSize && allocatedCount > 0, ASSERT_TEXT);
                MALLOC_ASSERT((uintptr_t) ptr >= (uintptr_t) this + sizeof(StartupBlock)
                                      && (uintptr_t) ptr + StartupBlock::msize(ptr)
                                                 <= (uintptr_t) this + slabSize,
                              ASSERT_TEXT);
                if (0 == --allocatedCount) {
                    ...
                    MALLOC_ASSERT((uintptr_t) newBump > (uintptr_t) this + sizeof(StartupBlock), ASSERT_TEXT);
                    bumpPtr = newBump;
                }
            }
            if (blockToRelease) {
 ...
 */
   ...
   simultaneously, so flag never becomes negative because of that. */
            ...
            void threadDtorDone() { flag.fetch_sub(1); }
            void processExit() {
                if (flag.fetch_add(skipDtor) != 0) { SpinWaitUntilEq(flag, skipDtor); }
            }
        };
        ...
        extern "C" void MallocInitializeITT() {
            ...
            if (!usedBySrcIncluded) tbb::detail::r1::__TBB_load_ittnotify();
#endif
        }

        void MemoryPool::initDefaultPool() { hugePages.init(); }
 ...
 */
        static bool initMemoryManager() {
            ...
            bool initOk = defaultMemPool->extMemPool.init(0, NULL, NULL, granularity,
    ...
    There is no need to call this routine if mallocInitialized==2 . */
        ...
        FreeObject *Block::allocateFromBumpPtr() {
            ...
            if (result) {
                ...
                if ((uintptr_t) bumpPtr < (uintptr_t) this + sizeof(Block)) { bumpPtr = NULL; }
                MALLOC_ASSERT(allocatedCount < (slabSize - sizeof(Block)) / objectSize, ASSERT_TEXT);
                allocatedCount++;
        ...
        inline FreeObject *Block::allocate() {
            ...
            if (FreeObject *result = allocateFromFreeList()) { return result; }
            MALLOC_ASSERT(!freeList, ASSERT_TEXT);

            /* if free list is empty, try thread local bump pointer allocation. */
            if (FreeObject *result = allocateFromBumpPtr()) { return result; }
        ...
        size_t Block::findObjectSize(void *object) const {
            ...
            if (!blSize) return StartupBlock::msize(object);
#endif
            // object can be aligned, so real size can be less than block's
            size_t size = blSize - ((uintptr_t) object - (uintptr_t) findObjectToFree(object));
        ...
        bool LocalLOCImpl<LOW_MARK, HIGH_MARK>::put(LargeMemoryBlock *object, ExtMemoryPool *extMemPool) {
            ...
            if (size > MAX_TOTAL_SIZE) return false;
            LargeMemoryBlock *localHead = head.exchange(NULL);

            object->prev = NULL;
            object->next = localHead;
            if (localHead) localHead->prev = object;
            else {
        ...
        LargeMemoryBlock *LocalLOCImpl<LOW_MARK, HIGH_MARK>::get(size_t size) {
            ...
            if (size > MAX_TOTAL_SIZE) return NULL;
            ...
            for (LargeMemoryBlock *curr = localHead; curr; curr = curr->next) {
                if (curr->unalignedSize == size) {
                    ...
                    if (curr->next) curr->next->prev = curr->prev;
                        ...
                        tail = curr->prev;
                    if (curr != localHead) curr->prev->next = curr->next;
                    else
                        localHead = curr->next;
                    totalSize -= size;
                    numOfBlocks--;
        ...
        void *MemoryPool::getFromLLOCache(TLSData *tls, size_t size, size_t alignment) {
            ...
            if (!lmb) lmb = extMemPool.mallocLargeObject(this, allocationSize);
            ...
            if (lmb) {
                ...
                uintptr_t alignedRight = alignDown((uintptr_t) lmb + lmb->unalignedSize - size, alignment);
                ...
                MALLOC_ASSERT((uintptr_t) lmb + lmb->unalignedSize >= (uintptr_t) alignedArea + size,
                              "Object doesn't fit the block.");
        ...
        void MemoryPool::putToLLOCache(TLSData *tls, void *object) {
            ...
            if (tls) {
                tls->markUsed();
                if (tls->lloc.put(header->memoryBlock, &extMemPool)) return;
            }
 ...
 */
        static void *allocateAligned(MemoryPool *memPool, size_t size, size_t alignment) {
            ...
            if (!isMallocInitialized())
                if (!doInitialization()) return NULL;
            ...
            if (size <= maxSegregatedObjectSize && alignment <= maxSegregatedObjectSize)
                ...
                if (alignment <= fittingAlignment) result = internalPoolMalloc(memPool, size);
            ...
            LargeObjAlloc:
                ...
                result = memPool->getFromLLOCache(
                        tls, size, largeObjectAlignment > alignment ? largeObjectAlignment : alignment);
            }
        ...
        static void *reallocAligned(MemoryPool *memPool, void *ptr, size_t newSize, size_t alignment = 0) {
            void *result;
            size_t copySize;
            ...
            if (isLargeObject<ourMem>(ptr)) {
                ...
                if (void *r = memPool->extMemPool.remap(
                            ptr, copySize, newSize,
 ...
 */
        ...
        bool isLargeObject(void *object) {
            if (!isAligned(object, largeObjectAlignment)) return false;
            LargeObjectHdr *header = (LargeObjectHdr *) object - 1;
            BackRefIdx idx = (memOrigin == unknownMem) ? safer_dereference(&header->backRefIdx)
                                                       : dereference(&header->backRefIdx);
            ...
            return idx.isLargeObject()
                   ...
                   && (uintptr_t) header->memoryBlock < (uintptr_t) header && getBackRef(idx) == header;
        }
        ...
        static inline bool isSmallObject(void *ptr) {
            ...
            if (isSmall) expectedBlock->checkFreePrecond(ptr);
            return isSmall;
        }
        ...
        static void *internalPoolMalloc(MemoryPool *memPool, size_t size) {
            ...
            if (size >= minLargeObjectSize) return memPool->getFromLLOCache(tls, size, largeObjectAlignment);
     ...
     */
            ...
            {
                if (FreeObject *result = mallocBlock->allocate()) return result;
            }
     ...
     */
            ...
            if (mallocBlock) {
                ...
                if (FreeObject *result = mallocBlock->allocateFromFreeList()) return result;
                /* Else something strange happened, need to retry from the beginning; */
                TRACEF(("[ScalableMalloc trace] Something is wrong: no objects in public free list; "
                        "reentering.\n"));
     ...
     */
            ...
            if (mallocBlock) {
                ...
                if (FreeObject *result = mallocBlock->allocate()) return result;
                /* Else something strange happened, need to retry from the beginning; */
                TRACEF(("[ScalableMalloc trace] Something is wrong: no objects in empty block; "
                        "reentering.\n"));
                return internalPoolMalloc(memPool, size);
            }
            /*
     ...
     */
        ...
        static void internalFree(void *object) { internalPoolFree(defaultMemPool, object, 0); }
    ...
    rml::MemPoolError pool_create_v1(intptr_t pool_id, const MemPoolPolicy *policy, rml::MemoryPool **pool) {
        if (!policy->pAlloc
            || policy->version < MemPoolPolicy::TBBMALLOC_POOL_VERSION
    ...
    void *pool_realloc(rml::MemoryPool *mPool, void *object, size_t size) {
        if (!object) return internalPoolMalloc((rml::internal::MemoryPool *) mPool, size);
        if (!size) {
    ...
    void *pool_aligned_malloc(rml::MemoryPool *mPool, size_t size, size_t alignment) {
        if (!isPowerOfTwo(alignment) || 0 == size) return NULL;
    ...
    void *pool_aligned_realloc(rml::MemoryPool *memPool, void *ptr, size_t size, size_t alignment) {
        ...
        if (!ptr) tmp = allocateAligned(mPool, size, alignment);
        else if (!size) {
...
extern "C" void __TBB_mallocProcessShutdownNotification(bool windows_process_dying) {
    ...
    if (!windows_process_dying) { doThreadShutdownNotification(NULL, /*main_thread=*/true); }
#if __TBB_MALLOC_LOCACHE_STAT
...
*/
    ...
    for (int i = 1; i <= nThreads && i < MAX_THREADS; ++i) STAT_print(i);
#endif
    if (!usedBySrcIncluded) MALLOC_ITT_FINI_ITTLIB();
}
...
extern "C" void scalable_free(void *object) { internalFree(object); }
...
extern "C" TBBMALLOC_EXPORT void __TBB_malloc_safer_free(void *object, void (*original_free)(void *)) {
    ...
    if (mallocInitialized.load(std::memory_order_acquire)
        && defaultMemPool->extMemPool.backend.ptrCanBeValid(object)) {
    ...
    if (original_free) original_free(object);
}
...
extern "C" void *scalable_realloc(void *ptr, size_t size) {
    ...
    if (!ptr) tmp = internalMalloc(size);
    else if (!size) {
        internalFree(ptr);
        return NULL;
    } else
    ...
    if (!ptr) {
        ...
        if (original_ptrs->msize) {
            ...
            if (tmp) {
                ...
                if (original_ptrs->free) { original_ptrs->free(ptr); }
            }
        } else
            tmp = NULL;
    }
...
extern "C" void *scalable_calloc(size_t nobj, size_t size) {
    ...
    if (result) memset(result, 0, arraySize);
    else
        errno = ENOMEM;
    return result;
}
...
extern "C" int scalable_posix_memalign(void **memptr, size_t alignment, size_t size) {
    if (!isPowerOfTwoAtLeast(alignment, sizeof(void *))) return EINVAL;
...
extern "C" void *scalable_aligned_realloc(void *ptr, size_t size, size_t alignment) {
    ...
    if (!ptr) tmp = allocateAligned(defaultMemPool, size, alignment);
    else if (!size) {
        scalable_free(ptr);
        return NULL;
    } else
...
extern "C" TBBMALLOC_EXPORT void *__TBB_malloc_safer_aligned_realloc(void *ptr, size_t size, size_t alignment,
                                                                     void *orig_function) {
        ...
        if (size) {
            ...
            if (original_ptrs->aligned_msize) {
                ...
                if (tmp) {
                    ...
                    if (original_ptrs->aligned_free) { original_ptrs->aligned_free(ptr); }
                }
            }
        } else {
            if (original_ptrs->aligned_free) { original_ptrs->aligned_free(ptr); }
...
extern "C" void scalable_aligned_free(void *ptr) { internalFree(ptr); }
...
extern "C" TBBMALLOC_EXPORT size_t __TBB_malloc_safer_aligned_msize(
        void *object, size_t alignment, size_t offset, size_t (*orig_aligned_msize)(void *, size_t, size_t)) {
    if (object) {
        // Check if the memory was allocated by scalable_malloc
        if (mallocInitialized.load(std::memory_order_acquire) && isRecognized(object))
            return internalMsize(object);
...
extern "C" int scalable_allocation_command(int cmd, void *param) {
    if (param) return TBBMALLOC_INVALID_PARAM;
