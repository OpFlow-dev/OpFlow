       ...
       Moreover it cancels the OP_CLEAN_TO_THRESHOLD operation. */
           ...
           lastGet - the same meaning as CacheBin::lastGet */
            ...
            public:
                OperationPreprocessor(typename LargeObjectCacheImpl<Props>::CacheBin *bin)
                    : bin(bin), lclTime(0), opGet(nullptr), opClean(nullptr), cleanTime(0), lastGetOpTime(0),
                      lastGet(0), updateUsedSize(0), head(nullptr), tail(nullptr), putListNum(0),
                      isCleanAll(false) {}
        ...
        public:
            CacheBinFunctor(typename LargeObjectCacheImpl<Props>::CacheBin *bin, ExtMemoryPool *extMemPool,
                ...
                : bin(bin), extMemPool(extMemPool), bitMask(bitMask), idx(idx), toRelease(nullptr),
                  needCleanup(false), currTime(0) {}
            void operator()(CacheBinOperation *opList);
       ...
       was used. age overflow is also processed correctly. */
        ...
        void LargeObjectCacheImpl<Props>::CacheBin::forgetOutdatedState(uintptr_t currTime) {
            ...
            if (threshold)
                ...
                doCleanup = sinceLastGet
                            > static_cast<uintptr_t>(Props::LongWaitFactor * (lastCleanedAge - lastGet));
    ...
    was used. age overflow is also processed correctly. */
        ...
        bool ExtMemoryPool::hardCachesCleanup(bool wait) {
            if (hardCachesCleanupInProgress.exchange(1, std::memory_order_acq_rel)) {
                ...
                while (hardCachesCleanupInProgress.exchange(1, std::memory_order_acq_rel)) backoff.pause();
            }
