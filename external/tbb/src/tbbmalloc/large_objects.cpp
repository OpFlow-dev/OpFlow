    ...
    namespace internal {
        ...
        void LargeObjectCache::init(ExtMemoryPool *memPool) {
            ...
            if (hugeSizeThreshold == 0) {
                ...
                long requestedThreshold = tbb::detail::r1::GetIntegralEnvironmentVariable(
       ...
       Moreover it cancels the OP_CLEAN_TO_THRESHOLD operation. */
           ...
           lastGet - the same meaning as CacheBin::lastGet */
            ...
            public:
                OperationPreprocessor(typename LargeObjectCacheImpl<Props>::CacheBin *bin)
                    : bin(bin), lclTime(0), opGet(NULL), opClean(NULL), cleanTime(0), lastGetOpTime(0),
                      updateUsedSize(0), head(NULL), isCleanAll(false) {}
        ...
        public:
            CacheBinFunctor(typename LargeObjectCacheImpl<Props>::CacheBin *bin, ExtMemoryPool *extMemPool,
                ...
                : bin(bin), extMemPool(extMemPool), bitMask(bitMask), idx(idx), toRelease(NULL),
                  needCleanup(false) {}
            void operator()(CacheBinOperation *opList);
        ...
        inline bool lessThanWithOverflow(intptr_t a, intptr_t b) {
            return (a < b && (b - a < UINTPTR_MAX / 2)) || (a > b && (a - b > UINTPTR_MAX / 2));
        }
#endif
        ...
        template <typename Props>
        void CacheBinFunctor<Props>::OperationPreprocessor::commitOperation(CacheBinOperation *op) const {
            // FencedStore( (intptr_t&)(op->status), CBST_DONE );
            op->status.store(CBST_DONE, std::memory_order_release);
        }

        template <typename Props>
        void CacheBinFunctor<Props>::OperationPreprocessor::addOpToOpList(CacheBinOperation *op,
                                                                          CacheBinOperation **opList) const {
            op->next = *opList;
            *opList = op;
        }

        template <typename Props>
        bool CacheBinFunctor<Props>::OperationPreprocessor::getFromPutList(CacheBinOperation *opGet,
                                                                           uintptr_t currTime) {
            if (head) {
        ...
        template <typename Props>
        void CacheBinFunctor<Props>::OperationPreprocessor::addToPutList(LargeMemoryBlock *h,
                                                                         LargeMemoryBlock *t, int num) {
            if (head) {
        ...
        template <typename Props>
        void CacheBinFunctor<Props>::OperationPreprocessor::operator()(CacheBinOperation *opList) {
                ...
                switch (op->type) {
                    ...
                    case CBOP_PUT_LIST: {
                        ...
                        while (opGet) {
                            ...
                            if (!getFromPutList(opGet, opCast<OpGet>(*opGet).currTime)) break;
                            opGet = next;
                        }
                    } break;
        ...
        template <typename Props>
        void CacheBinFunctor<Props>::operator()(CacheBinOperation *opList) {
            ...
            if (uintptr_t timeRange = prep.getTimeRange()) {
                ...
                if (CacheBinOperation *opGet = prep.opGet) {
                    ...
                    if (prep.lastGetOpTime) bin->setLastGet(prep.lastGetOpTime + endTime);
            ...
            if (CacheBinOperation *opClean = prep.opClean) {
                if (prep.isCleanAll) *opCast<OpCleanAll>(*opClean).res = bin->cleanAll(bitMask, idx);
                else
                    *opCast<OpCleanToThreshold>(*opClean).res
                            = bin->cleanToThreshold(prep.cleanTime, bitMask, idx);
            ...
            if (size_t size = prep.updateUsedSize) bin->updateUsedSize(size, bitMask, idx);
        }
        /* ----------------------------------------------------------------------------------------------------- */
        /* --------------------------- Methods for creating and executing operations --------------------------- */
        template <typename Props>
        void LargeObjectCacheImpl<Props>::CacheBin::ExecuteOperation(CacheBinOperation *op,
                                                                     ExtMemoryPool *extMemPool,
                                                                     BinBitMask *bitMask, int idx,
                                                                     bool longLifeTime) {
        ...
        template <typename Props>
        LargeMemoryBlock *LargeObjectCacheImpl<Props>::CacheBin::get(ExtMemoryPool *extMemPool, size_t size,
                                                                     BinBitMask *bitMask, int idx) {
        ...
        template <typename Props>
        void LargeObjectCacheImpl<Props>::CacheBin::putList(ExtMemoryPool *extMemPool, LargeMemoryBlock *head,
                                                            BinBitMask *bitMask, int idx) {
            MALLOC_ASSERT(sizeof(LargeMemoryBlock) + sizeof(CacheBinOperation) <= head->unalignedSize,
                          "CacheBinOperation is too large to be placed in LargeMemoryBlock!");
        ...
        template <typename Props>
        bool LargeObjectCacheImpl<Props>::CacheBin::cleanToThreshold(ExtMemoryPool *extMemPool,
                                                                     BinBitMask *bitMask, uintptr_t currTime,
                                                                     int idx) {
       ...
       was used. age overflow is also processed correctly. */
            if (last.load(std::memory_order_relaxed)
                && (intptr_t)(currTime - oldest.load(std::memory_order_relaxed))
        ...
        template <typename Props>
        bool LargeObjectCacheImpl<Props>::CacheBin::releaseAllToBackend(ExtMemoryPool *extMemPool,
                                                                        BinBitMask *bitMask, int idx) {
            LargeMemoryBlock *toRelease = NULL;
            ...
            while (toRelease) {
                LargeMemoryBlock *helper = toRelease->next;
                MALLOC_ASSERT(!helper || lessThanWithOverflow(helper->age, toRelease->age), ASSERT_TEXT);
                backend->returnLargeObject(toRelease);
                toRelease = helper;
            }
            return released;
        ...
        template <typename Props>
        void LargeObjectCacheImpl<Props>::CacheBin::updateUsedSize(ExtMemoryPool *extMemPool, size_t size,
                                                                   BinBitMask *bitMask, int idx) {
        ...
        template <typename Props>
        LargeMemoryBlock *LargeObjectCacheImpl<Props>::CacheBin::putList(LargeMemoryBlock *head,
                                                                         LargeMemoryBlock *tail,
                                                                         BinBitMask *bitMask, int idx,
                                                                         int num, size_t hugeSizeThreshold) {
            ...
            MALLOC_ASSERT(!last.load(std::memory_order_relaxed)
                                  || (last.load(std::memory_order_relaxed)->age != 0
                                      && last.load(std::memory_order_relaxed)->age != -1U),
                          ASSERT_TEXT);
            ...
            if (num) {
                ...
                cachedSize.store(cachedSize.load(std::memory_order_relaxed) + num * size,
        ...
        template <typename Props>
        LargeMemoryBlock *LargeObjectCacheImpl<Props>::CacheBin::get() {
        ...
        void LargeObjectCacheImpl<Props>::CacheBin::forgetOutdatedState(uintptr_t currTime) {
            ...
            if (threshold) doCleanup = sinceLastGet > Props::LongWaitFactor * threshold;
        ...
        template <typename Props>
        LargeMemoryBlock *LargeObjectCacheImpl<Props>::CacheBin::cleanToThreshold(uintptr_t currTime,
                                                                                  BinBitMask *bitMask,
                                                                                  int idx) {
            /* oldest may be more recent then age, that's why cast to signed type
    was used. age overflow is also processed correctly. */
            if (!last.load(std::memory_order_relaxed)
                || (intptr_t)(currTime - last.load(std::memory_order_relaxed)->age)
                           < ageThreshold.load(std::memory_order_relaxed))
                return nullptr;
            ...
            do {
                ...
                MALLOC_ASSERT(
                        !nextAge || lessThanWithOverflow(nextAge, last.load(std::memory_order_relaxed)->age),
                        ASSERT_TEXT);
                nextAge = last.load(std::memory_order_relaxed)->age;
#endif
                cachedSize.store(cachedSize.load(std::memory_order_relaxed)
                                         - last.load(std::memory_order_relaxed)->unalignedSize,
                                 std::memory_order_relaxed);
            ...
            if (last.load(std::memory_order_relaxed)) {
                ...
                last.load(std::memory_order_relaxed)->next = NULL;
            } else {
                toRelease = first;
                first = NULL;
                oldest.store(0, std::memory_order_relaxed);
                if (!usedSize.load(std::memory_order_relaxed)) bitMask->set(idx, false);
            }
        ...
        template <typename Props>
        LargeMemoryBlock *LargeObjectCacheImpl<Props>::CacheBin::cleanAll(BinBitMask *bitMask, int idx) {
            if (!last.load(std::memory_order_relaxed)) return NULL;

            LargeMemoryBlock *toRelease = first;
            last.store(NULL, std::memory_order_relaxed);
            first = NULL;
            oldest.store(0, std::memory_order_relaxed);
        ...
        size_t LargeObjectCacheImpl<Props>::CacheBin::reportStat(int num, FILE *f) {
            ...
            if (first)
                printf("%d(%lu): total %lu KB thr %ld lastCln %lu oldest %lu\n", num,
                       num * Props::CacheStep + Props::MinSize,
                       cachedSize.load(std::memory_order_relaxed) / 1024,
                       ageThresholdageThreshold.load(std::memory_order_relaxed), lastCleanedAge,
        ...
        template <typename Props>
        bool LargeObjectCacheImpl<Props>::regularCleanup(ExtMemoryPool *extMemPool, uintptr_t currTime,
                                                         bool doThreshDecr) {
            bool released = false;
            ...
            for (int i = bitMask.getMaxTrue(startSearchIdx); i >= 0; i = bitMask.getMaxTrue(i - 1)) {
                ...
                if (!doThreshDecr && tooLargeLOC.load(std::memory_order_relaxed) > 2
                    && binsSummary.isLOCTooLarge()) {
                ...
                if (doThreshDecr) bin[i].decreaseThreshold();

                if (bin[i].cleanToThreshold(extMemPool, &bitMask, currTime, i)) { released = true; }
            }
        ...
        void LargeObjectCacheImpl<Props>::reset() {
            tooLargeLOC.store(0, std::memory_order_relaxed);
            for (int i = numBins - 1; i >= 0; i--) bin[i].init();
            bitMask.reset();
        }
        ...
        size_t LargeObjectCacheImpl<Props>::getLOCSize() const {
            size_t size = 0;
            for (int i = numBins - 1; i >= 0; i--) size += bin[i].getSize();
            return size;
        }
        ...
        size_t LargeObjectCacheImpl<Props>::getUsedSize() const {
            size_t size = 0;
            for (int i = numBins - 1; i >= 0; i--) size += bin[i].getUsedSize();
            return size;
        }
        ...
        bool LargeObjectCache::doCleanup(uintptr_t currTime, bool doThreshDecr) {
            if (!doThreshDecr) extMemPool->allLocalCaches.markUnused();
        ...
        void LargeObjectCacheImpl<Props>::reportStat(FILE *f) {
            size_t cachedSize = 0;
            for (int i = 0; i < numBins; i++) cachedSize += bin[i].reportStat(i, f);
        ...
        void LargeObjectCache::updateCacheState(DecreaseOrIncrease op, size_t size) {
            if (size < maxLargeSize) largeCache.updateCacheState(extMemPool, op, size);
        ...
        int LargeObjectCache::sizeToIdx(size_t size) {
            ...
            return size < maxLargeSize ? LargeCacheType::sizeToIdx(size)
            ...
            for (LargeMemoryBlock *curr = list; curr; curr = toProcess) {
                ...
                for (LargeMemoryBlock *b = toProcess; b; b = n) {
                    ...
                    if (sizeToIdx(b->unalignedSize) == currIdx) {
                        tail->next = b;
                        tail = b;
                        if (toProcess == b) toProcess = toProcess->next;
                        else {
                            b->prev->next = b->next;
                ...
                tail->next = NULL;
                if (curr->unalignedSize < maxLargeSize) largeCache.putList(extMemPool, curr);
                else
        ...
        void LargeObjectCache::put(LargeMemoryBlock *largeBlock) {
            ...
            if (sizeInCacheRange(blockSize)) {
                ...
                if (blockSize < maxLargeSize) largeCache.putList(extMemPool, largeBlock);
                else
        ...
        LargeMemoryBlock *LargeObjectCache::get(size_t size) {
            ...
            if (sizeInCacheRange(size)) {
                return size < maxLargeSize ? largeCache.get(extMemPool, size)
                                           : hugeCache.get(extMemPool, size);
            }
            return NULL;
        }
        ...
        void ExtMemoryPool::freeLargeObject(LargeMemoryBlock *mBlock) { loc.put(mBlock); }

        void ExtMemoryPool::freeLargeObjectList(LargeMemoryBlock *head) { loc.putList(head); }

        bool ExtMemoryPool::softCachesCleanup() { return loc.regularCleanup(); }
