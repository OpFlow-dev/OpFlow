...
namespace tbb {
    namespace detail {
        namespace r1 {
            ...
            // Arena Slot
            //------------------------------------------------------------------------
            d1::task* arena_slot::get_task_impl(size_t T, execution_data_ext& ed, bool& tasks_omitted,
                ...
                if (tasks_omitted) { task_pool_ptr[T] = nullptr; }
                return nullptr;
            }
                    ...
                    if ((std::intptr_t)(head.load(std::memory_order_acquire)) > (std::intptr_t) T) {
                        ...
                        if ((std::intptr_t) H0 > (std::intptr_t) T) {
                            ...
                            __TBB_ASSERT(H0 == head.load(std::memory_order_relaxed)
                                                 && T == tail.load(std::memory_order_relaxed) && H0 == T + 1,
                ...
                do {
                    ...
                    if (result) {
                        if (isolation == no_isolation || isolation == task_accessor::isolation(*result)) {
                            if (!task_accessor::is_proxy_task(*result)) { break; }
                            task_proxy& tp = *static_cast<task_proxy*>(result);
                            ...
                            if (!task_proxy::is_shared(tp.task_and_tag) || !tp.outbox->recipient_is_idle()
                                || a.mailbox(slot_index).recipient_is_idle()) {
                                break;
                            }
                        }
