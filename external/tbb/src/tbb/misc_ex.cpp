...
#define curMaskSize sizeof(basic_mask_t) * num_masks
            affinity_helper::~affinity_helper() {
                if (threadMask) {
                    if (is_changed) { set_thread_affinity_mask(curMaskSize, threadMask); }
                    delete[] threadMask;
                }
            }
            void affinity_helper::protect_affinity_mask(bool restore_process_mask) {
                if (threadMask == nullptr && num_masks) {// TODO: assert num_masks validity?
                    ...
                    if (restore_process_mask) {
                        ...
                        if (is_changed) set_thread_affinity_mask(curMaskSize, process_mask);
                    } else {
...
#undef curMaskSize
            ...
            static void initialize_hardware_concurrency_info() {
                ...
                for (;;) {
                    ...
                    if (!err || errno != ERANGE || curMaskSize * CHAR_BIT >= 16 * 1024) break;
#else /* __unix__ */
                    int pid = getpid();
                    err = sched_getaffinity(pid, curMaskSize, processMask);
                    if (!err || errno != EINVAL || curMaskSize * CHAR_BIT >= 256 * 1024) break;
#endif
                    delete[] processMask;
                    numMasks <<= 1;
                }
                if (!err) {
                    ...
                    for (int m = 0; availableProcs < maxProcs && m < numMasks; ++m) {
                        for (std::size_t i = 0; (availableProcs < maxProcs) && (i < BasicMaskSize * CHAR_BIT);
                             ++i) {
                            if (CPU_ISSET(i, processMask + m)) ++availableProcs;
                        }
                    }
...
/* End of __TBB_USE_OS_AFFINITY_SYSCALL implementation */
        ...
        TODO:   Implement a dynamic scheme remapping workers depending on the pending
            ...
            int calculate_numa
                    [MaxProcessorGroups];//Array needed for FindProcessorGroupIndex to calculate Processor Group when number of threads > number of cores to distribute threads evenly between processor groups
            int numaSum;
            ...
            static BOOL(WINAPI *TBB_SetThreadGroupAffinity)(HANDLE hThread, const TBB_GROUP_AFFINITY *newAff,
                                                            TBB_GROUP_AFFINITY *prevAff);
            ...
            static void initialize_hardware_concurrency_info() {
                ...
                theProcessorGroups[0].numProcs = theProcessorGroups[0].numProcsRunningTotal
                ...
                if (nproc == number_of_processors && TBB_GetActiveProcessorCount) {
                    ...
                    if (ProcessorGroupInfo::NumGroups > 1) {
                        ...
                        for (WORD i = 0; i < ProcessorGroupInfo::NumGroups; ++i) {
                            ...
                            if (pgi.numProcs < min_procs)
                                min_procs
                                        = pgi.numProcs;//Finding the minimum number of processors in the Processor Groups
                            calculate_numa[i] = pgi.numProcs;
                            __TBB_ASSERT(pgi.numProcs <= (int) sizeof(DWORD_PTR) * CHAR_BIT, nullptr);
                            pgi.mask = pgi.numProcs == sizeof(DWORD_PTR) * CHAR_BIT
                                               ? ~(DWORD_PTR) 0
                                               : (DWORD_PTR(1) << pgi.numProcs) - 1;
                ...
                if (ProcessorGroupInfo::NumGroups > 1)
                    for (int i = 0; i < ProcessorGroupInfo::NumGroups; ++i)
                        PrintExtraVersionInfo("----- Group", "%d: size %d", i,
                                              theProcessorGroups[i].numProcs);
            }

            int NumberOfProcessorGroups() {
                __TBB_ASSERT(hardware_concurrency_info == do_once_state::initialized,
            ...
            int FindProcessorGroupIndex(int procIdx) {
                ...
                if (procIdx >= theProcessorGroups[current_grp_idx].numProcs
                    && procIdx < theProcessorGroups[ProcessorGroupInfo::NumGroups - 1].numProcsRunningTotal) {
                    ...
                    do {
                        ...
                        procIdx = procIdx - theProcessorGroups[current_grp_idx].numProcs;
                           ...
                           >= theProcessorGroups[ProcessorGroupInfo::NumGroups - 1].numProcsRunningTotal) {
            ...
            void MoveThreadIntoProcessorGroup(void *hThread, int groupIndex) {
                ...
                if (!TBB_SetThreadGroupAffinity) return;
