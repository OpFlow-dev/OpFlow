...
#define DYNAMIC_LINK_WARNING dynamic_link_warning
            ...
            static bool resolve_symbols(dynamic_link_handle module,
                                        const dynamic_link_descriptor descriptors[], std::size_t required) {
...
#define MAX_LOADED_MODULES 8// The number of maximum possible modules which can be loaded
            ...
            static struct handles_t {
                ...
                void free() {
                    ...
                    for (std::size_t i = 0; i < size; ++i) dynamic_unlink(my_handles[i]);
                }
            } handles;
            ...
            static void init_ap_data() {
                ...
                *(backslash + 1) = 0;
                ...
                Dl_info dlinfo;
                int res = dladdr((void *) &dynamic_link,
                                 &dlinfo);// any function inside the library can be used for the address
                if (!res) {
            ...
            static void init_dl_data() { init_ap_data(); }
    ...
    */
            ...
            static struct static_init_dl_data_t {
                static_init_dl_data_t() { init_dynamic_link_data(); }
            } static_init_dl_data;
#endif
            ...
            static bool weak_symbol_link(const dynamic_link_descriptor descriptors[], std::size_t required) {
                ...
                for (std::size_t k = 0; k < required; ++k)
                    if (!descriptors[k].ptr) return false;
                // Commit the entry points.
                for (std::size_t k = 0; k < required; ++k)
...
#else
            static bool weak_symbol_link(const dynamic_link_descriptor[], std::size_t) { return false; }
            ...
            bool dynamic_link(const char *library, const dynamic_link_descriptor descriptors[],
                ...
                dynamic_link_handle library_handle
                        = (flags & DYNAMIC_LINK_GLOBAL) ? global_symbols_link(library, descriptors, required)
                                                        : nullptr;
...
#pragma warning(pop)
            ...
            bool dynamic_link(const char *, const dynamic_link_descriptor *, std::size_t,
                              dynamic_link_handle *handle, int) {
                if (handle) *handle = 0;
                return false;
            }
            void dynamic_unlink(dynamic_link_handle) {}
