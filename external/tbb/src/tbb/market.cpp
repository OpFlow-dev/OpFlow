    ...
    namespace detail {
        namespace r1 {
            ...
            arena* market::select_next_arena(arena* hint) {
                ...
                if (hint) next_arena_priority_level = hint->my_priority_level;
                for (unsigned idx = 0; idx < next_arena_priority_level; ++idx) {
                    if (!my_arenas[idx].empty()) return &*my_arenas[idx].begin();
                }
            ...
            void market::insert_arena_into_list(arena& a) {
                ...
                __TBB_ASSERT(!my_next_arena || my_next_arena->my_priority_level < num_priority_levels,
                             nullptr);
            ...
            void market::remove_arena_from_list(arena& a) {
                ...
                if (my_next_arena == &a) my_next_arena = nullptr;
                my_next_arena = select_next_arena(my_next_arena);
            }
            ...
            market::market(unsigned workers_soft_limit, unsigned workers_hard_limit, std::size_t stack_size)
                : my_num_workers_hard_limit(workers_hard_limit),
                  my_num_workers_soft_limit(workers_soft_limit), my_next_arena(nullptr), my_ref_count(1),
                  my_stack_size(stack_size), my_workers_soft_limit_to_report(workers_soft_limit) {
            ...
            static unsigned calc_workers_soft_limit(unsigned workers_soft_limit,
                                                    unsigned workers_hard_limit) {
                if (int soft_limit = market::app_parallelism_limit()) workers_soft_limit = soft_limit - 1;
                ...
                if (workers_soft_limit >= workers_hard_limit) workers_soft_limit = workers_hard_limit - 1;
                return workers_soft_limit;
            }

            bool market::add_ref_unsafe(global_market_mutex_type::scoped_lock& lock, bool is_public,
                                        unsigned workers_requested, std::size_t stack_size) {
                market* m = theMarket;
                if (m) {
                    ++m->my_ref_count;
                    const unsigned old_public_count
                            = is_public ? m->my_public_ref_count++ : /*any non-zero value*/ 1;
                    lock.release();
                    if (old_public_count == 0)
                        set_active_num_workers(
                                calc_workers_soft_limit(workers_requested, m->my_num_workers_hard_limit));

                    // do not warn if default number of workers is requested
                    if (workers_requested != governor::default_num_threads() - 1) {
                        __TBB_ASSERT(
                                skip_soft_limit_warning > workers_requested,
                                "skip_soft_limit_warning must be larger than any valid workers_requested");
                        unsigned soft_limit_to_report
                                = m->my_workers_soft_limit_to_report.load(std::memory_order_relaxed);
                        if (soft_limit_to_report < workers_requested) {
                            runtime_warning("The number of workers is currently limited to %u. "
                                            "The request for %u workers is ignored. Further requests for "
                                            "more workers "
                            ...
                            m->my_workers_soft_limit_to_report.compare_exchange_strong(
                                    expected_limit, skip_soft_limit_warning);
                        }
                    }
            ...
            market& market::global_market(bool is_public, unsigned workers_requested,
                ...
                if (!market::add_ref_unsafe(lock, is_public, workers_requested, stack_size)) {
                    ...
                    const unsigned workers_hard_limit = max(
                            max(factor * governor::default_num_threads(), 256u), app_parallelism_limit());
                    const unsigned workers_soft_limit
                            = calc_workers_soft_limit(workers_requested, workers_hard_limit);
                    // Create the global market instance
                    std::size_t size = sizeof(market);
                    __TBB_ASSERT(__TBB_offsetof(market, my_workers) + sizeof(std::atomic<thread_data*>)
                                         == sizeof(market),
                    ...
                    if (is_public) m->my_public_ref_count.store(1, std::memory_order_relaxed);
            ...
            bool market::release(bool is_public, bool blocking_terminate) {
                market::enforce([this] { return theMarket == this; },
                                "Global market instance was destroyed prematurely?");
            ...
            void market::set_active_num_workers(unsigned soft_limit) {
                ...
                {
                    global_market_mutex_type::scoped_lock lock(theMarketMutex);
                    if (!theMarket) return;// actual value will be used at market creation
                    m = theMarket;
                    if (m->my_num_workers_soft_limit.load(std::memory_order_relaxed) == soft_limit) return;
                    ++m->my_ref_count;
                }
                ...
                int delta = 0;
                {
                    ...
                    if (m->my_num_workers_soft_limit.load(std::memory_order_relaxed) == 0
                        ...
                        for (unsigned level = 0; level < num_priority_levels; ++level)
                            for (arena_list_type::iterator it = arenas[level].begin();
                                 it != arenas[level].end(); ++it)
                    ...
                    if (m->my_num_workers_soft_limit.load(std::memory_order_relaxed) == 0) {
                        for (unsigned level = 0; level < num_priority_levels; ++level)
                            for (arena_list_type::iterator it = arenas[level].begin();
                                 it != arenas[level].end(); ++it)
                                if (it->has_enqueued_tasks()) m->enable_mandatory_concurrency_impl(&*it);
                    }
#endif
                ...
                if (delta != 0) m->my_server->adjust_job_count_estimate(delta);
            ...
            void market::detach_arena(arena& a) {
                ...
                if (a.my_aba_epoch == my_arenas_aba_epoch.load(std::memory_order_relaxed)) {
                    my_arenas_aba_epoch.store(my_arenas_aba_epoch.load(std::memory_order_relaxed) + 1,
            ...
            void market::try_destroy_arena(arena* a, uintptr_t aba_epoch, unsigned priority_level) {
                ...
                for (; it != my_arenas[priority_level].end(); ++it) {
                    if (a == &*it) {
                        ...
                        if (locked) my_arenas_list_mutex.unlock();
                        return;
                    }
                }
                my_arenas_list_mutex.unlock();
            ...
            arena* market::arena_in_need(arena* prev) {
                if (my_total_demand.load(std::memory_order_acquire) <= 0) return nullptr;
                ...
                if (is_arena_alive(prev)) return arena_in_need(my_arenas, prev);
            ...
            int market::update_allotment(arena_list_type* arenas, int workers_demand, int max_workers) {
                ...
                for (unsigned list_idx = 0; list_idx < num_priority_levels; ++list_idx) {
                    ...
                    for (arena_list_type::iterator it = arenas[list_idx].begin();
                         it != arenas[list_idx].end(); ++it) {
                        arena& a = *it;
                        ...
                        __TBB_ASSERT(a.my_num_workers_requested <= int(a.my_max_num_workers)
                                             || (a.my_max_num_workers == 0
                                                 && a.my_local_concurrency_requests > 0
                                                 && a.my_num_workers_requested == 1),
                                     nullptr);
                        ...
                        if (max_priority_level == num_priority_levels) { max_priority_level = list_idx; }
            ...
            bool market::is_arena_alive(arena* a) {
                ...
                for (unsigned idx = 0; idx < num_priority_levels; ++idx) {
                    if (is_arena_in_list(my_arenas[idx], a)) return true;
                }
                return false;
            }
            ...
            void market::enable_mandatory_concurrency(arena* a) {
                ...
                if (delta != 0) my_server->adjust_job_count_estimate(delta);
            }
            ...
            void market::mandatory_concurrency_disable(arena* a) {
                ...
                {
                    arenas_list_mutex_type::scoped_lock lock(my_arenas_list_mutex);
                    if (!a->my_global_concurrency_mode.load(std::memory_order_relaxed)) return;
                    ...
                    if (a->has_enqueued_tasks()) return;
            ...
            void market::adjust_demand(arena& a, int delta, bool mandatory) {
                ...
                a.my_adjust_demand_current_epoch.wait_until(target_epoch, /* context = */ target_epoch,
                                                            std::memory_order_relaxed);
            ...
            void market::process(job& j) {
                ...
                arena* a = td.my_arena;
                for (int i = 0; i < 2; ++i) {
                    while ((a = arena_in_need(a))) { a->process(td); }
            ...
            void market::acknowledge_close_connection() { destroy(); }
