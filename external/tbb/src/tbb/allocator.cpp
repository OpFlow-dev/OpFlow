...
namespace tbb {
    namespace detail {
        namespace r1 {
            ...
            static std::atomic<cache_aligned_allocate_handler_type> cache_aligned_allocate_handler {
                    &initialize_cache_aligned_allocate_handler};
...
#error Unknown OS
    ...
    If that allocator is not found, it links to malloc and free. */
            void initialize_handler_pointers() {
                ...
                cache_aligned_allocate_handler.store(cache_aligned_allocate_handler_unsafe,
            ...
            static void* initialize_cache_aligned_allocate_handler(std::size_t bytes, std::size_t alignment) {
                initialize_cache_aligned_allocator();
                __TBB_ASSERT(cache_aligned_allocate_handler != &initialize_cache_aligned_allocate_handler,
                             NULL);
            ...
            std::size_t __TBB_EXPORTED_FUNC cache_line_size() { return nfs_size; }
            ...
            void* __TBB_EXPORTED_FUNC cache_aligned_allocate(std::size_t size) {
                ...
                // Check for overflow
                if (size + cache_line_size < size) { throw_exception(exception_id::bad_alloc); }
                // scalable_aligned_malloc considers zero size request an error, and returns NULL
                if (size == 0) size = 1;

                void* result = cache_aligned_allocate_handler.load(std::memory_order_acquire)(
                        size, cache_line_size);
                if (!result) { throw_exception(exception_id::bad_alloc); }
            ...
            static void* std_cache_aligned_allocate(std::size_t bytes, std::size_t alignment) {
                ...
                __TBB_ASSERT((result - base) >= sizeof(std::uintptr_t),
                             "Cannot store a base pointer to the header");
            ...
            static void std_cache_aligned_deallocate(void* p) {
                if (p) {
                    __TBB_ASSERT(reinterpret_cast<std::uintptr_t>(p) >= 0x4096,
                    ...
                    __TBB_ASSERT(((base + nfs_size) & ~(nfs_size - 1)) == reinterpret_cast<std::uintptr_t>(p),
                                 "Incorrect alignment or not allocated by std_cache_aligned_deallocate?");
            ...
            bool __TBB_EXPORTED_FUNC is_tbbmalloc_used() {
                ...
                __TBB_ASSERT(handler_snapshot != &initialize_allocate_handler
                                     && deallocate_handler != nullptr,
                             NULL);
                // Cast to void avoids type mismatch errors on some compilers (e.g. __IBMCPP__)
                __TBB_ASSERT(
                        (reinterpret_cast<void*>(handler_snapshot) == reinterpret_cast<void*>(&std::malloc))
                                == (reinterpret_cast<void*>(deallocate_handler)
                                    == reinterpret_cast<void*>(&std::free)),
                        "Both shim pointers must refer to routines from the same package (either TBB or "
                        "CRT)");
