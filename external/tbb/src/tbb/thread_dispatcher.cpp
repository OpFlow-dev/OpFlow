...
namespace tbb {
    namespace detail {
        namespace r1 {
            ...
            thread_dispatcher::thread_dispatcher(threading_control& tc, unsigned hard_limit,
                                                 std::size_t stack_size)
                : my_threading_control(tc), my_num_workers_hard_limit(hard_limit), my_stack_size(stack_size) {
                my_server = governor::create_rml_server(*this);
                __TBB_ASSERT(my_server, "Failed to create RML server");
            }

            thread_dispatcher::~thread_dispatcher() { poison_pointer(my_server); }
            ...
            thread_dispatcher_client* thread_dispatcher::select_next_client(thread_dispatcher_client* hint) {
                ...
                if (hint) { next_client_priority_level = hint->priority_level(); }

                for (unsigned idx = 0; idx < next_client_priority_level; ++idx) {
                    if (!my_client_list[idx].empty()) { return &*my_client_list[idx].begin(); }
                }
            ...
            thread_dispatcher_client* thread_dispatcher::create_client(arena& a) {
                return new (cache_aligned_allocate(sizeof(thread_dispatcher_client)))
                        thread_dispatcher_client(a, my_clients_aba_epoch);
            }
            ...
            bool thread_dispatcher::try_unregister_client(thread_dispatcher_client* client,
                                                          std::uint64_t aba_epoch, unsigned priority) {
            ...
            void thread_dispatcher::insert_client(thread_dispatcher_client& client) {
                ...
                __TBB_ASSERT(!my_next_client || my_next_client->priority_level() < num_priority_levels,
                             nullptr);
            ...
            void thread_dispatcher::remove_client(thread_dispatcher_client& client) {
                ...
                if (my_next_client == &client) { my_next_client = nullptr; }
                my_next_client = select_next_client(my_next_client);
            }
            ...
            thread_dispatcher_client* thread_dispatcher::client_in_need(client_list_type* clients,
                                                                        thread_dispatcher_client* hint) {
            ...
            thread_dispatcher_client* thread_dispatcher::client_in_need(thread_dispatcher_client* prev) {
                ...
                if (is_client_alive(prev)) { return client_in_need(my_client_list, prev); }
            ...
            bool thread_dispatcher::is_any_client_in_need() {
                ...
                for (auto& priority_list : my_client_list) {
                    for (auto& client : priority_list) {
                        if (client.is_joinable()) { return true; }
                    }
                }
                return false;
            }
            ...
            void thread_dispatcher::acknowledge_close_connection() { my_threading_control.destroy(); }
