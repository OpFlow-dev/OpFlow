...
#pragma weak tcmGetVersionInfo
                ...
                tcm_result_t (*tcm_request_permit)(tcm_client_id_t client_id, tcm_permit_request_t request,
                                                   ...
                                                   tcm_permit_t* permit) {nullptr};
                tcm_result_t (*tcm_get_permit_data)(tcm_permit_handle_t permit_handle,
                                                    tcm_permit_t* permit) {nullptr};
...
#define TCMLIB_NAME LIBRARY_PREFIX "tcm" DEBUG_SUFFIX LIBRARY_EXTENSION
            ...
            public:
                ...
                unsigned priority_level() { return my_arena.priority_level(); }

                tcm_permit_request_t& permit_request() { return my_permit_request; }

                tcm_permit_handle_t& permit_handle() { return my_permit_handle; }
                ...
                void actualize_permit() {
                    ...
                    int delta {};
                    {
                        ...
                        if (!new_permit.flags.stale) {
                            ...
                            delta = update_concurrency(
                                    new_permit.state != TCM_PERMIT_STATE_INACTIVE ? new_concurrency : 0);
                        }
                    }
                    if (delta) { my_tcm_adaptor.notify_thread_request(delta); }
                }
                ...
                void request_permit(tcm_client_id_t client_id) {
                    ...
                    my_permit_request.min_sw_threads
                            = my_permit_request.max_sw_threads == 0 ? 0 : min_workers();
                    ...
                    __TBB_ASSERT(my_permit_request.max_sw_threads >= my_permit_request.min_sw_threads,
                                 nullptr);

                    tcm_result_t res = tcm_request_permit(client_id, my_permit_request, this,
                                                          &my_permit_handle, nullptr);
                    __TBB_ASSERT_EX(res == TCM_RESULT_SUCCESS, nullptr);
                }
            ...
            void tcm_adaptor::initialize() {
                tcm_functions_loaded
                        = dynamic_link(TCMLIB_NAME, tcm_link_table, /* tcm_link_table size = */ 11);
            }

            bool tcm_adaptor::is_initialized() { return tcm_functions_loaded; }
            ...
            bool tcm_adaptor::is_connected() { return my_impl != nullptr; }
