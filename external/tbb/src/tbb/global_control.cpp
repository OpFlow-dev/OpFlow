    ...
    namespace detail {
        namespace r1 {
            ...
            protected:
                ...
                std::set<global_control*, control_storage_comparator, tbb_allocator<global_control*>>
                        my_list {};
                spin_mutex my_list_mutex {};

            public:
                virtual std::size_t default_value() const = 0;
                virtual void apply_active(std::size_t new_active) { my_active_value = new_active; }
            ...
            class alignas(max_nfs_size) allowed_parallelism_control : public control_storage {
                ...
                virtual std::size_t active_value() override {
                    spin_mutex::scoped_lock lock(my_list_mutex);// protect my_list.empty() call
                    if (my_list.empty()) return default_value();
            ...
            public:
                std::size_t active_value_if_present() const { return !my_list.empty() ? my_active_value : 0; }
            };

            class alignas(max_nfs_size) stack_size_control : public control_storage {
                virtual std::size_t default_value() const override { return ThreadStackSize; }
                virtual void apply_active(std::size_t new_active) override {
                    control_storage::apply_active(new_active);
            ...
            class alignas(max_nfs_size) terminate_on_exception_control : public control_storage {
                virtual std::size_t default_value() const override { return 0; }
            };
            ...
            class alignas(max_nfs_size) lifetime_control : public control_storage {
                ...
                virtual void apply_active(std::size_t new_active) override {
                    if (new_active == 1) {
                        ...
                        if (market::theMarket) { market::add_ref_unsafe(lock, /*is_public*/ true); }
            ...
            static control_storage* controls[]
                    = {&allowed_parallelism_ctl, &stack_size_ctl, &terminate_on_exception_ctl, &lifetime_ctl};
#else
            static control_storage* controls[]
                    = {&allowed_parallelism_ctl, &stack_size_ctl, &terminate_on_exception_ctl};
#endif// __TBB_SUPPORTS_WORKERS_WAITING_IN_TERMINATE

            //! Comparator for a set of global_control objects
            inline bool control_storage_comparator::operator()(const global_control* lhs,
                                                               const global_control* rhs) const {
...
#if __TBB_SUPPORTS_WORKERS_WAITING_IN_TERMINATE
            unsigned market::is_lifetime_control_present() { return !lifetime_ctl.is_empty(); }
#endif// __TBB_SUPPORTS_WORKERS_WAITING_IN_TERMINATE
            ...
            public:
                ...
                static void destroy(d1::global_control& gc) {
                    ...
                    spin_mutex::scoped_lock lock(c->my_list_mutex);
#if __TBB_SUPPORTS_WORKERS_WAITING_IN_TERMINATE
                    __TBB_ASSERT(gc.my_param == global_control::scheduler_handle || !c->my_list.empty(),
                                 NULL);
#else
                    ...
                    if (new_active != old_active) { c->apply_active(new_active); }
                }
                ...
                static bool is_present(d1::global_control& gc) {
                    ...
                    spin_mutex::scoped_lock lock(c->my_list_mutex);
                    auto it = c->my_list.find(&gc);
                    if (it != c->my_list.end()) { return true; }
                    return false;
                }
#endif// TBB_USE_ASSERT
            };

            void __TBB_EXPORTED_FUNC create(d1::global_control& gc) { global_control_impl::create(gc); }
            void __TBB_EXPORTED_FUNC destroy(d1::global_control& gc) { global_control_impl::destroy(gc); }

            bool remove_and_check_if_empty(d1::global_control& gc) {
                return global_control_impl::remove_and_check_if_empty(gc);
            }
#if TBB_USE_ASSERT
            bool is_present(d1::global_control& gc) { return global_control_impl::is_present(gc); }
#endif// TBB_USE_ASSERT
