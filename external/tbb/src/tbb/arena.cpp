...
namespace tbb {
    namespace detail {
        namespace r1 {
            ...
            public:
                numa_binding_observer(d1::task_arena* ta, int num_slots, int numa_id, core_type_id core_type,
                                      int max_threads_per_core)
                    : task_scheduler_observer(*ta),
                      my_binding_handler(construct_binding_handler(num_slots, numa_id, core_type,
                ...
                ~numa_binding_observer() override { destroy_binding_handler(my_binding_handler); }
            };

            numa_binding_observer* construct_binding_observer(d1::task_arena* ta, int num_slots, int numa_id,
                                                              core_type_id core_type,
                                                              int max_threads_per_core) {
                numa_binding_observer* binding_observer = nullptr;
                if ((core_type >= 0 && core_type_count() > 1) || (numa_id >= 0 && numa_node_count() > 1)
                    || max_threads_per_core > 0) {
                    binding_observer = new (allocate_memory(sizeof(numa_binding_observer)))
                            numa_binding_observer(ta, num_slots, numa_id, core_type, max_threads_per_core);
            ...
            void arena::on_thread_leaving(unsigned ref_param) {
                ...
                __TBB_ASSERT(my_references.load(std::memory_order_relaxed) >= ref_param,
                ...
                if (ref_param == ref_external && !my_mandatory_concurrency.test()) { out_of_work(); }
                ...
                unsigned remaining_ref
                        = my_references.fetch_sub(ref_param, std::memory_order_release) - ref_param;
            ...
            std::size_t arena::occupy_free_slot_in_range(thread_data& tls, std::size_t lower,
                                                         std::size_t upper) {
            ...
            std::size_t arena::occupy_free_slot(thread_data& tls) {
                ...
                std::size_t index
                        = as_worker ? out_of_arena : occupy_free_slot_in_range(tls, 0, my_num_reserved_slots);
                if (index == out_of_arena) {
                    ...
                    if (index == out_of_arena) return out_of_arena;
                }
            ...
            std::uintptr_t arena::calculate_stealing_threshold() {
                ...
                return r1::calculate_stealing_threshold(reinterpret_cast<std::uintptr_t>(&anchor),
                                                        my_threading_control->worker_stack_size());
            }
            ...
            void arena::process(thread_data& tls) {
                ...
                if (tls.my_arena_slot->is_task_pool_published()) { tls.my_inbox.set_is_idle(false); }
                ...
                __TBB_ASSERT(!tls.my_last_observer,
                ...
                on_thread_leaving(ref_worker);
                __TBB_ASSERT(tls.my_arena == this,
                             "my_arena is used as a hint when searching the arena to join");
            }

            arena::arena(threading_control* control, unsigned num_slots, unsigned num_reserved_slots,
                ...
                __TBB_ASSERT(sizeof(my_slots[0]) % cache_line_size() == 0,
                             "arena::slot size not multiple of cache line size");
                ...
                my_default_ctx = new (cache_aligned_allocate(sizeof(d1::task_group_context)))
                        d1::task_group_context {d1::task_group_context::isolated,
                                                d1::task_group_context::fp_settings};
                // Construct slots. Mark internal synchronization elements for the tools.
                task_dispatcher* base_td_pointer
                        = reinterpret_cast<task_dispatcher*>(my_slots + my_num_slots);
            ...
            arena& arena::allocate_arena(threading_control* control, unsigned num_slots,
                                         unsigned num_reserved_slots, unsigned priority_level
#if __TBB_PREVIEW_PARALLEL_PHASE
                                         ,
                                         tbb::task_arena::leave_policy lp
#endif
            ) {
                __TBB_ASSERT(sizeof(base_type) + sizeof(arena_slot) == sizeof(arena),
                             "All arena data fields must go to arena_base");
                __TBB_ASSERT(sizeof(base_type) % cache_line_size() == 0,
                             "arena slots area misaligned: wrong padding");
            ...
            void arena::free_arena() {
                __TBB_ASSERT(is_alive(my_guard), nullptr);
                __TBB_ASSERT(!my_references.load(std::memory_order_relaxed),
                             "There are threads in the dying arena");
                __TBB_ASSERT(!my_total_num_workers_requested && !my_num_workers_allotted,
                ...
                for (unsigned i = 0; i < my_num_slots; ++i) {
                    ...
                    __TBB_ASSERT(my_slots[i].head == my_slots[i].tail,
                                 nullptr);// TODO: replace by is_quiescent_local_task_pool_empty
            ...
            bool arena::has_enqueued_tasks() { return !my_fifo_task_stream.empty(); }
            ...
            void arena::request_workers(int mandatory_delta, int workers_delta, bool wakeup_threads) {
                ...
                if (wakeup_threads) {
                    ...
                    get_waiting_threads_monitor().notify(
                            [&](market_context context) { return this == context.my_arena_addr; });
                }
            }
            ...
            bool arena::has_tasks() {
                ...
                tasks_are_available
                        = tasks_are_available || has_enqueued_tasks() || !my_resume_task_stream.empty();
#if __TBB_PREVIEW_CRITICAL_TASKS
            ...
            void arena::out_of_work() {
                ...
                bool disable_mandatory
                        = my_mandatory_concurrency.try_clear_if([this] { return !has_enqueued_tasks(); });
            ...
            std::pair<int, int> arena::update_request(int mandatory_delta, int workers_delta) {
                ...
                max_workers_request = clamp(
                        my_total_num_workers_requested, 0,
            ...
            arena& arena::create(threading_control* control, unsigned num_slots, unsigned num_reserved_slots,
                                 unsigned arena_priority_level, d1::constraints constraints
#if __TBB_PREVIEW_PARALLEL_PHASE
                                 ,
                                 tbb::task_arena::leave_policy lp
#endif
    ...
    namespace detail {
        namespace r1 {
            ...
            void assert_arena_priority_valid(tbb::task_arena::priority a_priority) {
                bool is_arena_priority_correct = a_priority == tbb::task_arena::priority::high
            ...
            tbb::task_arena::priority arena_priority(unsigned priority_level) {
                auto priority = tbb::task_arena::priority((d1::num_priority_levels - priority_level)
                                                          * d1::priority_stride);
            ...
            void __TBB_EXPORTED_FUNC initialize(d1::task_arena_base& ta) { task_arena_impl::initialize(ta); }
            void __TBB_EXPORTED_FUNC terminate(d1::task_arena_base& ta) { task_arena_impl::terminate(ta); }
            bool __TBB_EXPORTED_FUNC attach(d1::task_arena_base& ta) { return task_arena_impl::attach(ta); }
            void __TBB_EXPORTED_FUNC execute(d1::task_arena_base& ta, d1::delegate_base& d) {
                task_arena_impl::execute(ta, d);
            }
            void __TBB_EXPORTED_FUNC wait(d1::task_arena_base& ta) { task_arena_impl::wait(ta); }
            ...
            void __TBB_EXPORTED_FUNC enqueue(d1::task& t, d1::task_group_context& ctx,
                                             d1::task_arena_base* ta) {
            ...
            void task_arena_impl::initialize(d1::task_arena_base& ta) {
                ...
                numa_binding_observer* observer = construct_binding_observer(
                        static_cast<d1::task_arena*>(&ta),
                        arena::num_arena_slots(ta.my_max_concurrency, ta.my_num_reserved_slots),
                ...
                arena& a = arena::create(thr_control, unsigned(ta.my_max_concurrency),
                                         ta.my_num_reserved_slots, priority_level, arena_constraints
#if __TBB_PREVIEW_PARALLEL_PHASE
                                         ,
                                         ta.get_leave_policy()
#endif
            ...
            bool task_arena_impl::attach(d1::task_arena_base& ta) {
                ...
                if (td && td->my_arena) {
                    ...
                    __TBB_ASSERT(arena::num_arena_slots(ta.my_max_concurrency, ta.my_num_reserved_slots)
                                         == a->my_num_slots,
                                 nullptr);
                    ta.my_arena.store(a, std::memory_order_release);
            ...
            void task_arena_impl::enqueue(d1::task& t, d1::task_group_context* c, d1::task_arena_base* ta) {
                thread_data* td
                        = governor::get_thread_data();// thread data is only needed for FastRandom instance
                assert_pointer_valid(td, "thread_data pointer should not be null");
                arena* a = ta ? ta->my_arena.load(std::memory_order_relaxed) : td->my_arena;
            ...
            public:
                nested_arena_context(thread_data& td, arena& nested_arena, std::size_t slot_index)
                    ...
                    if (td.my_arena != &nested_arena) {
                        ...
                        if (td.my_inbox.is_idle_state(true)) td.my_inbox.set_is_idle(false);
                        task_dispatcher& task_disp = td.my_arena_slot->default_task_dispatcher();
                        td.enter_task_dispatcher(task_disp,
                                                 m_orig_execute_data_ext.task_disp->m_stealing_threshold);
                        ...
                        td.my_arena->my_observers.notify_entry_observers(td.my_last_observer,
                                                                         /* worker*/ false);
                    }
            ...
            class delegated_task : public d1::task {
                ...
                d1::task* execute(d1::execution_data& ed) override {
                    ...
                    ed_ext.task_disp->m_execute_data_ext.context
                            = ed_ext.task_disp->get_thread_data().my_arena->my_default_ctx;
                    bool fifo_task_allowed = ed_ext.task_disp->allow_fifo_task(true);
                    try_call([&] { m_delegate(); }).on_completion([&] {
                    ...
                    if (index1 == arena::out_of_arena) {
                        ...
                        do {
                            ...
                            if (index2 != arena::out_of_arena) {
                                ...
                                __TBB_ASSERT(!exec_context.my_exception.load(std::memory_order_relaxed),
                                             nullptr);// exception can be thrown above, not deferred
                                break;
                            }
                        ...
                        if (exception) {
                            __TBB_ASSERT(exec_context.is_group_execution_cancelled(),
                                         "The task group context with an exception should be canceled.");
            ...
            void task_arena_impl::wait(d1::task_arena_base& ta) {
                ...
                __TBB_ASSERT(td->my_arena != a || td->my_arena_index == 0,
                             "internal_wait is not supported within a worker context");
                if (a->my_max_num_workers != 0) {
                    while (a->num_workers_active() || !a->is_empty()) { yield(); }
                }
            }
            ...
            int task_arena_impl::max_concurrency(const d1::task_arena_base* ta) {
                ...
                if (ta && ta->my_max_concurrency == 1) { return 1; }
            ...
            void task_arena_impl::enter_parallel_phase(d1::task_arena_base* ta, std::uintptr_t /*reserved*/) {
                arena* a = ta ? ta->my_arena.load(std::memory_order_relaxed)
                              : governor::get_thread_data()->my_arena;
            ...
            void isolate_within_arena(d1::delegate_base& d, std::intptr_t isolation) {
                ...
                try_call([&] {
                    ...
                    isolation_type current_isolation
                            = isolation ? isolation : reinterpret_cast<isolation_type>(&d);
