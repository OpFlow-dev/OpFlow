    ...
    namespace detail {
        namespace r1 {
            ...
            public:
                ...
                ~numa_binding_observer() { destroy_binding_handler(my_binding_handler); }
            };
            ...
            void arena::free_arena() {
                ...
                __TBB_ASSERT(!my_num_workers_requested && !my_num_workers_allotted,
                             "Dying arena requests workers");
                __TBB_ASSERT(my_pool_state.load(std::memory_order_relaxed) == SNAPSHOT_EMPTY
                                     || !my_max_num_workers,
                ...
                for (unsigned i = 0; i < my_num_slots; ++i) {
                    ...
                    __TBB_ASSERT(my_slots[i].head == my_slots[i].tail,
                                 NULL);// TODO: replace by is_quiescent_local_task_pool_empty
                        ...
                        if (my_pool_state.compare_exchange_strong(expected_state, busy)) {
                            ...
                            for (k = 0; k < n; ++k) {
                                if (my_slots[k].task_pool.load(std::memory_order_relaxed) != EmptyTaskPool
                                    && my_slots[k].head.load(std::memory_order_relaxed)
                                               < my_slots[k].tail.load(std::memory_order_relaxed)) {
                            ...
                            if (my_pool_state.load(std::memory_order_acquire) == busy) {
                                ...
                                if (work_absent) {
                                    ...
                                    expected_state = busy;
                                    if (my_pool_state.compare_exchange_strong(expected_state,
                                                                              SNAPSHOT_EMPTY)) {
    ...
    namespace detail {
        namespace r1 {
            ...
            tbb::task_arena::priority arena_priority(unsigned priority_level) {
                auto priority = tbb::task_arena::priority((market::num_priority_levels - priority_level)
                                                          * d1::priority_stride);
            ...
            void task_arena_impl::initialize(d1::task_arena_base& ta) {
                ...
                arena* a = market::create_arena(ta.my_max_concurrency, ta.my_num_reserved_slots,
                                                priority_level, /* stack_size = */ 0);
                ...
                a->my_numa_binding_observer = construct_binding_observer(
                        static_cast<d1::task_arena*>(&ta), a->my_num_slots, ta.my_numa_id, ta.core_type(),
                ...
                nested_arena_context(thread_data& td, arena& nested_arena, std::size_t slot_index)
                    ...
                    if (td.my_arena != &nested_arena) {
                        ...
                        if (td.my_arena_index >= td.my_arena->my_num_reserved_slots) {
                            td.my_arena->my_market->adjust_demand(*td.my_arena, /* delta = */ -1,
                                                                  /* mandatory = */ false);
                        }
