    ...
    namespace detail {
        namespace r1 {
            ...
            void observer_list::clear() {
                {
                    ...
                    while (observer_proxy* p = next) {
                        ...
                        if (!obs || !(p = obs->my_proxy.exchange(nullptr))) { continue; }
                        ...
                        __TBB_ASSERT(p->my_ref_count.load(std::memory_order_relaxed) == 1,
                ...
                for (atomic_backoff backoff;; backoff.pause()) {
                    ...
                    if (my_head.load(std::memory_order_relaxed) == nullptr) { break; }
                }

                __TBB_ASSERT(my_head.load(std::memory_order_relaxed) == nullptr
                                     && my_tail.load(std::memory_order_relaxed) == nullptr,
                             nullptr);
            }
            ...
            void observer_list::remove(observer_proxy* p) {
                __TBB_ASSERT(my_head.load(std::memory_order_relaxed),
                             "Attempt to remove an item from an empty list");
                __TBB_ASSERT(!my_tail.load(std::memory_order_relaxed)->my_next,
                             "Last item's my_next must be nullptr");
                ...
                __TBB_ASSERT(
                        (my_head.load(std::memory_order_relaxed) && my_tail.load(std::memory_order_relaxed))
            ...
            void observer_list::remove_ref(observer_proxy* p) {
                ...
                while (r > 1) {
                    if (p->my_ref_count.compare_exchange_strong(r, r - 1)) { return; }
                }
                    ...
                    // Hold lock on list only long enough to advance to the next proxy in the list.
                    {
                        ...
                        do {
                            if (p) {
                                // We were already processing the list.
                                if (p != last) {
                                    __TBB_ASSERT(p->my_next,
                                                 "List items before 'last' must have valid my_next pointer");
                                    if (p == prev) remove_ref_fast(prev);// sets prev to nullptr if successful
                                    p = p->my_next;
                                } else {
                                    ...
                                    if (p) {
                                        lock.release();
                                        if (p != prev && prev) { remove_ref(prev); }
                                        remove_ref(p);
                                    }
                                    return;
                                }
                                ...
                                p = my_head.load(std::memory_order_relaxed);
                                __TBB_ASSERT(
                                        p, "Nonzero 'last' must guarantee that the global list is non-empty");
                            }
                            tso = p->my_observer;
                        } while (!tso);
                ...
                if (enable) {
                    if (!tso.my_proxy.load(std::memory_order_relaxed)) {
                        ...
                        if (p->my_observer->my_task_arena == nullptr) {
                            if (!(td && td->my_arena)) { td = governor::get_thread_data(); }
                    ...
                    if (observer_proxy* proxy = tso.my_proxy.exchange(nullptr)) {
                        ...
                        __TBB_ASSERT(proxy->my_ref_count.load(std::memory_order_relaxed) >= 1,
                        ...
                        spin_wait_until_eq(tso.my_busy_count,
                                           0);// other threads are still accessing the callback
                    }
                }
            }
