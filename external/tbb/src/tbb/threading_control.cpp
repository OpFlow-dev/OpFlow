    ...
    namespace detail {
        namespace r1 {
            ...
            std::pair<unsigned, unsigned> threading_control_impl::calculate_workers_limits() {
                ...
                unsigned workers_app_limit
                        = global_control_active_value_unsafe(global_control::max_allowed_parallelism);
                unsigned workers_hard_limit
                        = max(max(factor * governor::default_num_threads(), 256u), workers_app_limit);
            ...
            cache_aligned_unique_ptr<permit_manager>
            threading_control_impl::make_permit_manager(unsigned workers_soft_limit) {
                if (tcm_adaptor::is_initialized()) {
                    auto tcm = make_cache_aligned_unique<tcm_adaptor>();
                    if (tcm->is_connected()) { return tcm; }
                }
                return make_cache_aligned_unique<market>(workers_soft_limit);
            }

            cache_aligned_unique_ptr<thread_dispatcher>
            threading_control_impl::make_thread_dispatcher(threading_control& tc, unsigned workers_soft_limit,
                                                           unsigned workers_hard_limit) {
                stack_size_type stack_size
                        = global_control_active_value_unsafe(global_control::thread_stack_size);
            ...
            threading_control_impl::threading_control_impl(threading_control* tc) {
                ...
                my_thread_request_serializer = make_cache_aligned_unique<thread_request_serializer_proxy>(
                        *my_thread_dispatcher, workers_soft_limit);
            ...
            threading_control_impl::client_snapshot
            threading_control_impl::prepare_client_destruction(threading_control_client client) {
                auto td_client = client.get_thread_dispatcher_client();
                return {td_client->get_aba_epoch(), td_client->priority_level(), td_client,
                        client.get_pm_client()};
            }

            bool
            threading_control_impl::try_destroy_client(threading_control_impl::client_snapshot snapshot) {
                if (my_thread_dispatcher->try_unregister_client(snapshot.my_td_client, snapshot.aba_epoch,
                                                                snapshot.priority_level)) {
            ...
            void threading_control_impl::propagate_task_group_state(
                    std::atomic<uint32_t> d1::task_group_context::*mptr_state, d1::task_group_context& src,
                    uint32_t new_state) {
            ...
            void threading_control_impl::adjust_demand(threading_control_client tc_client,
                                                       int mandatory_delta, int workers_delta) {
            ...
            bool threading_control_impl::is_any_other_client_active() {
                return my_thread_request_serializer->num_workers_requested() > 0
                               ? my_thread_dispatcher->is_any_client_in_need()
                               : false;
            }
            ...
            void threading_control::add_ref(bool is_public) {
                ++my_ref_count;
                if (is_public) { my_public_ref_count++; }
            }

            bool threading_control::remove_ref(bool is_public) {
                if (is_public) {
                    __TBB_ASSERT(g_threading_control == this,
                                 "Global threading control instance was destroyed prematurely?");
            ...
            threading_control* threading_control::get_threading_control(bool is_public) {
                ...
                if (control) { control->add_ref(is_public); }
            ...
            threading_control* threading_control::create_threading_control() {
                ...
                try_call([&] {
                    ...
                    if (thr_control == nullptr) {
                        thr_control = new (cache_aligned_allocate(sizeof(threading_control)))
                                threading_control(/*public_ref = */ 1, /*private_ref = */ 1);
                        thr_control->my_pimpl
                                = make_cache_aligned_unique<threading_control_impl>(thr_control);
            ...
            bool threading_control::release(bool is_public, bool blocking_terminate) {
                ...
                if (do_release) {
                    __TBB_ASSERT(!my_public_ref_count.load(std::memory_order_relaxed),
                                 "No public references must remain if we remove the threading control.");
            ...
            threading_control::threading_control(unsigned public_ref, unsigned ref)
                : my_public_ref_count(public_ref), my_ref_count(ref) {}
            ...
            bool threading_control::unregister_public_reference(bool blocking_terminate) {
                __TBB_ASSERT(g_threading_control,
                             "Threading control should exist until last public reference");
                __TBB_ASSERT(g_threading_control->my_public_ref_count.load(std::memory_order_relaxed),
                             nullptr);
                return g_threading_control->release(/*public = */ true,
                                                    /*blocking_terminate = */ blocking_terminate);
            }
            ...
            threading_control::client_snapshot
            threading_control::prepare_client_destruction(threading_control_client client) {
            ...
            bool threading_control::try_destroy_client(threading_control::client_snapshot deleter) {
                ...
                if (res) { release(/*public = */ false, /*blocking_terminate = */ false); }
                return res;
            }
            ...
            bool threading_control::unregister_lifetime_control(bool blocking_terminate) {
                ...
                bool released {true};
                if (thr_control) {
                    released = thr_control->release(/*public = */ true,
                                                    /*blocking_terminate = */ blocking_terminate);
                }
            ...
            void threading_control::register_thread(thread_data& td) { my_pimpl->register_thread(td); }

            void threading_control::unregister_thread(thread_data& td) { my_pimpl->unregister_thread(td); }

            void threading_control::propagate_task_group_state(
                    std::atomic<uint32_t> d1::task_group_context::*mptr_state, d1::task_group_context& src,
                    uint32_t new_state) {
                my_pimpl->propagate_task_group_state(mptr_state, src, new_state);
            }

            std::size_t threading_control::worker_stack_size() { return my_pimpl->worker_stack_size(); }
            ...
            void threading_control::adjust_demand(threading_control_client client, int mandatory_delta,
                                                  int workers_delta) {
