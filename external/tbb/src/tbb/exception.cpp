...
namespace tbb {
    namespace detail {
        namespace r1 {
            ...
            const char* bad_last_alloc::what() const noexcept(true) {
                return "bad allocation in previous or concurrent attempt";
            }
            const char* user_abort::what() const noexcept(true) {
                return "User-initiated abort has terminated this operation";
            }
            const char* missing_wait::what() const noexcept(true) {
            ...
            /*[[noreturn]]*/ void do_throw(F throw_func) {
                if (terminate_on_exception()) { do_throw_noexcept(throw_func); }
                throw_func();
            }
...
#define DO_THROW(exc, init_args) PRINT_ERROR_AND_ABORT(#exc, #init_args)
            ...
            void throw_exception(exception_id eid) {
                switch (eid) {
                    case exception_id::bad_alloc:
                        DO_THROW(std::bad_alloc, ());
                        break;
                    case exception_id::bad_last_alloc:
                        DO_THROW(bad_last_alloc, ());
                        break;
                    case exception_id::user_abort:
                        DO_THROW(user_abort, ());
                        break;
                    case exception_id::nonpositive_step:
                        DO_THROW(std::invalid_argument, ("Step must be positive"));
                        break;
                    case exception_id::out_of_range:
                        DO_THROW(std::out_of_range, ("Index out of requested size range"));
                        break;
                    case exception_id::reservation_length_error:
                        DO_THROW(std::length_error,
                                 ("Attempt to exceed implementation defined length limits"));
                        break;
                    case exception_id::missing_wait:
                        DO_THROW(missing_wait, ());
                        break;
                    case exception_id::invalid_load_factor:
                        DO_THROW(std::out_of_range, ("Invalid hash load factor"));
                        break;
                    case exception_id::invalid_key:
                        DO_THROW(std::out_of_range, ("invalid key"));
                        break;
                    case exception_id::bad_tagged_msg_cast:
                        DO_THROW(std::runtime_error, ("Illegal tagged_msg cast"));
                        break;
#if __TBB_SUPPORTS_WORKERS_WAITING_IN_TERMINATE
                    case exception_id::unsafe_wait:
                        DO_THROW(unsafe_wait, ("Unsafe to wait further"));
                        break;
#endif
#if __TBB_PREVIEW_TASK_GROUP_EXTENSIONS
                    case exception_id::bad_task_handle:
                        DO_THROW(std::runtime_error, ("Attempt to schedule empty task_handle"));
                        break;
                    case exception_id::bad_task_handle_wrong_task_group:
                        DO_THROW(std::runtime_error,
                                 ("Attempt to schedule task_handle into different task_group"));
                        break;
#endif
                    default:
                        __TBB_ASSERT(false, "Unknown exception ID");
                }
