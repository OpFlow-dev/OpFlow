...
//------------------------------------------------------------------------
            ...
            void resume(suspend_point_type* sp) {
                ...
                if (sp->try_notify_resume()) {
                    ...
                    if (task_disp.m_properties.critical_task_allowed) {
                        ...
                        a.my_critical_task_stream.push(&sp->m_resume_task,
                                                       random_lane_selector(sp->m_random));
#endif
                    }
            ...
            task_dispatcher& create_coroutine(thread_data& td) {
                ...
                if (!task_disp) {
                    ...
                    task_disp->init_suspend_point(td.my_arena,
                                                  td.my_arena->my_threading_control->worker_stack_size());
                }
            ...
            void task_dispatcher::internal_suspend() {
                ...
                bool is_recalled = default_task_disp.get_suspend_point()->m_is_owner_recalled.load(
                        std::memory_order_acquire);
                task_dispatcher& target = is_recalled ? default_task_disp : create_coroutine(*m_thread_data);

                resume(target);

                if (m_properties.outermost) { recall_point(); }
            }
            ...
            bool task_dispatcher::resume(task_dispatcher& target) {
                // Do not create non-trivial objects on the stack of this function. They might never be destroyed
                {
                    ...
                    __TBB_ASSERT(td->my_task_dispatcher == this,
                                 "Thread data must be attached to this task dispatcher");
            ...
            void task_dispatcher::do_post_resume_action() {
                ...
                switch (td->my_post_resume_action) {
                    case post_resume_action::register_waiter: {
                        ...
                        static_cast<thread_control_monitor::resume_context*>(td->my_post_resume_arg)
                                ->notify();
                        break;
                    }
                    ...
                    case post_resume_action::notify: {
                        ...
                        auto is_our_suspend_point
                                = [sp](market_context ctx) { return std::uintptr_t(sp) == ctx.my_uniq_addr; };
                        td->my_arena->get_waiting_threads_monitor().notify(is_our_suspend_point);
                        break;
                    }
                    default:
                        __TBB_ASSERT(td->my_post_resume_action == post_resume_action::none,
                                     "Unknown post resume action");
                        __TBB_ASSERT(td->my_post_resume_arg == nullptr,
                                     "The post resume argument should not be set");
                }
            ...
            void notify_waiters(std::uintptr_t wait_ctx_addr) {
                auto is_related_wait_ctx
                        = [&](market_context context) { return wait_ctx_addr == context.my_uniq_addr; };

                governor::get_thread_data()->my_arena->get_waiting_threads_monitor().notify(
                        is_related_wait_ctx);
            }

            d1::wait_tree_vertex_interface*
            get_thread_reference_vertex(d1::wait_tree_vertex_interface* top_wait_context) {
                ...
                if (pos != reference_map.end()) {
                    ...
                    reference_map[top_wait_context] = ref_counter
                            = new (cache_aligned_allocate(sizeof(d1::reference_vertex)))
                                    d1::reference_vertex(top_wait_context, 0);
                }
