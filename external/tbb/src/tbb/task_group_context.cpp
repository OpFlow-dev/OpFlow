...
namespace tbb {
        ...
        namespace r1 {
            ...
            void task_group_context_impl::destroy(d1::task_group_context& ctx) {
                ...
                if (ctx.my_context_list != nullptr) {
                    __TBB_ASSERT(ctx.my_lifetime_state.load(std::memory_order_relaxed)
                                         == d1::task_group_context::lifetime_state::bound,
                                 nullptr);
                ...
                if (exception) { exception->destroy(); }
                ...
                ctx.my_lifetime_state.store(d1::task_group_context::lifetime_state::dead,
            ...
            void task_group_context_impl::initialize(d1::task_group_context& ctx) {
                ...
                ctx.my_lifetime_state.store(d1::task_group_context::lifetime_state::created,
                ...
                static_assert(sizeof(d1::cpu_ctl_env) <= sizeof(ctx.my_cpu_ctl_env),
                              "FPU settings storage does not fit to uint64_t");
                d1::cpu_ctl_env* ctl = new (&ctx.my_cpu_ctl_env) d1::cpu_ctl_env;
                if (ctx.my_traits.fp_settings) ctl->get_env();
            }
            ...
            void task_group_context_impl::bind_to_impl(d1::task_group_context& ctx, thread_data* td) {
                __TBB_ASSERT(!is_poisoned(ctx.my_context_list), nullptr);
                __TBB_ASSERT(ctx.my_lifetime_state.load(std::memory_order_relaxed)
                                     == d1::task_group_context::lifetime_state::locked,
                ...
                if (!ctx.my_traits.fp_settings) copy_fp_settings(ctx, *ctx.my_parent);

                // Condition below prevents unnecessary thrashing parent context's cache line
                if (ctx.my_parent->my_state.load(std::memory_order_relaxed)
                    != d1::task_group_context::may_have_children) {
                    ctx.my_parent->my_state.store(d1::task_group_context::may_have_children,
                                                  std::memory_order_relaxed);// full fence is below
                }
                if (ctx.my_parent->my_parent) {
                    ...
                    uintptr_t local_count_snapshot
                            = ctx.my_parent->my_context_list->epoch.load(std::memory_order_acquire);
                    // Speculative propagation of parent's state. The speculation will be
                    // validated by the epoch counters check further on.
                    ctx.my_cancellation_requested.store(
                            ctx.my_parent->my_cancellation_requested.load(std::memory_order_relaxed),
                            std::memory_order_relaxed);
                    ...
                    if (local_count_snapshot
                        != the_context_state_propagation_epoch.load(std::memory_order_relaxed)) {
                        // Another thread may be propagating state change right now. So resort to lock.
                        context_state_propagation_mutex_type::scoped_lock lock(
                                the_context_state_propagation_mutex);
                        ctx.my_cancellation_requested.store(
                                ctx.my_parent->my_cancellation_requested.load(std::memory_order_relaxed),
                                std::memory_order_relaxed);
                    }
            ...
            void task_group_context_impl::bind_to(d1::task_group_context& ctx, thread_data* td) {
                __TBB_ASSERT(!is_poisoned(ctx.my_context_list), nullptr);
                d1::task_group_context::lifetime_state state
                        = ctx.my_lifetime_state.load(std::memory_order_acquire);
                if (state <= d1::task_group_context::lifetime_state::locked) {
                    if (state == d1::task_group_context::lifetime_state::created &&
                        ...
                        ((std::atomic<typename std::underlying_type<
                                  d1::task_group_context::lifetime_state>::type>&) ctx.my_lifetime_state)
                                .compare_exchange_strong(
                                        (typename std::underlying_type<
                                                d1::task_group_context::lifetime_state>::type&) state,
                                        (typename std::underlying_type<
                                                d1::task_group_context::lifetime_state>::type)
                                                d1::task_group_context::lifetime_state::locked)
#else
                        ctx.my_lifetime_state.compare_exchange_strong(
                                state, d1::task_group_context::lifetime_state::locked)
#endif
                    ) {
                ...
                __TBB_ASSERT(ctx.my_lifetime_state.load(std::memory_order_relaxed)
                                     != d1::task_group_context::lifetime_state::created,
                             nullptr);
                __TBB_ASSERT(ctx.my_lifetime_state.load(std::memory_order_relaxed)
                                     != d1::task_group_context::lifetime_state::locked,
                             nullptr);
            ...
            void task_group_context_impl::propagate_task_group_state(
                    d1::task_group_context& ctx, std::atomic<T> d1::task_group_context::*mptr_state,
                    d1::task_group_context& src, T new_state) {
                __TBB_ASSERT(!is_poisoned(ctx.my_context_list), nullptr);
    ...
    */
                if ((ctx.*mptr_state).load(std::memory_order_relaxed) != new_state && &ctx != &src) {
                    for (d1::task_group_context* ancestor = ctx.my_parent; ancestor != nullptr;
            ...
            template <typename T>
            void thread_data::propagate_task_group_state(std::atomic<T> d1::task_group_context::*mptr_state,
                                                         d1::task_group_context& src, T new_state) {
                ...
                for (context_list::iterator it = my_context_list->begin(); it != my_context_list->end();
                     ++it) {
                    d1::task_group_context& ctx
                            = __TBB_get_object_ref(d1::task_group_context, my_node, &(*it));
                ...
                my_context_list->epoch.store(
                        the_context_state_propagation_epoch.load(std::memory_order_relaxed),
                        std::memory_order_release);
            }

            template <typename T>
            bool market::propagate_task_group_state(std::atomic<T> d1::task_group_context::*mptr_state,
                                                    d1::task_group_context& src, T new_state) {
                ...
                for (unsigned i = 0; i < num_workers; ++i) {
                    ...
                    if (td) td->propagate_task_group_state(mptr_state, src, new_state);
                }
            ...
            bool task_group_context_impl::cancel_group_execution(d1::task_group_context& ctx) {
                __TBB_ASSERT(!is_poisoned(ctx.my_context_list), nullptr);
                __TBB_ASSERT(ctx.my_cancellation_requested.load(std::memory_order_relaxed) <= 1,
                             "The cancellation state can be either 0 or 1");
                if (ctx.my_cancellation_requested.load(std::memory_order_relaxed)
                    || ctx.my_cancellation_requested.exchange(1)) {
                ...
                governor::get_thread_data()->my_arena->my_market->propagate_task_group_state(
                        &d1::task_group_context::my_cancellation_requested, ctx, uint32_t(1));
                return true;
            }
            ...
            void task_group_context_impl::copy_fp_settings(d1::task_group_context& ctx,
                                                           const d1::task_group_context& src) {
                __TBB_ASSERT(!is_poisoned(ctx.my_context_list), nullptr);
                ...
                const d1::cpu_ctl_env* src_ctl
                        = reinterpret_cast<const d1::cpu_ctl_env*>(&src.my_cpu_ctl_env);
