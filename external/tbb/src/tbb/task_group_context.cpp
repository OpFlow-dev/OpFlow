...
namespace tbb {
    namespace detail {
        namespace r1 {
            ...
            void task_group_context_impl::destroy(d1::task_group_context& ctx) {
                ...
                if (ctx.my_context_list != nullptr) {
                    __TBB_ASSERT(ctx.my_state.load(std::memory_order_relaxed)
                                         == d1::task_group_context::state::bound,
                                 nullptr);
            ...
            void task_group_context_impl::bind_to_impl(d1::task_group_context& ctx, thread_data* td) {
                __TBB_ASSERT(!is_poisoned(ctx.my_context_list), nullptr);
                __TBB_ASSERT(ctx.my_state.load(std::memory_order_relaxed)
                                     == d1::task_group_context::state::locked,
                ...
                if (ctx.my_parent->my_may_have_children.load(std::memory_order_relaxed)
                    ...
                    ctx.my_parent->my_may_have_children.store(
                            d1::task_group_context::may_have_children,
            ...
            void task_group_context_impl::bind_to(d1::task_group_context& ctx, thread_data* td) {
                ...
                if (state <= d1::task_group_context::state::locked) {
                    if (state == d1::task_group_context::state::created &&
                        ...
                        ((std::atomic<typename std::underlying_type<d1::task_group_context::state>::type>&)
                                 ctx.my_state)
                                .compare_exchange_strong(
                                        (typename std::underlying_type<d1::task_group_context::state>::type&)
                                                state,
                                        (typename std::underlying_type<d1::task_group_context::state>::type)
                                                d1::task_group_context::state::locked)
#else
                ...
                __TBB_ASSERT(ctx.my_state.load(std::memory_order_relaxed)
                                     != d1::task_group_context::state::created,
                             nullptr);
                __TBB_ASSERT(ctx.my_state.load(std::memory_order_relaxed)
                                     != d1::task_group_context::state::locked,
                             nullptr);
            ...
            void task_group_context_impl::propagate_task_group_state(
                    ...
                    std::atomic<std::uint32_t> d1::task_group_context::*mptr_state,
                    d1::task_group_context& src, std::uint32_t new_state) {
                __TBB_ASSERT(!is_poisoned(ctx.my_context_list), nullptr);
    ...
    */
            ...
            bool task_group_context_impl::cancel_group_execution(d1::task_group_context& ctx) {
                ...
                governor::get_thread_data()->my_arena->my_threading_control->propagate_task_group_state(
