...
#pragma warning(disable : 4311 4312)
            ...
            public:
                ...
                static T* fetch_add(std::atomic<word>& location, word addend,
                ...
                static T* exchange(std::atomic<word>& location, T* value, std::memory_order memory_order) {
                    return reinterpret_cast<T*>(
                            location.exchange(reinterpret_cast<word>(value), memory_order));
                }

                static T* compare_exchange_strong(std::atomic<word>& obj, const T* expected, const T* desired,
                                                  std::memory_order memory_order) {
                ...
                static void store(std::atomic<word>& location, const T* value,
                                  std::memory_order memory_order) {
...
#pragma warning(pop)
            ...
            struct queuing_rw_mutex_impl {
                ...
                static void acquire_internal_lock(d1::queuing_rw_mutex::scoped_lock& s) {
                    ...
                    while (!try_acquire_internal_lock(s)) { machine_pause(1); }
                }
                ...
                //! A helper function
                static void unblock_or_wait_on_internal_lock(d1::queuing_rw_mutex::scoped_lock& s,
                                                             uintptr_t flag) {
                    if (flag) {
                ...
                static void acquire(d1::queuing_rw_mutex& m, d1::queuing_rw_mutex::scoped_lock& s,
                                    bool write) {
                    ...
                    s.my_state.store(
                            d1::queuing_rw_mutex::scoped_lock::state_t(write ? STATE_WRITER : STATE_READER),
                    ...
                    queuing_rw_mutex::scoped_lock* predecessor
                            = m.q_tail.exchange(&s, std::memory_order_acq_rel);
                    ...
                    if (write) {// Acquiring for write
                        ...
                        if (predecessor) {
                            ...
                            __TBB_ASSERT(!s.my_prev.load(std::memory_order_relaxed),
                                         "the predecessor is already set");
                            if (tricky_pointer(predecessor) & FLAG) {
                                ...
                                if (pred_state == STATE_READER) {
                                    ...
                                    predecessor->my_state.compare_exchange_strong(
                                            pred_state, STATE_READER_UNBLOCKNEXT, std::memory_order_relaxed);
                                }
                                if (pred_state
                                    == STATE_ACTIVEREADER) {// either we initially read it or CAS failed
                            ...
                            __TBB_ASSERT(!predecessor->my_next.load(std::memory_order_relaxed),
                        ...
                        s.my_state.compare_exchange_strong(old_state, STATE_ACTIVEREADER,
                                                           std::memory_order_release,
                                                           std::memory_order_relaxed);
                        if (old_state != STATE_READER) {
#if __TBB_USE_ITT_NOTIFY
                            if (!sync_prepare_done) ITT_NOTIFY(sync_prepare, s.my_mutex);
#endif
                            // Failed to become active reader -> need to unblock the next waiting reader first
                            __TBB_ASSERT(s.my_state.load(std::memory_order_relaxed)
                   ...
                   and another thread can get our old state and left blocked */
                            ...
                            tricky_pointer::load(s.my_next, std::memory_order_relaxed)
                                    ->my_going.store(1U, std::memory_order_release);
                        }
                        __TBB_ASSERT(s.my_state.load(std::memory_order_relaxed) == STATE_ACTIVEREADER,
                                     "unlocked reader is active reader");
                    }
                ...
                static bool try_acquire(d1::queuing_rw_mutex& m, d1::queuing_rw_mutex::scoped_lock& s,
                                        bool write) {
                    __TBB_ASSERT(!s.my_mutex, "scoped_lock is already holding a mutex");
                    ...
                    s.my_state.store(d1::queuing_rw_mutex::scoped_lock::state_t(write ? STATE_WRITER
                                                                                      : STATE_ACTIVEREADER),
                                     std::memory_order_relaxed);
                    s.my_internal_lock.store(RELEASED, std::memory_order_relaxed);
                ...
                static void release(d1::queuing_rw_mutex::scoped_lock& s) {
                    ...
                    if (s.my_state.load(std::memory_order_relaxed) == STATE_WRITER) {// Acquired for write
                        ...
                        d1::queuing_rw_mutex::scoped_lock* next
                                = tricky_pointer::load(s.my_next, std::memory_order_acquire);
                        if (!next) {
                        ...
                        next->my_going.store(
                                2U,
                                std::memory_order_relaxed);// protect next queue node from being destroyed too early
                        ...
                        if (next->my_state.load(std::memory_order_acquire) == STATE_UPGRADE_WAITING) {
                            ...
                            d1::queuing_rw_mutex::scoped_lock* tmp = tricky_pointer::exchange(
                                    next->my_prev, nullptr, std::memory_order_release);
                            ...
                            __TBB_ASSERT(next->my_state.load(std::memory_order_relaxed)
                                                 & (STATE_COMBINED_WAITINGREADER | STATE_WRITER),
                                         "unexpected state");
                            __TBB_ASSERT(!(next->my_prev.load(std::memory_order_relaxed) & FLAG),
                                         "use of corrupted pointer!");
                            // Guarantee that above store of 2 into next->my_going happens-before resetting of next->my_prev
                    ...
                    retry:
                        ...
                        queuing_rw_mutex::scoped_lock* predecessor
                                = tricky_pointer::fetch_add(s.my_prev, FLAG, std::memory_order_acquire);
                        ...
                        if (predecessor) {
                            if (!(try_acquire_internal_lock(*predecessor))) {
                                ...
                                // Responsibility transition, the one who reads uncorrupted my_prev will do release.
                                tmp = tricky_pointer::compare_exchange_strong(
                                        s.my_prev, tricky_pointer(predecessor) | FLAG, predecessor,
                            ...
                            __TBB_ASSERT(
                                    predecessor
                                            && predecessor->my_internal_lock.load(std::memory_order_relaxed)
                                                       == ACQUIRED,
                                    "predecessor's lock is not acquired");
                            tricky_pointer::store(s.my_prev, predecessor, std::memory_order_relaxed);
                            acquire_internal_lock(s);
                            ...
                            d1::queuing_rw_mutex::scoped_lock* expected = &s;
                            if (!tricky_pointer::load(s.my_next, std::memory_order_acquire)
                                && !s.my_mutex->q_tail.compare_exchange_strong(expected, predecessor,
                                                                               std::memory_order_release)) {
                                spin_wait_while_eq(s.my_next, 0U, std::memory_order_acquire);
                            }
                            __TBB_ASSERT(!(s.my_next.load(std::memory_order_relaxed) & FLAG),
                                         "use of corrupted pointer");

                            // my_next is acquired either with load or spin_wait.
                            if (d1::queuing_rw_mutex::scoped_lock* const l_next = tricky_pointer::load(
                                        s.my_next,
                                        std::memory_order_relaxed)) {// I->next != nil, TODO: rename to next after clearing up and adapting the n in the comment two lines below
                                // Equivalent to I->next->prev = I->prev but protected against (prev[n]&FLAG)!=0
                                tmp = tricky_pointer::exchange(l_next->my_prev, predecessor,
                                                               std::memory_order_release);
                                // I->prev->next = I->next;
                                __TBB_ASSERT(tricky_pointer::load(s.my_prev, std::memory_order_relaxed)
                                                     == predecessor,
                                             nullptr);
                                predecessor->my_next.store(s.my_next.load(std::memory_order_relaxed),
                                                           std::memory_order_release);
                            }
                ...
                static bool downgrade_to_reader(d1::queuing_rw_mutex::scoped_lock& s) {
                    if (s.my_state.load(std::memory_order_relaxed) == STATE_ACTIVEREADER)
                    ...
                    if (!next) {
                        ...
                        if (&s == s.my_mutex->q_tail.load(std::memory_order_seq_cst)) {
                            ...
                            s.my_state.compare_exchange_strong(old_state, STATE_ACTIVEREADER,
                                                               std::memory_order_release,
                                                               std::memory_order_relaxed);
                            if (old_state == STATE_READER) return true;// Downgrade completed
                        }
                ...
                static bool upgrade_to_writer(d1::queuing_rw_mutex::scoped_lock& s) {
                    ...
                    __TBB_ASSERT(s.my_state.load(std::memory_order_relaxed) == STATE_ACTIVEREADER,
                                 "only active reader can be updated");
                ...
                requested:
                    ...
                    acquire_internal_lock(s);
                    d1::queuing_rw_mutex::scoped_lock* expected = &s;
                    if (!s.my_mutex->q_tail.compare_exchange_strong(expected, tricky_pointer(me) | FLAG,
                        ...
                        if (n_state & (STATE_COMBINED_READER | STATE_UPGRADE_REQUESTED)) {
                            ...
                            for (atomic_backoff b;
                                 tricky_pointer::load(s.my_next, std::memory_order_relaxed) == tmp;
                                 b.pause()) {
                            ...
                            __TBB_ASSERT(tricky_pointer::load(s.my_next, std::memory_order_relaxed)
                                                 != (tricky_pointer(next) | FLAG),
                                         nullptr);
                            goto requested;
                        } else {
                            __TBB_ASSERT(n_state & (STATE_WRITER | STATE_UPGRADE_WAITING),
                                         "unexpected state");
                            __TBB_ASSERT(
                                    (tricky_pointer(next) | FLAG)
                                            == tricky_pointer::load(s.my_next, std::memory_order_relaxed),
                    ...
                    }// if( this != my_mutex->q_tail... )
                    {
                        ...
                        s.my_state.compare_exchange_strong(old_state, STATE_UPGRADE_WAITING,
                                                           std::memory_order_release,
                                                           std::memory_order_relaxed);
                    }
                waiting:
                    __TBB_ASSERT(!(s.my_next.load(std::memory_order_relaxed) & FLAG),
                                 "use of corrupted pointer!");
                    __TBB_ASSERT(s.my_state & STATE_COMBINED_UPGRADING,
                                 "wrong state at upgrade waiting_retry");
                    ...
                    if (predecessor) {
                        bool success = try_acquire_internal_lock(*predecessor);
                        {
                            ...
                            predecessor->my_state.compare_exchange_strong(old_state, STATE_UPGRADE_WAITING,
                                                                          std::memory_order_release,
                                                                          std::memory_order_relaxed);
                        }
                        if (!success) {
                            ...
                            if (tricky_pointer(tmp) & FLAG) {
                                ...
                                tricky_pointer::spin_wait_while_eq(s.my_prev,
                                                                   tricky_pointer(predecessor) | FLAG);
                                release_internal_lock(*predecessor);
                            }
                        } else {
            ...
            void __TBB_EXPORTED_FUNC acquire(d1::queuing_rw_mutex& m, d1::queuing_rw_mutex::scoped_lock& s,
                                             bool write) {
                queuing_rw_mutex_impl::acquire(m, s, write);
            }

            bool __TBB_EXPORTED_FUNC try_acquire(d1::queuing_rw_mutex& m,
                                                 d1::queuing_rw_mutex::scoped_lock& s, bool write) {
