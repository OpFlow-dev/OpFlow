    ...
    namespace detail {
        namespace r1 {
            ...
            thread_request_serializer::thread_request_serializer(thread_dispatcher& td, int soft_limit)
                : my_thread_dispatcher(td), my_soft_limit(soft_limit) {}
            ...
            void thread_request_serializer::update(int delta) {
                ...
                if (prev_pending_delta == pending_delta_base) {
                    delta = int(my_pending_delta.exchange(pending_delta_base) & delta_mask)
                            - int(pending_delta_base);
                    mutex_type::scoped_lock lock(my_mutex);
                    my_total_request.store(my_total_request.load(std::memory_order_relaxed) + delta,
                                           std::memory_order_relaxed);
                    delta = limit_delta(delta, my_soft_limit,
                                        my_total_request.load(std::memory_order_relaxed));
            ...
            thread_request_serializer_proxy::thread_request_serializer_proxy(thread_dispatcher& td,
                                                                             int soft_limit)
                : my_serializer(td, soft_limit) {}
            ...
            int thread_request_serializer_proxy::num_workers_requested() {
                return my_serializer.num_workers_requested();
            }

            void thread_request_serializer_proxy::update(int delta) { my_serializer.update(delta); }

            void
            thread_request_serializer_proxy::enable_mandatory_concurrency(mutex_type::scoped_lock& lock) {
                lock.upgrade_to_writer();
                bool still_should_enable = my_num_mandatory_requests.load(std::memory_order_relaxed) > 0
                                           && !my_is_mandatory_concurrency_enabled
                                           && my_serializer.is_no_workers_avaliable();
            ...
            void
            thread_request_serializer_proxy::disable_mandatory_concurrency(mutex_type::scoped_lock& lock) {
                lock.upgrade_to_writer();
