    ...
    namespace detail {
        namespace r1 {
...
#endif
                if (status) handle_perror(status, "TBB failed to initialize task scheduler TLS\n");
            ...
            void governor::release_resources() {
                ...
                __TBB_ASSERT(!(__TBB_InitOnce::initialization_done() && theTLS.get()),
                             "TBB is unloaded while thread data still alive?");
            ...
            rml::tbb_server* governor::create_rml_server(rml::tbb_client& client) {
                ...
                if (!UsePrivateRML) {
                    ...
                    if (status != ::rml::factory::st_success) {
                        UsePrivateRML = true;
                        runtime_warning("rml::tbb_factory::make_server failed with status %x, falling back "
                                        "on private rml",
                                        status);
                    }
                }
            ...
            void governor::one_time_init() {
                if (!__TBB_InitOnce::initialization_done()) { DoOneTimeInitialization(); }
            }
...
*/
            static void get_stack_attributes(std::uintptr_t& stack_base, std::size_t& stack_size,
                                             std::size_t fallback_stack_size) {
                ...
                if (0 == pthread_getattr_np(pthread_self(), &np_attr_stack)) {
                    if (0 == pthread_attr_getstack(&np_attr_stack, &stack_limit, &np_stack_size)) {
                        ...
                        if (np_stack_size > 0) stack_size = np_stack_size;
                    }
            ...
            static void register_external_thread_destructor() {
                struct thread_destructor {
                    ~thread_destructor() { governor::terminate_external_thread(); }
                };
            ...
            void governor::auto_terminate(void* tls) {
                __TBB_ASSERT(get_thread_data_if_initialized() == nullptr
                                     || get_thread_data_if_initialized() == tls,
                             nullptr);
                if (tls) {
                    ...
                    if (td->my_arena_slot) {
                        ...
                        if (!is_thread_data_set(td)) { set_thread_data(*td); }
            ...
            void __TBB_EXPORTED_FUNC get(d1::task_scheduler_handle& handle) {
                handle.m_ctl = new (allocate_memory(sizeof(global_control)))
                        global_control(global_control::scheduler_handle, 1);
            }
            ...
            bool finalize_impl(d1::task_scheduler_handle& handle) {
                ...
                __TBB_ASSERT(is_present(*handle.m_ctl),
                             "finalize or release was already called on this object");
                ...
                if (threading_control::is_present()) {
                    ...
                    if (td) {
                        ...
                        if (task_disp->m_properties.outermost
                            && !td->my_is_worker) {// is not inside a parallel region
            ...
            bool __TBB_EXPORTED_FUNC finalize(d1::task_scheduler_handle& handle, std::intptr_t mode) {
                if (mode == d1::release_nothrowing) {
                    ...
                    release_impl(handle);
                    if (mode == d1::finalize_throwing && !ok) { throw_exception(exception_id::unsafe_wait); }
                    return ok;
                }
            }
...
#pragma weak __TBB_internal_get_default_concurrency
            ...
            extern "C" {
            void __TBB_internal_initialize_system_topology(size_t groups_num, int& numa_nodes_count,
                                                           int*& numa_indexes_list, int& core_types_count,
            ...
            binding_handler* __TBB_internal_allocate_binding_handler(int slot_num, int numa_id,
                                                                     int core_type_id,
            ...
            int __TBB_internal_get_default_concurrency(int numa_id, int core_type_id,
                                                       int max_threads_per_core);
            }
            ...
            static int dummy_get_default_concurrency(int, int, int) {
                return governor::default_num_threads();
            }

            // Handlers for communication with TBBbind
            static void (*initialize_system_topology_ptr)(size_t groups_num, int& numa_nodes_count,
                                                          int*& numa_indexes_list, int& core_types_count,
                                                          int*& core_types_indexes_list)
                    = nullptr;
            static void (*destroy_system_topology_ptr)() = dummy_destroy_system_topology;

            static binding_handler* (*allocate_binding_handler_ptr)(int slot_num, int numa_id,
                                                                    int core_type_id,
                                                                    int max_threads_per_core)
...
#define TBBBIND_2_5_NAME LIBRARY_PREFIX "tbbbind_2_5" DEBUG_SUFFIX LIBRARY_EXTENSION
            ...
            namespace system_topology {
...
#endif /* _WIN32 && !_WIN64 */
                        for (const auto& tbbbind_version :
                             {TBBBIND_2_5_NAME, TBBBIND_2_0_NAME, TBBBIND_NAME}) {
                            if (dynamic_link(tbbbind_version, TbbBindLinkTable, LinkTableSize, nullptr,
                ...
                void initialization_impl() {
                    ...
                    if (const char* tbbbind_name = load_tbbbind_shared_object()) {
                        initialize_system_topology_ptr(processor_groups_num(), numa_nodes_count,
                                                       numa_nodes_indexes, core_types_count,
                                                       core_types_indexes);
                ...
                void initialize() { atomic_do_once(initialization_impl, initialization_state); }

                void destroy() { destroy_system_topology_ptr(); }
            }// namespace system_topology

            binding_handler* construct_binding_handler(int slot_num, int numa_id, int core_type_id,
                                                       int max_threads_per_core) {
            ...
            void __TBB_EXPORTED_FUNC fill_numa_indices(int* index_array) {
                system_topology::initialize();
                std::memcpy(index_array, system_topology::numa_nodes_indexes,
                            system_topology::numa_nodes_count * sizeof(int));
            }

            int __TBB_EXPORTED_FUNC numa_default_concurrency(int node_id) {
                if (node_id >= 0) {
                    system_topology::initialize();
                    int result = get_default_concurrency_ptr(node_id,
            ...
            void __TBB_EXPORTED_FUNC fill_core_type_indices(int* index_array, intptr_t /*reserved*/) {
                system_topology::initialize();
                std::memcpy(index_array, system_topology::core_types_indexes,
                            system_topology::core_types_count * sizeof(int));
            }

            void constraints_assertion(d1::constraints c) {
                bool is_topology_initialized
                        = system_topology::initialization_state == do_once_state::initialized;
                __TBB_ASSERT_RELEASE(c.max_threads_per_core == system_topology::automatic
                ...
                __TBB_ASSERT_RELEASE(c.numa_id == system_topology::automatic
                                             || (is_topology_initialized
                                                 && std::find(numa_nodes_begin, numa_nodes_end, c.numa_id)
                                                            != numa_nodes_end),
                                     "The constraints::numa_id value is not known to the library. Use "
                                     "tbb::info::numa_nodes() to get the list of possible values.");
                ...
                __TBB_ASSERT_RELEASE(c.core_type == system_topology::automatic
                                             || (is_topology_initialized
                                                 && std::find(core_types_begin, core_types_end, c.core_type)
                                                            != core_types_end),
                                     "The constraints::core_type value is not known to the library. Use "
                                     "tbb::info::core_types() to get the list of possible values.");
            }

            int __TBB_EXPORTED_FUNC constraints_default_concurrency(const d1::constraints& c,
                                                                    intptr_t /*reserved*/) {
            ...
            int __TBB_EXPORTED_FUNC constraints_threads_per_core(const d1::constraints&,
                                                                 intptr_t /*reserved*/) {
