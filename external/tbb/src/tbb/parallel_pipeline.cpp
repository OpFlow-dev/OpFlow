        ...
        namespace r1 {
            ...
            public:
                ...
                pipeline(d1::task_group_context& cxt, std::size_t max_token)
                    : my_context(cxt), first_filter(nullptr), last_filter(nullptr),
                      input_tokens(Token(max_token)), end_of_input(false), wait_ctx(0) {
            ...
            public:
                ...
                input_buffer(bool ordered)
                    : array(nullptr), array_size(0), low_token(0), high_token(0), is_ordered(ordered),
                      end_of_input_tls(), end_of_input_tls_allocated(false) {
                ...
                ~input_buffer() {
                    ...
                    if (end_of_input_tls_allocated) { destroy_my_tls(); }
                }

                //! Define order when the first filter is serial_in_order.
                Token get_ordered_token() { return high_token++; }
    ...
    */
                bool try_put_token(task_info& info) {
                    ...
                    if (token != low_token) {
                        ...
                        if (token - low_token >= array_size) grow(token - low_token + 1);
                ...
                void try_to_spawn_task_for_next_token(StageTask& spawner, d1::execution_data& ed) {
                    ...
                    if (wakee.is_valid) spawner.spawn_stage_task(wakee, ed);
                }

                // end_of_input signal for parallel_pipeline, parallel input filters with 0 tokens allowed.
                void create_my_tls() {
                    int status = end_of_input_tls.create();
                    if (status) handle_perror(status, "TLS not allocated for filter");
                    end_of_input_tls_allocated = true;
                }
                void destroy_my_tls() {
                    int status = end_of_input_tls.destroy();
                    if (status) handle_perror(status, "Failed to destroy filter TLS");
                }
                bool my_tls_end_of_input() { return end_of_input_tls.get() != nullptr; }
                void set_my_tls_end_of_input() { end_of_input_tls.set(this); }
            };
            ...
            void input_buffer::grow(size_type minimum_size) {
                ...
                for (size_type i = 0; i < new_size; ++i) new_array[i].is_valid = false;
                Token t = low_token;
                for (size_type i = 0; i < old_size; ++i, ++t)
                    new_array[t & (new_size - 1)] = old_array[t & (old_size - 1)];
                array = new_array;
                array_size = new_size;
                if (old_array) cache_aligned_allocator<task_info>().deallocate(old_array, old_size);
            }
            ...
            private:
                ...
                void try_spawn_stage_task(d1::execution_data& ed) {
                    ...
                    if ((my_pipeline.input_tokens.fetch_sub(1, std::memory_order_release)) > 1) {
                        ...
                        r1::spawn(*alloc.new_object<stage_task>(ed, my_pipeline, alloc),
                                  my_pipeline.my_context);
                    }
                }
            ...
            public:
                ...
                stage_task(pipeline& pipeline, d1::small_object_allocator& alloc)
                    : my_pipeline(pipeline), my_filter(pipeline.first_filter), m_allocator(alloc),
                      my_at_start(true) {
                    task_info::reset();
                    my_pipeline.wait_ctx.reserve();
                }
                //! Construct stage_task for a subsequent stage in a pipeline.
                stage_task(pipeline& pipeline, d1::base_filter* filter, const task_info& info,
                           d1::small_object_allocator& alloc)
                    : task_info(info), my_pipeline(pipeline), my_filter(filter), m_allocator(alloc),
                      my_at_start(false) {
                    my_pipeline.wait_ctx.reserve();
                }
                ...
                void finalize(d1::execution_data& ed) { m_allocator.delete_object(this, ed); }
            ...
            bool stage_task::execute_filter(d1::execution_data& ed) {
                ...
                if (my_at_start) {
                    if (my_filter->is_serial()) {
                        ...
                        if (my_pipeline.end_of_input.load(std::memory_order_relaxed)) { return false; }
                        ...
                        if (!my_object
                            && (!my_filter->object_may_be_null()
                                || my_filter->my_input_buffer->my_tls_end_of_input())) {
                            my_pipeline.end_of_input.store(true, std::memory_order_relaxed);
                            return false;
                        }
                    }
                ...
                if (my_filter) {
                    ...
                    std::size_t ntokens_avail
                            = my_pipeline.input_tokens.fetch_add(1, std::memory_order_acquire);
            ...
            void pipeline::add_filter(d1::base_filter& new_fitler) {
                __TBB_ASSERT(new_fitler.next_filter_in_pipeline == d1::base_filter::not_in_pipeline(),
                             "filter already part of pipeline?");
                new_fitler.my_pipeline = this;
                if (first_filter == nullptr) first_filter = &new_fitler;
                else
                ...
                if (new_fitler.is_serial()) {
                    new_fitler.my_input_buffer = new (allocate_memory(sizeof(input_buffer)))
                            input_buffer(new_fitler.is_ordered());
                } else {
                    if (first_filter == &new_fitler && new_fitler.object_may_be_null()) {
                        //TODO: buffer only needed to hold TLS; could improve
                        new_fitler.my_input_buffer = new (allocate_memory(sizeof(input_buffer)))
                                input_buffer(/*is_ordered*/ false);
                        new_fitler.my_input_buffer->create_my_tls();
                    }
                }
            }

            void __TBB_EXPORTED_FUNC parallel_pipeline(d1::task_group_context& cxt, std::size_t max_token,
