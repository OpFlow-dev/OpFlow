        ...
        "my_slack<=0 && my_state==st_normal && I am on server's list of asleep threads" */
                ...
                protected:
                    private_worker(private_server& server, tbb_client& client, const std::size_t i)
                        : my_state(st_init), my_server(server), my_client(client), my_index(i), my_handle(),
                          my_next() {}
                };
...
#pragma warning(disable : 4510 4610)
                ...
                public:
                    padded_private_worker(private_server& server, tbb_client& client, const std::size_t i)
                        : private_worker(server, client, i) {
...
#pragma warning(pop)
        ...
        which in turn each wake up two threads, etc. */
                    void propagate_chain_reaction() {
                        ...
                        if (my_asleep_list_root.load(std::memory_order_relaxed)) wake_some(0);
                    }
                ...
                public:
                    ...
                    void request_close_connection(bool /*exiting*/) override {
                        for (std::size_t i = 0; i < my_n_thread; ++i) my_thread_array[i].start_shutdown();
                        remove_server_ref();
                    }
                    ...
                    unsigned default_concurrency() const override {
                        return governor::default_num_threads() - 1;
                    }
...
#pragma warning(disable : 4189)
                ...
                private_worker::thread_routine(void* arg) {
...
#pragma warning(pop)
                ...
                void private_worker::release_handle(thread_handle handle, bool join) {
                    if (join) thread_monitor::join(handle);
                    else
                        thread_monitor::detach_thread(handle);
                }

                void private_worker::start_shutdown() {
                    __TBB_ASSERT(my_state.load(std::memory_order_relaxed) != st_quit,
                                 "The quit state is expected to be set only once");
                ...
                void private_worker::run() noexcept {
                    ...
                    while (my_state.load(std::memory_order_seq_cst) != st_quit) {
                        if (my_server.my_slack.load(std::memory_order_acquire) >= 0) {
                            ...
                            __TBB_ASSERT(my_state.load(std::memory_order_relaxed) == st_quit || !my_next,
                                         "Thread monitor missed a spurious wakeup?");
                            my_server.propagate_chain_reaction();
                        }
                    }
                    my_client.cleanup(j);
                ...
                inline void private_worker::wake_or_launch() {
                    ...
                    switch (state) {
                        case st_starting:
                            __TBB_fallthrough;
                        case st_normal:
                            __TBB_ASSERT(!my_next,
                                         "Should not wake a thread while it's still in asleep list");
                            my_thread_monitor.notify();
                            break;
                        case st_init:
                            if (my_state.compare_exchange_strong(state, st_starting)) {
                                ...
                                my_handle = thread_monitor::launch(thread_routine, this,
                                                                   my_server.my_stack_size, &worker_idx);
#elif __TBB_USE_POSIX
                                {
                                    affinity_helper fpa;
                                    fpa.protect_affinity_mask(/*restore_process_mask=*/true);
                                    my_handle = thread_monitor::launch(thread_routine, this,
                                                                       my_server.my_stack_size);
                ...
                private_server::private_server(tbb_client& client)
                    : my_client(client), my_n_thread(client.max_job_count()),
                      my_stack_size(client.min_stack_size()), my_slack(0), my_ref_count(my_n_thread + 1),
                      my_thread_array(nullptr), my_asleep_list_root(nullptr)
#if TBB_USE_ASSERT
                      ,
                ...
                {
                    my_thread_array
                            = tbb::cache_aligned_allocator<padded_private_worker>().allocate(my_n_thread);
                ...
                private_server::~private_server() {
                    ...
                    for (std::size_t i = my_n_thread; i--;) my_thread_array[i].~padded_private_worker();
                    tbb::cache_aligned_allocator<padded_private_worker>().deallocate(my_thread_array,
                                                                                     my_n_thread);
                ...
                tbb_server* make_private_server(tbb_client& client) {
                    return new (tbb::cache_aligned_allocator<private_server>().allocate(1))
                            private_server(client);
                }
