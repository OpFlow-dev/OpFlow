...
namespace rml {
    namespace internal {
        ...
        extern "C" factory::status_type __RML_open_factory(factory& f, version_type& /*server_version*/,
...
#if USE_WINTHREAD
#elif USE_PTHREAD
        inline ipc_thread_monitor::handle_type ipc_thread_monitor::launch(void* (*thread_routine)(void*),
...
namespace tbb {
    namespace detail {
        ...
        namespace rml {
            ...
            char* get_stop_sem_name() { return get_sem_name(IPC_STOP_SEM_VAR_NAME, IPC_STOP_SEM_PREFIX); }
            ...
            extern "C" void set_stop_sem_name() { set_sem_name(IPC_STOP_SEM_VAR_NAME, IPC_STOP_SEM_PREFIX); }
...
#pragma warning(disable : 4189)
            ...
            ipc_worker::thread_routine(void* arg) {
...
#pragma warning(pop)
            ...
            void ipc_worker::release_handle(thread_handle handle, bool join) {
                if (join) ipc_thread_monitor::join(handle);
                else
            ...
            void ipc_worker::start_shutdown(bool join) {
                ...
                if (s == st_normal || s == st_starting) {
                    ...
                    if (s == st_normal) release_handle(my_handle, join);
                }
            }
            ...
            void ipc_worker::run() {
                ...
                while (state != st_quit && state != st_stop) {
                    if (my_server.my_slack >= 0) {
                        ...
                        if (state != st_quit && state != st_stop
                            && my_server.try_insert_in_asleep_list(*this)) {
            ...
            inline bool ipc_worker::wake_or_launch() {
                ...
                if ((my_state.load(std::memory_order_acquire) == st_init
                     && my_state.compare_exchange_strong(expected_init, st_starting))
                    || (my_state.load(std::memory_order_acquire) == st_stop
                        && my_state.compare_exchange_strong(excepted_stop, st_starting))) {
...
#endif
            __RML_DECL_THREAD_ROUTINE
            ipc_waker::thread_routine(void* arg) {
...
#pragma warning(pop)
                ...
                while (my_state.load(std::memory_order_acquire) != st_quit) {
                    ...
                    if (have_to_sleep) {
                        ...
                        if (my_state.load(std::memory_order_acquire) != st_quit
                            && my_server.my_slack.load(std::memory_order_acquire) < 0) {
                            my_thread_monitor.commit_wait(c);
                        } else {
                            // Invariant broken
                            my_thread_monitor.cancel_wait();
            ...
            inline bool ipc_waker::wake_or_launch() {
                ...
                if ((my_state.load(std::memory_order_acquire) == st_init
                     && my_state.compare_exchange_strong(excepted, st_starting))) {
                    // after this point, remove_server_ref() must be done by created thread
#if USE_WINTHREAD
                    my_handle = ipc_thread_monitor::launch(thread_routine, this, my_server.my_stack_size,
                                                           &this->my_index);
...
#endif
            __RML_DECL_THREAD_ROUTINE
            ipc_stopper::thread_routine(void* arg) {
...
#pragma warning(pop)
            ...
            ipc_server::ipc_server(tbb_client& client)
                : my_client(client), my_stack_size(client.min_stack_size()), my_thread_array(nullptr),
                  my_join_workers(false), my_waker(nullptr), my_stopper(nullptr) {
                my_ref_count = 1;
                my_slack = 0;
#if TBB_USE_ASSERT
            ...
            ipc_server::~ipc_server() {
                ...
                for (size_t i = my_n_thread; i--;) my_thread_array[i].~padded_ipc_worker();
            ...
            inline bool ipc_server::try_insert_in_asleep_list(ipc_worker& t) {
                ...
                if (!lock.try_acquire(my_asleep_list_mutex)) return false;
            ...
            inline void ipc_server::release_active_thread() { release_thread_sem(my_active_sem); }

            inline bool ipc_server::wait_stop_thread() {
                struct timespec ts;
                if (clock_gettime(CLOCK_REALTIME, &ts) == 0) {
                    ts.tv_sec++;
                    if (sem_timedwait(my_stop_sem, &ts) == 0) { return true; }
                }
                return false;
            }

            inline void ipc_server::add_stop_thread() { sem_post(my_stop_sem); }
                ...
                ipc_worker** w = wakee;
                {
                    ...
                    while (active_threads > 0 && my_asleep_list_root.load(std::memory_order_relaxed)
                        ...
                        if (additional_slack > 0) {
                            if (additional_slack + my_slack.load(std::memory_order_acquire)
                                <= 0)// additional demand does not exceed surplus supply
                                break;
                            --additional_slack;
                        } else {
            ...
            void ipc_server::wake_one_forced(int additional_slack) {
                ...
                while (w > wakee) {
                    if (!(*--w)->wake_or_launch()) {
                        add_stop_thread();
                        do { } while (!try_insert_in_asleep_list_forced(**w)); }
                }
            }
                ...
                ipc_worker* next = nullptr;
                {
                    ...
                    if (my_asleep_list_root.load(std::memory_order_relaxed)) {
                        ...
                        if (current->my_state.load(std::memory_order_relaxed) == ipc_worker::st_normal) {
                            ...
                            while (next != nullptr
                                   && next->my_state.load(std::memory_order_relaxed)
            ...
            void ipc_server::adjust_job_count_estimate(int delta) {
                ...
                if (my_n_thread > 1) {
                    if (delta < 0) {
                        ...
                        if (!my_waker->wake_or_launch()) { add_stop_thread(); }
            ...
            void rml_atexit() { release_resources(); }
            ...
            extern "C" tbb_factory::status_type __TBB_make_rml_server(tbb_factory& /*f*/, tbb_server*& server,
                                                                      tbb_client& client) {
                ...
                atexit(rml_atexit);
#endif /* USE_PTHREAD */
                if (getenv("RML_DEBUG")) { runtime_warning("IPC server is started"); }
                return tbb_factory::st_success;
            }

            extern "C" void __TBB_call_with_my_server_info(::rml::server_info_callback_t /*cb*/,
                                                           void* /*arg*/) {}
