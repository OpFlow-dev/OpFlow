    ...
    namespace detail {
        ...
        inline namespace d0 {
            ...
            concept function_node_body
                    = std::copy_constructible<Body> && std::invocable<Body&, const Input&>
            ...
            concept join_node_function_object
                    = std::copy_constructible<FunctionObject> && std::invocable<FunctionObject&, const Input&>
            ...
            concept multifunction_node_body
                    = std::copy_constructible<Body> && std::invocable<Body&, const Input&, OutputPortsType&>;

            template <typename Sequencer, typename Value>
            concept sequencer
                    = std::copy_constructible<Sequencer> && std::invocable<Sequencer&, const Value&>
            ...
            concept async_node_body
                    = std::copy_constructible<Body> && std::invocable<Body&, const Input&, GatewayType&>;
    ...
    namespace detail {
        namespace d2 {
            ...
            inline std::pair<graph_task*, graph_task*> order_tasks(graph_task* first, graph_task* second) {
                if (second->priority > first->priority) return std::make_pair(second, first);
            ...
            public:
                ...
                void merge(const message_metainfo& other) {
                    ...
                    my_waiters.insert_after(my_waiters.before_begin(), other.waiters().begin(),
                                            other.waiters().end());
                }
...
#define __TBB_FLOW_GRAPH_METAINFO_ARG(metainfo)
            ...
            public:
                ...
                bool try_put(const T& t) { return internal_try_put(t); }
                ...
                bool try_put_and_wait(const T& t) {
                    ...
                    bool res = internal_try_put(
                            t, message_metainfo {message_metainfo::waiters_type {&msg_wait_vertex}});
                    if (res) {
                        __TBB_ASSERT(graph_reference().my_context != nullptr,
                                     "No wait_context associated with the Flow Graph");
        ...
        unexpected results. */
            ...
            private:
                ...
                graph_task* try_put_task_impl(
                        const input_type& __TBB_FLOW_GRAPH_METAINFO_ARG(const message_metainfo& metainfo)) {
#if __TBB_PREVIEW_FLOW_GRAPH_TRY_PUT_AND_WAIT
...
#endif
                    {
                        ...
                        for (auto waiter : metainfo.waiters()) { waiter->reserve(1); }
                    ...
                    for (auto waiter : predecessor_metainfo.waiters()) { waiter->release(1); }
#else
            ...
            protected:
                ...
                virtual void reset_receiver(reset_flags f) {
                    my_current_count = 0;
                    if (f & rf_clear_edges) { my_predecessor_count = my_initial_predecessor_count; }
                }
...
#include "detail/_flow_graph_types_impl.h"
            ...
            inline graph::graph()
                : my_wait_context_vertex(0), my_nodes(nullptr), my_nodes_last(nullptr),
                  my_task_arena(nullptr) {
                prepare_task_arena();
                own_context = true;
                cancelled = false;
            ...
            inline graph::graph(task_group_context& use_this_context)
                : my_wait_context_vertex(0), my_context(&use_this_context), my_nodes(nullptr),
                  my_nodes_last(nullptr), my_task_arena(nullptr) {
                prepare_task_arena();
                own_context = false;
                cancelled = false;
                caught_exception = false;
            ...
            inline void graph::cancel() { my_context->cancel_group_execution(); }
            ...
            inline graph_node::graph_node(graph& g) : my_graph(g) { my_graph.register_node(this); }
...
#include "detail/_flow_graph_node_impl.h"
            ...
            public:
                ...
                __TBB_requires(input_node_body<Body, Output>) __TBB_NOINLINE_SYM
                    ...
                    : graph_node(g), my_active(false), my_body(new input_body_leaf<output_type, Body>(body)),
                      my_init_body(new input_body_leaf<output_type, Body>(body)), my_successors(this),
                ...
                __TBB_NOINLINE_SYM input_node(const input_node& src)
                    : graph_node(src.my_graph), sender<Output>(), my_active(false),
...
#if __TBB_PREVIEW_FLOW_GRAPH_TRY_PUT_AND_WAIT
            private:
                bool try_reserve(output_type& v, message_metainfo&) override { return try_reserve(v); }

                bool try_get(output_type& v, message_metainfo&) override { return try_get(v); }
            ...
            private:
                ...
                graph_task* apply_body_bypass() {
                    output_type v;
                    if (!try_reserve_apply_body(v)) return nullptr;
            ...
            public:
                ...
                __TBB_requires(function_node_body<Body, Input, Output>) __TBB_NOINLINE_SYM
                    ...
                    : graph_node(g), input_impl_type(g, concurrency, body, a_priority), fOutput_type(g) {
            ...
            __TBB_requires(std::default_initializable<Input>&&
                ...
                : public graph_node,
                  public multifunction_input<
                          Input,
                          typename wrap_tuple_elements<std::tuple_size<Output>::value,// #elements in tuple
            ...
            public:
                ...
                typedef multifunction_input<input_type, output_ports_type, Policy, internals_allocator>
                        input_impl_type;
                typedef function_input_queue<input_type, internals_allocator> input_queue_type;
            ...
            public:
                template <typename Body>
                __TBB_requires(multifunction_node_body<Body, Input, output_ports_type>) __TBB_NOINLINE_SYM
                    ...
                    fgt_multioutput_node_with_body<N>(CODEPTR(), FLOW_MULTIFUNCTION_NODE, &this->my_graph,
                                                      static_cast<receiver<input_type>*>(this),
            ...
            public:
                ...
                __TBB_NOINLINE_SYM split_node(const split_node& other)
                    : graph_node(other.my_graph), base_type(other),
                      my_output_ports(
                              init_output_ports<output_ports_type>::call(other.my_graph, my_output_ports)) {
                    fgt_multioutput_node<N>(CODEPTR(), FLOW_SPLIT_NODE, &this->my_graph,
                                            static_cast<receiver<input_type>*>(this), this->output_ports());
                }
            ...
            public:
                ...
                template <typename Body, typename... Args>
                __TBB_requires(continue_node_body<Body, Output>)
                        continue_node(const node_set<Args...>& nodes, Body body, Policy p = Policy(),
                                      node_priority_t a_priority = no_priority)
                ...
                __TBB_requires(continue_node_body<Body, Output>) __TBB_NOINLINE_SYM
                        continue_node(graph& g, int number_of_predecessors, Body body, Policy = Policy(),
                                      node_priority_t a_priority = no_priority)
                    : graph_node(g), input_impl_type(g, number_of_predecessors, body, a_priority),
                      fOutput_type(g) {
                    fgt_node_with_body(CODEPTR(), FLOW_CONTINUE_NODE, &this->my_graph,
                                       static_cast<receiver<input_type>*>(this),
                                       static_cast<sender<output_type>*>(this), this->my_body);
                ...
                template <typename Body, typename... Args>
                __TBB_requires(continue_node_body<Body, Output>)
                        continue_node(const node_set<Args...>& nodes, int number_of_predecessors, Body body,
                                      Policy p = Policy(), node_priority_t a_priority = no_priority)
                ...
                __TBB_NOINLINE_SYM continue_node(const continue_node& src)
                    : graph_node(src.my_graph), input_impl_type(src), function_output<Output>(src.my_graph) {
                    fgt_node_with_body(CODEPTR(), FLOW_CONTINUE_NODE, &this->my_graph,
                                       static_cast<receiver<input_type>*>(this),
                                       static_cast<sender<output_type>*>(this), this->my_body);
                }
            ...
            private:
                graph_task* try_put_task_impl(
                        const T& t __TBB_FLOW_GRAPH_METAINFO_ARG(const message_metainfo& metainfo)) {
                    graph_task* new_task
                            = my_successors.try_put_task(t __TBB_FLOW_GRAPH_METAINFO_ARG(metainfo));
            ...
            protected:
                ...
                void handle_operations_impl(buffer_operation* op_list, derived_type* derived) {
                    ...
                    if (try_forwarding && !forwarder_busy) {
                        if (is_graph_active(this->my_graph)) {
                            ...
                            d1::small_object_allocator allocator {};
                            graph_task* new_task
                                    = allocator.new_object<task_type>(graph_reference(), allocator, *this);
                ...
                inline graph_task* grab_forwarding_task(buffer_operation& op_data) { return op_data.ltask; }
            ...
            private:
                ...
                void try_put_and_add_task(graph_task*& last_task) {
                    graph_task* new_task = my_successors.try_put_task(
                            this->back() __TBB_FLOW_GRAPH_METAINFO_ARG(this->back_metainfo()));
                    if (new_task) {
                        // workaround for icc bug
                        graph& g = this->my_graph;
        ...
        It also calls r.remove_predecessor(*this) to remove this node as a predecessor. */
         ...
         true = an item is reserved */
            ...
            private:
                ...
                void try_put_and_add_task(graph_task*& last_task) {
                    graph_task* new_task = this->my_successors.try_put_task(
                            this->front() __TBB_FLOW_GRAPH_METAINFO_ARG(this->front_metainfo()));
            ...
            protected:
                void reset_node(reset_flags f) override { base_type::reset_node(f); }
            };// queue_node
            ...
            public:
                ...
                __TBB_requires(sequencer<Sequencer, T>) __TBB_NOINLINE_SYM
                    ...
                    : queue_node<T>(g), my_sequencer(new function_body_leaf<T, size_t, Sequencer>(s)) {
            ...
            protected:
                ...
                void internal_pop(prio_operation* op) override {
                    ...
                    *(op->elem) = prio();
#if __TBB_PREVIEW_FLOW_GRAPH_TRY_PUT_AND_WAIT
                    if (op->metainfo) { *(op->metainfo) = std::move(prio_metainfo()); }
#endif
                    op->status.store(SUCCEEDED, std::memory_order_release);
                    prio_pop();
                }
...
#if __TBB_PREVIEW_FLOW_GRAPH_TRY_PUT_AND_WAIT
                    for (auto waiter : reserved_metainfo.waiters()) { waiter->release(1); }
            ...
            private:
                ...
                void try_put_and_add_task(graph_task*& last_task) {
                    graph_task* new_task = this->my_successors.try_put_task(
                            this->prio() __TBB_FLOW_GRAPH_METAINFO_ARG(this->prio_metainfo()));
                    if (new_task) {
                        // workaround for icc bug
                        graph& graph_ref = this->graph_reference();
                        last_task = combine_tasks(graph_ref, last_task, new_task);
            ...
            private:
                ...
                void heapify() {
                    ...
                    for (; mark < this->my_tail; ++mark) {// for each unheaped element
                        ...
                        this->place_item(cur_pos,
                                         to_place __TBB_FLOW_GRAPH_METAINFO_ARG(std::move(metainfo)));
                    }
                }
    ...
    message is dropped. */
            ...
            private:
                ...
                graph_task* decrement_counter(long long delta) {
                    if (delta > 0 && size_t(delta) > my_threshold) { delta = my_threshold; }

                    {
                        spin_mutex::scoped_lock lock(my_mutex);
                        if (delta > 0 && size_t(delta) > my_count) {
                            if (my_tries > 0) { my_future_decrement += (size_t(delta) - my_count); }
                            my_count = 0;
                        ...
                        __TBB_ASSERT(my_count <= my_threshold,
                                     "counter values are truncated to be inside the [0, threshold] interval");
                    }
                    return forward_task();
                }
                ...
                graph_task* forward_task() {
                    ...
                    if ((my_predecessors.try_reserve(v __TBB_FLOW_GRAPH_METAINFO_ARG(metainfo))) == true) {
                        reserved = true;
                        if ((rval = my_successors.try_put_task(v __TBB_FLOW_GRAPH_METAINFO_ARG(metainfo)))
                            != nullptr) {
                            {
                                spin_mutex::scoped_lock lock(my_mutex);
                                ++my_count;
            ...
            public:
                ...
                limiter_node(graph& g, size_t threshold)
                    : graph_node(g), my_threshold(threshold), my_count(0), my_tries(0),
                      my_future_decrement(0), my_predecessors(this), my_successors(this), decrement(this) {
                    initialize();
                }
            ...
            private:
                ...
                graph_task* try_put_task_impl(
                    ...
                    {
                        spin_mutex::scoped_lock lock(my_mutex);
                        if (my_count + my_tries >= my_threshold) return nullptr;
                        else
                            ++my_tries;
                    }
...
#include "detail/_flow_graph_join_impl.h"
            ...
            class join_node<OutputTuple, reserving>
                : public unfolded_join_node<std::tuple_size<OutputTuple>::value, reserving_port, OutputTuple,
                                            reserving> {
            private:
...
#if __TBB_PREVIEW_FLOW_GRAPH_NODE_SET
                template <typename... Args>
                __TBB_NOINLINE_SYM join_node(const node_set<Args...>& nodes, reserving = reserving())
            ...
            class join_node<OutputTuple, queueing>
                : public unfolded_join_node<std::tuple_size<OutputTuple>::value, queueing_port, OutputTuple,
                                            queueing> {
            private:
                static const int N = std::tuple_size<OutputTuple>::value;
...
#if __TBB_PREVIEW_FLOW_GRAPH_NODE_SET
                template <typename... Args>
                __TBB_NOINLINE_SYM join_node(const node_set<Args...>& nodes, queueing = queueing())
                    : join_node(nodes.graph_reference()) {
                    make_edges_in_order(nodes, *this);
                }
#endif
            ...
            template <typename OutputTuple, typename K, typename... Functions, std::size_t... Idx>
            void join_node_function_objects_helper(std::index_sequence<Idx...>)
                requires(std::tuple_size_v<OutputTuple> == sizeof...(Functions))
                        && (...
                            && join_node_function_object<Functions, std::tuple_element_t<Idx, OutputTuple>,
                                                         K>);
            ...
            class join_node<OutputTuple, key_matching<K, KHash>>
                : public unfolded_join_node<std::tuple_size<OutputTuple>::value, key_matching_port,
                                            OutputTuple, key_matching<K, KHash>> {
            private:
                static const int N = std::tuple_size<OutputTuple>::value;
                typedef unfolded_join_node<N, key_matching_port, OutputTuple, key_matching<K, KHash>>
                        unfolded_type;
            ...
            public:
                ...
                __TBB_requires(join_node_functions<OutputTuple, K, __TBB_B0, __TBB_B1>) __TBB_NOINLINE_SYM
                ...
                template <typename __TBB_B0, typename __TBB_B1, typename __TBB_B2, typename __TBB_B3,
                          typename __TBB_B4>
                __TBB_requires(
                        join_node_functions<OutputTuple, K, __TBB_B0, __TBB_B1, __TBB_B2, __TBB_B3, __TBB_B4>)
                        __TBB_NOINLINE_SYM
                ...
                template <typename __TBB_B0, typename __TBB_B1, typename __TBB_B2, typename __TBB_B3,
                          typename __TBB_B4, typename __TBB_B5>
                __TBB_requires(join_node_functions<OutputTuple, K, __TBB_B0, __TBB_B1, __TBB_B2, __TBB_B3,
                                                   __TBB_B4, __TBB_B5>) __TBB_NOINLINE_SYM
                        join_node(graph& g, __TBB_B0 b0, __TBB_B1 b1, __TBB_B2 b2, __TBB_B3 b3, __TBB_B4 b4,
                                  __TBB_B5 b5)
                ...
                template <typename __TBB_B0, typename __TBB_B1, typename __TBB_B2, typename __TBB_B3,
                          typename __TBB_B4, typename __TBB_B5, typename __TBB_B6>
                __TBB_requires(join_node_functions<OutputTuple, K, __TBB_B0, __TBB_B1, __TBB_B2, __TBB_B3,
                                                   __TBB_B4, __TBB_B5, __TBB_B6>) __TBB_NOINLINE_SYM
                        join_node(graph& g, __TBB_B0 b0, __TBB_B1 b1, __TBB_B2 b2, __TBB_B3 b3, __TBB_B4 b4,
                                  __TBB_B5 b5, __TBB_B6 b6)
                ...
                template <typename __TBB_B0, typename __TBB_B1, typename __TBB_B2, typename __TBB_B3,
                          typename __TBB_B4, typename __TBB_B5, typename __TBB_B6, typename __TBB_B7>
                __TBB_requires(join_node_functions<OutputTuple, K, __TBB_B0, __TBB_B1, __TBB_B2, __TBB_B3,
                                                   __TBB_B4, __TBB_B5, __TBB_B6, __TBB_B7>) __TBB_NOINLINE_SYM
                        join_node(graph& g, __TBB_B0 b0, __TBB_B1 b1, __TBB_B2 b2, __TBB_B3 b3, __TBB_B4 b4,
                                  __TBB_B5 b5, __TBB_B6 b6, __TBB_B7 b7)
                ...
                __TBB_requires(join_node_functions<OutputTuple, K, __TBB_B0, __TBB_B1, __TBB_B2, __TBB_B3,
                        ...
                        __TBB_NOINLINE_SYM
                        join_node(graph& g, __TBB_B0 b0, __TBB_B1 b1, __TBB_B2 b2, __TBB_B3 b3, __TBB_B4 b4,
                                  __TBB_B5 b5, __TBB_B6 b6, __TBB_B7 b7, __TBB_B8 b8)
                ...
                template <typename __TBB_B0, typename __TBB_B1, typename __TBB_B2, typename __TBB_B3,
                          typename __TBB_B4, typename __TBB_B5, typename __TBB_B6, typename __TBB_B7,
                          typename __TBB_B8, typename __TBB_B9>
                __TBB_requires(
                        join_node_functions<OutputTuple, K, __TBB_B0, __TBB_B1, __TBB_B2, __TBB_B3, __TBB_B4,
                                            __TBB_B5, __TBB_B6, __TBB_B7, __TBB_B8, __TBB_B9>)
                        __TBB_NOINLINE_SYM
                        join_node(graph& g, __TBB_B0 b0, __TBB_B1 b1, __TBB_B2 b2, __TBB_B3 b3, __TBB_B4 b4,
                                  __TBB_B5 b5, __TBB_B6 b6, __TBB_B7 b7, __TBB_B8 b8, __TBB_B9 b9)
                ...
                __TBB_requires((sizeof...(Bodies) == 0)
                               || join_node_functions<OutputTuple, K, Bodies...>) __TBB_NOINLINE_SYM
...
#include "detail/_flow_graph_indexer_impl.h"
            ...
            public:
                ...
                __TBB_NOINLINE_SYM indexer_node(graph& g) : unfolded_type(g) {
                    fgt_multiinput_node<N>(CODEPTR(), FLOW_INDEXER_NODE, &this->my_graph, this->input_ports(),
                                           static_cast<sender<output_type>*>(this));
                }
            ...
            class indexer_node /*default*/ : public unfolded_indexer_node<
            ...
            public:
                composite_node(graph& g) : graph_node(g) { fgt_composite(CODEPTR(), this, &g); }
            ...
            public:
                ...
                async_body(const Body& body, gateway_type* gateway) : base_type(gateway), my_body(body) {}

                void operator()(const Input& v, Ports&) noexcept(
                        noexcept(tbb::detail::invoke(my_body, v, std::declval<gateway_type&>()))) {
            ...
            __TBB_requires(
                    std::default_initializable<Input>&& std::copy_constructible<Input>) class async_node
                ...
                typedef multifunction_input<Input, typename base_type::output_ports_type, Policy,
            ...
            private:
                ...
                public:
                    ...
                    void reserve_wait() override {
                        fgt_async_reserve(static_cast<typename async_node::receiver_type*>(my_node),
                                          &my_node->my_graph);
                    ...
                    bool try_put(const Output& i) override { return my_node->try_put_impl(i); }
                ...
                bool try_put_impl(const Output& i) {
                    ...
                    while (!tasks.empty()) { enqueue_in_graph_arena(this->my_graph, tasks.pop_front()); }
            ...
            public:
                template <typename Body>
                __TBB_requires(async_node_body<Body, input_type, gateway_type>) __TBB_NOINLINE_SYM
                    ...
                    : base_type(g, concurrency,
                                ...
                                a_priority),
                      my_gateway(self()) {
                    fgt_multioutput_node_with_body<1>(CODEPTR(), FLOW_ASYNC_NODE, &this->my_graph,
                                                      static_cast<receiver<input_type>*>(this),
                                                      this->output_ports(), this->my_body);
                }
                ...
                template <typename Body>
                __TBB_requires(async_node_body<Body, input_type, gateway_type>) __TBB_NOINLINE_SYM
                        async_node(graph& g, size_t concurrency, Body body, node_priority_t a_priority)
                ...
                template <typename Body>
                Body copy_function_object() {
                    typedef multifunction_body<input_type, typename base_type::output_ports_type>
                            mfn_body_type;
                    typedef async_body<Input, typename base_type::output_ports_type, gateway_type, Body>
                            async_body_type;
                    mfn_body_type& body_ref = *this->my_body;
                    async_body_type ab = *static_cast<async_body_type*>(
                            dynamic_cast<multifunction_body_leaf<
                                    input_type, typename base_type::output_ports_type, async_body_type>&>(
                                    body_ref)
                                    .get_body_ptr());
                    return ab.get_body();
                }
...
#include "detail/_flow_graph_node_set_impl.h"
            ...
            public:
                ...
                bool register_successor(successor_type& s) override {
                    ...
                    if (my_buffer_is_valid && is_graph_active(my_graph)) {
                        ...
                        if (ret) {
                            ...
                            graph_task* t
                                    = allocator.new_object<task_type>(graph_reference(), allocator, *this, s);
                ...
                bool try_get(input_type& v, message_metainfo& metainfo) override {
                    ...
                    if (my_buffer_is_valid) {
                        ...
                        for (auto msg_waiter : metainfo.waiters()) { msg_waiter->reserve(1); }
                        return true;
                    }
                    return false;
                }
                ...
                //! Reserves an item
                bool try_reserve(T& v) override { return try_get(v); }
...
#if __TBB_PREVIEW_FLOW_GRAPH_TRY_PUT_AND_WAIT
                    for (auto msg_waiter : my_buffered_metainfo.waiters()) { msg_waiter->release(1); }
            ...
            protected:
                ...
                graph_task* try_put_task_impl(
                        const input_type& v __TBB_FLOW_GRAPH_METAINFO_ARG(const message_metainfo& metainfo)) {
                    my_buffer = v;
...
#endif
                    graph_task* rtask = my_successors.try_put_task(
                            v __TBB_FLOW_GRAPH_METAINFO_ARG(my_buffered_metainfo));
                ...
                struct register_predecessor_task : public graph_task {
                    register_predecessor_task(graph& g, d1::small_object_allocator& allocator,
                    ...
                    d1::task* execute(d1::execution_data& ed) override {
                        ...
                        if (!register_predecessor(s, o)) { register_successor(o, s); }
            ...
            protected:
                ...
                graph_task* try_put_task(const T& v) override {
                    ...
                    return this->my_buffer_is_valid ? nullptr
                                                    : this->try_put_task_impl(v __TBB_FLOW_GRAPH_METAINFO_ARG(
                                                              message_metainfo {}));
                }
            ...
            inline void set_name(const graph& g, const char* name) { fgt_graph_desc(&g, name); }
