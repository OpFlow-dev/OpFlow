    ...
    namespace detail {
        ...
        namespace d0 {
            ...
            concept node_body_return_type
                    = std::same_as<OutputType, tbb::detail::d1::continue_msg> || std::same_as<OutputType,
                                                                                              ReturnType>;

            template <typename Body, typename Output>
            concept continue_node_body
                    = std::copy_constructible<Body> && requires(Body &body,
                                                                const tbb::detail::d1::continue_msg &v) {
            ...
            concept join_node_function_object
                    = std::copy_constructible<FunctionObject> && requires(FunctionObject &func,
                                                                          const Input &v) {
                { func(v) } -> adaptive_same_as<Key>;
            };

            template <typename Body, typename Output>
            concept input_node_body
                    = std::copy_constructible<Body> && requires(Body &body,
                                                                tbb::detail::d1::flow_control &fc) {
            ...
            concept sequencer
                    = std::copy_constructible<Sequencer> && requires(Sequencer &seq, const Value &value) {
                { seq(value) } -> adaptive_same_as<std::size_t>;
            };

            template <typename Body, typename Input, typename GatewayType>
            concept async_node_body = std::copy_constructible<Body> && requires(Body &body, const Input &v,
                                                                                GatewayType &gateway) {
                body(v, gateway);
            };
...
#include "detail/_flow_graph_types_impl.h"
            ...
            inline graph::graph(task_group_context &use_this_context)
                : my_wait_context(0), my_context(&use_this_context), my_nodes(NULL), my_nodes_last(NULL),
                  my_task_arena(NULL) {
                prepare_task_arena();
                own_context = false;
                cancelled = false;
...
#include "detail/_flow_graph_node_impl.h"
            ...
            public:
                ...
                __TBB_NOINLINE_SYM explicit buffer_node(graph &g)
                    : graph_node(g),
                      reservable_item_buffer<T, internals_allocator>(), receiver<T>(), sender<T>(),
                      my_successors(this), forwarder_busy(false) {
...
#include "detail/_flow_graph_join_impl.h"
            ...
            template <typename OutputTuple, typename K, typename... Functions, std::size_t... Idx>
                    void join_node_function_objects_helper(std::index_sequence<Idx...>) requires(
                            std::tuple_size_v<OutputTuple> == sizeof...(Functions))
                    && (...
