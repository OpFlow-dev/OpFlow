    ...
    namespace detail {
        namespace d1 {
            ...
            struct internal_ets_key_selector {
                ...
                static key_type current_key() { return std::this_thread::get_id(); }
            };
...
// Intel Compiler on OSX cannot create atomics objects that instantiated from non-fundamental types
            ...
            struct ets_key_selector<ets_suspend_aware> {
                ...
                static key_type current_key() { return r1::current_suspend_point(); }
            };
#endif
        ...
        Each array in the list is half the size of its predecessor. */
                ...
                void table_elementwise_copy(const ets_base& other,
                    ...
                    for (array* r = other.my_root.load(std::memory_order_relaxed); r; r = r->next) {
                        for (std::size_t i = 0; i < r->size(); ++i) {
                            slot& s1 = r->at(i);
                            if (!s1.empty()) {
                                for (std::size_t j = root->start(
                                             std::hash<key_type> {}(s1.key.load(std::memory_order_relaxed)));
                                     ; j = (j + 1) & mask) {
                                    slot& s2 = root->at(j);
                                    if (s2.empty()) {
            ...
            public:
                ...
                enumerable_thread_specific_iterator(
                    ...
                    : my_container(other.my_container), my_index(other.my_index),
                ...
                friend enumerable_thread_specific_iterator operator+(std::ptrdiff_t offset,
                                                                     enumerable_thread_specific_iterator v) {
                ...
                Value& operator[](std::ptrdiff_t k) const { return *(*my_container)[my_index + k].value(); }
            ...
            public:
                ...
                segmented_iterator(const segmented_iterator<SegmentedContainer, U>& other)
                    : my_segcont(other.my_segcont), outer_iter(other.outer_iter),
            ...
            struct construct_by_args : no_assign {
                stored_pack<P...> pack;
                void construct(void* where) {
                    call([where](const typename std::decay<P>::type&... args) { new (where) T(args...); },
                         pack);
                }
            ...
            class callback_leaf : public callback_base, Constructor {
                ...
                callback_base* clone() const override { return make(*this); }
                ...
                void construct(void* where) override { Constructor::construct(where); }
...
@ingroup containers */
            ...
            class enumerable_thread_specific : ets_base<ETS_key_type> {
                ...
                public:
                    ...
                    generic_range_type(I begin_, I end_, std::size_t grainsize_ = 1)
            ...
            public:
                ...
                using const_iterator
                        = enumerable_thread_specific_iterator<internal_collection_type, const value_type>;
                ...
                template <typename Finit, typename = typename std::enable_if<is_callable_no_args<
                ...
                explicit enumerable_thread_specific(const T& exemplar)
                    : my_construct_callback(callback_leaf<construct_by_exemplar<T>>::make(exemplar)) {}
            ...
            private:
                ...
                void internal_copy(const enumerable_thread_specific<T, A2, C2>& other) {
                    ...
                    static_assert((is_compatible_ets<T, typename std::decay<decltype(other)>::type>::value),
