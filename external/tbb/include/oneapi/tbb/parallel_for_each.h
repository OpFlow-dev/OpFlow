...
#pragma warning(push)
            ...
            struct feeder_item_task : public task {
                ...
                feeder_item_task(ItemType&& input_item, feeder_type& feeder, small_object_allocator& alloc)
                    : item(std::forward<ItemType>(input_item)), my_feeder(feeder), my_allocator(alloc) {}
                ...
                static auto call(const BodyType& call_body, ItemType& call_item, FeederType& call_feeder,
                                 first_priority)
                        -> decltype(parallel_for_each_operator_selector<Body>::call(call_body,
                                                                                    std::move(call_item),
                                                                                    &call_feeder),
                                    void()) {
                    parallel_for_each_operator_selector<Body>::call(call_body, std::move(call_item),
                                                                    &call_feeder);
                }

                template <typename BodyType, typename ItemType, typename FeederType>
                static void call(const BodyType& call_body, ItemType& call_item, FeederType& call_feeder,
                                 second_priority) {
    ...
    @ingroup algorithms **/
            ...
            class feeder_impl : public feeder<Item> {
                ...
                void internal_add_copy_impl(std::false_type, const Item&) {
                    __TBB_ASSERT(false, "Overloading for r-value reference doesn't work or it's not movable "
                                        "and not copyable object");
                }
    ...
    @ingroup algorithms **/
            ...
            struct for_each_iteration_task : public task {
                ...
                for_each_iteration_task(Iterator input_item_ptr, const Body& body,
                                        feeder_impl<Body, Item>* feeder_ptr, wait_context& wait_context)
                    : item_ptr(input_item_ptr), my_body(body), my_feeder_ptr(feeder_ptr),
                      parent_wait_context(wait_context) {}

                void finalize() { parent_wait_context.release(); }
            ...
            struct input_block_handling_task : public task {
                ...
                input_block_handling_task(wait_context& root_wait_context, task_group_context& e_context,
                    ...
                    for (auto* it = task_pool.begin(); it != task_pool.end(); ++it) {
                        new (it) iteration_task(iteration_task_iterator_type(item_it++), body, feeder_ptr,
                                                my_wait_context);
                    }
                }
    ...
    @ingroup algorithms **/
            ...
            struct forward_block_handling_task : public task {
                ...
                forward_block_handling_task(Iterator first, std::size_t size, wait_context& w_context,
...
#pragma ivdep
    ...
    @ingroup algorithms */
            ...
            using iterator_tag_dispatch = typename std::conditional<
                    ...
                    typename std::conditional<std::is_base_of<std::forward_iterator_tag, tag<It>>::value,
            ...
            public:
                for_each_root_task_base(Iterator first, Iterator last, const Body& body,
                    ...
                    : my_first(first), my_last(last), my_wait_context(w_context),
                      my_execution_context(e_context), my_body(body),
            ...
            private:
                task* execute(execution_data& ed) override {
                    ...
                    auto block_handling_task = alloc.new_object<block_handling_type>(
                            ...
                            this->my_execution_context, this->my_body, this->my_feeder_holder.feeder_ptr(),
                            alloc);
  ...
  * Splits input range to blocks
    ...
    @ingroup algorithms */
            ...
            concept parallel_for_each_iterator_body
                    = parallel_for_each_body<Body, iterator_reference_type<Iterator>,
    ...
    @ingroup algorithms */
            ...
            void run_parallel_for_each(Iterator first, Iterator last, const Body& body,
                ...
                if (!(first == last)) {
                    ...
                    for_each_root_task<Iterator, Body, ItemType> root_task(first, last, body, w_context,
                                                                           context);
...
**/
    ...
    See also requirements on \ref parallel_for_each_body_req "parallel_for_each Body". **/
            ...
            __TBB_requires(std::input_iterator<Iterator>&& parallel_for_each_iterator_body<
                           Body, Iterator>) void parallel_for_each(Iterator first, Iterator last,
                                                                   const Body& body) {
            ...
            __TBB_requires(
                    container_based_sequence<Range, std::input_iterator_tag>&& parallel_for_each_range_body<
