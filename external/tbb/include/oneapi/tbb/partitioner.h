...
namespace tbb {
    namespace detail {
        ...
        namespace d1 {
            ...
            struct adaptive_mode : partition_type_base<Partition> {
                ...
                adaptive_mode()
                    : my_divisor(get_initial_auto_partitioner_divisor() / 4 * my_partition::factor) {}
    ...
    otherwise it represents equal-size split. */
            ...
            template <typename Range>
            struct proportion_helper<
                    Range, typename std::enable_if<supports_proportional_splitting<Range>::value>::type> {
            ...
            struct linear_affinity_mode : proportional_mode<Partition> {
                ...
                linear_affinity_mode()
                    : proportional_mode<Partition>(), my_head(get_initial_partition_head()),
            ...
            struct dynamic_grainsize_mode : Mode {
                ...
                bool check_being_stolen(Task& t,
                                        const execution_data& ed) {// part of old should_execute_range()
                    if (!(self().my_divisor
            ...
            public:
                ...
                void execute(StartType& start, Range& range, execution_data& ed) {
                    ...
                    while (range.is_divisible()) start.offer_work(split_obj, ed);
                    start.run_body(range);
                }
                void spawn_task(task& t, task_group_context& ctx) { spawn(t, ctx); }
            };
            ...
            private:
                ...
                public:
                    bool should_execute_range(const execution_data& ed) {
                        if (num_chunks < VICTIM_CHUNKS && is_stolen_task(ed)) num_chunks = VICTIM_CHUNKS;
                        return num_chunks == 1;
                    }
