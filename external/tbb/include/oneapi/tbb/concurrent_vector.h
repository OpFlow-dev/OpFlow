    ...
    namespace detail {
        namespace d1 {
            ...
            public:
                ...
                vector_iterator&
                operator=(const vector_iterator<vector_type, typename vector_type::value_type>& other) {
            ...
            typename vector_iterator<Vector, T>::difference_type
            operator-(const vector_iterator<Vector, T>& i, const vector_iterator<Vector, U>& j) {
            ...
            class concurrent_vector : private segment_table<T, Allocator, concurrent_vector<T, Allocator>,
            ...
            public:
                ...
                explicit concurrent_vector(const allocator_type& alloc) noexcept : base_type(alloc) {}

                explicit concurrent_vector(size_type count, const value_type& value,
                                           const allocator_type& alloc = allocator_type())
                    : concurrent_vector(alloc) {
                    try_call([&] { grow_by(count, value); }).on_exception([&] { base_type::clear(); });
                }
                ...
                concurrent_vector(InputIterator first, InputIterator last,
                                  const allocator_type& alloc = allocator_type())
                    : concurrent_vector(alloc) {
                    try_call([&] { grow_by(first, last); }).on_exception([&] { base_type::clear(); });
                }

                concurrent_vector(const concurrent_vector& other)
                    : base_type(segment_table_allocator_traits::select_on_container_copy_construction(
                            other.get_allocator())) {
                ...
                concurrent_vector(concurrent_vector&& other) noexcept : base_type(std::move(other)) {}
                ...
                // Concurrent growth
                iterator grow_by(size_type delta) { return internal_grow_by_delta(delta); }
                ...
                iterator grow_to_at_least(size_type n) { return internal_grow_to_at_least(n); }
                iterator grow_to_at_least(size_type n, const value_type& value) {
                    return internal_grow_to_at_least(n, value);
                }

                iterator push_back(const value_type& item) { return internal_emplace_back(item); }

                iterator push_back(value_type&& item) { return internal_emplace_back(std::move(item)); }
                ...
                reference front() { return internal_subscript(0); }

                const_reference front() const { return internal_subscript(0); }

                reference back() { return internal_subscript(size() - 1); }

                const_reference back() const { return internal_subscript(size() - 1); }
                ...
                void resize(size_type n) { internal_resize(n); }

                void resize(size_type n, const value_type& val) { internal_resize(n, val); }

                void shrink_to_fit() { internal_compact(); }

                void swap(concurrent_vector& other) noexcept(is_noexcept_swap) { base_type::swap(other); }
            ...
            private:
                ...
                using segment_element_allocator_traits
                        = tbb::detail::allocator_traits<segment_element_allocator_type>;
                ...
                allocate_long_table(const typename base_type::atomic_segment* embedded_table,
                    ...
                    if (this->get_table() != embedded_table) { return nullptr; }

                    // Allocate long segment table and fill with null pointers
                    segment_table_type new_segment_table = segment_table_allocator_traits::allocate(
                            base_type::get_allocator(), this->pointers_per_long_table);
                    // Copy segment pointers from the embedded table
                    for (size_type segment_index = 0; segment_index < this->pointers_per_embedded_table;
                         ++segment_index) {
                    ...
                    for (size_type segment_index = this->pointers_per_embedded_table;
                        ...
                        segment_table_allocator_traits::construct(base_type::get_allocator(),
                                                                  &new_segment_table[segment_index], nullptr);
                    }
                ...
                segment_type create_segment(segment_table_type table, segment_index_type seg_index,
                                            size_type index) {
                    ...
                    if (seg_index < first_block) {
                        ...
                        try_call([&] {
                            new_segment = segment_element_allocator_traits::allocate(segment_allocator,
                                                                                     first_block_size);
                        }).on_exception([&] {
                            segment_type disabled_segment = nullptr;
                            if (table[0].compare_exchange_strong(disabled_segment,
                                                                 this->segment_allocation_failure_tag)) {
                        ...
                        if (table[0].compare_exchange_strong(disabled_segment, new_segment)) {
                            ...
                            for (size_type i = 1; i < first_block && i < this->pointers_per_embedded_table;
                                 ++i) {
                            ...
                            segment_element_allocator_traits::deallocate(segment_allocator, new_segment,
                                                                         first_block_size);
                        ...
                        if (index == offset) {
                            __TBB_ASSERT(table[seg_index].load(std::memory_order_relaxed) == nullptr,
                ...
                size_type number_of_elements_in_segment(segment_index_type seg_index) {
                    ...
                    if (seg_index == 0) {
                        ...
                        if (curr_vector_size < curr_segment_base) { return 0; }
                        return curr_segment_base * 2 > curr_vector_size ? curr_vector_size - curr_segment_base
                                                                        : curr_segment_base;
                    }
                }
                ...
                void deallocate_segment(segment_type address, segment_index_type seg_index) {
                    ...
                    if (seg_index >= first_block) {
                        segment_element_allocator_traits::deallocate(segment_allocator, address,
                                                                     this->segment_size(seg_index));
                    } else if (seg_index == 0) {
                        size_type elements_to_deallocate
                                = first_block > 0 ? this->segment_size(first_block) : this->segment_size(0);
                        segment_element_allocator_traits::deallocate(segment_allocator, address,
                ...
                void copy_segment(segment_index_type seg_index, segment_type from, segment_type to) {
                    ...
                    try_call([&] {
                        for (; i != number_of_elements_in_segment(seg_index); ++i) {
                            segment_table_allocator_traits::construct(base_type::get_allocator(), to + i,
                                                                      from[i]);
                        }
                    }).on_exception([&] {
                        ...
                        segment_index_type last_segment
                                = this->segment_index_of(this->my_size.load(std::memory_order_relaxed));
                        ...
                        for (segment_index_type j = seg_index + 1; j != last_segment; ++j) {
                            ...
                            if (curr_segment) {
                                zero_unconstructed_elements(curr_segment + this->segment_base(j),
                                                            this->segment_size(j));
                            }
                        }
                ...
                iterator internal_emplace_back(Args&&... args) {
                    ...
                    auto element_address
                            = &base_type::template internal_subscript</*allow_out_of_range_access=*/true>(
                                    old_size);
                ...
                template <typename... Args>
                void internal_loop_construct(segment_table_type table, size_type start_idx, size_type end_idx,
                                             const Args&... args) {
                ...
                iterator internal_grow(size_type start_idx, size_type end_idx, const Args&... args) {
                    ...
                    return iterator(
                            *this, start_idx,
                            &base_type::template internal_subscript</*allow_out_of_range_access=*/false>(
                                    start_idx));
                }

                template <typename... Args>
                iterator internal_grow_by_delta(size_type delta, const Args&... args) {
                    if (delta == size_type(0)) { return end(); }
                ...
                iterator internal_grow_to_at_least(size_type new_size, const Args&... args) {
                    ...
                    if (delta > 0) { return internal_grow(old_size, new_size, args...); }
                ...
                void internal_resize(size_type n, const Args&... args) {
                    ...
                    if (n > old_size) {
                        ...
                        // Delete segments
                        for (size_type seg_idx = this->segment_index_of(n - 1) + 1; seg_idx <= last_segment;
                             ++seg_idx) {
                        ...
                        size_type last_index_to_destroy = std::min(
                                this->segment_base(n_segment) + this->segment_size(n_segment), old_size);
                ...
                void internal_compact() {
                    ...
                    const segment_index_type k_stop
                            = curr_size
                                      ? this->segment_index_of(curr_size - 1) + 1
                                      : 0;// number of segments to store existing items: 0=>0; 1,2=>1; 3,4=>2; [5-8]=>3;..
                    const segment_index_type first_block
                            = this->my_first_block;// number of merged segments, getting values from atomics

                    segment_index_type k = first_block;
                    if (k_stop < first_block) {
                        k = k_stop;
                    } else {
                        while (k < k_stop && incompact_predicate(this->segment_size(k) * sizeof(value_type)))
                            k++;
                    }

                    if (k_stop == k_end && k == first_block) { return; }
                    ...
                    if (k != first_block && k) {
                        ...
                        segment_table_allocator_traits::deallocate(base_type::get_allocator(), buffer_table,
                                                                   max_block);
                    }
            ...
            concurrent_vector(It, It, Alloc = Alloc()) -> concurrent_vector<iterator_value_t<It>, Alloc>;
#endif

            template <typename T, typename Allocator>
            void swap(concurrent_vector<T, Allocator>& lhs, concurrent_vector<T, Allocator>& rhs) {
                lhs.swap(rhs);
            }
