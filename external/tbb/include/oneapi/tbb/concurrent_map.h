    ...
    namespace detail {
        namespace d2 {
            ...
            class concurrent_map
                : public concurrent_skip_list<map_traits<
                          Key, Value, Compare, concurrent_geometric_level_generator<32>, Allocator, false>> {
                using base_type = concurrent_skip_list<map_traits<
                        Key, Value, Compare, concurrent_geometric_level_generator<32>, Allocator, false>>;
            ...
            public:
                ...
                mapped_type& at(const key_type& key) {
                    ...
                    if (it == this->end()) { throw_exception(exception_id::invalid_key); }
                    return it->second;
                }
                ...
                mapped_type& operator[](key_type&& key) {
                    ...
                    if (it == this->end()) {
                        it = this->emplace(std::piecewise_construct, std::forward_as_tuple(std::move(key)),
                                           std::tuple<>())
                                     .first;
                    }
                    return it->second;
            ...
            template <typename Key, typename T, typename Comp = std::less<std::remove_const_t<Key>>,
            ...
            template <typename It, typename Alloc, typename = std::enable_if_t<is_input_iterator_v<It>>,
                      typename = std::enable_if_t<is_allocator_v<Alloc>>>
            concurrent_map(It, It, Alloc) -> concurrent_map<iterator_key_t<It>, iterator_mapped_t<It>,
                                                            std::less<iterator_key_t<It>>, Alloc>;

            template <typename Key, typename T, typename Alloc,
                      typename = std::enable_if_t<is_allocator_v<Alloc>>>
            concurrent_map(std::initializer_list<std::pair<Key, T>>, Alloc)
                    -> concurrent_map<std::remove_const_t<Key>, T, std::less<std::remove_const_t<Key>>,
                                      Alloc>;
            ...
            class concurrent_multimap
                : public concurrent_skip_list<map_traits<
                          Key, Value, Compare, concurrent_geometric_level_generator<32>, Allocator, true>> {
                using base_type = concurrent_skip_list<map_traits<
                        Key, Value, Compare, concurrent_geometric_level_generator<32>, Allocator, true>>;
            ...
            concurrent_multimap(std::initializer_list<std::pair<Key, T>>, Alloc)
                    -> concurrent_multimap<std::remove_const_t<Key>, T, std::less<std::remove_const_t<Key>>,
                                           Alloc>;
