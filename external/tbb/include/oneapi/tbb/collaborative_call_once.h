...
#pragma warning(disable : 4324)
            ...
            constexpr std::uintptr_t collaborative_once_references_mask
                    = collaborative_once_max_references - 1;
            ...
            public:
                ...
                public:
                    lifetime_guard(collaborative_once_runner& r) : m_runner(r) { m_runner.m_ref_count++; }
                    ~lifetime_guard() { m_runner.m_ref_count--; }
                };
                ...
                ~collaborative_once_runner() {
                    ...
                    if (m_is_ready.load(std::memory_order_relaxed)) { m_storage.~storage_t(); }
                }

                std::uintptr_t to_bits() { return reinterpret_cast<std::uintptr_t>(this); }

                static collaborative_once_runner* from_bits(std::uintptr_t bits) {
                    __TBB_ASSERT((bits & collaborative_once_references_mask) == 0,
                                 "invalid pointer, last log2(max_nfs_size) bits must be zero");
                    return reinterpret_cast<collaborative_once_runner*>(bits);
                }

                template <typename F>
                void run_once(F&& f) {
                    __TBB_ASSERT(!m_is_ready.load(std::memory_order_relaxed),
                                 "storage with task_arena and wait_context is already initialized");
                        ...
                        isolated_execute([&] {
                            task_group_context context {task_group_context::bound,
                                                        task_group_context::default_traits
                                                                | task_group_context::concurrent_wait};
            ...
            class collaborative_once_flag : no_copy {
                ...
                void do_collaborative_call_once(Fn&& f) {
                    ...
                    do {
                        if (expected == state::uninitialized
                            && m_state.compare_exchange_strong(expected, runner.to_bits())) {
                            // Winner thread
                            runner.run_once([&] {
                                try_call([&] { std::forward<Fn>(f)(); }).on_exception([&] {
                            ...
                            do {
                                ...
                                // "expected > state::done" prevents storing values, when state is uninitialized or done
                                     ...
                                     && !m_state.compare_exchange_strong(expected, expected + 1));

                            if (auto shared_runner = collaborative_once_runner::from_bits(
                                        expected & ~collaborative_once_references_mask)) {
...
#if TBB_USE_ASSERT
            public:
                ~collaborative_once_flag() { m_state.store(state::dead, std::memory_order_relaxed); }
#endif
            };
