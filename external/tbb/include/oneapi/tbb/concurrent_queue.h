...
namespace tbb {
    namespace detail {
        namespace d2 {
            ...
            class concurrent_queue {
                ...
                using queue_allocator_type =
                        typename allocator_traits_type::template rebind_alloc<queue_representation_type>;
            ...
            public:
                ...
                explicit concurrent_queue(const allocator_type& a)
                    ...
                    __TBB_ASSERT(is_aligned(&my_queue_representation->head_counter, max_nfs_size),
                                 "alignment error");
                    __TBB_ASSERT(is_aligned(&my_queue_representation->tail_counter, max_nfs_size),
                                 "alignment error");
                ...
                concurrent_queue(const concurrent_queue& src)
                    : concurrent_queue(queue_allocator_traits::select_on_container_copy_construction(
                            src.get_allocator())) {
                    my_queue_representation->assign(*src.my_queue_representation, my_allocator,
                                                    copy_construct_item);
                }
                ...
                // Move constructors
                concurrent_queue(concurrent_queue&& src) : concurrent_queue(std::move(src.my_allocator)) {
                    internal_swap(src);
                }
        ...
        Returns true if successful; false otherwise. */
                ...
                __TBB_nodiscard bool empty() const { return my_queue_representation->empty(); }
            ...
            private:
                ...
                void internal_push(Args&&... args) {
                    ...
                    my_queue_representation->choose(k).push(k, *my_queue_representation, my_allocator,
            ...
            concurrent_queue(It, It, Alloc = Alloc()) -> concurrent_queue<iterator_value_t<It>, Alloc>;
        ...
        namespace r1 {
            ...
            TBB_EXPORT std::uint8_t* __TBB_EXPORTED_FUNC
            allocate_bounded_queue_rep(std::size_t queue_rep_size);
            TBB_EXPORT void __TBB_EXPORTED_FUNC deallocate_bounded_queue_rep(std::uint8_t* mem,
                                                                             std::size_t queue_rep_size);
            TBB_EXPORT void __TBB_EXPORTED_FUNC abort_bounded_queue_monitors(concurrent_monitor* monitors);
            TBB_EXPORT void __TBB_EXPORTED_FUNC notify_bounded_queue_monitor(concurrent_monitor* monitors,
                                                                             std::size_t monitor_tag,
                                                                             std::size_t ticket);
            TBB_EXPORT void __TBB_EXPORTED_FUNC wait_bounded_queue_monitor(concurrent_monitor* monitors,
                                                                           std::size_t monitor_tag,
        ...
        namespace d2 {
            ...
            class concurrent_bounded_queue {
                ...
                void internal_wait(r1::concurrent_monitor* monitors, std::size_t monitor_tag,
            ...
            public:
                ...
                explicit concurrent_bounded_queue(const allocator_type& a)
                    ...
                    queue_allocator_traits::construct(my_allocator, my_queue_representation);
                    my_capacity = std::size_t(-1)
                                  / (queue_representation_type::item_size > 1
                ...
                concurrent_bounded_queue(const concurrent_bounded_queue& src)
                    : concurrent_bounded_queue(queue_allocator_traits::select_on_container_copy_construction(
                            src.get_allocator())) {
                    my_queue_representation->assign(*src.my_queue_representation, my_allocator,
                                                    copy_construct_item);
                }
                ...
                bool try_push(const T& value) { return internal_push_if_not_full(value); }

                bool try_push(T&& value) { return internal_push_if_not_full(std::move(value)); }
                ...
                /** Does not wait for item to become available.
        Returns true if successful; false otherwise. */
                bool pop(T& result) { return internal_pop(&result); }

                bool try_pop(T& result) { return internal_pop_if_present(&result); }

                void abort() { internal_abort(); }

                // Return the number of items in the queue; thread unsafe
                std::ptrdiff_t size() const { return my_queue_representation->size(); }
            ...
            private:
                ...
                void internal_push(Args&&... args) {
                    ...
                    if (static_cast<std::ptrdiff_t>(
                                my_queue_representation->head_counter.load(std::memory_order_relaxed))
                        ...
                        auto pred = [&] {
                            ...
                            return static_cast<std::ptrdiff_t>(my_queue_representation->head_counter.load(
                    ...
                    __TBB_ASSERT((static_cast<std::ptrdiff_t>(my_queue_representation->head_counter.load(
                                          std::memory_order_relaxed))
                                  > target),
                                 nullptr);
                    my_queue_representation->choose(ticket).push(ticket, *my_queue_representation,
                                                                 my_allocator, std::forward<Args>(args)...);
                ...
                bool internal_push_if_not_full(Args&&... args) {
                    ...
                    } while (!my_queue_representation->tail_counter.compare_exchange_strong(ticket,
                                                                                            ticket + 1));
                    ...
                    do {
                        ...
                        if (static_cast<std::ptrdiff_t>(
                            ...
                            auto pred = [&] {
                                ...
                                return static_cast<std::ptrdiff_t>(my_queue_representation->tail_counter.load(
                                               std::memory_order_relaxed))
                                       <= target;
                            };
                        ...
                        __TBB_ASSERT(static_cast<std::ptrdiff_t>(my_queue_representation->tail_counter.load(
                                             std::memory_order_relaxed))
                                             > target,
                                     nullptr);
                ...
                bool internal_pop_if_present(void* dst) {
                    ...
                    do {
                        ...
                        do {
                            ...
                            // Queue had item with ticket k when we looked.  Attempt to get that item.
                            // Another thread snatched the item, retry.
                        } while (!my_queue_representation->head_counter.compare_exchange_strong(ticket,
                                                                                                ticket + 1));
