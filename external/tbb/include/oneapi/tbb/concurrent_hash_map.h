...
namespace tbb {
    namespace detail {
        namespace d2 {
            ...
            inline void* const rehash_req_flag = reinterpret_cast<void*>(std::size_t(3));
            // Rehashed empty bucket flag
            inline void* const empty_rehashed_flag = reinterpret_cast<void*>(std::size_t(0));
            ...
            public:
                ...
                static constexpr size_type first_block
                        = 8;//including embedded_block. perfect with bucket size 16, so the allocations are power of 4096
                ...
                const bucket_allocator_type& get_allocator() const { return my_allocator; }

                bucket_allocator_type& get_allocator() { return my_allocator; }
                ...
                void enable_segment(segment_index_type k, bool is_initial = false) {
                    ...
                    if (k >= first_block) {
                        ...
                        }).on_exception([&] { my_table[k].store(nullptr, std::memory_order_relaxed); });
                ...
                void mark_rehashed_levels(hashcode_type h) noexcept {
                    ...
                    while (segment_ptr_type seg = my_table[++s].load(std::memory_order_relaxed))
                        if (rehash_required(seg[h].node_list.load(std::memory_order_relaxed))) {
                            seg[h].node_list.store(reinterpret_cast<node_base*>(empty_rehashed_flag),
                ...
                bool check_rehashing_collision(const hashcode_type h, hashcode_type m_old,
                    ...
                    if ((h & m_old) != (h & m)) {// mask changed for this hashcode, rare event
                        ...
                        for (++m_old; !(h & m_old);
                             m_old <<= 1)// at maximum few rounds depending on the first block size
                            ;
                        m_old = (m_old << 1) - 1;// get full mask from a bit
                        __TBB_ASSERT((m_old & (m_old + 1)) == 0 && m_old <= m, nullptr);
                        // check whether it is rehashing/ed
                        if (!rehash_required(
                                    get_bucket(h & m_old)->node_list.load(std::memory_order_acquire))) {
                            return true;
                        }
                    }
                    return false;
                ...
                segment_index_type insert_new_node(bucket* b, node_base* n, hashcode_type mask) {
                    size_type sz
                            = ++my_size;// prefix form is to enforce allocation after the first item inserted
                    add_to_bucket(b, n);
                    // check load factor
                    if (sz >= mask) {// TODO: add custom load_factor
                        segment_index_type new_seg = tbb::detail::log2(mask + 1);//optimized segment_index_of
                        __TBB_ASSERT(is_valid(my_table[new_seg - 1].load(std::memory_order_relaxed)),
                ...
                void internal_swap_content(hash_map_base& table) {
                    ...
                    for (size_type i = 0; i < embedded_buckets; i++) {
                        ...
                        my_embedded_segment[i].node_list.store(
                                table.my_embedded_segment[i].node_list.load(std::memory_order_relaxed),
                                std::memory_order_relaxed);
                ...
                void internal_move(hash_map_base&& other) {
                    ...
                    for (size_type i = 0; i < embedded_buckets; ++i) {
                        my_embedded_segment[i].node_list.store(other.my_embedded_segment[i].node_list,
                                                               std::memory_order_relaxed);
            ...
            protected:
                ...
                std::atomic<size_type>
                        my_size;// It must be in separate cache line from my_mask due to performance effects
                // Zero segment
            ...
            public:
                ...
                hash_map_iterator(const hash_map_iterator<Container, typename Container::value_type>& other)
                    : my_map(other.my_map), my_index(other.my_index), my_bucket(other.my_bucket),
                      my_node(other.my_node) {}

                hash_map_iterator&
                operator=(const hash_map_iterator<Container, typename Container::value_type>& other) {
                    my_map = other.my_map;
            ...
            private:
                ...
                template <typename Key, typename T, typename HashCompare, typename A
                          ...
                          ,
                          typename M>
                __TBB_requires(tbb::detail::hash_compare<HashCompare, Key>&& ch_map_rw_scoped_lockable<M>)
#else
                          >
                ...
                hash_map_iterator(const Container& map, std::size_t index, const bucket* b, node_base* n)
                    ...
                    if (map_base::is_valid(n)) my_node = static_cast<node*>(n);
                    else if (b)
            ...
            public:
                ...
                // Split range.
                hash_map_range(hash_map_range& r, split) : my_end(r.my_end), my_grainsize(r.my_grainsize) {
                ...
                hash_map_range(const map_type& map, size_type grainsize_ = 1)
                    : my_begin(Iterator(map, 0, map.my_embedded_segment,
                      ...
                      my_end(Iterator(map, map.my_mask.load(std::memory_order_relaxed) + 1, nullptr,
                                      nullptr)),
            ...
            void hash_map_range<Iterator>::set_midpoint() const {
                ...
                if (m > my_grainsize) {
                    ...
                    my_midpoint
                            = Iterator(*my_begin.my_map, m, b, b->node_list.load(std::memory_order_relaxed));
                } else {
                    my_midpoint = my_end;
                }
            ...
            template <typename Key, typename T, typename HashCompare = d1::tbb_hash_compare<Key>,
                      typename Allocator = tbb_allocator<std::pair<const Key, T>>
#if __TBB_PREVIEW_CONCURRENT_HASH_MAP_EXTENSIONS
                      ,
                      typename MutexType = spin_rw_mutex>
            __TBB_requires(tbb::detail::hash_compare<HashCompare, Key>&& ch_map_rw_scoped_lockable<MutexType>)
#else
                      >
            __TBB_requires(tbb::detail::hash_compare<HashCompare, Key>)
#endif
            ...
            protected:
                ...
                static node* allocate_node_copy_construct(bucket_allocator_type& allocator, const Key& key,
                                                          const T* t) {
                    return create_node(allocator, key, *t);
                }

                static node* allocate_node_move_construct(bucket_allocator_type& allocator, const Key& key,
                                                          const T* t) {
                    return create_node(allocator, key, std::move(*const_cast<T*>(t)));
                }

                template <typename K = Key>
                static node* allocate_node_default_construct(bucket_allocator_type& allocator, const K& key,
                                                             const T*) {
                    // Emplace construct an empty T object inside the pair
                    return create_node(allocator, std::piecewise_construct, std::forward_as_tuple(key),
                ...
                public:
                    bucket_accessor(concurrent_hash_map* base, const hashcode_type h, bool writer = false) {
                    ...
                    inline void acquire(concurrent_hash_map* base, const hashcode_type h,
                        ...
                        if (rehash_required(my_b->node_list.load(std::memory_order_acquire))
                            ...
                            if (rehash_required(my_b->node_list.load(std::memory_order_relaxed)))
                                base->rehash_bucket(my_b, h);// recursive rehashing
                        } else
                            bucket::scoped_type::acquire(my_b->mutex, writer);
                        __TBB_ASSERT(!rehash_required(my_b->node_list.load(std::memory_order_relaxed)),
                                     nullptr);
                    }
                ...
                void rehash_bucket(bucket* b_new, const hashcode_type hash) {
                    ...
                    hashcode_type mask = (hashcode_type(1) << tbb::detail::log2(hash))
            ...
            public:
                ...
                concurrent_hash_map(const concurrent_hash_map& table)
                    : concurrent_hash_map(node_allocator_traits::select_on_container_copy_construction(
                              table.get_allocator())) {
                    try_call([&] { internal_copy(table); }).on_exception([&] { this->clear(); });
                }
                ...
                concurrent_hash_map(std::initializer_list<value_type> il,
                                    ...
                                    const allocator_type& a = allocator_type())
                    : concurrent_hash_map(compare, a) {
                    try_call([&] { internal_copy(il.begin(), il.end(), il.size()); }).on_exception([&] {
                        this->clear();
                    });
                }
                ...
                concurrent_hash_map& operator=(concurrent_hash_map&& table) {
                    if (this != &table) {
                        ...
                        internal_move_assign(std::move(table),
        ...
        Also enables using of find() and count() concurrent methods in serial context. */
                void rehash(size_type sz = 0) {
                    ...
                    for (; b <= mask; b++, bp++) {
                        ...
                        __TBB_ASSERT(this->is_valid(n) || empty_rehashed(n) || rehash_required(n),
                                     "Broken internal structure");
                        __TBB_ASSERT(
                                *reinterpret_cast<intptr_t*>(&bp->mutex) == 0,
                                "concurrent or unexpectedly terminated operation during rehash() execution");
                        if (rehash_required(
                                    n)) {// rehash bucket, conditional because rehashing of a previous bucket may affect this one
                            hashcode_type h = b;
                            bucket* b_old = bp;
                            do {
                                __TBB_ASSERT(h > 1, "The lowermost buckets can't be rehashed");
                                hashcode_type m = (hashcode_type(1) << tbb::detail::log2(h))
                                                  - 1;// get parent mask from the topmost bit
                            ...
                            this->mark_rehashed_levels(
                                    h);// mark all non-rehashed children recursively across all segments
                            node_base* prev = nullptr;
                            ...
                            while (this->is_valid(curr)) {
                                ...
                                if ((curr_node_hash & mask) != h) {// should be rehashed
                                    ...
                                    __TBB_ASSERT(
                                            !rehash_required(
                                                    b_new->node_list.load(std::memory_order_relaxed)),
                ...
                void clear() {
                    ...
                    do {
                        __TBB_ASSERT(this->is_valid(this->my_table[s].load(std::memory_order_relaxed)),
                                     "wrong mask or concurrent grow");
                ...
                //------------------------------------------------------------------------
                iterator begin() {
                    return iterator(*this, 0, this->my_embedded_segment,
                ...
                bool insert(const value_type& value) {
                    return lookup</*insert*/ true>(value.first, &value.second, nullptr, /*write=*/false,
                                                   &allocate_node_copy_construct);
                }
            ...
            protected:
                ...
                node* allocate_node_helper(const K& key, const T* t, AllocateNodeType allocate_node,
                                           std::true_type) {
                ...
                bool lookup(const K& key, const T* t, const_accessor* result, bool write,
                            AllocateNodeType allocate_node, node* tmp_n = nullptr) {
                ...
                restart: {//lock scope
                    ...
                    if (OpInsert) {
                        ...
                        if (!n) {
                            if (!tmp_n) {
                                tmp_n = allocate_node_helper(key, t, allocate_node,
                            ...
                            if (this->check_mask_race(h, m)) goto restart;// b.release() is done in ~b().
                ...
                check_growth:
                    ...
                    if (grow_segment) { this->enable_segment(grow_segment); }
                ...
                bool generic_move_insert(Accessor&& result, value_type&& value) {
                    result.release();
                    return lookup</*insert*/ true>(value.first, &value.second, accessor_location(result),
                ...
                bool generic_emplace(Accessor&& result, Args&&... args) {
                    ...
                    return lookup</*insert*/ true>(node_ptr->value().first, nullptr,
                                                   accessor_location(result), is_write_access_needed(result),
                ...
                bool exclude(const_accessor& item_accessor) {
                    ...
                    do {
                        ...
                        if (curr == nullptr) {// someone else was first
                            if (this->check_mask_race(hash, mask)) continue;
                ...
                search:
                    ...
                    while (this->is_valid(erase_node)
                           && !my_hash_compare.equal(key, static_cast<node*>(erase_node)->value().first)) {
                        prev = erase_node;
                        erase_node = erase_node->next;
                    }

                    if (erase_node == nullptr) {// not found, but mask could be changed
                        if (this->check_mask_race(hash, mask)) goto restart;
                        return false;
                ...
                std::pair<I, I> internal_equal_range(const K& key, I end_) const {
                    ...
                    while (rehash_required(b->node_list.load(std::memory_order_relaxed))) {
                        m = (hashcode_type(1) << tbb::detail::log2(h))
                            - 1;// get parent mask from the topmost bit
                ...
                void internal_copy(const concurrent_hash_map& source) {
                    ...
                    if (this->my_mask.load(std::memory_order_relaxed) == mask) {      // optimized version
                        ...
                        for (hashcode_type k = 0; k <= mask; k++) {
                            ...
                            __TBB_ASSERT(!rehash_required(dst->node_list.load(std::memory_order_relaxed)),
                            ...
                            if (rehash_required(n)) {// source is not rehashed, items are in previous buckets
                                ...
                                dst->node_list.store(reinterpret_cast<node_base*>(rehash_req_flag),
                                                     std::memory_order_relaxed);
                            } else
                                for (; n; n = static_cast<node*>(n->next)) {
                                    node* node_ptr = create_node(base_type::get_allocator(), n->value().first,
                                                                 n->value().second);
                ...
                void internal_copy(I first, I last, size_type reserve_size) {
                    ...
                    for (; first != last; ++first) {
                        ...
                        __TBB_ASSERT(!rehash_required(b->node_list.load(std::memory_order_relaxed)),
                                     "Invalid bucket in destination table");
                        node* node_ptr
                                = create_node(base_type::get_allocator(), (*first).first, (*first).second);
        ...
        Must not be called concurrently with erasure operations. */
            ...
            template <typename It, typename HashCompare = d1::tbb_hash_compare<iterator_key_t<It>>,
            ...
            template <typename It, typename Alloc, typename = std::enable_if_t<is_input_iterator_v<It>>,
                      typename = std::enable_if_t<is_allocator_v<Alloc>>>
            ...
            template <typename Key, typename T,
                      ...
                      typename = std::enable_if_t<is_allocator_v<Alloc>>,
                      typename = std::enable_if_t<!is_allocator_v<HashCompare>>>
            concurrent_hash_map(std::initializer_list<std::pair<Key, T>>, HashCompare = HashCompare(),
                                Alloc = Alloc())
            ...
            inline bool operator==(const concurrent_hash_map<Key, T, HashCompare, A1>& a,
                ...
                typename concurrent_hash_map<Key, T, HashCompare, A1>::const_iterator i(a.begin()),
                        i_end(a.end());
