...
namespace tbb {
    namespace detail {
        namespace d1 {
            ...
            public:
                //! Constructors
                rw_mutex() noexcept : m_state(0) { create_itt_sync(this, "tbb::rw_mutex", ""); }
            ...
            private:
                ...
                bool upgrade() {
                    ...
                    while ((s & READERS) == ONE_READER || !(s & WRITER_PENDING)) {
                        if (m_state.compare_exchange_strong(s, s | WRITER | WRITER_PENDING)) {
                            auto wakeup_condition = [&] {
                                return (m_state.load(std::memory_order_relaxed) & READERS) == ONE_READER;
                            };
                            while ((m_state.load(std::memory_order_relaxed) & READERS) != ONE_READER) {
                                adaptive_wait_on_address(this, wakeup_condition, WRITER_CONTEXT);
                            }

                            __TBB_ASSERT((m_state.load(std::memory_order_relaxed) & (WRITER_PENDING | WRITER))
                ...
                void downgrade() {
                    ...
                    if (!(m_state & WRITER_PENDING)) { r1::notify_by_address(this, READER_CONTEXT); }
