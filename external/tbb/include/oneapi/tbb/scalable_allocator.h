...
/** The "posix_memalign" analogue.
    @ingroup memory_allocation */
TBBMALLOC_EXPORT int __TBB_EXPORTED_FUNC scalable_posix_memalign(void** memptr, size_t alignment,
                                                                 size_t size);
...
namespace rml {
    ...
    struct MemPoolPolicy {
        ...
        MemPoolPolicy(rawAllocType pAlloc_, rawFreeType pFree_, std::size_t granularity_ = 0,
            ...
            : pAlloc(pAlloc_), pFree(pFree_), granularity(granularity_), version(TBBMALLOC_POOL_VERSION),
              fixedPool(fixedPool_), keepAllMemory(keepAllMemory_), reserved(0) {}
    };
    ...
    TBBMALLOC_EXPORT void* pool_aligned_realloc(MemoryPool* mPool, void* ptr, std::size_t size,
        ...
        namespace d1 {
            ...
            public:
                ...
                __TBB_nodiscard T* allocate(std::size_t n) {
                    ...
                    if (!p) { throw_exception(std::bad_alloc()); }
                    return p;
                }

                //! Free previously allocated block of memory
                void deallocate(T* p, std::size_t) { scalable_free(p); }
            ...
            inline bool operator==(const scalable_allocator<T>&, const scalable_allocator<U>&) noexcept {
                return true;
            }
