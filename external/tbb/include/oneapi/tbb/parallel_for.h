...
namespace tbb {
    namespace detail {
        ...
        inline namespace d0 {
            ...
            concept parallel_for_body
                    = std::copy_constructible<Body> && requires(const std::remove_reference_t<Body>& body,
                                                                Range& range) {
                body(range);
            };
            ...
            concept parallel_for_index
                    = std::constructible_from<Index, int> && std::copyable<Index> && requires(
                            const std::remove_reference_t<Index>& lhs,
            ...
            concept parallel_for_function
                    = requires(const std::remove_reference_t<Function>& func, Index index) {
                func(index);
            };
...
**/
    ...
    See also requirements on \ref range_req "Range" and \ref parallel_for_body_req "parallel_for Body". **/
            ...
            __TBB_requires(tbb_range<Range>&& parallel_for_body<Body, Range>) void parallel_for(
                ...
                start_for<Range, Body, const __TBB_DEFAULT_PARTITIONER>::run(range, body,
            ...
            void parallel_for_impl(Index first, Index last, Index step, const Function& f,
                ...
                if (step <= 0) throw_exception(exception_id::nonpositive_step);// throws std::invalid_argument
                else if (first < last) {
            ...
            __TBB_requires(parallel_for_index<Index>&& parallel_for_function<
                ...
                parallel_for_impl<Index, Function, const simple_partitioner>(first, last, step, f,
                                                                             partitioner);
            }
            ...
            __TBB_requires(parallel_for_index<Index>&& parallel_for_function<
                ...
                parallel_for_impl<Index, Function, const static_partitioner>(first, last, step, f,
                                                                             partitioner);
            }
            ...
            template <typename Index, typename Function>
            __TBB_requires(parallel_for_index<Index>&& parallel_for_function<
                           Function, Index>) void parallel_for(Index first, Index last, const Function& f) {
                parallel_for_impl<Index, Function, const auto_partitioner>(first, last, static_cast<Index>(1),
                                                                           f, auto_partitioner());
            }
