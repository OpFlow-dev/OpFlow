...
namespace tbb {
    namespace detail {
        namespace d1 {
            ...
            public:
                //! Constructors
                mutex() { create_itt_sync(this, "tbb::mutex", ""); };

                //! Destructor
                ~mutex() {
                    __TBB_ASSERT(!my_flag.load(std::memory_order_relaxed),
                ...
                void lock() {
                    ...
                    while (!try_lock()) { my_flag.wait(true, /* context = */ 0, std::memory_order_relaxed); }
                }
