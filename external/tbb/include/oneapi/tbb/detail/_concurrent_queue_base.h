...
#pragma warning(disable : 4146)
            ...
            public:
                ...
                void push(ticket_type k, queue_rep_type& base, queue_allocator_type& allocator,
                    ...
                    p->mask.store(p->mask.load(std::memory_order_relaxed) | uintptr_t(1) << index,
                ...
                bool pop(void* dst, ticket_type k, queue_rep_type& base, queue_allocator_type& allocator) {
                    ...
                    bool success = false;
                    {
                        ...
                        micro_queue_pop_finalizer<self_type, value_type, page_allocator_type> finalizer(
                                *this, page_allocator, k + queue_rep_type::n_queue,
                ...
                micro_queue& assign(const micro_queue& src, queue_allocator_type& allocator,
                    ...
                    if (is_valid_page(srcp)) {
                        ...
                        size_type index = modulo_power_of_two(head_counter.load(std::memory_order_relaxed)
                                                                      / queue_rep_type::n_queue,
                                                              items_per_page);
                        size_type end_in_first_page
                                = (index + n_items < items_per_page) ? (index + n_items) : items_per_page;

                        try_call([&] {
                            head_page.store(make_copy(allocator, srcp, index, end_in_first_page, g_index,
                                                      construct_item),
                                            std::memory_order_relaxed);
                        }).on_exception([&] {
                        ...
                        try_call([&] {
                            if (srcp != src.tail_page.load(std::memory_order_relaxed)) {
                                for (srcp = srcp->next; srcp != src.tail_page.load(std::memory_order_relaxed);
                                     srcp = srcp->next) {
                                    cur_page->next = make_copy(allocator, srcp, 0, items_per_page, g_index,
                                                               construct_item);
                                ...
                                size_type last_index
                                        = modulo_power_of_two(tail_counter.load(std::memory_order_relaxed)
                                                                      / queue_rep_type::n_queue,
                                                              items_per_page);
                                if (last_index == 0) last_index = items_per_page;

                                cur_page->next
                                        = make_copy(allocator, srcp, 0, last_index, g_index, construct_item);
                                cur_page = cur_page->next;
                            }
                ...
                padded_page* make_copy(queue_allocator_type& allocator, const padded_page* src_page,
                                       size_type begin_in_page, size_type end_in_page, ticket_type& g_index,
                                       item_constructor_type construct_item) {
                    page_allocator_type page_allocator(allocator);
            ...
            public:
                ...
                void assign(const concurrent_queue_rep& src, queue_allocator_type& alloc,
                    ...
                    try_call([&] {
                        ...
                        for (size_type i = 0; i < queue_idx + 1; ++i) { array[i].clear(alloc); }
                        head_counter.store(0, std::memory_order_relaxed);
                        tail_counter.store(0, std::memory_order_relaxed);
                        n_invalid_entries.store(0, std::memory_order_relaxed);
                    });

                    __TBB_ASSERT(head_counter.load(std::memory_order_relaxed)
...
#pragma warning(pop)
            ...
            protected:
...
#endif
                    std::size_t i
                            = modulo_power_of_two(k / queue_rep_type::n_queue, my_queue_rep->items_per_page);
                ...
                bool get_item(Value*& item, std::size_t k) {
                    if (k == my_queue_rep->tail_counter.load(std::memory_order_relaxed)) {
                        ...
                        __TBB_ASSERT(p, nullptr);
                        std::size_t i = modulo_power_of_two(k / queue_rep_type::n_queue,
            ...
            class concurrent_queue_iterator
                ...
                using base_type
                        = concurrent_queue_iterator_base<typename std::remove_cv<Value>::type, Allocator>;
        ...
        If Value==const Container::value_type, then this routine is a conversion constructor. */
                concurrent_queue_iterator(
                        const concurrent_queue_iterator<Container, typename Container::value_type, Allocator>&
                                other)
                    : base_type(other) {}
            ...
            public:
                concurrent_queue_iterator&
                operator=(const concurrent_queue_iterator<Container, typename Container::value_type,
                                                          Allocator>& other) {
                    this->assign(other);
                    return *this;
                }
