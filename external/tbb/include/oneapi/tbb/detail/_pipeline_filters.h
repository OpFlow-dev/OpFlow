    ...
    namespace detail {
        ...
        namespace d1 {
            ...
            private:
                ...
                static base_filter* not_in_pipeline() {
                    return reinterpret_cast<base_filter*>(std::intptr_t(-1));
                }
            ...
            protected:
                explicit base_filter(unsigned int m)
                    : next_filter_in_pipeline(not_in_pipeline()), my_input_buffer(nullptr), my_filter_mode(m),
                      my_pipeline(nullptr) {}

                // signal end-of-input for concrete_filters
                void set_end_of_input() { r1::set_end_of_input(*this); }
        ...
        Note it can be called concurrently even for serial filters.*/
            ...
            class flow_control {
                ...
                template <typename Body, typename InputType, typename OutputType>
                friend class concrete_filter;
                template <typename Output>
                __TBB_requires(std::copyable<Output>) friend class input_node;
...
// Emulate std::is_trivially_copyable (false positives not allowed, false negatives suboptimal but safe).
            ...
            class concrete_filter : public base_filter {
                ...
                void* operator()(void* input) override {
                    ...
                    output_pointer temp_output = output_helper::create_token(
            ...
            public:
                concrete_filter(unsigned int m, const Body& body)
                    : base_filter(m | filter_may_emit_null), my_body(body) {}
            };
            ...
            inline filter_node_ptr::filter_node_ptr(filter_node* nd) : my_node(nd) {
                if (my_node) { my_node->add_ref(); }
            }

            inline filter_node_ptr::~filter_node_ptr() {
                if (my_node) { my_node->remove_ref(); }
            }
            ...
            inline void filter_node_ptr::operator=(const filter_node_ptr& rhs) { *this = rhs.my_node; }
            ...
            inline filter_node_ptr::operator bool() const { return my_node != nullptr; }
            ...
            class filter_node_leaf : public filter_node {
                ...
                base_filter* create_filter() const override {
                    return new (r1::allocate_memory(sizeof(concrete_filter<InputType, OutputType, Body>)))
                            concrete_filter<InputType, OutputType, Body>(my_mode, my_body);
                }
            ...
            template <typename Body,
                      typename Input = typename body_types<decltype(&Body::operator())>::input_type>
            using filter_input =
                    typename std::conditional<std::is_same<Input, flow_control>::value, void, Input>::type;
