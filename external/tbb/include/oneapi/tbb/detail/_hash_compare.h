    ...
    namespace detail {
        namespace d1 {
            ...
            public:
                ...
                std::size_t operator()(const Key& key) const { return std::size_t(my_hasher(key)); }

                bool operator()(const Key& key1, const Key& key2) const { return my_equal(key1, key2); }
                ...
                hasher hash_function() const { return my_hasher; }

                key_equal key_eq() const { return my_equal; }
...
#pragma warning(pop)
        ...
        inline namespace d0 {
            ...
            concept hash_compare = std::copy_constructible<HashCompare> && requires(
                    const std::remove_reference_t<HashCompare>& hc, const Key& key1, const Key& key2) {
...
// Apple clang and MSVC defines their own specializations for std::hash<std::basic_string<T, Traits, Alloc>>
    ...
    public:
        std::size_t operator()(const std::basic_string<CharT, Traits, Allocator>& s) const {
            std::size_t h = 0;
            for (const CharT* c = s.c_str(); *c; ++c) { h = h * hash_multiplier ^ char_hash(*c); }
            return h;
        }
