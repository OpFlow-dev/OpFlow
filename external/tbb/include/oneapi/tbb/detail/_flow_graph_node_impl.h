...
public:
    ...
    function_input_base(graph& g, size_t max_concurrency, node_priority_t a_priority)
        : my_graph_ref(g), my_max_concurrency(max_concurrency), my_concurrency(0), my_priority(a_priority),
          my_queue(!has_policy<rejecting, Policy>::value ? new input_queue_type() : NULL),
          my_predecessors(this), forwarder_busy(false) {
...
protected:
    ...
    graph& graph_reference() const override { return my_graph_ref; }
...
private:
    ...
    public:
        ...
        graph_task* bypass_t;
        operation_type(const input_type& e, op_type t) : type(char(t)), elem(const_cast<input_type*>(&e)) {}
    ...
    void internal_forward(operation_type* op) {
        op->bypass_t = NULL;
        if (my_concurrency < my_max_concurrency) op->bypass_t = perform_queued_requests();
        if (op->bypass_t) op->status.store(SUCCEEDED, std::memory_order_release);
        else {
            forwarder_busy = false;
            op->status.store(FAILED, std::memory_order_release);
        }
    ...
    graph_task* internal_try_put_bypass(const input_type& t) {
        ...
        my_aggregator.execute(&op_data);
        if (op_data.status == SUCCEEDED) { return op_data.bypass_t; }
        return NULL;
    }
    ...
    graph_task* try_put_task_impl(const input_type& t, /*lightweight=*/std::true_type) {
        if (my_max_concurrency == 0) {
            ...
            if (check_op.status == SUCCEEDED) { return apply_body_bypass(t); }
    ...
    graph_task* create_body_task(const input_type& input) {
        if (!is_graph_active(my_graph_ref)) { return nullptr; }
    ...
    inline void spawn_forward_task() {
        ...
        if (tp) { spawn_in_graph_arena(graph_reference(), *tp); }
    }
...
public:
    ...
    function_input(const function_input& src)
        : base_type(src), my_body(src.my_init_body->clone()), my_init_body(src.my_init_body->clone()) {}
...
struct clear_element {
    ...
    static bool this_empty(P& p) {
        if (std::get<N - 1>(p).successors().empty()) return clear_element<N - 1>::this_empty(p);
        return false;
    }
#endif
};
...
class multifunction_input
    : public function_input_base<Input, Policy, A, multifunction_input<Input, OutputPortSet, Policy, A>> {
public:
    ...
    Body copy_function_object() {
        ...
        return *static_cast<Body*>(
                dynamic_cast<multifunction_body_leaf<input_type, output_ports_type, Body>&>(body_ref)
                        .get_body_ptr());
    }
...
inline void check_task_and_spawn(graph& g, graph_task* t) {
    if (t && t != SUCCESSFULLY_ENQUEUED) { spawn_in_graph_arena(g, *t); }
}
...
public:
    ...
    continue_input(graph& g, Body& body, node_priority_t a_priority)
        : continue_receiver(/*number_of_predecessors=*/0, a_priority), my_graph_ref(g),
          my_body(new function_body_leaf<input_type, output_type, Body>(body)),
          my_init_body(new function_body_leaf<input_type, output_type, Body>(body)) {}

    template <typename Body>
    continue_input(graph& g, int number_of_predecessors, Body& body, node_priority_t a_priority)
        : continue_receiver(number_of_predecessors, a_priority), my_graph_ref(g),
          my_body(new function_body_leaf<input_type, output_type, Body>(body)),
          my_init_body(new function_body_leaf<input_type, output_type, Body>(body)) {}

    continue_input(const continue_input& src)
        : continue_receiver(src), my_graph_ref(src.my_graph_ref), my_body(src.my_init_body->clone()),
...
protected:
    ...
    graph_task* execute() override {
        if (!is_graph_active(my_graph_ref)) { return NULL; }
#if _MSC_VER && !__INTEL_COMPILER
#pragma warning(push)
#pragma warning(disable : 4127) /* suppress conditional expression is constant */
#endif
...
protected:
    graph_task* try_put_task(const output_type& i) { return my_successors.try_put_task(i); }
