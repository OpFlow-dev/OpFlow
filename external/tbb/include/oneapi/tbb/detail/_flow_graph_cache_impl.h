...
public:
    ...
    void remove(T& n) {
        ...
        for (size_t i = internal_size(); i != 0; --i) {
            T& s = internal_pop();
            if (&s == &n) break;// only remove one predecessor per request
            internal_push(s);
        }
    }
...
public:
    ...
    bool try_reserve(output_type& v) {
        ...
        do {
            ...
            {
                typename mutex_type::scoped_lock lock(this->my_mutex);
                if (reserved_src.load(std::memory_order_relaxed) || this->internal_empty()) return false;
...
public:
    ...
    void register_successor(successor_type& r) {
        ...
        if (r.priority() != no_priority) my_successors.push_front(&r);
        else
    ...
    void clear() { my_successors.clear(); }
...
public:
    ...
    void register_successor(successor_type& r) {
        ...
        if (r.is_continue_receiver()) { r.register_predecessor(*my_owner); }
    }
...
public:
    ...
    bool gather_successful_try_puts(const T& t, graph_task_list& tasks) {
        ...
        while (i != this->my_successors.end()) {
            ...
            if (new_task) {
                ++i;
                if (new_task != SUCCESSFULLY_ENQUEUED) { tasks.push_back(*new_task); }
