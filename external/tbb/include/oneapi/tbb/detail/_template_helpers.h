...
namespace tbb {
  ...
  situations that inherently break strict ISO C++ aliasing rules. */
            ...
            using iterator_key_t =
                    typename std::remove_const<typename iterator_value_t<Iterator>::first_type>::type;
            ...
            public:
                static_assert(std::is_nothrow_copy_constructible<Func>::value
                              ...
                              "Throwing an exception during the Func copy or move construction cause an "
                              "unexpected behavior.");
