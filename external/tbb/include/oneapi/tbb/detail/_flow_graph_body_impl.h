...
namespace graph_policy_namespace {
    ...
    __TBB_requires(tbb::detail::hash_compare<KHash, K>) struct key_matching {
        typedef K key_type;
...
public:
    ...
    input_body_leaf* clone() override { return new input_body_leaf<Output, Body>(body); }
...
public:
    ...
    function_body_leaf* clone() override { return new function_body_leaf<Input, Output, B>(body); }
...
public:
    ...
    B get_body() { return body; }
    function_body_leaf* clone() override { return new function_body_leaf<Input, continue_msg, B>(body); }
...
public:
    function_body_leaf(const B& _body) : body(_body) {}
    Output operator()(const continue_msg& i) override { return body(i); }
    B get_body() { return body; }
    function_body_leaf* clone() override { return new function_body_leaf<continue_msg, Output, B>(body); }
...
public:
    type_to_key_function_body_leaf(const B& _body) : body(_body) {}
    const Output& operator()(const Input& i) override { return body(i); }
    type_to_key_function_body_leaf* clone() override {
        return new type_to_key_function_body_leaf<Input, Output&, B>(body);
    }
...
public:
    ...
    task* execute(execution_data& ed) override {
        ...
        if (SUCCESSFULLY_ENQUEUED == next_task) next_task = nullptr;
        else if (next_task)
...
public:
    apply_body_task_bypass(graph& g, small_object_allocator& allocator, NodeType& n, const Input& i,
                           node_priority_t node_priority = no_priority)
        : graph_task(g, allocator, node_priority), my_node(n), my_input(i) {}
...
protected:
    graph_task* try_put_task(const DecrementType& value) override {
        ...
        if (!result) result = SUCCESSFULLY_ENQUEUED;
        return result;
    }

    graph& graph_reference() const override { return my_node->my_graph; }
...
class threshold_regulator<T, continue_msg, void> : public continue_receiver, no_copy {
    ...
    graph_task* execute() override { return my_node->decrement_counter(1); }
