...
namespace tbb {
    namespace detail {
        ...
        namespace d2 {
            ...
            class graph_task;
            inline graph_task* const SUCCESSFULLY_ENQUEUED = (graph_task*) -1;
            typedef unsigned int node_priority_t;
            inline const node_priority_t no_priority = node_priority_t(0);
            ...
            enum reset_flags {
                rf_reset_protocol = 0,
                rf_reset_bodies
                = 1 << 0,// delete the current node body, reset to a copy of the initial node body.
            ...
            public:
                trackable_messages_graph_task(graph& g, d1::small_object_allocator& allocator,
                    ...
                    : graph_task(g, allocator, node_priority), my_msg_wait_context_vertices(msg_waiters) {
                    ...
                    for (auto& msg_waiter : my_msg_wait_context_vertices) {
                        ...
                        d1::wait_tree_vertex_interface* ref_vertex
                                = is_this_thread_in_graph_arena(g)
                                          ? r1::get_thread_reference_vertex(msg_waiter)
                                          : msg_waiter;
                        last_iterator = my_msg_reference_vertices.emplace_after(last_iterator, ref_vertex);
            ...
            protected:
                ...
                void finalize(const d1::execution_data& ed) {
                    ...
                    if (msg_reference_vertices.empty()) {
                        for (auto& msg_waiter : wait_context_vertices) { msg_waiter->release(1); }
                    } else {
                        for (auto& msg_waiter : msg_reference_vertices) { msg_waiter->release(1); }
                    }
                }
            ...
            public:
                ...
                graph_task& pop_front() {
                    ...
                    if (!my_first) { my_next_ptr = &my_first; }
                    return *result;
                }
            };
    ...
    matches the number of release_wait calls. */
                ...
                void wait_for_all() {
                    ...
                    try_call([this] {
                        my_task_arena->execute(
                                [this] { d1::wait(my_wait_context_vertex.get_context(), *my_context); });
            ...
            inline graph_task::graph_task(graph& g, d1::small_object_allocator& allocator,
                ...
                my_reference_vertex = is_this_thread_in_graph_arena(g)
                                              ? r1::get_thread_reference_vertex(graph_wait_context_vertex)
            ...
            inline graph_task* prioritize_task(graph& g, graph_task& gt) {
                if (no_priority == gt.priority) return &gt;
                ...
                d1::small_object_allocator allocator;
                d1::task* critical_task
                        = allocator.new_object<priority_task_selector>(g.my_priority_queue, allocator);
