...
namespace tbb {
    namespace detail {
        ...
        namespace r1 {
            ...
            TBB_EXPORT void __TBB_EXPORTED_FUNC spawn(d1::task& t, d1::task_group_context& ctx,
                                                      d1::slot_id id);
            TBB_EXPORT void __TBB_EXPORTED_FUNC execute_and_wait(d1::task& t, d1::task_group_context& t_ctx,
                                                                 d1::wait_context&,
        ...
        namespace d1 {
            ...
            inline void resume(suspend_point tag) { r1::resume(tag); }
            ...
            public:
                ...
                ~wait_context() { __TBB_ASSERT(!continue_execution(), NULL); }

                void reserve(std::uint32_t delta = 1) { add_reference(delta); }

                void release(std::uint32_t delta = 1) { add_reference(-std::int64_t(delta)); }
            };
            ...
            inline task_group_context* context(const execution_data& ed) { return ed.context; }

            inline slot_id original_slot(const execution_data& ed) { return ed.original_slot; }

            inline slot_id affinity_slot(const execution_data& ed) { return ed.affinity_slot; }

            inline slot_id execution_slot(const execution_data& ed) { return r1::execution_slot(&ed); }
            ...
            inline void execute_and_wait(task& t, task_group_context& t_ctx, wait_context& wait_ctx,
                                         task_group_context& w_ctx) {
