...
private:
    ...
    public:
        ...
        reserving_port_operation(const T &e, op_type t) : type(char(t)), my_arg(const_cast<T *>(&e)) {}
...
protected:
    ...
    graph_task *try_put_task(const T &) override { return nullptr; }

    graph &graph_reference() const override { return my_join->graph_ref; }
...
public:
    ...
    void set_join_node_pointer(reserving_forwarding_base *join) { my_join = join; }
...
private:
    ...
    public:
        ...
        queueing_port_operation(const T &e, op_type t) : type(char(t)), my_val(e), bypass_t(nullptr) {}
...
public:
    ...
    void set_join_node_pointer(queueing_forwarding_base *join) { my_join = join; }
    ...
    void reset_receiver(reset_flags) { item_buffer<T>::reset(); }
...
public:
    ...
    void reset_receiver(reset_flags) { buffer_type::reset(); }
...
public:
    ...
    join_node_FE(const join_node_FE &other)
        : reserving_forwarding_base((other.reserving_forwarding_base::graph_ref)), my_node(nullptr) {
        ports_with_no_inputs = N;
        join_helper<N>::set_join_node_pointer(my_inputs, this);
    }
    ...
    void increment_port_count() override { ++ports_with_no_inputs; }
...
public:
    ...
    join_node_FE(const join_node_FE &other)
        : queueing_forwarding_base((other.queueing_forwarding_base::graph_ref)), my_node(nullptr) {
        ports_with_no_items = N;
        join_helper<N>::set_join_node_pointer(my_inputs, this);
    }
...
public:
    ...
    typedef type_to_key_function_body_leaf<count_element_type, unref_key_type &, key_to_count_func>
        ...
        while (op_list) {
            ...
            switch (current->type) {
                ...
                case inc_count: {// called from input ports
                    ...
                    if (++(p->my_value) == size_t(N)) { current->bypass_t = fill_output_buffer(t); }
                }
                    current->status.store(SUCCEEDED, std::memory_order_release);
                    break;
...
public:
    ...
    join_node_FE(const join_node_FE &other)
        : forwarding_base_type((other.forwarding_base_type::graph_ref)), key_to_count_buffer_type(),
          output_buffer_type() {
        my_node = nullptr;
        join_helper<N>::set_join_node_pointer(my_inputs, this);
...
public:
    ...
    join_node_base(const join_node_base &other)
        : graph_node(other.graph_node::my_graph), input_ports_type(other), sender<OutputTuple>(),
          forwarder_busy(false), my_successors(this) {
        input_ports_type::set_my_node(this);
        my_aggregator.initialize_handler(handler_type(this));
    }
...
struct key_from_message_body {
    K operator()(const T &t) const { return key_from_message<K>(t); }
};
// Adds const to reference type
template <typename K, typename T>
struct key_from_message_body<K &, T> {
    const K &operator()(const T &t) const { return key_from_message<const K &>(t); }
};
...
public:
    typedef typename wrap_key_tuple_elements<2, key_matching_port, key_matching<K, KHash>, OutputTuple>::type
            input_ports_type;
    typedef OutputTuple output_type;
...
public:
    ...
    unfolded_join_node(graph &g, Body0 body0, Body1 body1)
        : base_type(g, func_initializer_type(new type_to_key_function_body_leaf<T0, K, Body0>(body0),
...
public:
    typedef typename wrap_key_tuple_elements<3, key_matching_port, key_matching<K, KHash>, OutputTuple>::type
            input_ports_type;
    typedef OutputTuple output_type;
...
public:
    typedef typename wrap_key_tuple_elements<4, key_matching_port, key_matching<K, KHash>, OutputTuple>::type
            input_ports_type;
    typedef OutputTuple output_type;
...
public:
    typedef typename wrap_key_tuple_elements<5, key_matching_port, key_matching<K, KHash>, OutputTuple>::type
            input_ports_type;
    typedef OutputTuple output_type;
...
public:
    typedef typename wrap_key_tuple_elements<6, key_matching_port, key_matching<K, KHash>, OutputTuple>::type
            input_ports_type;
    typedef OutputTuple output_type;
...
public:
    typedef typename wrap_key_tuple_elements<7, key_matching_port, key_matching<K, KHash>, OutputTuple>::type
            input_ports_type;
    typedef OutputTuple output_type;
...
public:
    typedef typename wrap_key_tuple_elements<8, key_matching_port, key_matching<K, KHash>, OutputTuple>::type
            input_ports_type;
    typedef OutputTuple output_type;
...
#endif /* __TBB_PREVIEW_MESSAGE_BASED_KEY_MATCHING */
    template <typename Body0, typename Body1, typename Body2, typename Body3, typename Body4, typename Body5,
              typename Body6, typename Body7>
    unfolded_join_node(graph &g, Body0 body0, Body1 body1, Body2 body2, Body3 body3, Body4 body4, Body5 body5,
                       Body6 body6, Body7 body7)
        : base_type(g, func_initializer_type(new type_to_key_function_body_leaf<T0, K, Body0>(body0),
                                             new type_to_key_function_body_leaf<T1, K, Body1>(body1),
                                             new type_to_key_function_body_leaf<T2, K, Body2>(body2),
                                             new type_to_key_function_body_leaf<T3, K, Body3>(body3),
...
public:
    typedef typename wrap_key_tuple_elements<9, key_matching_port, key_matching<K, KHash>, OutputTuple>::type
            input_ports_type;
    typedef OutputTuple output_type;
...
#endif /* __TBB_PREVIEW_MESSAGE_BASED_KEY_MATCHING */
    template <typename Body0, typename Body1, typename Body2, typename Body3, typename Body4, typename Body5,
              typename Body6, typename Body7, typename Body8>
    unfolded_join_node(graph &g, Body0 body0, Body1 body1, Body2 body2, Body3 body3, Body4 body4, Body5 body5,
                       Body6 body6, Body7 body7, Body8 body8)
        : base_type(g, func_initializer_type(new type_to_key_function_body_leaf<T0, K, Body0>(body0),
                                             new type_to_key_function_body_leaf<T1, K, Body1>(body1),
                                             new type_to_key_function_body_leaf<T2, K, Body2>(body2),
                                             new type_to_key_function_body_leaf<T3, K, Body3>(body3),
...
public:
    typedef typename wrap_key_tuple_elements<10, key_matching_port, key_matching<K, KHash>, OutputTuple>::type
            input_ports_type;
    typedef OutputTuple output_type;
...
#endif /* __TBB_PREVIEW_MESSAGE_BASED_KEY_MATCHING */
    template <typename Body0, typename Body1, typename Body2, typename Body3, typename Body4, typename Body5,
              typename Body6, typename Body7, typename Body8, typename Body9>
    unfolded_join_node(graph &g, Body0 body0, Body1 body1, Body2 body2, Body3 body3, Body4 body4, Body5 body5,
                       Body6 body6, Body7 body7, Body8 body8, Body9 body9)
        : base_type(g, func_initializer_type(new type_to_key_function_body_leaf<T0, K, Body0>(body0),
                                             new type_to_key_function_body_leaf<T1, K, Body1>(body1),
                                             new type_to_key_function_body_leaf<T2, K, Body2>(body2),
                                             new type_to_key_function_body_leaf<T3, K, Body3>(body3),
