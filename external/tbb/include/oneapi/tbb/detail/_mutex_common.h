...
namespace tbb {
    namespace detail {
        inline namespace d0 {
            ...
            concept mutex_scoped_lock
                    = std::default_initializable<Lock> && std::constructible_from<Lock, Mutex&> && requires(
            ...
            template <typename Lock, typename Mutex>
            concept rw_mutex_scoped_lock = mutex_scoped_lock<Lock, Mutex> && std::constructible_from<
                    Lock, Mutex&, bool> && requires(Lock& lock, Mutex& mutex) {
            ...
            template <typename Mutex>
            concept rw_scoped_lockable
                    = scoped_lockable<Mutex> && rw_mutex_scoped_lock<typename Mutex::scoped_lock, Mutex>;
