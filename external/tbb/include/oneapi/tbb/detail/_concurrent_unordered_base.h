    ...
    namespace detail {
        namespace d1 {
            ...
            public:
                ...
                solist_iterator&
                operator=(const solist_iterator<Container, typename Container::value_type>& other) {
                    my_node_ptr = other.my_node_ptr;
                    return *this;
                }
                ...
                solist_iterator& operator++() {
                    ...
                    while (next_node && next_node->is_dummy()) { next_node = next_node->next(); }
            ...
            public:
                ...
                void init(sokey_type key) { my_order_key = key; }

                sokey_type order_key() const { return my_order_key; }
                ...
                node_ptr next() const { return my_next.load(std::memory_order_acquire); }

                void set_next(node_ptr next_node) { my_next.store(next_node, std::memory_order_release); }
            ...
            public:
                ...
                ~value_node() {}
                value_type* storage() { return reinterpret_cast<value_type*>(&my_value); }
            ...
            public:
                ...
                explicit concurrent_unordered_base(size_type bucket_count, const hasher& hash = hasher(),
                    ...
                    : my_size(0), my_bucket_count(round_up_to_power_of_two(bucket_count)),
                      my_max_load_factor(float(initial_max_load_factor)), my_hash_compare(hash, equal),
                      my_head(sokey_type(0)), my_segments(alloc) {}
                ...
                template <typename InputIterator>
                concurrent_unordered_base(InputIterator first, InputIterator last, size_type bucket_count,
                                          const allocator_type& alloc)
                ...
                concurrent_unordered_base(const concurrent_unordered_base& other)
                    : my_size(other.my_size.load(std::memory_order_relaxed)),
                      ...
                      my_max_load_factor(other.my_max_load_factor), my_hash_compare(other.my_hash_compare),
                      my_head(other.my_head.order_key()), my_segments(other.my_segments) {
                ...
                concurrent_unordered_base(const concurrent_unordered_base& other, const allocator_type& alloc)
                    : my_size(other.my_size.load(std::memory_order_relaxed)),
                      my_bucket_count(other.my_bucket_count.load(std::memory_order_relaxed)),
                      my_max_load_factor(other.my_max_load_factor), my_hash_compare(other.my_hash_compare),
                      my_head(other.my_head.order_key()), my_segments(other.my_segments, alloc) {
                    try_call([&] { internal_copy(other); }).on_exception([&] { clear(); });
                }
                ...
                concurrent_unordered_base(concurrent_unordered_base&& other)
                    : my_size(other.my_size.load(std::memory_order_relaxed)),
                      ...
                      my_hash_compare(std::move(other.my_hash_compare)), my_head(other.my_head.order_key()),
                ...
                ~concurrent_unordered_base() { internal_clear(); }
                ...
                concurrent_unordered_base& operator=(concurrent_unordered_base&& other) noexcept(
                ...
                void
                swap(concurrent_unordered_base& other) noexcept(unordered_segment_table::is_noexcept_swap) {
                    if (this != &other) {
                ...
                void clear() noexcept { internal_clear(); }
                    ...
                    if (!nh.empty()) {
                        ...
                        if (insert_result.inserted) {
                            ...
                            __TBB_ASSERT(insert_result.remaining_node == nullptr,
                                         "internal_insert_node should not return the remaining node if the "
                                         "insertion succeeded");
                ...
                iterator unsafe_erase(const_iterator first, const_iterator last) {
                    while (first != last) { first = unsafe_erase(first); }
                    return iterator(first.get_node_ptr());
                }

                size_type unsafe_erase(const key_type& key) { return internal_erase_by_key(key); }
                ...
                size_type unsafe_max_bucket_count() const { return max_size(); }
                ...
                void rehash(size_type bucket_count) {
                    ...
                    if (current_bucket_count < bucket_count) {
                        ...
                        my_bucket_count.compare_exchange_strong(current_bucket_count,
                                                                round_up_to_power_of_two(bucket_count));
                    }
                }
                ...
                public:
                    ...
                    const_range_type(const_range_type& range, split)
                        : my_instance(range.my_instance), my_begin_node(range.my_midpoint_node),
                    ...
                    const_range_type(const concurrent_unordered_base& table)
                        : my_instance(table), my_begin_node(const_cast<node_ptr>(&table.my_head)),
                          my_end_node(nullptr) {
                        set_midpoint();
                    }
                    ...
                    void set_midpoint() const {
                        if (my_begin_node == my_end_node) {
                            ...
                            sokey_type begin_key
                                    = my_begin_node != nullptr ? my_begin_node->order_key() : invalid_key;
                            sokey_type end_key
            ...
            private:
                ...
                class unordered_segment_table
                    : public segment_table<std::atomic<node_ptr>, allocator_type, unordered_segment_table,
                                           pointers_per_embedded_table> {
                    using self_type = unordered_segment_table;
                    using atomic_node_ptr = std::atomic<node_ptr>;
                    using base_type = segment_table<std::atomic<node_ptr>, allocator_type,
                ...
                public:
                    ...
                    segment_type create_segment(typename base_type::segment_table_type,
...
// GCC 11.1 issues a warning here that incorrect destructor might be called for dummy_nodes
                ...
                std::pair<value_node_ptr, bool> search_after(node_ptr& prev, sokey_type order_key,
                                                             const key_type& key) {
                    ...
                    while (curr != nullptr
                           && (curr->order_key() < order_key
                               || (curr->order_key() == order_key
                                   && !my_hash_compare(
                                           traits_type::get_key(static_cast<value_node_ptr>(curr)->value()),
                                           key)))) {
                        prev = curr;
                        curr = curr->next();
                    }
                ...
                void init_bucket(size_type bucket) {
                    ...
                    __TBB_ASSERT(my_segments[parent_bucket].load(std::memory_order_acquire) != nullptr,
                ...
                value_node_ptr create_node(sokey_type order_key, Args&&... args) {
                    ...
                    value_node_allocator_traits::construct(value_node_allocator, new_node->storage(),
            ...
            protected:
                ...
                void internal_merge(SourceType&& source) {
                    static_assert(
                            std::is_same<node_type, typename std::decay<SourceType>::type::node_type>::value,
                            ...
                            if (allow_multimapping || !contains(traits_type::get_key(curr->value()))) {
                                ...
                                if (!insert(std::move(curr_node)).second) {
                                    ...
                                    __TBB_ASSERT(source_prev->next() == next_node,
                                                 "Concurrent operations with the source container in merge "
                                                 "are prohibited");
            ...
            private:
                ...
                void unlink_node(node_ptr prev_node, node_ptr node_to_unlink, node_ptr next_node) {
                    __TBB_ASSERT(
                            prev_node->next() == node_to_unlink && node_to_unlink->next() == next_node,
                        ...
                        if (curr->order_key() > order_key) {
                            ...
                            do {
                                last = last->next();
                                     ...
                                     && my_hash_compare(
                                             traits_type::get_key(static_cast<value_node_ptr>(last)->value()),
                                             key));
                ...
                void internal_copy(const concurrent_unordered_base& other) {
                    ...
                    for (node_ptr node = other.my_head.next(); node != nullptr; node = node->next()) {
                        ...
                        if (!node->is_dummy()) {
                            ...
                            my_segments[reverse_bits(node->order_key())].store(new_node,
                                                                               std::memory_order_relaxed);
                        }
                ...
                void internal_move_construct_with_allocator(concurrent_unordered_base&& other,
                ...
                void internal_move_construct_with_allocator(concurrent_unordered_base&& other,
                    ...
                    if (alloc == other.my_segments.get_allocator()) {
                        ...
                        try_call([&] { internal_move(std::move(other)); }).on_exception([&] { clear(); });
                    }
                }
