        ...
        namespace d2 {
            ...
            public:
                ...
                static skip_list_node* create(container_allocator_type& alloc, size_type height) {
                    ...
                    static_assert(std::is_same<typename allocator_traits::value_type, std::uint8_t>::value,
                ...
                void set_index_number(size_type index_num) { my_index_number = index_num; }

                size_type index_number() const { return my_index_number; }
            ...
            protected:
                ...
                using node_allocator_type =
                        typename allocator_traits_type::template rebind_alloc<std::uint8_t>;
            ...
            public:
                ...
                explicit concurrent_skip_list(const key_compare& comp,
                    ...
                    : my_node_allocator(alloc), my_compare(comp), my_head_ptr(nullptr), my_size(0),
                      my_max_height(0) {}
                ...
                concurrent_skip_list(const concurrent_skip_list& other)
                    : my_node_allocator(node_allocator_traits::select_on_container_copy_construction(
                            other.get_allocator())),
                      my_compare(other.my_compare), my_rng(other.my_rng), my_head_ptr(nullptr), my_size(0),
                      my_max_height(0) {
                    internal_copy(other);
                ...
                concurrent_skip_list(const concurrent_skip_list& other, const allocator_type& alloc)
                    : my_node_allocator(alloc), my_compare(other.my_compare), my_rng(other.my_rng),
                      my_head_ptr(nullptr), my_size(0), my_max_height(0) {
                    internal_copy(other);
                    __TBB_ASSERT(my_size == other.my_size, "Wrong size of copy-constructed container");
                }
                ...
                concurrent_skip_list(concurrent_skip_list&& other, const allocator_type& alloc)
                    : my_node_allocator(alloc), my_compare(other.my_compare), my_rng(std::move(other.my_rng)),
                      my_head_ptr(nullptr) {
                ...
                concurrent_skip_list& operator=(concurrent_skip_list&& other) {
                    if (this != &other) {
                        ...
                        internal_move_assign(std::move(other),
                ...
                std::pair<iterator, bool> insert(const value_type& value) { return internal_insert(value); }
                ...
                std::pair<iterator, bool> insert(node_type&& nh) {
                    if (!nh.empty()) {
                        ...
                        if (insert_result.second) { d1::node_handle_accessor::deactivate(nh); }
                ...
                iterator unsafe_erase(const_iterator pos) { return unsafe_erase(get_iterator(pos)); }
                ...
                size_type unsafe_erase(const key_type& key) { return internal_erase(key); }
                ...
                node_type unsafe_extract(const_iterator pos) {
                    ...
                    return extract_result.first
                                   ? d1::node_handle_accessor::construct<node_type>(extract_result.first)
                                   : node_type();
                }

                node_type unsafe_extract(iterator pos) { return unsafe_extract(const_iterator(pos)); }

                node_type unsafe_extract(const key_type& key) { return unsafe_extract(find(key)); }
                ...
                iterator find(const key_type& key) { return iterator(internal_find(key)); }

                const_iterator find(const key_type& key) const { return const_iterator(internal_find(key)); }
                ...
                size_type count(const key_type& key) const { return internal_count(key); }
                ...
                iterator begin() { return iterator(internal_begin()); }

                const_iterator begin() const { return const_iterator(internal_begin()); }

                const_iterator cbegin() const { return const_iterator(internal_begin()); }
                ...
                __TBB_nodiscard bool empty() const { return 0 == size(); }
                ...
                template <typename K>
                typename std::enable_if<is_transparent<K>::value, std::pair<iterator, iterator>>::type
                ...
                public:
                    ...
                    bool empty() const { return my_begin.my_node_ptr->next(0) == my_end.my_node_ptr; }

                    bool is_divisible() const {
                        return my_level != 0 ? my_begin.my_node_ptr->next(my_level - 1) != my_end.my_node_ptr
                                             : false;
                    }
            ...
            private:
                ...
                node_ptr internal_find_multi(const K& key) const {
                    ...
                    for (size_type h = my_max_height.load(std::memory_order_acquire); h > 0; --h) {
                        ...
                        if (curr != old_curr && found(curr, key)) { return curr; }
                        old_curr = curr;
                    }
                    return nullptr;
                }
                ...
                node_ptr internal_find_unique(const K& key) const {
                    ...
                    return (it == end() || my_compare(key, container_traits::get_key(*it))) ? nullptr
                                                                                            : it.my_node_ptr;
                }
                        ...
                        if (!allow_multimapping) {
                            ...
                            for (size_type h = prev->height(); h > 0; --h) {
                                ...
                                while (curr && cmp(get_key(curr), key)) {
                                    ...
                                    if (h < curr->height()) { h = curr->height(); }
                                    curr = prev->next(h - 1);
                                }
                            }
                ...
                template <typename Comparator>
                void fill_prev_curr_arrays(array_type& prev_nodes, array_type& curr_nodes, node_ptr node,
                                           const key_type& key, const Comparator& cmp, node_ptr head) {
                    ...
                    if (curr_max_height < node_height) {
                        std::fill(prev_nodes.begin() + curr_max_height, prev_nodes.begin() + node_height,
                                  head);
                        std::fill(curr_nodes.begin() + curr_max_height, curr_nodes.begin() + node_height,
                                  nullptr);
                    }
                ...
                void fill_prev_array_for_existing_node(array_type& prev_nodes, node_ptr node) {
                    ...
                    for (size_type level = node->height(); level > 0; --level) {
                        while (prev->next(level - 1) != node) { prev = prev->next(level - 1); }
                        prev_nodes[level - 1] = prev;
                    }
                }
                ...
                std::pair<iterator, bool> internal_insert(Args&&... args) {
                    ...
                    if (!insert_result.second) { delete_value_node(new_node); }
                    return insert_result;
                }
                ...
                std::pair<iterator, bool> internal_insert_node(node_ptr new_node) {
                    ...
                    for (;;) {
                        fill_prev_curr_arrays(prev_nodes, curr_nodes, new_node, get_key(new_node), compare,
                                              head_node);
                        ...
                        if (!prev->atomic_next(0).compare_exchange_strong(next, new_node)) { continue; }
                        ...
                        for (std::size_t level = 1; level < new_height; ++level) {
                            ...
                            for (;;) {
                                ...
                                for (size_type lev = level; lev != new_height; ++lev) {
                                    curr_nodes[lev]
                                            = internal_find_position(lev, prev_nodes[lev], new_node, compare);
                                }
                            }
                        }
                        ++my_size;
            ...
            protected:
                ...
                void internal_merge(SourceType&& source) {
                    ...
                    static_assert((std::is_same<node_type, typename source_type::node_type>::value),
            ...
            private:
                ...
                node_ptr create_head_node() { return create_node(max_level); }
                ...
                void delete_node(node_ptr node) { list_node_type::destroy(my_node_allocator, node); }
                ...
                node_ptr get_head() const { return my_head_ptr.load(std::memory_order_acquire); }
                ...
                static iterator get_iterator(const_iterator it) { return iterator(it.my_node_ptr); }
            ...
            public:
                ...
                std::size_t operator()() {
                    ...
                    std::size_t result
                            = max_level - std::size_t(tbb::detail::log2(engines.local()() + 1)) - 1;
