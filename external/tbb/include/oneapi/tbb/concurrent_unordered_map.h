    ...
    namespace detail {
        namespace d1 {
            ...
            public:
                ...
                concurrent_unordered_map(const concurrent_unordered_map& other, const allocator_type& alloc)
                    : base_type(other, alloc) {}
                concurrent_unordered_map(concurrent_unordered_map&&) = default;
                concurrent_unordered_map(concurrent_unordered_map&& other, const allocator_type& alloc)
                ...
                mapped_type& operator[](const key_type& key) {
                    ...
                    if (where == this->end()) {
                        where = this->emplace(std::piecewise_construct, std::forward_as_tuple(key),
                                              std::tuple<>())
                                        .first;
                    }
                    return where->second;
                ...
                mapped_type& operator[](key_type&& key) {
                    ...
                    if (where == this->end()) {
                        where = this->emplace(std::piecewise_construct, std::forward_as_tuple(std::move(key)),
                                              std::tuple<>())
                                        .first;
                    }
                    return where->second;
                ...
                mapped_type& at(const key_type& key) {
                    ...
                    if (where == this->end()) { throw_exception(exception_id::invalid_key); }
                    return where->second;
                }
                ...
                const mapped_type& at(const key_type& key) const {
                    ...
                    if (where == this->end()) { throw_exception(exception_id::out_of_range); }
                    return where->second;
                }
                ...
                template <typename P>
                typename std::enable_if<std::is_constructible<value_type, P&&>::value, iterator>::type
                insert(const_iterator hint, P&& value) {
                    return this->emplace_hint(hint, std::forward<P>(value));
                }

                template <typename OtherHash, typename OtherKeyEqual>
                void merge(concurrent_unordered_map<key_type, mapped_type, OtherHash, OtherKeyEqual,
                ...
                template <typename OtherHash, typename OtherKeyEqual>
                void merge(concurrent_unordered_multimap<key_type, mapped_type, OtherHash, OtherKeyEqual,
                                                         allocator_type>& source) {
                    this->internal_merge(source);
                }
            ...
            concurrent_unordered_map(It, It, std::size_t = {}, Hash = Hash(), KeyEq = KeyEq(),
                                     Alloc = Alloc())
                    -> concurrent_unordered_map<iterator_key_t<It>, iterator_mapped_t<It>, Hash, KeyEq,
                                                Alloc>;

            template <typename Key, typename T, typename Hash = std::hash<std::remove_const_t<Key>>,
            ...
            template <typename It, typename Hash = std::hash<iterator_key_t<It>>,
                      ...
                      typename = std::enable_if_t<!is_allocator_v<KeyEq>>,
                      typename = std::enable_if_t<!std::is_integral_v<Hash>>>
            concurrent_unordered_multimap(It, It, std::size_t = {}, Hash = Hash(), KeyEq = KeyEq(),
                                          Alloc = Alloc())
                    -> concurrent_unordered_multimap<iterator_key_t<It>, iterator_mapped_t<It>, Hash, KeyEq,
                                                     Alloc>;
            ...
            concurrent_unordered_multimap(std::initializer_list<std::pair<Key, T>>, std::size_t, Alloc)
                    -> concurrent_unordered_multimap<std::remove_const_t<Key>, T,
                                                     std::hash<std::remove_const_t<Key>>,
                                                     std::equal_to<std::remove_const_t<Key>>, Alloc>;
