    ...
    namespace detail {
        namespace d1 {
            ...
            public:
                ...
                concurrent_priority_queue(InputIterator begin, InputIterator end, const Compare& compare,
                                          const allocator_type& alloc = allocator_type())
                ...
                concurrent_priority_queue(std::initializer_list<value_type> init, const Compare& compare,
                                          const allocator_type& alloc = allocator_type())
                ...
                concurrent_priority_queue(const concurrent_priority_queue& other)
                    : mark(other.mark), my_size(other.my_size.load(std::memory_order_relaxed)),
                      my_compare(other.my_compare), data(other.data) {
                    my_aggregator.initialize_handler(functor {this});
                }

                concurrent_priority_queue(const concurrent_priority_queue& other, const allocator_type& alloc)
                    : mark(other.mark), my_size(other.my_size.load(std::memory_order_relaxed)),
                      my_compare(other.my_compare), data(other.data, alloc) {
                    my_aggregator.initialize_handler(functor {this});
                }

                concurrent_priority_queue(concurrent_priority_queue&& other)
                    : mark(other.mark), my_size(other.my_size.load(std::memory_order_relaxed)),
                      my_compare(other.my_compare), data(std::move(other.data)) {
                    my_aggregator.initialize_handler(functor {this});
                }

                concurrent_priority_queue(concurrent_priority_queue&& other, const allocator_type& alloc)
                    : mark(other.mark), my_size(other.my_size.load(std::memory_order_relaxed)),
                      my_compare(other.my_compare), data(std::move(other.data), alloc) {
                    my_aggregator.initialize_handler(functor {this});
                }
       ...
       This operation reads shared data and will trigger a race condition. */
                ...
                void push(const value_type& value) {
                    ...
                    if (op_data.status == FAILED) throw_exception(exception_id::bad_alloc);
                }
       ...
       This operation can be safely used concurrently with other push, try_pop or emplace operations. */
            ...
            private:
                ...
                void push_back_helper_impl(const T&, /*is_copy_constructible = */ std::false_type) {
                    __TBB_ASSERT(false, "error: calling tbb::concurrent_priority_queue.push(const "
        ...
        mark through my_size-1. */
            ...
            concurrent_priority_queue(It, It, Alloc)
                    -> concurrent_priority_queue<iterator_value_t<It>, std::less<iterator_value_t<It>>,
                                                 Alloc>;
