...
TEST_CASE("Task handle being non copyable") {
    static_assert((!std::is_copy_constructible<oneapi::tbb::task_handle>::value)
...
TEST_CASE("Task handle blocks wait") {
    ...
    oneapi::tbb::task_handle h = tg.defer([&] { completed = true; });
    ...
    arena.execute([&] { tg.run(std::move(h)); });
    CHECK_MESSAGE(completed == false,
                  "Deferred task should not be run until run(task_handle) and wait is called");
    start_wait = true;
    wait_thread.join();
}
