...
void TestWorkersConstraints() {
    ...
    for (size_t num = 2; num < limit_par; num++) RunWorkersLimited(num);
    for (size_t num = limit_par; num > 1; num--) RunWorkersLimited(num);
}
...
//! \brief \ref interface \ref requirement
TEST_CASE("setting max number of threads") { TestWorkersConstraints(); }
//! Testing concurrenct setting concurrency
//! \brief \ref interface \ref requirement
TEST_CASE("concurrenct setting concurrency") { TestConcurrentSetUseConcurrency(); }

//! Testing auto initialization
//! \brief \ref interface \ref requirement
TEST_CASE("auto initialization") { TestAutoInit(); }
...
TEST_CASE("Test worker threads remain inactive in enforced serial execution mode") {
    ...
    // Warm-up threads
    tbb::parallel_for(std::size_t(0), num_threads, [&](std::size_t) { barrier.wait(); });

    tbb::global_control control(tbb::global_control::max_allowed_parallelism, 1);

    std::thread thr([&] { tbb::parallel_for(0, 100000, [&](int) { utils::doDummyWork(100); }); });
