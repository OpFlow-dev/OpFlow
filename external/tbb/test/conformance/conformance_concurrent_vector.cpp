...
static void CheckVector(const vector_t& cv, std::size_t expected_size, std::size_t /*old_size*/) {
    ...
    for (int j = 0; j < int(expected_size); ++j) { CHECK((cv[j].bar() == ~j)); }
}
...
void TestResizeAndCopy() {
    ...
    for (int old_size = 0; old_size <= 0; NextSize(old_size)) {
        for (int new_size = 0; new_size <= 8; NextSize(new_size)) {
            ...
            for (int j = 0; j < new_size; ++j) { v[j].bar() = ~j; }
...
void TestCopyAssignment() {
    ...
    for (int dst_size = 1; dst_size <= 128; NextSize(dst_size)) {
        for (int src_size = 2; src_size <= 128; NextSize(src_size)) {
            vector_t u(move_support_tests::FooIterator(0), move_support_tests::FooIterator(src_size),
                       init_alloc);
            for (int i = 0; i < src_size; ++i) REQUIRE(u[i].bar() == i);
            ...
            for (int i = 0; i < src_size; ++i) { REQUIRE(v[i].bar() == i); }
...
void TestCapacity() {
    ...
    using vector_t = oneapi::tbb::concurrent_vector<move_support_tests::Foo, allocator_t>;
    allocator_t::init_counters();
    for (std::size_t old_size = 0; old_size <= 11000;
         old_size = (old_size < 5 ? old_size + 1 : 3 * old_size)) {
        for (std::size_t new_size = 0; new_size <= 11000;
             new_size = (new_size < 5 ? new_size + 1 : 3 * new_size)) {
            std::size_t count = move_support_tests::foo_count;
            {
                vector_t v;
                v.reserve(old_size);
                ...
                if (oneapi::tbb::detail::log2(/*reserved size*/ old_size | 1)
                    > oneapi::tbb::detail::log2(fill_size | 1)) {
...
void TestExceptions() {
            ...
            for (int m = zero_method + 1; m < all_methods; ++m) {
                ...
                try {
                    switch (m) {
                        ...
                        case assign_ir: {
                            victim.assign(move_support_tests::FooIterator(0),
                                          move_support_tests::FooIterator(N));
                        } break;
                        case reserve: {
                            try {
                                victim.reserve(victim.max_size() + 1);
                            } catch (std::length_error&) {
                            } catch (...) { INFO("ERROR: unrecognized exception - known compiler issue\n"); }
                            victim.reserve(N);
                        } break;
                        case compact: {
                            if (t) move_support_tests::max_foo_count = 0;
                            else
                                allocator_t::set_limits();// reset limits
                            victim.reserve(2);
                    ...
                    allocator_t::set_limits();
                    move_support_tests::max_foo_count = 0;
                            ...
                            if (!t) {
                                ...
                                for (i = 1;; ++i)
                                    if (!victim[i].zero_bar()) break;
                                        ...
                                        REQUIRE(victim[i].bar()
                                                == (m == assign_ir ? i : move_support_tests::initial_bar));
                                    }
                                for (; size_t(i) < size; ++i) { REQUIRE(!victim[i].zero_bar()); }
    ...
    } catch (...) { REQUIRE_MESSAGE(false, "unexpected exception"); }
}
#endif
...
void verify_vector_partially_copied(
        oneapi::tbb::concurrent_vector<move_support_tests::FooWithAssign, allocator_t> const& victim,
        size_t planned_victim_size,
        oneapi::tbb::concurrent_vector<move_support_tests::FooWithAssign, allocator_t> const& src,
        bool is_memory_allocation_failure) {
    if (is_memory_allocation_failure) {// allocator generated exception
        using vector_t = oneapi::tbb::concurrent_vector<move_support_tests::FooWithAssign, allocator_t>;
        REQUIRE_MESSAGE(victim == vector_t(src.begin(), src.begin() + victim.size(), src.get_allocator()),
                        "failed to properly copy of source ?");
    } else {
        REQUIRE_MESSAGE(std::equal(victim.begin(), victim.begin() + planned_victim_size, src.begin()),
                        "failed to properly copy items before the exception?");
        REQUIRE_MESSAGE((std::all_of(victim.begin() + planned_victim_size, victim.end(),
                                     is_state_predicate<move_support_tests::Foo::ZeroInitialized>())),
                        "failed to zero-initialize items left not constructed after the exception?");
    }
}
...
void test_ex_assign_operator() {
    ...
    for (int t = 0; t < 2; ++t) {// exception type
        ...
        REQUIRE_THROWS_AS(
                [&]() {
                    move_support_tests::LimitFooCountInScope foo_limit(
                            move_support_tests::foo_count + planned_victim_size, t);
                    move_support_tests::LimitAllocatedItemsInScope<allocator_t> allocator_limit(
                            allocator_t::items_allocated + planned_victim_size, !t);
        ...
        if (!t) { verify_c_vector_capacity_is_below(victim.capacity(), N); }
...
void TestIteratorTraits() {
    AssertSameType(static_cast<typename Iterator::difference_type*>(nullptr),
                   static_cast<std::ptrdiff_t*>(nullptr));
    ...
    AssertSameType(static_cast<typename Iterator::iterator_category*>(nullptr),
                   static_cast<std::random_access_iterator_tag*>(nullptr));
    T x;
...
void TestSerialMoveInShrinkToFit() {
    ...
    static_assert(std::is_nothrow_move_constructible<dummy>::value,
                  "incorrect test setup or broken configuration?");
    {
        dummy src(0);
        REQUIRE_MESSAGE(is_state<StateTrackableBase::MoveInitialized>(dummy(std::move_if_noexcept(src))),
    ...
    REQUIRE_MESSAGE(c_vector_before_shrink.content_location_changed(c_vector),
                    "incorrect test setup? shrink_to_fit should cause moving elements to other memory "
                    "locations while it is not");
    REQUIRE_MESSAGE((std::all_of(c_vector.begin(), c_vector.end(),
                                 is_state_predicate<StateTrackableBase::MoveInitialized>())),
...
struct default_container_traits {
    ...
    construct_container(typename std::aligned_storage<sizeof(container_type)>::type& storage,
    ...
    static container_type&
    construct_container(typename std::aligned_storage<sizeof(container_type)>::type& storage,
                        iterator_type begin, iterator_type end, allocator_type const& a) {
        container_type* ptr = reinterpret_cast<container_type*>(&storage);
...
struct c_vector_type : default_container_traits {
    ...
    static bool equal(oneapi::tbb::concurrent_vector<element_type, allocator_type> const& c, iterator begin,
                      iterator end) {
...
void TestSerialGrowByWithMoveIterators() {
    ...
    dst.grow_by(std::make_move_iterator(fixture.source.begin()),
                std::make_move_iterator(fixture.source.end()));
        ...
        void operator()(const oneapi::tbb::blocked_range<std::size_t>& range) const {
            for (std::size_t i = range.begin(); i != range.end(); ++i) {
                ...
                if (p - my_vector.begin() < typename MyVector::difference_type(req))
                    CHECK((p->state == desired_state
                           || p->state == move_support_tests::Foo::ZeroInitialized));
...
void TestConcurrentGrowToAtLeastImpl() {
    ...
    for (std::size_t s = 1; s < 1000; s *= 10) {
        oneapi::tbb::parallel_for(oneapi::tbb::blocked_range<std::size_t>(0, 10000 * s, s),
                                  GrowToAtLeast<MyVector>(use_two_arg_form, v, copy_from),
    ...
    std::size_t allocations = v.get_allocator().allocations, frees = v.get_allocator().frees;
...
struct CheckElement {
    ...
    void operator()(const oneapi::tbb::concurrent_vector<int>::const_range_type& range) const {
        for (iterator i = range.begin(); i != range.end(); ++i) {
            if (*i != int(i - base)) { REPORT("ERROR for v[%ld]\n", long(i - base)); }
        }
    }
...
void TestParallelFor(std::size_t nthread) {
    ...
    for (int i = 0; std::size_t(i) < v.size(); ++i) {
        if (v[i] != i) { REPORT("ERROR for v[%ld]\n", i); }
    }
}
...
private:
    struct sum_number_of_parts {
        static std::size_t sum(std::size_t accumulator, grain_map::range_part const& rp) {
            return accumulator + rp.number_of_parts;
        }
    };
    ...
    void operator()(const oneapi::tbb::blocked_range<std::size_t>& range) const {
        ...
        for (std::size_t index = 0; index < my_grain_map.batched.size(); ++index) {
            ...
            switch (batch_part.method) {
                case grain_map::grow_by_range: {
                    my_vector.grow_by(move_support_tests::FooIterator(current_adding_index_in_cvector),
                                      move_support_tests::FooIterator(end));
                } break;
                ...
                default: {
                    REQUIRE_MESSAGE(false, "using unimplemented method of batch add in ConcurrentGrow test.");
                } break;
            };
        ...
        for (; current_adding_index_in_cvector < range.end(); ++current_adding_index_in_cvector) {
            ...
            if (!items_left_to_add[method_index]) {
                ...
                method_index = std::distance(
                        items_left_to_add.begin(),
                        std::find_if(items_left_to_add.begin(), items_left_to_add.end(), &not_zero::is));
                REQUIRE_MESSAGE(method_index < my_grain_map.distributed.size(),
                                "incorrect test setup - wrong expected distribution: left free space but no "
                                "elements to add?");
            };
...
void TestConcurrentGrowBy() {
    ...
    MyAllocator::init_counters();
    {
        ...
        static const std::size_t number_of_grains
                = 8;//this should be (power of two) in order to get minimal ranges equal to grain_size
        static const std::size_t range_size = grain_size * number_of_grains;

        MyAllocator a;
        MyVector v(a);
        oneapi::tbb::parallel_for(oneapi::tbb::blocked_range<std::size_t>(0, range_size, grain_size),
                                  GrowBy<MyVector>(v, m, part_weight), oneapi::tbb::simple_partitioner());
        ...
        for (std::size_t i = 0; i < range_size; ++i) {
            ...
            else if (v[i].state == move_support_tests::Foo::DirectInitialized)
                ++direct_inits;
            else if (v[i].state == move_support_tests::Foo::CopyInitialized)
                ++copy_inits;
            else if (v[i].state == move_support_tests::Foo::MoveInitialized)
                ++move_inits;
            else {
        ...
        std::size_t expected_direct_inits = 0, expected_def_inits = 0, expected_copy_inits = 0,
                    expected_move_inits = 0;
        for (std::size_t i = 0; i < utils::array_length(concurrent_grow_single_range_map); ++i) {
            ...
            switch (rp.expected_element_state) {
                case move_support_tests::Foo::DefaultInitialized: {
                    expected_def_inits += rp.number_of_parts;
                } break;
                case move_support_tests::Foo::DirectInitialized: {
                    expected_direct_inits += rp.number_of_parts;
                } break;
                case move_support_tests::Foo::MoveInitialized: {
                    expected_move_inits += rp.number_of_parts;
                } break;
                case move_support_tests::Foo::CopyInitialized: {
                    expected_copy_inits += rp.number_of_parts;
                } break;
                default: {
                    REQUIRE_MESSAGE(false, "unexpected expected state");
                } break;
            };
        }
    ...
    std::size_t items_allocated = MyAllocator::items_allocated, items_freed = MyAllocator::items_freed;
    std::size_t allocations = MyAllocator::allocations, frees = MyAllocator::frees;
    REQUIRE(items_allocated == items_freed);
    REQUIRE(allocations == frees);
}
...
void test_ex_move_assignment_memory_failure() {
    using fixture_type = move_support_tests::DefaultStatefulFixtureHelper<c_vector_type,
                                                                          /*POCMA = */ std::false_type>::type;
    using arena_allocator_fixture_type
            = move_support_tests::ArenaAllocatorFixture<move_support_tests::FooWithAssign,
    ...
    REQUIRE_THROWS_AS(
            [&]() {
                move_support_tests::LimitAllocatedItemsInScope<allocator_type> allocator_limit(
...
void test_ex_move_assignment_element_ctor_exception() {
    using fixture_type
            = move_support_tests::DefaultStatefulFixtureHelper<c_vector_type, std::false_type>::type;
    using arena_allocator_fixture_type
            = move_support_tests::ArenaAllocatorFixture<move_support_tests::FooWithAssign, std::false_type>;
    using vector_type = fixture_type::container_type;
...
namespace push_back_exception_safety_helpers {
    ...
    struct fixture {
        ...
        void test(void (*p_test)(vector_t&)) {
            ...
            REQUIRE_MESSAGE(is_state<move_support_tests::Foo::ZeroInitialized>(v[0]),
                            "incorrectly filled item during exception in emplace_back?");
        }
    };
}// namespace push_back_exception_safety_helpers
...
void TestEmplaceBackExceptionSafety() {
    ...
    struct test {
        static void test_emplace(fixture_t::vector_t& v) { v.emplace_back(1, 2); }
    };
...
void TestDeductionGuides() {
    ...
    static_assert(std::is_same<decltype(v2), TVector<ComplexType, std::allocator<ComplexType>>>::value);

    // check TVector(std::initializer_list<T>)
    TVector v3(l);
    static_assert(std::is_same<decltype(v3), TVector<ComplexType>>::value);
    ...
    static_assert(
            std::is_same<decltype(v6),
                         TVector<ComplexType, oneapi::tbb::cache_aligned_allocator<ComplexType>>>::value);
...
void TestVectorComparisons() {
    ...
    using three_way_comparable_vector
            = oneapi::tbb::concurrent_vector<comparisons_testing::ThreeWayComparable>;
...
TEST_CASE("test type matching") { test_member_types<oneapi::tbb::concurrent_vector>(); }

//! Test sequential access to elements
//! \brief \ref interface \ref requirement
TEST_CASE("testing sequential for") { TestSequentialFor<move_support_tests::FooWithAssign>(); }

//! Test of assign, grow, copying with various sizes
//! \brief \ref interface \ref requirement
TEST_CASE("testing resize and copy") { TestResizeAndCopy(); }

//! Test the assignment operator and swap
//! \brief \ref interface \ref requirement
TEST_CASE("testing copy assignment") { TestCopyAssignment(); }
...
//! \brief \ref interface \ref requirement
TEST_CASE("testing capacity") { TestCapacity(); }

#if TBB_USE_EXCEPTIONS
//! Test exceptions
//! \brief \ref requirement
TEST_CASE("testing exceptions") { TestExceptions(); }

//! Test of push_back move exception safety
//! \brief \ref requirement
TEST_CASE("testing push_back move exception safety") { TestPushBackMoveExceptionSafety(); }

//! Test of emplace back move exception safety
//! \brief \ref requirement
TEST_CASE("testing emplace back exception safety") { TestEmplaceBackExceptionSafety(); }

//! Test exceptions guarantees for assign operator
//! \brief \ref requirement
TEST_CASE("testing exception safety guaranteees for assign operator") { test_ex_assign_operator(); }
...
//! \brief \ref requirement
TEST_CASE("test exception safety on concurrent_vector move assignment") { test_ex_move_assignment(); }
#endif
//! Test push_back in move only container
//! \brief \ref requirement
TEST_CASE("testing push_back move only container") { TestPushBackMoveOnlyContainer(); }
...
TEST_CASE("testing std::iterator_traits for concurrent_vector::iterator") {
    TestIteratorTraits<oneapi::tbb::concurrent_vector<move_support_tests::Foo>::iterator,
...
TEST_CASE("testing std::iterator_traits for concurrent_vector::const_iterator") {
    TestIteratorTraits<oneapi::tbb::concurrent_vector<move_support_tests::Foo>::const_iterator,
                       const move_support_tests::Foo>();
}

//! Test initializer_list support
//! \brief \ref interface \ref requirement
TEST_CASE("testing initializer_list support") { TestInitList(); }

//! Test move constructor
//! \brief \ref interface \ref requirement
TEST_CASE("testing move constructor") { move_support_tests::test_move_constructor<c_vector_type>(); }

//! Test move assign operator
//! \brief \ref interface \ref requirement
TEST_CASE("testing move assign operator") { move_support_tests::test_move_assignment<c_vector_type>(); }
...
//! \brief \ref requirement
TEST_CASE("testing serial grow_by with move iterator") { TestSerialGrowByWithMoveIterators(); }

//! Test grow_by with move iterator
//! \brief \ref requirement
TEST_CASE("testing serial move in shrink_to_fit") { TestSerialMoveInShrinkToFit(); }
...
//! \brief \ref interface \ref requirement
TEST_CASE("testing comparison on assign operations") { TestComparison(); }
...
//! \brief \ref requirement
TEST_CASE("testing deduction guides") { TestDeductionGuides<oneapi::tbb::concurrent_vector>(); }
#endif

//! Test concurrent_vector comparisons
//! \brief \ref interface \ref requirement
TEST_CASE("concurrent_vector comparisons") { TestVectorComparisons(); }

//! Test concurrent_vector iterators comparisons
//! \brief \ref interface \ref requirement
TEST_CASE("concurrent_vector iterators comparisons") { TestVectorIteratorComparisons(); }
