                ...
                for (int col_x = -10; col_x < 10; ++col_x) {
                    for (int col_y = col_x; col_y < 10; ++col_y) {
                        ...
                        for (int col_grain = 1; col_grain < 10; ++col_grain) {
                            ...
                            utils::AssertSameType(static_cast<range_type::row_range_type::const_iterator*>(0),
                                                  static_cast<row_type*>(0));
                            utils::AssertSameType(static_cast<range_type::col_range_type::const_iterator*>(0),
                                                  static_cast<col_type*>(0));
                            utils::AssertSameType(r.rows(),
                                                  oneapi::tbb::blocked_range<row_type>(row_i, row_j, 1));
                            utils::AssertSameType(r.cols(),
                                                  oneapi::tbb::blocked_range<col_type>(col_i, col_j, 1));
                            REQUIRE(r.empty() == (row_x == row_y || col_x == col_y));
                            REQUIRE(r.is_divisible()
                                    == (row_y - row_x > row_grain || col_y - col_x > col_grain));
                            if (r.is_divisible()) {
...
struct Striker {
    ...
    void operator()(const oneapi::tbb::blocked_range2d<int>& r) const {
        for (oneapi::tbb::blocked_range<int>::const_iterator i = r.rows().begin(); i != r.rows().end(); ++i)
            for (oneapi::tbb::blocked_range<int>::const_iterator j = r.cols().begin(); j != r.cols().end();
                 ++j)
                ++Array[i][j];
    }
};
...
TEST_CASE("Deduction guides") {
    ...
    static_assert(
            std::is_same<decltype(r1),
                         oneapi::tbb::blocked_range2d<decltype(v)::iterator, decltype(v2)::iterator>>::value);
