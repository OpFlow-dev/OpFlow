...
using input_msg
        = conformance::message</*default_ctor*/ true, /*copy_ctor*/ true /*enable for queue_node successor*/,
...
TEST_CASE("limiter_node limiting") {
    ...
    for (int i = 0; i < limit * 2; ++i) node1.try_put(input_msg(1));
    g.wait_for_all();

    CHECK_MESSAGE((conformance::get_values(node2).size() == limit),
