...
struct correctness_test_case {
    ...
    static void run_validate_and_reset(oneapi::tbb::task_group_context* context_ptr) {
        for (auto& elem : data_array) elem.store(0, std::memory_order_relaxed);

        parallel_invoke_call<TaskCount, functor>::perform(context_ptr);
        for (std::size_t i = 0; i < TaskCount; i++) {
            REQUIRE_MESSAGE(data_array[i] == 1,
                            "Some task was executed more than once, or was not executed.");
            data_array[i] = 0;
        }
    }
};
...
TEST_CASE("Test correctness") { correctness_test(); }
...
struct exception_handling_test_case {
    ...
    struct functor {
        ...
        void operator()() const {
            REQUIRE_MESSAGE(Position < TaskCount, "Wrong structure configuration.");
            if (exception_mask & (1 << Position)) { ThrowTestException(); }
        }
    };
...
private:
    static std::uint64_t
            exception_mask;// each bit represents whether the function should throw exception or not
};
...
void simple_test_nothrow() { ++g_CurExecuted; }
...
struct ParInvokeLauncher {
    ...
    void operator()() const {
        ...
        for (int i = 0; i <= 9; ++i) func_array[i] = &simple_test_nothrow;
        ...
        oneapi::tbb::parallel_invoke(func_array[0], func_array[1], func_array[2], func_array[3],
                                     ...
                                     func_array[8], func_array[9], my_ctx);
    }
