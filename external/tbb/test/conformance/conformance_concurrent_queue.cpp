...
public:
    ...
    std::size_t capacity() const { return my_capacity; }

    void set_capacity(const std::size_t n) { my_capacity = n; }
...
void TestEmptiness() {
    ...
    TestEmptyQueue<oneapi::tbb::concurrent_bounded_queue<move_support_tests::Foo,
                                                         test_allocator<move_support_tests::Foo>>,
...
void TestFullness() {
    ...
    TestFullQueue<oneapi::tbb::concurrent_bounded_queue<move_support_tests::Foo,
                                                        test_allocator<move_support_tests::Foo>>,
                  move_support_tests::Foo>();
}
...
void TestClearWorks() {
    ...
    TestClear<oneapi::tbb::concurrent_bounded_queue<move_support_tests::Foo,
                                                    test_allocator<move_support_tests::Foo>>>();
}
...
void TestQueueIteratorWorks() {
    TestIterator<oneapi::tbb::concurrent_queue<move_support_tests::Foo,
                                               test_allocator<move_support_tests::Foo>>>();
    TestIterator<oneapi::tbb::concurrent_bounded_queue<move_support_tests::Foo,
                                                       test_allocator<move_support_tests::Foo>>>();
}
...
public:
    ...
    ConcQWithSizeWrapper(const ConcQWithSizeWrapper& q, const A& a)
        : oneapi::tbb::concurrent_queue<T, A>(q, a) {}
...
enum state_type { LIVE = 0x1234, DEAD = 0xDEAD };
...
public:
    Bar& operator*() const { return *bar_ptr; }
...
bool operator==(const BarIterator& bia, const BarIterator& bib) { return bia.bar_ptr == bib.bar_ptr; }
...
public:
    state_type state;
    typedef enum { PREPARATION, COPY_CONSTRUCT } mode_type;
    ...
    BarEx(const BarEx& a_bar) : state(LIVE) {
        ...
        my_id = a_bar.my_id;
        if (mode == PREPARATION)
            if (!(++count % 100)) { TBB_TEST_THROW(Bar_exception()); }
...
void TestConstructors() {
    ...
    for (size_type size = 0; size < 1001; ++size) {
        for (size_type i = 0; i < size; ++i) src_queue.push(T(i + (i ^ size)));
    ...
    T bar_array[1001];
    for (size_type size = 0; size < 1001; ++size) {
        for (size_type i = 0; i < size; ++i) { bar_array[i] = T(i + (i ^ size)); }
    ...
    for (size_type size = 0; size < 1001; ++size) {
        ...
        for (size_type i = 0; i < size; ++i, ++qit) { locations[i] = &(*qit); }
        ...
        CHECK_FAST_MESSAGE((size == size_of_queue && size_of_queue == dst_queue.size()),
                           "not working move constructor?");

        CHECK_FAST_MESSAGE(std::equal(locations.begin(), locations.end(), dst_queue.unsafe_begin(),
                                      [](const T* t1, const T& r2) { return t1 == &r2; }),
...
void TestQueueConstructors() {
    ...
    TestConstructors<oneapi::tbb::concurrent_bounded_queue<Bar>, Bar, BarIterator,
                     oneapi::tbb::concurrent_bounded_queue<BarEx>, BarEx>();
}
...
struct TestNegativeQueueBody {
    ...
    TestNegativeQueueBody(oneapi::tbb::concurrent_bounded_queue<T>& q, std::size_t n)
        : queue(q), nthread(n) {}
    void operator()(std::size_t k) const {
        if (k == 0) {
            int number_of_pops = int(nthread) - 1;
            // Wait for all pops to pend.
            while (int(queue.size()) > -number_of_pops) { utils::yield(); }
...
struct Body {
    ...
    void operator()(std::size_t thread_id) const {
        ...
        for (std::size_t j = 0; j < M; ++j) {
            ...
            CHECK_FAST_MESSAGE((f.get_thread_id() == nthread || serial[f.get_thread_id()] < f.get_serial()),
                               "partial order violation");
            serial[f.get_thread_id()] = f.get_serial();
            sum += int(f.get_serial() - 1);
        }
        Sum[thread_id] = sum;
        for (std::size_t k = 0; k < 3; ++k) PopKind[k] += pop_kind[k];
    }
};
...
void TestPushPop(typename CQ::size_type prefill, std::ptrdiff_t capacity, std::size_t nthread) {
    ...
    if (signed_prefill + 1 >= capacity) { return; }

    bool success = false;
    for (std::size_t k = 0; k < 3; ++k) { PopKind[k] = 0; }
    ...
    for (std::size_t trial = 0; !success; ++trial) {
        ...
        int sum = 0;
        for (std::size_t k = 0; k < nthread; ++k) { sum += Sum[k]; }
        ...
        if (sum != expected) { REPORT("sum=%d expected=%d\n", sum, expected); }
        ...
        if (nthread > 1 && prefill == 0) {
            ...
            for (std::size_t k = 0; k < 2; ++k) {
                ...
                if (PopKind[k] < min_requirement) {
                    if (trial >= max_trial) {
                        REPORT("Warning: %d threads had only %ld pop_if_present operations %s after %d "
                               "trials (expected at least %d). "
                               "This problem may merely be unlucky scheduling. "
                               "Investigate only if it happens repeatedly.\n",
                               nthread, long(PopKind[k]), k == 0 ? "failed" : "succeeded", max_trial,
                               min_requirement);
                    } else {
                        success = false;
                    }
...
void TestConcurrentPushPop() {
    for (std::size_t nthread = MinThread; nthread <= MaxThread; ++nthread) {
        ...
        for (std::size_t prefill = 0; prefill < 64; prefill += (1 + prefill / 3)) {
            TestPushPop<ConcQPushPopWrapper<move_support_tests::Foo>, move_support_tests::Foo>(
                    prefill, std::ptrdiff_t(-1), nthread);
            TestPushPop<ConcQPushPopWrapper<move_support_tests::Foo>, move_support_tests::Foo>(
                    prefill, std::ptrdiff_t(1), nthread);
            TestPushPop<ConcQPushPopWrapper<move_support_tests::Foo>, move_support_tests::Foo>(
                    prefill, std::ptrdiff_t(2), nthread);
            TestPushPop<ConcQPushPopWrapper<move_support_tests::Foo>, move_support_tests::Foo>(
                    prefill, std::ptrdiff_t(10), nthread);
            TestPushPop<ConcQPushPopWrapper<move_support_tests::Foo>, move_support_tests::Foo>(
                    prefill, std::ptrdiff_t(100), nthread);
        }
        for (std::size_t prefill = 0; prefill < 64; prefill += (1 + prefill / 3)) {
            TestPushPop<oneapi::tbb::concurrent_bounded_queue<move_support_tests::Foo,
                                                              test_allocator<move_support_tests::Foo>>,
                ...
                try {
                    ...
                    switch (m) {
                        ...
                        case m_pop:
                            ...
                            REQUIRE_MESSAGE(ptrdiff_t(queue_test.size()) == n_pushed,
                                            "queue size must be equal to n pushed");
                            for (int k = 0; k < n_pushed; k++) {
                                T elt;
                                queue_test.try_pop(elt);
                            }
...
public:
    ...
    void operator()(std::size_t thread_id) const {
        if (thread_id == max) {
            while (q->size() < std::ptrdiff_t(max)) { utils::yield(); }
            q->abort();
            return;
        }
        try {
...
public:
    ...
    void operator()(std::size_t thread_id) const {
        ...
        if (thread_id == std::size_t(max)) {
            while (q->size() > prefill - max) { utils::yield(); }
...
void TestAbort() {
    for (std::size_t nthreads = MinThread; nthreads <= MaxThread; ++nthreads) {
        ...
        for (std::size_t i = 0; i < 10; ++i) {
            ...
            REQUIRE_MESSAGE(failed_pushes == nthreads,
                            "All threads should have failed to push an element to zero-sized queue");
        ...
        for (std::size_t i = 0; i < 10; ++i) {
            ...
            if (nthreads >= 2)
                REQUIRE_MESSAGE(
                        failed_pushes == nthreads - 2,
                        "nthreads-2 threads should have failed to push an element to queue of size 2");
            int e;
            while (iq2.try_pop(e))
                ;
        }
        ...
        for (std::size_t i = 0; i < 10; ++i) {
            ...
            REQUIRE_MESSAGE(num_popped <= 2,
                            "at most 2 elements should have been popped from queue of size 2");
            if (nthreads >= 2)
                REQUIRE_MESSAGE(
                        failed_pops == nthreads - 2,
                        "nthreads-2 threads should have failed to pop an element from queue of size 2");
            else {
                int e;
                iq3.pop(e);
            }
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            utils::NativeParallelFor(nthreads + 1, my_push_body2);
            REQUIRE_MESSAGE(num_pushed <= cap,
                            "at most cap elements should have been pushed to queue of size cap");
            if (nthreads >= cap)
                REQUIRE_MESSAGE(
                        failed_pushes == nthreads - cap,
                        "nthreads-cap threads should have failed to push an element to queue of size cap");
            SimplePopBody my_pop_body(&iq4, nthreads, num_pushed);
            utils::NativeParallelFor(nthreads + 1, my_pop_body);
            REQUIRE_MESSAGE((int) num_popped <= cap,
                            "at most cap elements should have been popped from queue of size cap");
            if (nthreads >= cap)
                REQUIRE_MESSAGE(
                        failed_pops == nthreads - cap,
                        "nthreads-cap threads should have failed to pop an element from queue of size cap");
            else {
                int e;
                while (iq4.try_pop(e))
                    ;
...
struct MoveOperationTracker {
    ...
    MoveOperationTracker(const MoveOperationTracker&) { ++copy_constructor_called_times; }
    MoveOperationTracker(MoveOperationTracker&&) { ++move_constructor_called_times; }
...
void FillTest(Queue& q, const std::vector<typename Queue::value_type>& vec) {
    for (typename std::vector<typename Queue::value_type>::const_iterator it = vec.begin(); it != vec.end();
         ++it)
...
void bounded_queue_specific_test(oneapi::tbb::concurrent_queue<T, A>&,
                                 const std::vector<T>&) { /* do nothing */
}
...
void TestMoveConstructors() {
    ...
    const std::size_t size = 10;
    for (std::size_t i = 0; i < size; ++i) src_queue.push(T(i + (i ^ size)));
    CHECK(T::construction_num == 2 * size);
    CHECK(T::destruction_num == size);

    const T* locations[size];
    typename CQ::const_iterator qit = src_queue.unsafe_begin();
    for (std::size_t i = 0; i < size; ++i, ++qit) locations[i] = &(*qit);
...
struct emplacer {
    ...
    static void emplace(CQ& queue, Args&&... val) {
        queue.emplace(std::forward<Args>(val)...);
    }
};
...
TEST_CASE("testing constructors") { TestQueueConstructors(); }

//! Test work with empty queue
//! \brief \ref interface \ref requirement
TEST_CASE("testing work with empty queue") { TestEmptiness(); }

//! Test set capacity operation
//! \brief \ref interface \ref requirement
TEST_CASE("testing set capacity operation") { TestFullness(); }

//! Test clean operation
//! \brief \ref interface \ref requirement
TEST_CASE("testing clean operation") { TestClearWorks(); }

//! Test move constructors
//! \brief \ref interface \ref requirement
TEST_CASE("testing move constructor") { TestMoveConstruction(); }

//! Test move support in push and pop
//! \brief \ref requirement
TEST_CASE("testing move support in push and pop") { TestMoveSupportInPushPop(); }

//! Test emplace operation
//! \brief \ref interface \ref requirement
TEST_CASE("testing emplace") { TestEmplace(); }
...
//! Test iterators
//! \brief \ref interface \ref requirement
TEST_CASE("testing iterators") { TestQueueIteratorWorks(); }

//! Test concurrent operations support
//! \brief \ref requirement
TEST_CASE("testing concurrent operations support") { TestConcurrentPushPop(); }

#if TBB_USE_EXCEPTIONS
//! Test exception safety
//! \brief \ref requirement
TEST_CASE("testing exception safety") { TestExceptions(); }

//! Test abort operation
//! \brief \ref interface \ref requirement
TEST_CASE("testing abort operation") { TestAbort(); }
#endif
...
void TestMoveQueueUnequal() {
    ...
    REQUIRE_MESSAGE(allocator_type::items_allocated == q2_items_allocated,
                    "More than expected memory allocated?");
    REQUIRE_MESSAGE(std::all_of(q1.unsafe_begin(), q1.unsafe_end(),
                                is_state_predicate<move_support_tests::Foo::MoveInitialized>()),
                    "Container did not move construct some elements");
    REQUIRE_MESSAGE(std::all_of(q2.unsafe_begin(), q2.unsafe_end(),
                                is_state_predicate<move_support_tests::Foo::MovedFrom>()),
...
void test_check_move_allocator(Container& src, Container& dst, Container& cpy) {
    ...
    REQUIRE_MESSAGE(std::equal(dst.unsafe_begin(), dst.unsafe_end(), cpy.unsafe_begin()),
...
void test_move_assignment_test_equal() {
    ...
    REQUIRE_MESSAGE(src.get_allocator() == dst.get_allocator(),
                    "Incorrect test setup: allocators should be equal");
    cpy = src;
    dst = std::move(src);
    ...
    REQUIRE_MESSAGE(src_bnd.get_allocator() == dst_bnd.get_allocator(),
                    "Incorrect test setup: allocators should be equal");
    cpy_bnd = src_bnd;
...
void test_move_assignment_test_unequal() {
    ...
    REQUIRE_MESSAGE(src.get_allocator() != dst.get_allocator(),
                    "Incorrect test setup: allocators should be unequal");
    cpy = src;
    dst = std::move(src);

    tbb::concurrent_bounded_queue<std::vector<int, stateful_allocator<int>>, stateful_allocator<int>> src_bnd(
            src_alloc);
    tbb::concurrent_bounded_queue<std::vector<int, stateful_allocator<int>>, stateful_allocator<int>> dst_bnd(
            dst_alloc);
    tbb::concurrent_bounded_queue<std::vector<int, stateful_allocator<int>>, stateful_allocator<int>> cpy_bnd(
            src_alloc);
    REQUIRE_MESSAGE(src_bnd.get_allocator() != dst_bnd.get_allocator(),
                    "Incorrect test setup: allocators should be unequal");
    src_bnd.push(v);
    ...
    test_check_move_allocator<
            tbb::concurrent_queue<std::vector<int, stateful_allocator<int>>, stateful_allocator<int>>>(
            src, dst, cpy);
    REQUIRE_MESSAGE(dst.unsafe_size() == cpy.unsafe_size(), "Queues are not equal");

    test_check_move_allocator<tbb::concurrent_bounded_queue<std::vector<int, stateful_allocator<int>>,
                                                            stateful_allocator<int>>>(src_bnd, dst_bnd,
                                                                                      cpy_bnd);
