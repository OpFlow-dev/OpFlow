...
void test_inheritance() {
    ...
    CHECK_MESSAGE((std::is_base_of<graph_node, multifunction_node<I, O>>::value),
                  "multifunction_node should be derived from graph_node");
    CHECK_MESSAGE((std::is_base_of<receiver<I>, multifunction_node<I, O>>::value),
                  "multifunction_node should be derived from receiver<Input>");
}
...
TEST_CASE("multifunction_node with rejecting policy") {
    conformance::test_rejecting<
            oneapi::tbb::flow::multifunction_node<int, std::tuple<int>, oneapi::tbb::flow::rejecting>>();
}

//! Test nodes for execution with priority in single-threaded configuration
//! \brief \ref interface
TEST_CASE("multifunction_node priority") {
    conformance::test_priority<oneapi::tbb::flow::multifunction_node<input_msg, std::tuple<int>>, input_msg>(
...
TEST_CASE("multifunction_node constructors") {
    ...
    multifunction_node<int, std::tuple<int>, lightweight> lw_node2(g, serial, fun, lightweight(),
                                                                   oneapi::tbb::flow::node_priority_t(1));
}
...
TEST_CASE("multifunction_node buffering") {
    ...
    conformance::test_buffering<
            oneapi::tbb::flow::multifunction_node<input_msg, std::tuple<int>, oneapi::tbb::flow::rejecting>,
            input_msg>(oneapi::tbb::flow::unlimited, fun);
}
...
TEST_CASE("multifunction_node broadcast") {
    ...
    conformance::test_forwarding<oneapi::tbb::flow::multifunction_node<input_msg, std::tuple<int>>, input_msg,
                                 int>(1, oneapi::tbb::flow::unlimited, fun);
}

//! Test the body object passed to a node is copied
//! \brief \ref interface
TEST_CASE("multifunction_node copy body") {
    conformance::test_copy_body_function<oneapi::tbb::flow::multifunction_node<int, std::tuple<int>>,
                                         conformance::copy_counting_object<int>>(
            oneapi::tbb::flow::unlimited);
}
...
TEST_CASE("multifunction_node body") {
    conformance::test_body_exec<oneapi::tbb::flow::multifunction_node<input_msg, std::tuple<output_msg>,
                                                                      oneapi::tbb::flow::rejecting>,
                                input_msg, output_msg>(oneapi::tbb::flow::unlimited);
}
...
TEST_CASE("multifunction_node output_ports") {
    ...
    CHECK_MESSAGE(
            (std::is_same<oneapi::tbb::flow::multifunction_node<int, std::tuple<int>>::output_ports_type&,
                          decltype(node.output_ports())>::value),
            "multifunction_node output_ports should returns a tuple of output ports");
}
...
TEST_CASE("Test function_node Output and Input class") {
    ...
    conformance::test_output_input_class<oneapi::tbb::flow::multifunction_node<Body, std::tuple<Body>>,
                                         Body>();
}
