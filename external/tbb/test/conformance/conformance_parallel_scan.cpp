...
TEST_CASE_TEMPLATE("Test parallel scan with body", Partitioner, default_partitioner_tag,
                   oneapi::tbb::simple_partitioner, oneapi::tbb::auto_partitioner) {
    ...
    for (size_t i = 0; i < size; ++i) {
        input[i] = int(i / 2);
        if (i) control[i] = control[i - 1] + input[i];
        else
            control[i] = input[i];
    }
...
TEST_CASE_TEMPLATE("Test parallel scan with body", Partitioner, default_partitioner_tag,
    ...
    parallel_scan_wrapper<Partitioner>()(
            oneapi::tbb::blocked_range<std::size_t>(0U, size, 1U), std::size_t(0),
            [&](const oneapi::tbb::blocked_range<std::size_t>& r, std::size_t sum,
                bool is_final) -> std::size_t {
                std::size_t temp = sum;
                for (std::size_t i = r.begin(); i < r.end(); ++i) {
                    temp = temp + input[i];
                    if (is_final) output[i] = temp;
                }
                return temp;
            },
...
void test_pscan_invoke(const std::vector<std::size_t>& desired_vector,
                       std::vector<std::size_t>& result_vector, Args&&... args) {
...
TEST_CASE("parallel_scan and std::invoke") {
    ...
    for (std::size_t i = 1; i < iterations; ++i) { desired_vector[i] = desired_vector[i - 1] + i; }
