...
std::vector<my_input_tuple> get_values(conformance::test_push_receiver<my_input_tuple> &rr) {
    ...
    while (rr.try_get(tmp)) { messages.push_back(tmp); }
    return messages;
}
...
TEST_CASE("join_node copy constructor") {
    ...
    oneapi::tbb::flow::continue_node<int> node0(g, [](oneapi::tbb::flow::continue_msg) { return 1; });
    ...
    CHECK_MESSAGE((conformance::get_values(node2).size() == 0 && values.size() == 1),
    ...
    g.wait_for_all();
    CHECK_MESSAGE((std::get<0>(conformance::get_values(node3)[0]) == 2),
                  "Copied node doesn`t copy messages in the input ports");
}
...
TEST_CASE("join_node queueing policy and broadcast property") {
    ...
    oneapi::tbb::flow::continue_node<input_msg> c1(
            g, [](oneapi::tbb::flow::continue_msg) { return input_msg(1); });
    ...
    oneapi::tbb::flow::function_node<my_input_tuple, my_input_tuple> f3(
            g, oneapi::tbb::flow::unlimited, [&](const my_input_tuple &t) {
                CHECK_MESSAGE((std::get<0>(t) == number), "Messages must be in first-in first-out order");
                CHECK_MESSAGE((std::get<1>(t) == static_cast<float>(number) + 0.5f),
                              "Messages must be in first-in first-out order");
...
struct MyHash {
    std::size_t hash(const KeyType &k) const { return k * 2000 + 3; }

    bool equal(const KeyType &k1, const KeyType &k2) const { return hash(k1) == hash(k2); }
};
...
TEST_CASE("join_node key_matching policy") {
    ...
    oneapi::tbb::flow::join_node<std::tuple<oneapi::tbb::flow::continue_msg, float>,
                                 oneapi::tbb::flow::key_matching<int, MyHash<int>>>
            testing_node(g, body1, body2);
...
TEST_CASE("join_node tag_matching policy") {
    ...
    auto body2 = [](const float &val) -> oneapi::tbb::flow::tag_value {
        return static_cast<oneapi::tbb::flow::tag_value>(val);
    };

    oneapi::tbb::flow::join_node<std::tuple<oneapi::tbb::flow::continue_msg, float>,
                                 oneapi::tbb::flow::tag_matching>
            testing_node(g, body1, body2);
...
TEST_CASE("join_node output_ports") {
    ...
    CHECK_MESSAGE((std::is_same<oneapi::tbb::flow::join_node<std::tuple<int>>::input_ports_type &,
                                decltype(node.input_ports())>::value),
                  "join_node input_ports should returns a tuple of input ports");
}
...
TEST_CASE("key_matching join_node invoke semantics") {
    test_invoke_basic</*K = */ std::size_t>(&test_invoke::SmartID<std::size_t>::get_id,
                                            &test_invoke::SmartID<std::size_t>::id);
    test_invoke_basic</*K = */ const std::size_t &>(&test_invoke::SmartID<std::size_t>::get_id_ref,
                                                    &test_invoke::SmartID<std::size_t>::get_id_ref);
}
