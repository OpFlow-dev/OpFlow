...
using input_msg = conformance::message</*default_ctor*/ true, /*copy_ctor*/ true,
...
TEST_CASE("split_node buffering") {
    ...
    float tmp2 = -1;
    input_msg tmp3(-1);
    CHECK_MESSAGE((oneapi::tbb::flow::output_port<0>(testing_node).try_get(tmp1) == false && tmp1 == -1),
                  "Value should be discarded after rejection");
    CHECK_MESSAGE((oneapi::tbb::flow::output_port<1>(testing_node).try_get(tmp2) == false && tmp2 == -1.f),
                  "Value should be discarded after rejection");
    CHECK_MESSAGE((oneapi::tbb::flow::output_port<2>(testing_node).try_get(tmp3) == false && tmp3 == -1),
                  "Value should be discarded after rejection");
}
...
TEST_CASE("split_node copy constructor") {
    oneapi::tbb::flow::graph g;
    oneapi::tbb::flow::continue_node<std::tuple<int>> node0(
            g, [](oneapi::tbb::flow::continue_msg) { return std::tuple<int>(1); });
...
TEST_CASE("split_node superclasses") {
    CHECK_MESSAGE((std::is_base_of<oneapi::tbb::flow::graph_node, my_split_type>::value),
                  "split_node should be derived from graph_node");
    CHECK_MESSAGE((std::is_base_of<oneapi::tbb::flow::receiver<my_input_tuple>, my_split_type>::value),
                  "split_node should be derived from receiver<T>");
}
...
TEST_CASE("split_node output_ports") {
    ...
    CHECK_MESSAGE((std::is_same<my_split_type::output_ports_type&, decltype(node.output_ports())>::value),
                  "split_node output_ports should returns a tuple of output ports");
}
