...
TEST_CASE("indexer_node superclasses") {
    CHECK_MESSAGE((std::is_base_of<oneapi::tbb::flow::graph_node, my_indexer_type>::value),
                  "indexer_node should be derived from graph_node");
}
...
TEST_CASE("indexer_node behaviour") {
    ...
    oneapi::tbb::flow::continue_node<input_msg> c1(
            g, [](oneapi::tbb::flow::continue_msg) { return input_msg(5); });

    my_indexer_type testing_node(g);

    oneapi::tbb::flow::function_node<my_output_type> f3(
            g, oneapi::tbb::flow::unlimited, [](const my_output_type &v) {
                if (v.tag() == 0) {
                    ...
                    CHECK_MESSAGE((oneapi::tbb::flow::cast_to<input_msg>(v) == 5),
                                  "Expected to receive input_msg(5)");
                }
            });
...
TEST_CASE("indexer_node output_type") {
    CHECK_MESSAGE(
            (conformance::check_output_type<my_output_type,
                                            oneapi::tbb::flow::tagged_msg<size_t, int, float, input_msg>>()),
            "indexer_node output_type should returns a tagged_msg");
}
