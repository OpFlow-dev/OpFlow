...
void test_continue_node_rf_reset_protocol() {
    ...
    continue_node<int> source(g, 2, [&](const continue_msg&) {
        flag = true;
        return 1;
    });
...
void test_limiter_node_rf_reset_protocol() {
    ...
    for (int i = 0; i < limit * 2; ++i) testing_node.try_put(1);
    g.wait_for_all();

    CHECK_MESSAGE((conformance::get_values(suc_node).size() == limit),
                  "Descendant of the node needs be receive limited number of messages");
...
TEST_CASE("graph reset with rf_clear_edges") {
    ...
    CHECK_MESSAGE((conformance::get_values(successor).size() == 0),
                  "Message should not pass when edges doesn't exist");
    CHECK_MESSAGE((conformance::get_values(successor2).size() == 0),
                  "Message should not pass when edge doesn't exist");
    CHECK_MESSAGE((conformance::get_values(successor3).size() == 0),
                  "Message should not pass when edge doesn't exist");
}
...
TEST_CASE("graph cancel") {
    ...
    const unsigned N = 10;
    for (unsigned i = 0; i < N; ++i) f.try_put(0);
