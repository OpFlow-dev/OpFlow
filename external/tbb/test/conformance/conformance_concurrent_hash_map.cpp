...
struct Emplace {
    ...
    static void apply_impl(DataStateTrackedTable& table, int i) {
        Accessor a;
        REQUIRE_MESSAGE((table.emplace(a, MyKey::make(i), (i + 1))), "already present while should not ?");
        CHECK((*a).second == i + 1);
...
TEST_CASE("testing consruction with hash_compare") { TestHashCompareConstructors(); }
