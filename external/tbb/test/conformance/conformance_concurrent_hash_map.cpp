...
protected:
    ...
    int data;
    enum state_t { LIVE = 0x1234, DEAD = 0x5678 } my_state;
    void operator=(const MyData&);// Deny access
public:
    MyData(int i = 0) {
        my_state = LIVE;
        data = i;
        if (MyDataCountLimit && MyDataCount + 1 >= MyDataCountLimit) { TBB_TEST_THROW(MyException {}); }
        ++MyDataCount;
    }
...
class MyHashCompare {
public:
    bool equal(const MyKey& j, const MyKey& k) const { return j.key == k.key; }

    std::size_t hash(const MyKey& k) const { return k.key; }
};
...
public:
    ...
    std::size_t hash(const MyKey&) const { return 1; }
};
...
void test_member_types() {
    ...
    static_assert(std::is_same<typename container_type::allocator_type,
...
void TestRehash() {
    ...
    for (; it != w.end(); i++) w.count((it++)->first);
    CHECK(i == 1);
...
void TestIteratorsAndRanges() {
    ...
    using test_allocator_type2
            = StaticSharedCountingAllocator<std::allocator<std::pair<const MyKey, MyData2>>>;
    using YourTable1
            = oneapi::tbb::concurrent_hash_map<MyKey, MyData2, YourHashCompare, test_allocator_type2>;
...
void TestExceptions() {
    using allocator_type
            = only_node_counting_allocator<oneapi::tbb::tbb_allocator<std::pair<const MyKey, MyData2>>>;
    using throwing_table = oneapi::tbb::concurrent_hash_map<MyKey, MyData2, MyHashCompare, allocator_type>;
    enum methods { zero_method = 0, ctor_copy, op_assign, op_insert, all_methods };
    ...
    try {
        for (int t = 0; t < 2; t++)// exception type
            for (int m = zero_method + 1; m < all_methods; m++) {
                ...
                try {
                    switch (m) {
                        ...
                        case op_insert: {
                            ...
                            if (MyDataCountLimit) --MyDataCountLimit;
                ...
                } catch (...) { REQUIRE_MESSAGE(false, "Unrecognized exception"); }
            }
...
struct hash_map_traits : default_container_traits {
    ...
    template <typename T>
    struct hash_compare {
        bool equal(const T& lhs, const T& rhs) const { return lhs == rhs; }
        size_t hash(const T& k) const { return my_hash_func(k); }
    ...
    struct apply {
        using type = oneapi::tbb::concurrent_hash_map<element_type, element_type, hash_compare<element_type>,
                                                      allocator_type>;
    };
    ...
    static bool equal(hash_map_type const& c, iterator begin, iterator end) {
        ...
        for (iterator it = begin; it != end; ++it) {
            if (c.count((*it).first) == 0) { return false; }
        }
        return true;
    }
};
...
struct Emplace {
    ...
    static void apply_impl(DataStateTrackedTable& table, int i) {
        Accessor a;
        CHECK_FAST_MESSAGE((table.emplace(a, MyKey::make(i), (i + 1))), "already present while should not ?");
...
public:
    void operator()(const oneapi::tbb::blocked_range<int>& range) const {
        for (int i = range.begin(); i != range.end(); ++i) Op::apply(my_table, i);
    }
    TableOperation(test_table_type& table) : my_table(table) {}
};

bool UseKey(size_t i) { return (i & 3) != 3; }
...
struct Find {
    static void apply(test_table_type& table, int i) {
        ...
        if (b) {
            if (!UseKey(i)) REPORT("Line %d: unexpected key %d present\n", __LINE__, i);
            CHECK_FAST(ca->second.value_of() == i * i);
            CHECK_FAST((*ca).second.value_of() == i * i);
            if (i & 1) ca->second.set_value(~ca->second.value_of());
            else
                (*ca).second.set_value(~ca->second.value_of());
        } else {
            if (UseKey(i)) REPORT("Line %d: key %d missing\n", __LINE__, i);
        }
    }
};
...
void DoConcurrentOperations(TableType& table, int n, const char* what, std::size_t nthread) {
    ...
    oneapi::tbb::parallel_for(oneapi::tbb::blocked_range<int>(0, n, 100),
                              TableOperation<Op, TableType>(table));
}
...
void TraverseTable(test_table_type& table, size_t n, size_t expected_size) {
    ...
    for (test_table_type::iterator i = table.begin(); i != table.end(); ++i) {
        ...
        std::pair<test_table_type::const_iterator, test_table_type::const_iterator> cer
...
struct InsertErase {
    static void apply(YourTable& table, int i) {
        if (i % 3) {
            int key = i % IE_SIZE;
            if (table.insert(std::make_pair(MyKey::make(key), MyData2()))) ++InsertEraseCount[key];
        } else {
            int key = i % IE_SIZE;
            if (i & 1) {
                YourTable::accessor res;
                if (table.find(res, MyKey::make(key)) && table.erase(res)) --InsertEraseCount[key];
            } else {
...
struct ParallelTraverseBody {
    ...
    ParallelTraverseBody(AtomicByte array_[], size_t n_) : n(n_), array(array_) {}
...
void TestInsertFindErase(std::size_t nthread) {
    ...
    for (int i = 0; i < n; ++i) m += UseKey(i);
    {
        ...
        for (int i = 0; i < 2; ++i) {
            if (i == 0)
                DoConcurrentOperations<InsertInitList, test_table_type>(
                        table, n, "insert(std::initializer_list)", nthread);
            else
        ...
        if (nthread > 1) {
            YourTable ie_table;
            for (int i = 0; i < IE_SIZE; ++i) InsertEraseCount[i] = 0;
            DoConcurrentOperations<InsertErase, YourTable>(ie_table, n / 2, "insert_erase", nthread);
            for (int i = 0; i < IE_SIZE; ++i) CHECK(InsertEraseCount[i] == ie_table.count(MyKey::make(i)));
...
public:
    AddToTable(test_table_type& table, std::size_t nthread, int m)
        : my_table(table), my_nthread(nthread), my_m(m) {}
    void operator()(std::size_t) const {
        for (int i = 0; i < my_m; ++i) {
            ...
            int k;
            {
                test_table_type::accessor a;
                MyKey key = MyKey::make(i);
                if (my_table.insert(a, key)) a->second.set_value(1);
                else
                    a->second.set_value(a->second.value_of() + 1);
                k = a->second.value_of();
            }
            if (std::size_t(k) == my_nthread) Counter = i + 1;
        }
    }
};
...
struct non_default_constructible_hash_compare : oneapi::tbb::detail::d1::tbb_hash_compare<Key> {
    non_default_constructible_hash_compare() {
        REQUIRE_MESSAGE(false, "Hash compare object must not default construct during the construction of "
                               "hash_map with compare argument");
    }
...
void TestHashCompareConstructors() {
    using key_type = int;
    using map_type = oneapi::tbb::concurrent_hash_map<key_type, key_type,
                                                      non_default_constructible_hash_compare<key_type>>;
...
void TestCHMapComparisons() {
    ...
    using two_way_comparable_container
            = oneapi::tbb::concurrent_hash_map<comparisons_testing::TwoWayComparable,
...
public:
    ...
    HeterogeneousKey(int key) : my_key(key) {
        ++heterogeneous_keys_count;
    }
...
struct HeterogeneousHashCompare {
    ...
    std::size_t hash(const int& key) const { return my_hash_object(key); }

    bool equal(const int& key1, const int& key2) const { return key1 == key2; }
    ...
    template <bool IsConstructible>
    bool equal(const HeterogeneousKey<IsConstructible>& key1,
               const HeterogeneousKey<IsConstructible>& key2) const {
...
void test_heterogeneous_find() {
    ...
    REQUIRE_MESSAGE(key_type::heterogeneous_keys_count == 0,
                    "Temporary key object was created during find call with const_accessor (no element)");
    ...
    REQUIRE_MESSAGE(key_type::heterogeneous_keys_count == 0,
                    "Temporary key object was created during find call with accessor (no element)");

    bool tmp_result = chmap.emplace(cacc, std::piecewise_construct,
                                    std::forward_as_tuple(key_type::construct_flag {}, 1),
    ...
    REQUIRE(regular_result);
    REQUIRE_MESSAGE(regular_result == heterogeneous_result,
                    "Incorrect heterogeneous find result with const_accessor (element exists)");
    REQUIRE_MESSAGE(cacc->first.integer_key() == 1, "Incorrect accessor returned");
    REQUIRE_MESSAGE(key_type::heterogeneous_keys_count == 0,
                    "Temporary key object was created during find call with const_accessor (element exists)");
    cacc.release();
    ...
    REQUIRE(regular_result);
    REQUIRE_MESSAGE(regular_result == heterogeneous_result,
                    "Incorrect heterogeneous find result with accessor (element exists)");
    REQUIRE_MESSAGE(acc->first.integer_key() == 1, "Incorrect accessor returned");
    REQUIRE_MESSAGE(key_type::heterogeneous_keys_count == 0,
                    "Temporary key object was created during find call with accessor (element exists)");
    key_type::reset();
}
...
void test_heterogeneous_count() {
    ...
    REQUIRE_MESSAGE(regular_count == heterogeneous_count,
                    "Incorrect heterogeneous count result (no element)");
    REQUIRE_MESSAGE(key_type::heterogeneous_keys_count == 0,
                    "Temporary key object was created during count call (no element)");

    chmap.emplace(std::piecewise_construct, std::forward_as_tuple(key_type::construct_flag {}, 1),
                  std::forward_as_tuple(100));
    ...
    REQUIRE_MESSAGE(regular_count == heterogeneous_count,
                    "Incorrect heterogeneous count result (element exists)");
    REQUIRE_MESSAGE(key_type::heterogeneous_keys_count == 0,
                    "Temporary key object was created during count call (element exists)");
    key_type::reset();
}
...
void test_heterogeneous_equal_range() {
    ...
    REQUIRE_MESSAGE(regular_result == heterogeneous_result,
                    "Incorrect heterogeneous equal_range result (non const, no element)");
    REQUIRE_MESSAGE(key_type::heterogeneous_keys_count == 0,
                    "Temporary key object was created during equal_range call (non const, no element)");
    ...
    REQUIRE_MESSAGE(key_type::heterogeneous_keys_count == 0,
                    "Temporary key object was created during equal_range call (const, no element)");
    ...
    REQUIRE(regular_result.second == chmap.end());
    REQUIRE_MESSAGE(regular_result == heterogeneous_result,
                    "Incorrect heterogeneous equal_range result (non const, element exists)");
    REQUIRE_MESSAGE(key_type::heterogeneous_keys_count == 0,
                    "Temporary key object was created during equal_range call (non const, element exists)");
    ...
    REQUIRE_MESSAGE(key_type::heterogeneous_keys_count == 0,
                    "Temporary key object was created during equal_range call (const, element exists)");
    key_type::reset();
}
...
void test_heterogeneous_insert() {
    ...
    using chmap_type
            = oneapi::tbb::concurrent_hash_map<key_type, DefaultConstructibleValue, HeterogeneousHashCompare>;
    ...
    REQUIRE_MESSAGE(cacc->second.value() == DefaultConstructibleValue::default_value,
    ...
    REQUIRE_MESSAGE(key_type::heterogeneous_keys_count == 2,
                    "Only one extra heterogeneous key should be created");
    ...
    REQUIRE_MESSAGE(acc->second.value() == DefaultConstructibleValue::default_value,
                    "Value should be default constructed");
...
void test_heterogeneous_erase() {
    ...
    chmap.emplace(std::piecewise_construct, std::forward_as_tuple(key_type::construct_flag {}, 1),
                  std::forward_as_tuple(100));
    chmap.emplace(std::piecewise_construct, std::forward_as_tuple(key_type::construct_flag {}, 2),
...
TEST_CASE("testing construction with hash_compare") { TestHashCompareConstructors(); }

//! Test concurrent_hash_map member types
//! \brief \ref interface \ref requirement
TEST_CASE("test types") { test_member_types<oneapi::tbb::concurrent_hash_map>(); }

//! Test swap and clear operations
//! \brief \ref interface \ref requirement
TEST_CASE("test copy operations") { TestCopy(); }

//! Test rehash operation
//! \brief \ref interface \ref requirement
TEST_CASE("test rehash operation") { TestRehash(); }

//! Test assignment operation
//! \brief \ref interface \ref requirement
TEST_CASE("test assignment operation") { TestAssignment(); }

//! Test iterators and ranges
//! \brief \ref interface \ref requirement
TEST_CASE("test iterators and ranges") { TestIteratorsAndRanges(); }

//! Test work with initializer_list
//! \brief \ref interface \ref requirement
TEST_CASE("test work with initializer_list") { TestInitList(); }

#if TBB_USE_EXCEPTIONS
//! Test exception safety
//! \brief \ref requirement
TEST_CASE("test exception safety") { TestExceptions(); }
...
//! \brief \ref interface \ref requirement
TEST_CASE("testing move constructor") { move_support_tests::test_move_constructor<hash_map_traits>(); }

//! Test move assign operator
//! \brief \ref interface \ref requirement
TEST_CASE("testing move assign operator") { move_support_tests::test_move_assignment<hash_map_traits>(); }
...
//! \brief \ref requirement
TEST_CASE("testing allocator traits") { test_allocator_traits_support<hash_map_traits>(); }
...
//! \brief \ref interface
TEST_CASE("testing deduction guides") { TestDeductionGuides<oneapi::tbb::concurrent_hash_map>(); }
#endif// __TBB_CPP17_DEDUCTION_GUIDES_PRESENT

//! \brief \ref interface \ref requirement
TEST_CASE("concurrent_hash_map comparisons") { TestCHMapComparisons(); }

//! \brief \ref interface \ref requirement
TEST_CASE("concurrent_hash_map iterator comparisons") { TestCHMapIteratorComparisons(); }

//! \brief \ref interface \ref requirement
TEST_CASE("test concurrent_hash_map heterogeneous lookup") { test_heterogeneous_lookup(); }

//! \brief \ref interface \ref requirement
TEST_CASE("test concurrent_hash_map heterogeneous insert") { test_heterogeneous_insert(); }

//! \brief \ref interface \ref requirement
TEST_CASE("test concurrent_hash_map heterogeneous erase") { test_heterogeneous_erase(); }
