...
static const oneapi::tbb::filter_mode filter_table[]
        = {oneapi::tbb::filter_mode::parallel, oneapi::tbb::filter_mode::serial_in_order,
...
void TestSingleFilter(Body body, Cotnext&... context) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        oneapi::tbb::parallel_pipeline(n_tokens, oneapi::tbb::make_filter<void, void>(mode, body),
                                       context...);
    }
}
...
void RunPipeline(const oneapi::tbb::filter<I, O>& filter) {
    ...
    auto f_end = oneapi::tbb::make_filter<O, void>(oneapi::tbb::filter_mode::serial_in_order, [](O) {});
...
void RootSequence(Iterator1 first, Iterator1 last, Iterator2 res) {
    ...
    oneapi::tbb::parallel_pipeline(
            n_tokens, oneapi::tbb::make_filter<void, ValueType>(
                              ...
                              & oneapi::tbb::make_filter<ValueType, ValueType>(
                                      oneapi::tbb::filter_mode::parallel, [](ValueType p) { return p * p; })
                              & oneapi::tbb::make_filter<ValueType, void>(
                                      oneapi::tbb::filter_mode::serial_in_order, [&res](ValueType x) {
                                          *res = x;
                                          ++res;
                                      }));
}
...
TEST_CASE("Testing pipeline correctness") {
    ...
    for (std::size_t i = 0; i < max_counter; i++) input[i] = (double) i;
...
//! \brief \ref interface \ref requirement
TEST_CASE_TEMPLATE("Filter creation testing", T, std::tuple<size_t, int>, std::tuple<int, double>,
                   std::tuple<unsigned int*, size_t>, std::tuple<unsigned short, unsigned short*>,
    ...
    for (int i = 0; i < 3; i++) {
        ...
        static_assert(std::is_same<oneapi::tbb::filter<I, O>, decltype(made_filter1)>::value,
                      "make_filter wrong result type");
        RunPipeline(made_filter1);

        auto made_filter2 = oneapi::tbb::make_filter(mode, [](I) -> O { return O(); });
        static_assert(std::is_same<oneapi::tbb::filter<I, O>, decltype(made_filter2)>::value,
                      "make_filter wrong result type");
...
//! \brief \ref interface \ref requirement
TEST_CASE_TEMPLATE("Testing filters concatenation", T, std::tuple<size_t, int>, std::tuple<int, double>,
                   std::tuple<unsigned int*, size_t>, std::tuple<unsigned short, unsigned short*>,
                   std::tuple<double*, unsigned short*>,
                   std::tuple<std::unique_ptr<int>, std::unique_ptr<int>>) {
    using I = typename std::tuple_element<0, T>::type;
    ...
    for (int fi = 0; fi < 27; fi++) {
        ...
        oneapi::tbb::filter<void, I> filter1
                = oneapi::tbb::filter<void, I>(filter_table[i], input_filter<I>());
        oneapi::tbb::filter<I, O> filter2 = oneapi::tbb::filter<I, O>(filter_table[j], middle_filter<I, O>());
        oneapi::tbb::filter<O, void> filter3
                = oneapi::tbb::filter<O, void>(filter_table[k], output_filter<O>());
...
void doWork() {
    for (int i = 0; i < 10; ++i) utils::yield();
}
...
TEST_CASE("Testing filter modes") {
    for (auto concurrency_level : utils::concurrency_range()) {
        ...
        oneapi::tbb::filter<void, short> filter1(
                oneapi::tbb::filter_mode::serial_out_of_order,
                [&serial_checker](oneapi::tbb::flow_control& fc) {
                    ...
                    if (serial_checker >= (short) max_counter) { fc.stop(); }
                    return check_value;
                });

        short serial_checker2 {0};
        oneapi::tbb::filter<short, short> filter2(
                oneapi::tbb::filter_mode::serial_in_order, [&serial_checker2](int) {
                    auto check_value = ++serial_checker2;
                    doWork();
                    CHECK_MESSAGE(check_value == serial_checker2,
                                  "a serial filter was executed concurrently");
                    return check_value;
                });
        ...
        oneapi::tbb::filter<int, void> filter4(
                oneapi::tbb::filter_mode::serial_in_order, [&order_checker](int value) {
...
TEST_CASE("Testing max token number") {
    for (unsigned int i = 1; i < n_tokens; i++) {
        ...
        oneapi::tbb::filter<void, int> filter1(
                oneapi::tbb::filter_mode::parallel, [&active_tokens](oneapi::tbb::flow_control& fc) {
                    ++active_tokens;
                    doWork();
        ...
        oneapi::tbb::filter<int, int> filter2(
                oneapi::tbb::filter_mode::parallel, [&active_tokens](int value) {
                    ++active_tokens;
                    doWork();
        ...
        oneapi::tbb::filter<int, void> filter3(
                oneapi::tbb::filter_mode::serial_out_of_order, [&active_tokens](int) {
                    ++active_tokens;
                    doWork();
                    CHECK_MESSAGE(active_tokens <= n_tokens, "max number of tockens is exceed");
                    --active_tokens;
...
TEST_CASE_TEMPLATE("Deduction guides testing", T, int, unsigned int, double) {
    ...
    oneapi::tbb::filter fclv(oneapi::tbb::filter_mode::serial_in_order,
                             [](const int&) -> double { return 0.0; });
    oneapi::tbb::filter fc(oneapi::tbb::filter_mode::serial_in_order,
                           [](const int) -> double { return 0.0; });
...
void test_pipeline_invoke_basic(const MiddleFilterBody& middle_body, const LastFilterBody& last_body) {
    ...
    auto first_body = [&](oneapi::tbb::flow_control& fc) -> middle_filter_type {
        if (++counter > input_count) { fc.stop(); }
        return middle_filter_type {output_filter_type {&signal_point}};
    };

    auto first_filter = oneapi::tbb::make_filter<void, middle_filter_type>(
            oneapi::tbb::filter_mode::serial_in_order, first_body);
    auto middle_filter = oneapi::tbb::make_filter<middle_filter_type, output_filter_type>(
            oneapi::tbb::filter_mode::serial_in_order, middle_body);
    auto last_filter = oneapi::tbb::make_filter<output_filter_type, void>(
            oneapi::tbb::filter_mode::serial_in_order, last_body);
...
TEST_CASE("parallel_pipeline and std::invoke") {
    ...
    test_pipeline_invoke_basic(
            &middle_filter_type::get_id,
            &output_filter_type::operate);// Pointer to non-static function as middle filter
    test_pipeline_invoke_basic(&middle_filter_type::id,
                               &output_filter_type::operate);// Pointer to non-static member as middle filter
}
