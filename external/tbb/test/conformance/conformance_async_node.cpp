...
TEST_CASE("async_node constructors") {
    ...
    async_node<int, int, lightweight> lw_node2(g, serial, fun, lightweight(),
...
TEST_CASE("async_node buffering") {
    conformance::dummy_functor<int> fun;
    conformance::test_buffering<oneapi::tbb::flow::async_node<input_msg, int>, input_msg>(
            oneapi::tbb::flow::unlimited, fun);
}

//! Test priorities work in single-threaded configuration
//! \brief \ref requirement
TEST_CASE("async_node priority support") {
    conformance::test_priority<oneapi::tbb::flow::async_node<input_msg, int>, input_msg>(
...
TEST_CASE("Test async_node body") {
    conformance::test_body_exec<oneapi::tbb::flow::async_node<input_msg, output_msg>, input_msg, output_msg>(
            oneapi::tbb::flow::unlimited);
}
...
TEST_CASE("async_node superclasses") {
    ...
    conformance::test_inheritance<oneapi::tbb::flow::async_node<input_msg, output_msg>, input_msg,
                                  output_msg>();
}
...
TEST_CASE("async_node broadcast") {
    ...
    conformance::test_forwarding<oneapi::tbb::flow::async_node<input_msg, int>, input_msg, int>(
            1, oneapi::tbb::flow::unlimited, fun);
}
...
TEST_CASE("async_node body copying") {
    conformance::test_copy_body_function<oneapi::tbb::flow::async_node<int, int>,
...
TEST_CASE("async_node with rejecting policy") {
    ...
    std::thread thr;
    async_node_type testing_node {g, tbb::flow::unlimited, [&](const int& input, gateway_type& gateway) {
    ...
    g.wait_for_all();
    CHECK_MESSAGE((flag.load()), "The body of assync_node must submits the messages to an external activity "
                                 "for processing outside of the graph");
    thr.join();
}
...
TEST_CASE("async_node and std::invoke") {
    ...
    auto async_body = &test_invoke::SmartID<std::size_t>::template send_id_to_gateway<
            typename async_node_type::gateway_type>;

    graph g;
    start_node_type starter(g, serial,
                            [](std::size_t i) -> test_invoke::SmartID<std::size_t> { return {i}; });
