...
public:
    adder(oneapi::tbb::flow::graph &g)
        : base_type(g), j(g), f(g, oneapi::tbb::flow::unlimited, f_body()), qn(g) {
        make_edge(j, f);
        make_edge(f, qn);

        base_type::input_ports_type input_tuple(oneapi::tbb::flow::input_port<0>(j),
                                                oneapi::tbb::flow::input_port<1>(j));
...
TEST_CASE("composite_node superclasses") {
    CHECK_MESSAGE((std::is_base_of<oneapi::tbb::flow::graph_node, adder>::value),
                  "composite_node should be derived from graph_node");
}
...
TEST_CASE("composite_node ports") {
    ...
    using InputTupleType = std::tuple<tbb::flow::continue_msg, std::tuple<int, int>, int, int, int, int, int,
                                      int, int, int, int, int, int, int>;

    using OutputTupleType = std::tuple<tbb::flow::continue_msg, std::tuple<int, int>,
                                       oneapi::tbb::flow::tagged_msg<size_t, int, float>, int, int, int, int,
    ...
    oneapi::tbb::flow::multifunction_node<int, std::tuple<int, int>> m_fxn(g, oneapi::tbb::flow::unlimited,
                                                                           body());
    ...
    auto input_tuple = std::tie(ct, s, m_fxn, fxn, bc, oneapi::tbb::flow::input_port<0>(j), lim, q,
                                oneapi::tbb::flow::input_port<0>(ind), pq, ovw, wo, bf, seq);
    auto output_tuple = std::tie(ct, j, ind, fxn, src, bc, oneapi::tbb::flow::output_port<0>(s), lim,
                                 oneapi::tbb::flow::output_port<0>(m_fxn), q, pq, ovw, wo, bf, seq);
    ...
    compare<NUM_INPUTS - 1, decltype(a_node_input_ports_ptr), decltype(input_tuple)>::compare_refs(
            a_node_input_ports_ptr, input_tuple);
    CHECK_MESSAGE(NUM_INPUTS == std::tuple_size<decltype(a_node_input_ports_ptr)>::value,
                  "not all declared input ports were bound to nodes");

    auto a_node_output_ports_ptr = a_node.output_ports();
    compare<NUM_OUTPUTS - 1, decltype(a_node_output_ports_ptr), decltype(output_tuple)>::compare_refs(
            a_node_output_ports_ptr, output_tuple);
    CHECK_MESSAGE((NUM_OUTPUTS == std::tuple_size<decltype(a_node_output_ports_ptr)>::value),
                  "not all declared output ports were bound to nodes");
    ...
    compare<NUM_INPUTS - 1, decltype(b_node_input_ports_ptr), decltype(input_tuple)>::compare_refs(
            b_node_input_ports_ptr, input_tuple);
    CHECK_MESSAGE(NUM_INPUTS == std::tuple_size<decltype(b_node_input_ports_ptr)>::value,
                  "not all declared input ports were bound to nodes");
    ...
    compare<NUM_OUTPUTS - 1, decltype(c_node_output_ports_ptr), decltype(output_tuple)>::compare_refs(
            c_node_output_ports_ptr, output_tuple);
    CHECK_MESSAGE(NUM_OUTPUTS == std::tuple_size<decltype(c_node_output_ports_ptr)>::value,
                  "not all declared input ports were bound to nodes");
}
