...
class TestFunctor {
public:
    void operator()(T index) const { pfor_buffer[index]++; }
};
...
class FooRange {
    ...
    template <typename Flavor_, std::size_t Pad_>
    friend void Flog();
    template <size_t Pad_>
...
public:
    ~FooBody() {
        --FooBodyCount;
        for (std::size_t i = 0; i < sizeof(*this); ++i) reinterpret_cast<char*>(this)[i] = -1;
    }
    // Copy constructor
...
void Flog() {
    for (int i = 0; i < N; ++i) {
        for (int mode = 0; mode < 4; ++mode) {
            ...
            for (int a_i = 0; a_i < N; a_i++) { Array[a_i].store(0, std::memory_order_relaxed); }
            FooBodyCount = 1;
            switch (mode) {
                case 0: {
                ...
                case 1: {
                    Invoker<Flavor, const oneapi::tbb::simple_partitioner, FooRange<Pad>, FooBody<Pad>>
                            invoke_for;
                    invoke_for(rc, fc, oneapi::tbb::simple_partitioner());
                } break;
                case 2: {
                    Invoker<Flavor, const oneapi::tbb::auto_partitioner, FooRange<Pad>, FooBody<Pad>>
                            invoke_for;
                    invoke_for(rc, fc, oneapi::tbb::auto_partitioner());
                } break;
                case 3: {
                    static oneapi::tbb::affinity_partitioner affinity;
                    Invoker<Flavor, oneapi::tbb::affinity_partitioner, FooRange<Pad>, FooBody<Pad>>
                            invoke_for;
                    invoke_for(rc, fc, affinity);
                } break;
            }
            CHECK(std::find_if_not(
                          Array, Array + i,
                          [](const std::atomic<int>& v) { return v.load(std::memory_order_relaxed) == 1; })
                  == Array + i);
            CHECK(std::find_if_not(
                          Array + i, Array + N,
                          [](const std::atomic<int>& v) { return v.load(std::memory_order_relaxed) == 0; })
                  == Array + N);
    ...
    for (T begin = 0; begin < pfor_buffer_test_size - 1; begin += pfor_buffer_test_size / 10 + 1) {
        ...
        for (step = 1; step < pfor_buffer_test_size; step++) {
            ...
            for (T i = 0; i < pfor_buffer_actual_size; i++) {
                if (pfor_buffer[i] != 0) { CHECK_MESSAGE(false, "parallel_for processed an extra element"); }
            }
        }
    }
}
...
public:
    ...
    SmartIndex(std::size_t ri, std::vector<std::size_t>& cv) : real_index(ri), change_vector(&cv) {}
    SmartIndex(const SmartIndex& other) : real_index(other.real_index), change_vector(other.change_vector) {}
    ~SmartIndex() = default;
    ...
    bool operator<(const SmartIndex& other) const { return real_index < other.real_index; }
    ...
    std::size_t operator-(const SmartIndex& other) const { return real_index - other.real_index; }

    SmartIndex operator+(std::size_t k) { return {real_index + k, *change_vector}; }
...
void test_pfor_body_invoke() {
    ...
    for (std::size_t item : change_vector) { CHECK(item == number_of_overloads); }
}
...
TEST_CASE("Basic parallel_for") {
    ...
    oneapi::tbb::parallel_for(std::size_t(0), iterations, [&](std::size_t) { counter++; });

    oneapi::tbb::parallel_for(
            std::size_t(0), iterations, [&](std::size_t) { counter++; }, oneapi::tbb::simple_partitioner());

    oneapi::tbb::parallel_for(
            std::size_t(0), iterations, [&](std::size_t) { counter++; }, oneapi::tbb::auto_partitioner());

    oneapi::tbb::parallel_for(
            std::size_t(0), iterations, [&](std::size_t) { counter++; }, oneapi::tbb::static_partitioner());

    oneapi::tbb::affinity_partitioner aff;
    oneapi::tbb::parallel_for(
            std::size_t(0), iterations, [&](std::size_t) { counter++; }, aff);
...
//! \brief \ref interface \ref requirement
TEST_CASE_TEMPLATE("parallel_for with step support", T, short, unsigned short, int, unsigned int, long,
                   unsigned long, long long, unsigned long long, std::size_t) {
