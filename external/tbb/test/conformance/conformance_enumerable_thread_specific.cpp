...
public:
    minimalNComparable() : utils::NoAssign(), my_value(0) {
        ++construction_counter;
        is_constructed = true;
    }
    minimalNComparable(const minimalNComparable& m) : utils::NoAssign(), my_value(m.my_value) {
        ++construction_counter;
        is_constructed = true;
    }
    ~minimalNComparable() {
        ++destruction_counter;
        CHECK_FAST(is_constructed);
...
public:
    Thrower() {
        ...
        if (++gThrowValue == targetThrowValue) { throw std::bad_alloc(); }
#endif
    }
};
...
struct test_helper<minimalNComparable<N>> {
    ...
    static inline void sum(minimalNComparable<N>& sum, const int addend) {
        sum.set_value(sum.value() + addend);
    }
    static inline void sum(minimalNComparable<N>& sum, const double addend) {
        sum.set_value(sum.value() + static_cast<int>(addend));
    }
    static inline void sum(minimalNComparable<N>& sum, const minimalNComparable<N>& addend) {
        sum.set_value(sum.value() + addend.value());
    }
    static inline void set(minimalNComparable<N>& v, const int value) {
        v.set_value(static_cast<int>(value));
    }
...
struct test_helper<ThrowingConstructor> {
    ...
    static inline void sum(ThrowingConstructor& sum, const double addend) {
        sum.m_cnt += static_cast<int>(addend);
    }
    static inline void sum(ThrowingConstructor& sum, const ThrowingConstructor& addend) {
        sum.m_cnt += addend.m_cnt;
    }
...
struct FunctorFinit<ThrowingConstructor, Value> {
    ...
    FunctorFinit(SecretTagType) { ++FinitCounter; }
    ~FunctorFinit() { --FinitCounter; }
    ThrowingConstructor operator()() {
        ThrowingConstructor temp;
        temp.m_cnt = Value;
        return temp;
    }
};
...
struct FunctorAddCombineRef<minimalNComparable<N>> {
    minimalNComparable<N> operator()(const minimalNComparable<N>& left,
                                     const minimalNComparable<N>& right) const {
        minimalNComparable<N> result;
...
struct FunctorAddCombine {
    T operator()(T left, T right) const { return FunctorAddCombineRef<T>()(left, right); }
};
...
template <typename T>
T FunctionAdd(T left, T right) {
    return FunctionAddByRef(left, right);
}
...
void run_parallel_scalar_tests_nocombine(const char* /* test_name */, const char* allocator_name) {
    ...
    struct fail_on_exception_guard {
        ...
        ~fail_on_exception_guard() {
            if (!dismiss) { FAIL("The exception is not expected"); }
        }
    } guard;
    T default_value {};
    guard.dismiss = true;
    ...
    gThrowValue = 0;
    {
        ...
        for (int p = std::max(MinThread, 2); p <= MaxThread; ++p) {
            ...
            for (int t = -1; t < REPETITIONS; ++t) {
                ...
                oneapi::tbb::parallel_for(oneapi::tbb::blocked_range<int>(0, N * p, RANGE_MIN),
                                          parallel_scalar_body<T, Allocator>(finit_ets, allocator_name));
                REQUIRE(!finit_ets.empty());
                ...
                typedef typename oneapi::tbb::enumerable_thread_specific<T, Allocator<T>,
                                                                         oneapi::tbb::ets_key_per_instance>
                        cached_ets_type;

                cached_ets_type cconst(sums);
                oneapi::tbb::parallel_for(oneapi::tbb::blocked_range<int>(0, N * p, RANGE_MIN),
                                          [&](const oneapi::tbb::blocked_range<int>&) {
                                              bool exists = false;
...
#endif
                {
                    for (int t = -1; t < REPETITIONS; ++t) {
                        ...
                        oneapi::tbb::parallel_for(
                                oneapi::tbb::blocked_range<int>(0, N * p, RANGE_MIN),
                                parallel_scalar_body<T, Allocator>(static_sums, allocator_name));
                        REQUIRE(!static_sums.empty());
...
struct parallel_vector_reduce_body {
    ...
    parallel_vector_reduce_body(parallel_vector_reduce_body<R, T>&, oneapi::tbb::split) : count(0) {
...
void run_parallel_vector_tests(const char* /* test_name */, const char* allocator_name) {
    ...
    for (int p = std::max(MinThread, 2); p <= MaxThread; ++p) {
        ...
        for (int t = -1; t < REPETITIONS; ++t) {
            ...
            for (typename oneapi::tbb::flattened2d<ets_type>::const_iterator i = fvs.begin(); i != fvs.end();
                 ++i) {
                ++elem_cnt;
            };
            ...
            REQUIRE(vvs.empty());
            oneapi::tbb::parallel_for(oneapi::tbb::blocked_range<int>(0, N * p, RANGE_MIN),
                                      parallel_vector_for_body<T, Allocator>(vvs, allocator_name));
            REQUIRE(!vvs.empty());
...
void run_cross_type_vector_tests(const char* /* test_name */) {
    ...
    for (int p = std::max(MinThread, 2); p <= MaxThread; ++p) {
        ...
        for (int t = -1; t < REPETITIONS; ++t) {
            typedef typename oneapi::tbb::enumerable_thread_specific<
                    container_type, Allocator<container_type>, oneapi::tbb::ets_no_key>
                    ets_nokey_type;
            typedef typename oneapi::tbb::enumerable_thread_specific<
                    container_type, Allocator<container_type>, oneapi::tbb::ets_key_per_instance>
                    ets_tlskey_type;
            ets_nokey_type vs;
            ...
            size_t ccount = fvs.size();
            size_t elem_cnt = 0;
            for (typename oneapi::tbb::flattened2d<ets_nokey_type>::const_iterator i = fvs.begin();
                 i != fvs.end(); ++i) {
                ++elem_cnt;
            };
            REQUIRE(ccount == elem_cnt);
            ...
            elem_cnt = 0;
            for (typename oneapi::tbb::flattened2d<ets_nokey_type>::iterator i = fvs.begin(); i != fvs.end();
                 ++i) {
                ++elem_cnt;
            };
            REQUIRE(ccount == elem_cnt);

            oneapi::tbb::flattened2d<ets_nokey_type> fvs2
                    = flatten2d(vs3, vs3.begin(), std::next(vs3.begin()));
...
void run_serial_scalar_tests(const char* /* test_name */) {
    ...
    for (int t = -1; t < REPETITIONS; ++t) {
        for (int i = 0; i < N; ++i) { test_helper<T>::sum(sum, 1); }
    }
...
void run_serial_vector_tests(const char* /* test_name */) {
    ...
    for (int t = -1; t < REPETITIONS; ++t) {
        ...
        for (int i = 0; i < N; ++i) { v.push_back(one); }
        for (typename std::vector<T, oneapi::tbb::tbb_allocator<T>>::const_iterator i = v.begin();
             i != v.end(); ++i)
...
void run_parallel_tests(const char* allocator_name) {
    ...
    run_parallel_scalar_tests_nocombine<minimalNComparable<>, Allocator>("minimalNComparable<>",
                                                                         allocator_name);
    run_parallel_scalar_tests<ThrowingConstructor, Allocator>("ThrowingConstructor", allocator_name);
    run_parallel_vector_tests<int, Allocator>("std::vector<int, oneapi::tbb::tbb_allocator<int> >",
                                              allocator_name);
    run_parallel_vector_tests<double, Allocator>("std::vector<double, oneapi::tbb::tbb_allocator<double> >",
                                                 allocator_name);
}

void run_cross_type_tests() {
    // cross-type scalar tests are part of run_parallel_scalar_tests_nocombine
    run_cross_type_vector_tests<int, oneapi::tbb::tbb_allocator>(
            "std::vector<int, oneapi::tbb::tbb_allocator<int> >");
    run_cross_type_vector_tests<double, oneapi::tbb::tbb_allocator>(
            "std::vector<double, oneapi::tbb::tbb_allocator<double> >");
}
...
struct MakeETS_Functor {
    ETS operator()(typename std::decay<P>::type&&... params) { return ETS(std::move(params)...); }
};
template <typename T, template <class> class Allocator, typename... P>
oneapi::tbb::enumerable_thread_specific<T, Allocator<T>>
MakeETS(oneapi::tbb::detail::stored_pack<P...> pack) {
    typedef oneapi::tbb::enumerable_thread_specific<T, Allocator<T>> result_type;
    return oneapi::tbb::detail::call_and_return<result_type>(MakeETS_Functor<result_type, P...>(),
                                                             std::move(pack));
}

template <typename T, template <class> class Allocator, typename InitSrc, typename InitDst,
          typename Validator>
...
struct Validator {
    void operator()(const T& value) { REQUIRE(test_helper<T>::get(value) == Expected); }
...
#define EXPECTED 3142
    ...
    ets_copy_assign_test<std::pair<int, T>, Allocator>(oneapi::tbb::detail::save_pack(17, src_init),
                                                       std::make_pair(-1, T()), Validator<T, EXPECTED>(),
                                                       allocator_name);
#undef EXPECTED
}
...
void run_assignment_and_copy_constructor_tests(const char* allocator_name) {
    ...
    run_assign_and_copy_constructor_test<minimalNComparable<line_size - 1>, Allocator>(
            "minimalNComparable<line_size-1>", allocator_name);
    run_assign_and_copy_constructor_test<minimalNComparable<line_size>, Allocator>(
            "minimalNComparable<line_size>", allocator_name);
    run_assign_and_copy_constructor_test<minimalNComparable<line_size + 1>, Allocator>(
            "minimalNComparable<line_size+1>", allocator_name);
...
struct HasNoDefaultConstructorFinit {
    HasNoDefaultConstructor operator()() { return HasNoDefaultConstructor(SecretTag); }
};
...
void TestInstantiation(const char* /* allocator_name */) {
    ...
    oneapi::tbb::enumerable_thread_specific<HasNoDefaultConstructor, Allocator<HasNoDefaultConstructor>> ets2(
            x);
    ets2.local();
    ...
    oneapi::tbb::enumerable_thread_specific<HasNoDefaultConstructor, Allocator<HasNoDefaultConstructor>> ets3(
            f);
    ets3.local();
    ets3.combine(HasNoDefaultConstructorCombine());

    // Test instantiation with multiple arguments
    oneapi::tbb::enumerable_thread_specific<HasSpecialAndMoveCtor, Allocator<HasSpecialAndMoveCtor>> ets4(
            SecretTag, 0x42, "meaningless");
    ets4.local();
    ...
    oneapi::tbb::enumerable_thread_specific<HasSpecialAndMoveCtor, Allocator<HasSpecialAndMoveCtor>> ets5(
            SecretTag);
    ets5.local();
...
void TestMemberTypes() {
    ...
    static_assert(std::is_same<typename ets_container_type::pointer,
                               typename std::allocator_traits<allocator_type>::pointer>::value,
                  "Incorrect container pointer member type");
    static_assert(std::is_same<typename ets_container_type::const_pointer,
...
void TestETSIterator() {
    ...
    if (utils::get_platform_max_threads() == 1) {
        ...
        auto fill_ets_body = [&]() {
            ets.local() = 42;
            ++sync_counter;
            while (sync_counter != expected_ets_size) utils::yield();
        };
        ...
        auto iter = ets.begin();
        while (iter != ets.end()) { ++counter % 2 == 0 ? ++iter : iter++; }
        REQUIRE(counter == expected_ets_size);
        while (iter != ets.begin()) { --counter % 2 == 0 ? --iter : iter--; }
        REQUIRE(counter == 0);
        auto citer = cets.begin();
        while (citer != cets.end()) { ++counter % 2 == 0 ? ++citer : citer++; }
        REQUIRE(counter == expected_ets_size);
        while (citer != cets.begin()) { --counter % 2 == 0 ? --citer : citer--; }
        REQUIRE(counter == 0);
...
void TestETSIteratorComparisons() {
    ...
    auto fill_ets_body = [&](int) {
        ets.local() = 42;
        ++sync_counter;
        while (sync_counter != expected_ets_size) std::this_thread::yield();
    };
...
TEST_CASE("Assignment and copy constructor") {
    AlignMask = cache_allocator_mask;
    run_assignment_and_copy_constructor_tests<oneapi::tbb::cache_aligned_allocator>(
...
TEST_CASE("Basic ETS functionality") {
    ...
    for (int i : sequence) { ref_combined += i; }
    ref_combined += 42;
    int ets_combined = ets.combine([](int x, int y) { return x + y; });
...
//! \brief \ref interface \ref requirement
TEST_CASE("Member types") { TestMemberTypes(); }

//! \brief \ref interface \ref requirement
TEST_CASE("enumerable_thread_specific iterator") { TestETSIterator(); }

//! \brief \ref interface \ref requirement
TEST_CASE("enumerable_thread_specific iterator comparisons") { TestETSIteratorComparisons(); }
