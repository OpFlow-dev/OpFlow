...
public:
    minimal() : utils::NoAssign(), my_value(0) {
        ++construction_counter;
        is_constructed = true;
    }
    minimal(const minimal& m) : utils::NoAssign(), my_value(m.my_value) {
        ++construction_counter;
        is_constructed = true;
    }
    ~minimal() {
...
struct test_helper<minimal<N>> {
    ...
    static inline void sum(minimal<N>& sum, const double addend) {
        sum.set_value(sum.value() + static_cast<int>(addend));
    }
    static inline void sum(minimal<N>& sum, const minimal<N>& addend) {
...
void run_assignment_and_copy_constructor_tests(const char* allocator_name) {
    ...
    run_assign_and_copy_constructor_test<minimal<line_size - 1>, Allocator>("minimal<line_size-1>",
                                                                            allocator_name);
