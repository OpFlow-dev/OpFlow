    ...
    for (int x = -10; x < 10; ++x) {
        for (int y = -10; y < 10; ++y) {
            ...
            for (std::size_t k = 1; k < 10; ++k) {
                ...
                utils::AssertSameType(static_cast<range_type::const_iterator*>(nullptr),
                                      static_cast<AbstractValueType*>(nullptr));
...
struct Striker {
    void operator()(const oneapi::tbb::blocked_range<int>& r) const {
        for (oneapi::tbb::blocked_range<int>::const_iterator i = r.begin(); i != r.end(); ++i) ++Array[i];
    }
};
...
TEST_CASE("Basic serial") { SerialTest(); }
