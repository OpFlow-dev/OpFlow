...
public:
    ...
    minimalCombinable& operator=(const minimalCombinable& other) {
...
struct FunctorAddCombine {
    T operator()(T left, T right) const { return left + right; }
};
...
public:
    ...
    void operator()(const oneapi::tbb::blocked_range<int>& r) const {
        for (int i = r.begin(); i != r.end(); ++i) { sums.local() += 1; }
    }
};
...
void RunParallelScalarTests(const char* /* test_name */) {
    for (int p = MinThread; p <= MaxThread; ++p) {
        ...
        for (int t = -1; t < REPETITIONS; ++t) {
            ...
            oneapi::tbb::combinable<T> sums;
            oneapi::tbb::parallel_for(oneapi::tbb::blocked_range<int>(0, N, 10000),
                                      ParallelScalarBody<T>(sums));
            ...
            oneapi::tbb::parallel_for(oneapi::tbb::blocked_range<int>(0, N, 10000),
                                      ParallelScalarBody<T>(sums_to_clear));
            sums_to_clear.clear();
            ...
            oneapi::tbb::parallel_for(oneapi::tbb::blocked_range<int>(0, N, 10000),
                                      ParallelScalarBodyNoInit<T>(finit_combinable));
...
public:
    ParallelVectorForBody(oneapi::tbb::combinable<std::vector<T, oneapi::tbb::tbb_allocator<T>>>& _locals)
        : locals(_locals) {}

    void operator()(const oneapi::tbb::blocked_range<int>& r) const {
        T one = 1;

        for (int i = r.begin(); i < r.end(); ++i) { locals.local().push_back(one); }
    }
};
...
template <typename T>
void RunAssignmentAndCopyConstructorTest(const char* /* test_name */) {
    ...
    REQUIRE_MESSAGE(7 == create1.combine(my_combine<T>),
                    "Unexpected combine result for combinable object preinitialized with functor");

    // test copy construction with function initializer
    oneapi::tbb::combinable<T> copy1(create1);
    REQUIRE_MESSAGE(7 == copy1.combine(my_combine<T>),
                    "Unexpected combine result for copy-constructed combinable object");

    // test copy assignment with function initializer
    FunctorAddFinit<T> my_finit_decl;
    oneapi::tbb::combinable<T> assign1(my_finit_decl);
    assign1 = create1;
    REQUIRE_MESSAGE(7 == assign1.combine(my_combine<T>),
                    "Unexpected combine result for copy-assigned combinable object");

    // test move construction with function initializer
    oneapi::tbb::combinable<T> move1(std::move(create1));
    REQUIRE_MESSAGE(7 == move1.combine(my_combine<T>),
                    "Unexpected combine result for move-constructed combinable object");

    // test move assignment with function initializer
    oneapi::tbb::combinable<T> move2;
    move2 = std::move(copy1);
    REQUIRE_MESSAGE(7 == move2.combine(my_combine<T>),
                    "Unexpected combine result for move-assigned combinable object");
...
void RunMoveSemanticsForStateTrackableObjectTest() {
    ...
    REQUIRE_MESSAGE(move1.local().state == StateTrackableBase::Unspecified,
                    "Unexpected value in move-constructed combinable object");

    // Move assignment must not cause copying of stored values
    copy1 = std::move(move1);
    REQUIRE_MESSAGE(copy1.local().state == StateTrackableBase::Unspecified,
                    "Unexpected value in move-assigned combinable object");
...
void RunLocalAllocationsTests() {
    ...
    for (int i = 1 <= MinThread ? MinThread : 1; i <= MaxThread; ++i) {
        ...
        for (int j = 0; j < 100; ++j) { TestLocalAllocations(i); }
        // REMARK("  done\n");
    }
}
...
//! \brief \ref interface \ref requirement
TEST_CASE("Assignment and copy constructor test") { RunAssignmentAndCopyConstructorTests(); }

//! Test move support
//! \brief \ref interface \ref requirement
TEST_CASE("Move semantics") { RunMoveSemanticsForStateTrackableObjectTest(); }
