...
void test_inheritance() {
    ...
    CHECK_MESSAGE((std::is_base_of<graph_node, input_node<O>>::value),
                  "input_node should be derived from graph_node");
    CHECK_MESSAGE((std::is_base_of<sender<O>, input_node<O>>::value),
                  "input_node should be derived from sender<Output>");
    CHECK_MESSAGE((!std::is_base_of<receiver<O>, input_node<O>>::value),
                  "input_node cannot have predecessors");
}

//! Test the body object passed to a node is copied
//! \brief \ref interface
TEST_CASE("input_node and body copying") {
    conformance::test_copy_body_function<oneapi::tbb::flow::input_node<int>,
...
TEST_CASE("input_node copy constructor") {
    ...
    conformance::copy_counting_object<output_msg> b2
            = copy_body<conformance::copy_counting_object<output_msg>, input_node<output_msg>>(node_copy);
...
TEST_CASE("input_node body") {
    ...
    CHECK_MESSAGE((conformance::get_values(node2).size() == counting_threshold),
                  "Descendant of the node needs to be receive N messages");
    CHECK_MESSAGE((fun.execute_count == counting_threshold + 1),
                  "Body of the node needs to be executed N + 1 times");
}
...
struct function_node_counter {
    ...
    int operator()(int) {
        ...
        CHECK_MESSAGE((input_node_counter::count <= function_node_counter::count + 1),
                      "input_node `try_get()' call testing: a call to body is made only when the internal "
                      "buffer is empty");
        return 1;
    }
};
...
//! \brief \ref requirement
TEST_CASE("input_node `try_get()' call testing: a call to body is made only when the internal buffer is "
          "empty.") {
    ...
    oneapi::tbb::flow::function_node<int, int, oneapi::tbb::flow::rejecting> fnode(
            g, oneapi::tbb::flow::serial, fun2);
