...
void test_cancellation_on_exception(bool reset_ctx) {
    ...
    for (std::size_t i = 0; i < iter_counter; ++i) {
        ...
        } catch (int ex) { REQUIRE(ex == 1); }
        if (reset_ctx) { test_context.reset(); }
        wait.reserve(1);
    }
    wait.release(1);

    REQUIRE_MESSAGE(task.execute_counter() == (reset_ctx ? iter_counter : 1), "Some task was not executed");
    REQUIRE_MESSAGE(task.cancel_counter() == iter_counter,
                    "Some task was not canceled after the exception occurs");
    task.reset();
}
...
TEST_CASE("Test that task was executed p times") {
    ...
    REQUIRE_MESSAGE(CountingTask<>::execute_counter() == iter_counter,
                    "The task was not executed necessary times");
...
TEST_CASE("Simple test parallelism usage") {
    ...
    auto barrier_wait = [&barrier] { barrier.wait(); };
    ...
    std::vector<task_type, tbb::cache_aligned_allocator<task_type>> vector_test_task(
            threads_num, task_type(barrier_wait, wait));
...
TEST_CASE("Test parallelism usage with parallel_for") {
    ...
    auto parallel_for_func = [&pfor_counter, pfor_iter_count] {
        tbb::parallel_for(tbb::blocked_range<std::size_t>(0, pfor_iter_count),
                          [&pfor_counter](tbb::blocked_range<std::size_t>& range) {
                              for (auto it = range.begin(); it != range.end(); ++it) { ++pfor_counter; }
                          });
    };
    ...
    using task_type = CountingTask<decltype(barrier_wait)>;
    std::vector<task_type, tbb::cache_aligned_allocator<task_type>> vector_test_task(
            task_threads_num, task_type(barrier_wait, wait));
    ...
    for (std::size_t i = 0; i < iter_count; ++i) {
        ...
        for (std::size_t k = 0; k < pfor_threads_num; ++k) { pfor_threads.emplace_back(parallel_for_func); }
    ...
    REQUIRE_MESSAGE(task_type::execute_counter() == task_threads_num * iter_count,
                    "Some task was not executed");
    REQUIRE_MESSAGE(task_type::cancel_counter() == 0, "Some task was canceled");
    REQUIRE_MESSAGE(pfor_counter == iter_count * pfor_threads_num * pfor_iter_count,
                    "Some parallel_for thread was not finished");
    task_type::reset();
}
...
TEST_CASE("Test parallelism usage with spawn tasks in different threads") {
    ...
    for (std::size_t i = 0; i < iter_count; ++i) {
        ...
        for (std::size_t k = 0; k < threads_num - 1; ++k) { threads.emplace_back(thread_func, k); }

        for (auto& thread : threads) {
            if (thread.joinable()) { thread.join(); }
        }

        tbb::detail::d1::execute_and_wait(vector_test_task[threads_num - 1], test_context, wait,
                                          test_context);
        wait.reserve(threads_num);
    }
    wait.release(threads_num);
...
public:
    ...
    void operator()() const {
        ...
        if (start_idx < my_task_pool.size()) {
            for (std::size_t idx = start_idx; idx != std::min(my_task_pool.size(), start_idx + delta);
                 ++idx) {
...
TEST_CASE("Actively adding tasks") {
    ...
    for (std::size_t i = 0; i < task_number; ++i) { task_pool.emplace_back(task_body, wait); }

    SpawningTask first_task(task_body, wait);
    tbb::detail::d1::execute_and_wait(first_task, test_context, wait, test_context);

    REQUIRE_MESSAGE(SpawningTask::execute_counter() == task_number + 1,
                    "Some tasks were not executed");// Is it right?
...
struct suspended_task : public tbb::detail::d1::task {
    ...
    task* execute(tbb::detail::d1::execution_data&) override {
        tbb::parallel_for(
                tbb::blocked_range<std::size_t>(0, 100000),
                [](const tbb::blocked_range<std::size_t>& range) {
                    // Make some heavy work
                    std::atomic<int> sum {};
                    for (auto it = range.begin(); it != range.end(); ++it) { ++sum; }
                },
...
TEST_CASE("Isolation + resumable tasks") {
    ...
    tbb::parallel_for(
            tbb::blocked_range<std::size_t>(0, 100000),
            [&suspend_flag, &test_context, &suspend_count,
             &resume_count](const tbb::blocked_range<std::size_t>& range) {
                int ticket = 0;
                for (auto it = range.begin(); it != range.end(); ++it) { ticket = suspend_flag++; }
                ...
                if (ticket % 5 == 0) {
                    ...
                    tbb::this_task_arena::isolate([&wait, &test_context, &test_task] {
                        ...
                        tbb::task::suspend(
                                [&wait, &test_context, &test_task, thread_id](tbb::task::suspend_point tag) {
...
struct bypass_task : public tbb::detail::d1::task {
    ...
    bypass_task(tbb::detail::d1::wait_context& wait, task_pool_type& task_pool, std::atomic<int>& resume_flag,
                tbb::task::suspend_point& suspend_tag)
    ...
    task* execute(tbb::detail::d1::execution_data&) override {
        ...
        int expected = 1;
        if (my_resume_flag.compare_exchange_strong(expected, 2)) { tbb::task::resume(my_suspend_tag); }
...
TEST_CASE("Bypass suspended by resume") {
    ...
    for (std::size_t i = 0; i < utils::get_platform_max_threads(); ++i) {
        ...
        if (ticket < test_task_pool.size()) { tbb::detail::d1::spawn(test_task_pool[ticket], test_context); }
    }
...
TEST_CASE("Critical tasks + resume") {
    ...
    for (std::size_t i = 0; i < task_number; ++i) { test_tasks.emplace_back(task_body, wait); }
    ...
    test_arena.execute([&wait, &test_tasks, &test_arena, &test_context, &resumed, task_number] {
        tbb::this_task_arena::isolate(
                [&wait, &test_tasks, &test_arena, &test_context, &resumed, task_number] {
                    do {
                        wait.reserve(task_number / 2);
                        tbb::parallel_for(tbb::blocked_range<std::size_t>(task_number / 2, task_number),
                                          [&test_tasks, &test_arena,
                                           &test_context](tbb::blocked_range<std::size_t>& range) {
    ...
    test_arena.execute([&wait, &test_context] { tbb::detail::d1::wait(wait, test_context); });
}
...
TEST_CASE("Stress testing") {
    ...
    auto task_body = [] {
        tbb::parallel_for(tbb::blocked_range<std::size_t>(0, 1000),
                          [](tbb::blocked_range<std::size_t>&) { utils::doDummyWork(100); });
    };
    using task_type = CountingTask<decltype(task_body)>;
    ...
    for (std::size_t j = 0; j < task_number; ++j) { test_tasks.emplace_back(task_body, wait); }

    test_arena.execute(
            [&test_tasks, &task_body, &wait, &test_context, &test_arena, iter_counter, task_number] {
                for (std::size_t i = 0; i < iter_counter; ++i) {

                    for (std::size_t j = 0; j < task_number; ++j) { test_arena.enqueue(task_body); }
...
TEST_CASE("All workers sleep") {
    ...
    for (std::size_t i = 0; i < thread_number - 1; ++i) { test_gr.run(resumble_task); }

    barrier.wait();
    barrier.wait();
    TestCPUUserTime(thread_number);

    for (auto sp : suspend_points) tbb::task::resume(sp);
    test_gr.wait();
}
...
TEST_CASE("Enqueue with exception") {
    ...
    auto task_body = [] { utils::doDummyWork(100); };

    std::atomic<bool> end_flag {false};
    auto check_body = [&end_flag] { end_flag.store(true, std::memory_order_relaxed); };
...
struct resubmitting_task : public tbb::detail::d1::task {
    ...
    tbb::detail::d1::task* execute(tbb::detail::d1::execution_data&) override {
        if (counter-- > 0) { submit(*this, my_arena, my_ctx, true); }
        return nullptr;
    }
...
TEST_CASE("Test with priority inversion") {
    ...
    auto critical_work = [&] { utils::doDummyWork(10); };
    ...
    for (std::size_t i = 0; i < thread_number - 1; ++i) { high_priority_threads[i].join(); }
}
        ...
        tbb::task_scheduler_handle handle {tbb::attach {}};
        {
            ...
            a.enqueue([] {
                tbb::parallel_for(0, 100,
                                  [](int) { std::this_thread::sleep_for(std::chrono::nanoseconds(10)); });
            });
...
TEST_CASE("Try to force Leaked proxy observers warning") {
    ...
    for (int i = 0; i < 1000; ++i) { observers.emplace_back(arena); }

    for (auto& observer : observers) { observer.observe(true); }

    arena.enqueue([] { tbb::parallel_for(0, 100000, [](int) { utils::doDummyWork(1000); }); });
}
...
TEST_CASE("Force thread limit on per-thread reference_vertex") {
    ...
    for (int i = 0; i < num_groups; ++i) { groups[i].run(std::move(handles[i])); }

    for (int i = 0; i < num_groups; ++i) { groups[i].wait(); }
}
