...
public:
    ...
    static bool AreEqual(Minimal& a, Minimal& b) { return a.val == b.val; }
};
...
void validate(Range test_range, Range sorted_range, std::size_t size) {
    for (std::size_t i = 0; i < size; i++) { REQUIRE(test_range[i] == sorted_range[i]); }
}
...
void validate(Minimal* test_range, Minimal* sorted_range, std::size_t size) {
    for (std::size_t i = 0; i < size; i++) { REQUIRE(Minimal::AreEqual(test_range[i], sorted_range[i])); }
}

//! A validate() specialized to concurrent_vector<Minimal> since it does not define an operator==
void validate(tbb::concurrent_vector<Minimal>::iterator test_range,
              tbb::concurrent_vector<Minimal>::iterator sorted_range, std::size_t size) {
    for (std::size_t i = 0; i < size; i++) { REQUIRE(Minimal::AreEqual(test_range[i], sorted_range[i])); }
}
...
*/
bool fill_ranges(std::string* iter, std::string* sorted_list, std::size_t size,
                 const std::less<std::string>& compare) {
    static char test_case = 0;
...
//! \brief \ref error_guessing
TEST_CASE("Float array sorting test (default comparator)") { parallel_sort_test_suite<float>(); }
...
//! \brief \ref error_guessing
TEST_CASE("Array of strings sorting test (default comparator)") { parallel_sort_test_suite<std::string>(); }
...
TEST_CASE("Array sorting test (default comparator)") {
    for (auto concurrency_level : utils::concurrency_range()) {
        ...
        for (int i = 0; i < vector_size; ++i) test_array[i] = rand() % vector_size;
