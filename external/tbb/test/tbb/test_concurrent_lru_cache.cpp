...
TEST_CASE("basic test for return value") {
    ...
    int dummy_key = 1;
    REQUIRE_MESSAGE(dummy_f(dummy_key) == cache[dummy_key].value(),
...
TEST_CASE("basic test for unused objects") {
    ...
    REQUIRE_MESSAGE(preset_object.source.instances_count() > 1, "cache should store some unused objects");
}
...
TEST_CASE("basic test for unused object limit") {
    ...
    for (std::size_t i = 0; i < preset_object.number_of_lru_history_items + 1; ++i) preset_object.cache[i];
...
TEST_CASE("basic test for eviction order") {
    ...
    REQUIRE_MESSAGE(preset_object.number_of_lru_history_items > 2, "incorrect test setup");
...
TEST_CASE("basic test for handling case when number_of_lru_history_items is zero") {
    auto foo = [](int) { return utils::LifeTrackableObject {}; };
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        REQUIRE_MESSAGE(utils::LifeTrackableObject::is_alive(obj_addr) == false,
                        "when number_of_lru_history_items is zero, element must be erased after use");
    }
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        cache[1];
        REQUIRE_MESSAGE(utils::LifeTrackableObject::is_alive(obj_addr),
                        "the object with the key=1 was destroyed but should not");
