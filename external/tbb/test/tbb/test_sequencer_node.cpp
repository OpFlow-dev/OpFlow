...
struct touches {
    ...
    bool validate_touches() {
        ...
        for (int n = 0; n < N; ++n) all_touches[n] = false;
        ...
        for (int n = 0; n < N; ++n) {
            if (!all_touches[n]) printf("No touch at %d, my_num_threads = %d\n", n, my_num_threads);
...
struct parallel_gets : utils::NoAssign {
    ...
    parallel_gets(tbb::flow::sequencer_node<T> &q, int num_threads, touches<T> &t)
        : my_q(q), my_num_threads(num_threads), my_touches(t) {}
...
struct parallel_put_get : utils::NoAssign {
    ...
    parallel_put_get(tbb::flow::sequencer_node<T> &s1, tbb::flow::sequencer_node<T> &s2, int num_threads,
                     std::atomic<int> &counter, touches<T> &t)
        : my_s1(s1), my_s2(s2), my_num_threads(num_threads), my_counter(counter), my_touches(t) {}
...
void test_follows_and_precedes_api() {
    ...
    follows_and_precedes_testing::test_follows<int, tbb::flow::sequencer_node<int>>(
            messages_for_follows, [](const int &i) -> std::size_t { return i; });

    follows_and_precedes_testing::test_precedes<int, tbb::flow::sequencer_node<int>>(
...
void test_seq_node_try_put_and_wait() {
    ...
    // test_buffer_push returns the index from which the items processed during wait_for_all() starts
    {
        ...
        std::size_t after_start = test_buffer_push<tbb::flow::sequencer_node<int>>(
...

    {
        ...
        std::size_t after_start = test_buffer_pull<tbb::flow::sequencer_node<int>>(
                start_work_items, wait_message, occupier, new_work_items, processed_items, simple_sequencer);
        ...
        CHECK_MESSAGE(processed_items[check_index++] == occupier,
                      "Unexpected items processing by try_put_and_wait");
        for (auto item : start_work_items) {
            CHECK_MESSAGE(processed_items[check_index++] == item,
                          "try_put_and_wait should process start_work_items FIFO");
        }
    ...
    // Test reserve
    {
        ...
        for (int threshold : thresholds) {
            ...
            std::size_t after_start = test_buffer_reserve<tbb::flow::sequencer_node<int>>(
            ...
            CHECK_MESSAGE(
                    after_start == start_work_items.size() + 1,
                    "start_work_items, occupier and wait_message should be processed by try_put_and_wait");
            for (auto item : start_work_items) {
                CHECK_MESSAGE(processed_items[check_index++] == item,
                              "try_put_and_wait should process start_work_items FIFO");
...
TEST_CASE("constraints for sequencer_node object") {
    ...
    static_assert(
            !utils::well_formed_instantiation<tbb::flow::sequencer_node, test_concepts::NonCopyAssignable>);
}

template <typename T, typename Sequencer>
concept can_call_sequencer_node_ctor
        = requires(tbb::flow::graph &graph, Sequencer seq, tbb::flow::buffer_node<int> &f) {
...
#if __TBB_PREVIEW_FLOW_GRAPH_TRY_PUT_AND_WAIT
//! \brief \ref error_guessing
TEST_CASE("test sequencer_node try_put_and_wait") { test_seq_node_try_put_and_wait(); }
#endif
