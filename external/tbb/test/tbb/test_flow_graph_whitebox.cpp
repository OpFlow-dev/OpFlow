...
template <typename T>
struct receiverBody {
    tbb::flow::continue_msg operator()(const T& /*in*/) { return tbb::flow::continue_msg(); }
};
...
void TestSplitNode() {
    ...
    CHECK_MESSAGE((tbb::flow::output_port<0>(snode).my_successors.empty()),
                  "Constructed split_node has successors");
    // tbb::flow::output_port<0>(snode)
    tbb::flow::make_edge(tbb::flow::output_port<0>(snode), rcvr);
    CHECK_MESSAGE((!(tbb::flow::output_port<0>(snode).my_successors.empty())),
                  "after make_edge, split_node has no successor.");
    snode.try_put(std::tuple<int>(1));
    g.wait_for_all();
    g.reset();
    CHECK_MESSAGE((!(tbb::flow::output_port<0>(snode).my_successors.empty())),
                  "after reset(), split_node has no successor.");
    g.reset(tbb::flow::rf_clear_edges);
    CHECK_MESSAGE((tbb::flow::output_port<0>(snode).my_successors.empty()),
                  "after reset(rf_clear_edges), split_node has a successor.");
}
...
void TestBufferingNode(const char* name) {
    ...
    tbb::flow::function_node<int, int, tbb::flow::rejecting> fnode(g, tbb::flow::serial,
                                                                   serial_fn_body<int>(serial_fn_state0));
    INFO("Testing " << name << ":");
    for (int icnt = 0; icnt < 2; icnt++) {
        ...
        if (reverse_edge) {
            ...
            CHECK_MESSAGE((!bnode.my_successors.empty()),
                          "buffering node has no successor after forwarding message");
        }
    ...
    g.reset(tbb::flow::rf_clear_edges);
    CHECK_MESSAGE((bnode.my_successors.empty()),
                  "buffering node has a successor after reset(rf_clear_edges)");
    ...
    for (int icnt = 0; icnt < 2; ++icnt) {
        ...
        std::thread t([&] {
            ...
            if (icnt == 0) {  // first time through, let the continue_node fire
                ...
                INFO(" reset");
                CHECK_MESSAGE((!cnode.my_successors.empty()),
                              "Empty successors in built graph (before reset)");
                CHECK_MESSAGE((cnode.my_predecessor_count == 2), "predecessor_count reset (before reset)");
                g.reset();// should still be the same
                CHECK_MESSAGE((!cnode.my_successors.empty()),
                              "Empty successors in built graph (after reset)");
                ...
                CHECK_MESSAGE((cnode.my_current_count == 0),
                              "state of continue_receiver incorrect after reset(rf_clear_edges)");
                CHECK_MESSAGE((cnode.my_successors.empty()),
                              "buffering node has a successor after reset(rf_clear_edges)");
                CHECK_MESSAGE((cnode.my_predecessor_count == cnode.my_initial_predecessor_count),
                              "predecessor count not reset");
            }
        });

        utils::SpinWaitWhileEq(serial_fn_state0,
                               0);// waiting for the first message to arrive in function_node
        ...
        if (icnt == 0) {
            ...
            utils::SpinWaitWhileEq(serial_fn_state0,
                                   0);// waiting for the second message to arrive in function_node
            // Now the body of function_node 0 is executing.
            serial_fn_state0 = 0;// release the node

            utils::SpinWaitWhileEq(serial_continue_state0, 0);// waiting for continue_node to start
            CHECK_MESSAGE((cnode.my_current_count == 0),
                          " my_current_count not reset before body of continue_node started");
...
void TestFunctionNode() {
    ...
    tbb::flow::function_node<int, int, tbb::flow::rejecting> fnode0(g, tbb::flow::serial,
                                                                    serial_fn_body<int>(serial_fn_state0));
    tbb::flow::function_node<int, int /*, tbb::flow::queueing*/> fnode1(
            g, tbb::flow::serial, serial_fn_body<int>(serial_fn_state0));
    ...
    t2.join();
    g.reset(tbb::flow::rf_clear_edges);
    CHECK_MESSAGE((fnode0.my_predecessors.empty() && qnode0.my_successors.empty()),
                  "function_node edge not removed");
...
void TestSimpleSuccessorArc(const char* name) {
    tbb::flow::graph g;
    {
...
template <>
void TestSimpleSuccessorArc<tbb::flow::tag_matching>(const char* name) {
    tbb::flow::graph g;
    {
        INFO("Join<" << name << "> successor test ");
        typedef std::tuple<int, int> my_tuple;
        tbb::flow::join_node<my_tuple, tbb::flow::tag_matching> qj(g, tag_func<int>(1), tag_func<int>(1));
...
void TestJoinNode() {
    ...
    INFO(" reserving preds");
    {
        ...
        CHECK_MESSAGE((!(tbb::flow::input_port<0>(rj).my_predecessors.empty())),
                      "reversed port missing predecessor");
        CHECK_MESSAGE(((tbb::flow::input_port<1>(rj).my_predecessors.empty())), "non-reversed port has pred");
        g.reset();
        CHECK_MESSAGE(((tbb::flow::input_port<0>(rj).my_predecessors.empty())),
                      "reversed port has pred after reset()");
        CHECK_MESSAGE(((tbb::flow::input_port<1>(rj).my_predecessors.empty())),
                      "non-reversed port has pred after reset()");
        q1.try_put(2);
        g.wait_for_all();// quiesce
        CHECK_MESSAGE((!(tbb::flow::input_port<1>(rj).my_predecessors.empty())),
                      "reversed port missing predecessor");
...
struct DecrementerHelper {
    ...
    static DType makeDType() { return DType(1); }
};
...
struct DecrementerHelper<tbb::flow::continue_msg> {
    ...
    static tbb::flow::continue_msg makeDType() { return tbb::flow::continue_msg(); }
};
...
void TestMultifunctionNode() {
    ...
    for (int ii = 0; ii < 2; ++ii) {
        ...
        if (ii == 1) {
            INFO(" rf_clear_edges");
            g.reset(tbb::flow::rf_clear_edges);
            CHECK_MESSAGE((tbb::flow::output_port<0>(mf).my_successors.empty()),
                          "output_port<0> not reset (rf_clear_edges)");
            CHECK_MESSAGE((tbb::flow::output_port<1>(mf).my_successors.empty()),
                          "output_port<1> not reset (rf_clear_edges)");
        } else {
            g.reset();
        }
        CHECK_MESSAGE((mf.my_predecessors.empty()), "edge didn't reset");
        CHECK_MESSAGE(((ii == 0 && !qin.my_successors.empty()) || (ii == 1 && qin.my_successors.empty())),
                      "edge didn't reset");
    }
    INFO(" done\n");
}
...
void TestIndexerNode() {
    ...
    g.reset(tbb::flow::rf_clear_edges);
    CHECK_MESSAGE((inode.my_successors.empty()),
                  "successor of indexer_node not removed by reset(rf_clear_edges)");
    INFO(" done\n");
}

template <typename Node>
void TestScalarNode(const char* name) {
    tbb::flow::graph g;
    Node on(g);
...
struct seq_body {
    size_t operator()(const int& in) { return size_t(in / 3); }
};
...
void TestInputNode() {
    ...
    CHECK_MESSAGE((tbb::flow::input_port<0>(jn).my_predecessors.empty()),
                  "successor of input_node has pred after reset.");
    INFO(" done\n");
}
...
//! Test sequencer_node
//! \brief \ref error_guessing
TEST_CASE("Test sequencer node") { TestSequencerNode(); }

TEST_SUITE("Test multifunction node") {
    //! Test multifunction_node with rejecting policy
    //! \brief \ref error_guessing
    TEST_CASE("with rejecting policy") { TestMultifunctionNode<tbb::flow::rejecting>(); }

    //! Test multifunction_node with queueing policy
    //! \brief \ref error_guessing
    TEST_CASE("with queueing policy") { TestMultifunctionNode<tbb::flow::queueing>(); }
}

//! Test input_node
//! \brief \ref error_guessing
TEST_CASE("Test input node") { TestInputNode(); }

//! Test continue_node
//! \brief \ref error_guessing
TEST_CASE("Test continue node") { TestContinueNode(); }

//! Test function_node
//! \brief \ref error_guessing
TEST_CASE("Test function node" * doctest::may_fail()) { TestFunctionNode(); }

//! Test join_node
//! \brief \ref error_guessing
TEST_CASE("Test join node") { TestJoinNode(); }
...
//! Test indexer_node
//! \brief \ref error_guessing
TEST_CASE("Test indexer node") { TestIndexerNode(); }

//! Test split_node
//! \brief \ref error_guessing
TEST_CASE("Test split node") { TestSplitNode(); }
...
TEST_CASE("try_get in inactive graph") {
    ...
    tbb::flow::input_node<int> src(g, [&](tbb::flow_control& fc) {
        fc.stop();
        return 0;
    });
    deactivate_graph(g);
