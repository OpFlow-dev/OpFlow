...
void check_outputCheck(int nUsed, int maxCnt) {
    for (int i = 0; i < nUsed; ++i) {
        for (int j = 0; j < maxCnt; ++j) { CHECK_MESSAGE(outputCheck[i][j], ""); }
    }
}

void reset_outputCheck(int nUsed, int maxCnt) {
    for (int i = 0; i < nUsed; ++i) {
        for (int j = 0; j < maxCnt; ++j) { outputCheck[i][j] = false; }
    }
}
...
public:
    my_input_body(TT multiplier, int init_val, int addto)
        : my_mult(multiplier), my_count(init_val), addend(addto) {}
...
public:
    ...
    static void add_input_nodes(indexer_node_type &my_indexer, tbb::flow::graph &g, int nInputs) {
        for (int i = 0; i < nInputs; ++i) {
            my_input_node_type *new_node
                    = new my_input_node_type(g, my_input_body<IT>((IT)(ELEM + 1), i, nInputs));
...
public:
    static void print_remark() { INFO("Parallel test of indexer_node< " << name_of<IT>::name()); }
    static void add_input_nodes(indexer_node_type &my_indexer, tbb::flow::graph &g, int nInputs) {
        for (int i = 0; i < nInputs; ++i) {
...
public:
    ...
    static void test() {
        ...
        for (int i = 0; i < MaxPorts; ++i) {
            for (int j = 0; j < MaxNInputs; ++j) { all_input_nodes[i][j] = nullptr; }
        }
...
public:
    ...
    static void print_remark() { INFO("Serial test of indexer_node< " << name_of<IT>::name()); }
    static void fill_one_queue(int maxVal, IType &my_indexer) {
        for (int i = 0; i < maxVal; ++i) {
...
void test_one_serial(IType &my_indexer, tbb::flow::graph &g) {
    ...
    for (int i = 0; i < Count; ++i) { serial_queue_helper<SIZE, IType>::put_one_queue_val(i, my_indexer); }
...
public:
    static void test() {
        ...
        serial_queue_helper<SIZE, IType>::print_remark();
        INFO(" >\n");
        ...
        for (int e = 0; e < ELEMS; ++e) { test_one_serial<IType, TType, SIZE>(indexer_vector[e], g); }
    }
...
template <template <typename> class TestType,// serial_test or parallel_test
          ...
          typename T9 = void>// type of the inputs to the indexer_node
class generate_test {
public:
    typedef tbb::flow::indexer_node<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9> indexer_node_type;
    static void do_test() { TestType<indexer_node_type>::test(); }
};

//specializations for indexer node inputs
template <template <typename> class TestType, typename T0, typename T1, typename T2, typename T3, typename T4,
...
template <template <typename> class TestType, typename T0, typename T1, typename T2, typename T3>
...
template <template <typename> class TestType, typename T0, typename T1, typename T2>
...
template <template <typename> class TestType, typename T0, typename T1>
...
template <template <typename> class TestType, typename T0>
...
bool check_edge(tbb::flow::graph &g, tbb::flow::broadcast_node<input_t> &start,
                tbb::flow::buffer_node<tagged_msg_t> &buf, input_t input_value) {
...
void check_edge(tbb::flow::graph &g, BN &bn, tbb::flow::buffer_node<indexer_output_t> &buf, Type,
                tbb::detail::index_sequence<Seq...>) {
    sink(check_edge<indexer_output_t>(g, std::get<Seq>(bn), buf,
                                      typename std::tuple_element<Seq, Type>::type(Seq))...);
}
...
void test_try_put_and_wait() {
    ...
    arena.execute([] {
        ...
        tbb::flow::function_node<output_type, int> function(
                g, tbb::flow::serial, [&](output_type tag_msg) noexcept {
                    if (tag_msg.tag() == 0) {
        ...
        for (auto item : new_work_items) {
            ...
            CHECK_MESSAGE(processed_items2[check_index2++] == float(item),
...
TEST_CASE("Serial and parallel test") {
    ...
    for (int p = 0; p < 2; ++p) {
        ...
        generate_test<serial_test, float, double, int, double, double, float, long, int, float,
                      long>::do_test();
#endif
...
#if __TBB_PREVIEW_FLOW_GRAPH_TRY_PUT_AND_WAIT
//! \brief \ref error_guessing
TEST_CASE("test indexer_node try_put_and_wait") { test_try_put_and_wait(); }
#endif
