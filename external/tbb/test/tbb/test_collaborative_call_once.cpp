...
TEST_CASE("handles exceptions - state reset") {
    ...
    b = false;
    REQUIRE_NOTHROW(
            tbb::collaborative_call_once(flag, setBAndCancel));// Flag still set, so it shouldn't be called.
    REQUIRE(!b);
}
...
TEST_CASE("multiple arenas") {
    ...
    a2.execute([&] {
        ...
        tbb::parallel_for(0, num_threads,
                          [&](int) { CHECK(utils::ConcurrencyTracker::PeakParallelism() == 1); });
        tbb::collaborative_call_once(flag, [&] {
            barrier.wait();
            tbb::parallel_for(0, num_threads, [&](int) { barrier.wait(); });
        });
        // To sync collaborative_once_flag lifetime
        barrier.wait();
    });
