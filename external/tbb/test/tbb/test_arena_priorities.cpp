...
namespace HighPriorityArenasTakeExecutionPrecedence {
    ...
    using arena_info = std::tuple<std::unique_ptr<tbb::task_arena>, concurrency_type,
                                  tbb::task_arena::priority, std::unique_ptr<tbb::task_group>>;

    enum arena_info_keys { arena_pointer, arena_concurrency, arena_priority, associated_task_group };
    ...
    void prepare_logging_data(std::vector<tbb::task_arena::priority>& task_log, unsigned overall_tasks_num) {
        ...
        for (auto& record : task_log) record = tbb::task_arena::priority::normal;
    }
    ...
    tbb::task_arena* decide_on_arguments(const FirstArenaArg& first_arg, const int reserved_for_masters,
    ...
    tbb::task_arena* allocate_and_construct_arena(int arena_max_concurrency,
                                                  tbb::task_arena::priority a_priority) {
    ...
    void submit_work(std::vector<arena_info>& arenas, unsigned repeats, utils::SpinBarrier& barrier) {
        for (auto& item : arenas) {
            ...
            arena.execute([repeats, &barrier, &tg, priority_value, concurrency]() {
                for (unsigned i = 0; i < repeats * concurrency; ++i) {
                    tg->run([&barrier, priority_value]() {
                        while (!g_work_submitted.load(std::memory_order_acquire)) utils::yield();
    ...
    void wait_work_completion(std::vector<arena_info>& arenas, std::size_t max_num_threads,
                              unsigned overall_tasks_num) {
        if (max_num_threads > 1)
            while (g_task_num < overall_tasks_num) utils::yield();
    ...
    void test() {
        ...
        for (const auto& progressing_arenas_num : progressing_arenas) {
            ...
            projected_concurrency
                    = std::max(min_arena_concurrency, projected_concurrency);// implementation detail
            ...
            for (unsigned arena_idx = 0; arena_idx < overall_arenas_num; ++arena_idx) {
                ...
                if (threads_left < actual_concurrency
                    || arena_idx
                               == adjusted_progressing_arenas
                                          - 1)// give all remaining threads to last progressing arena
                {
                ...
                actual_concurrency
                        = std::max(min_arena_concurrency, actual_concurrency);// implementation detail

                tbb::task_arena* arena = allocate_and_construct_arena(concurrency, a_priority);
                arenas.push_back(std::make_tuple(std::unique_ptr<tbb::task_arena>(arena), actual_concurrency,
                                                 a_priority,
            ...
            for (auto& item : arenas) overall_tasks_num += std::get<arena_concurrency>(item) * repeats;
...

            {
                ...
                unsigned last_task_idx
                        = std::min((repeats + 1) * unsigned(max_num_threads), overall_tasks_num);
                for (unsigned i = 0; i < last_task_idx; ++i) {
                    tbb::task_arena::priority p = g_task_info[i];
                    ++per_priority_tasks_num[p];

                    overall_progressing_arenas_tasks_num
                            += (int) (end_ptr != std::find(priorities, end_ptr, p));

                    if (i < max_num_threads || i >= repeats * max_num_threads) ++wasted_tasks[p];
                }
                ...
                for (unsigned i = 0; i < adjusted_progressing_arenas; ++i) {
                    ...
                    CHECK_MESSAGE(expected_tasks_num == per_priority_tasks_num[p],
                                  "Unexpected number of executed tasks in arena with index "
                                          << i << " and concurrency = " << concurrency);

                    expected_overall_progressing_arenas_tasks_num += expected_tasks_num;
                }
                CHECK_MESSAGE(expected_overall_progressing_arenas_tasks_num
                                      == overall_progressing_arenas_tasks_num,
            ...
            }
            {
                ...
                for (auto& e : per_priority_tasks_num) {
                    ...
                    auto it = std::find_if(arenas.begin(), arenas.end(), [priority](arena_info& info) {
                        return std::get<arena_priority>(info) == priority;
                    });
                    auto per_arena_tasks_num = repeats * std::get<arena_concurrency>(*it);
                    CHECK_MESSAGE(tasks_num == per_arena_tasks_num - wasted_tasks[priority],
                                  "Incorrect number of tasks from deferred (non-progressing) arenas were "
                                  "executed.");
                }
            }// Other epochs
...
TEST_CASE("Arena priorities") { HighPriorityArenasTakeExecutionPrecedence::test(); }
