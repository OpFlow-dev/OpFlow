...
struct Accumulator {
    ValueType operator()(const tbb::blocked_range<ValueType*>& r, ValueType value) const {
        for (ValueType* pv = r.begin(); pv != r.end(); ++pv) value += *pv;
        return value;
    }
};
...
public:
    ...
    ParallelSumTester() : m_range(nullptr, nullptr) {
        ...
        for (ValueType i = 0; i < count; ++i) m_array[i] = i + 1;
    ...
    void CheckParallelReduce() {
        ...
        ValueType result2 = reduce_invoker<ValueType>(
                m_range,
                [](const tbb::blocked_range<ValueType*>& r, ValueType value) -> ValueType {
                    for (const ValueType* pv = r.begin(); pv != r.end(); ++pv) value += *pv;
                    return value;
                },
                Sum(), partitioner);
...
namespace test_cancellation {
    ...
    struct ReduceFunctorToCancel {
        ...
        void operator()(const tbb::blocked_range<std::size_t>& br) { result = ReduceToCancel {}(br, result); }

        void join(ReduceFunctorToCancel& rhs) { result = JoinToCancel {}(result, rhs.result); }
    };// struct ReduceFunctorToCancel
    ...
    class ParallelReduceRunner {
        ...
        static_assert(Mode >= 0 && Mode <= maxParallelReduceRunnerMode,
                      "Incorrect mode for ParallelReduceTask");
    ...
    public:
        ParallelReduceRunner(tbb::task_group_context& ctx) : my_ctx(ctx) {}
    ...
    class ParallelDeterministicReduceRunner {
        ...
        static_assert(Mode >= 0 && Mode <= maxParallelDeterministicReduceRunnerMode,
                      "Incorrect Mode for deterministic_reduce task");
    ...
    public:
        ParallelDeterministicReduceRunner(tbb::task_group_context& ctx) : my_ctx(ctx) {}
    ...
    struct ParallelReduceTestRunner<maxParallelReduceRunnerMode> {
        static void run() { run_parallel_reduce_cancellation_test<maxParallelReduceRunnerMode>(); }
    };// struct ParallelReduceTestRunner<maxParallelReduceRunnerMode>
...
concept can_call_functional_preduce
        = can_call_preduce_with_partitioner<const Range&, const Value&, const RealBody&, const Reduction&>;

template <typename Range, typename Value, typename RealBody, typename Reduction>
concept can_call_functional_pdet_reduce
        = can_call_pdet_reduce_with_partitioner<const Range&, const Value&, const RealBody&,
                                                const Reduction&>;
...
void test_preduce_range_constraints() {
    ...
    static_assert(!can_call_functional_preduce<NonDestructible, int, CorrectFunc<NonDestructible>,
                                               CorrectReduction>);
    static_assert(
            !can_call_functional_preduce<NonSplittable, int, CorrectFunc<NonSplittable>, CorrectReduction>);
    static_assert(!can_call_functional_preduce<NoEmpty, int, CorrectFunc<NoEmpty>, CorrectReduction>);
    static_assert(
            !can_call_functional_preduce<EmptyNonConst, int, CorrectFunc<EmptyNonConst>, CorrectReduction>);
    static_assert(!can_call_functional_preduce<WrongReturnEmpty, int, CorrectFunc<WrongReturnEmpty>,
                                               CorrectReduction>);
    static_assert(
            !can_call_functional_preduce<NoIsDivisible, int, CorrectFunc<NoIsDivisible>, CorrectReduction>);
    static_assert(!can_call_functional_preduce<IsDivisibleNonConst, int, CorrectFunc<IsDivisibleNonConst>,
                                               CorrectReduction>);
    static_assert(!can_call_functional_preduce<WrongReturnIsDivisible, int,
                                               CorrectFunc<WrongReturnIsDivisible>, CorrectReduction>);
}
...
void test_preduce_func_constraints() {
    ...
    static_assert(!can_call_functional_preduce<CorrectRange, int, NoOperatorRoundBrackets<CorrectRange>,
                                               CorrectReduction>);
    static_assert(!can_call_functional_preduce<CorrectRange, int, OperatorRoundBracketsNonConst<CorrectRange>,
                                               CorrectReduction>);
    static_assert(!can_call_functional_preduce<
                  CorrectRange, int, WrongFirstInputOperatorRoundBrackets<CorrectRange>, CorrectReduction>);
    static_assert(!can_call_functional_preduce<
                  CorrectRange, int, WrongSecondInputOperatorRoundBrackets<CorrectRange>, CorrectReduction>);
    static_assert(
            !can_call_functional_preduce<CorrectRange, int, WrongReturnOperatorRoundBrackets<CorrectRange>,
                                         CorrectReduction>);
}
...
void test_preduce_combine_constraints() {
    ...
    static_assert(!can_call_functional_preduce<CorrectRange, int, CorrectFunc<CorrectRange>,
                                               NoOperatorRoundBrackets<int>>);
    static_assert(!can_call_functional_preduce<CorrectRange, int, CorrectFunc<CorrectRange>,
                                               OperatorRoundBracketsNonConst<int>>);
    static_assert(!can_call_functional_preduce<CorrectRange, int, CorrectFunc<CorrectRange>,
...
void test_pdet_reduce_range_constraints() {
    ...
    static_assert(
            !can_call_imperative_pdet_reduce<WrongReturnIsDivisible, CorrectBody<WrongReturnIsDivisible>>);

    static_assert(can_call_functional_pdet_reduce<Correct, int, CorrectFunc<Correct>, CorrectReduction>);
    static_assert(
            !can_call_functional_pdet_reduce<NonCopyable, int, CorrectFunc<NonCopyable>, CorrectReduction>);
    static_assert(!can_call_functional_pdet_reduce<NonDestructible, int, CorrectFunc<NonDestructible>,
                                                   CorrectReduction>);
    static_assert(!can_call_functional_pdet_reduce<NonSplittable, int, CorrectFunc<NonSplittable>,
                                                   CorrectReduction>);
    static_assert(!can_call_functional_pdet_reduce<NoEmpty, int, CorrectFunc<NoEmpty>, CorrectReduction>);
    static_assert(!can_call_functional_pdet_reduce<EmptyNonConst, int, CorrectFunc<EmptyNonConst>,
                                                   CorrectReduction>);
    static_assert(!can_call_functional_pdet_reduce<WrongReturnEmpty, int, CorrectFunc<WrongReturnEmpty>,
                                                   CorrectReduction>);
    static_assert(!can_call_functional_pdet_reduce<NoIsDivisible, int, CorrectFunc<NoIsDivisible>,
                                                   CorrectReduction>);
    static_assert(!can_call_functional_pdet_reduce<IsDivisibleNonConst, int, CorrectFunc<IsDivisibleNonConst>,
                                                   CorrectReduction>);
    static_assert(!can_call_functional_pdet_reduce<WrongReturnIsDivisible, int,
                                                   CorrectFunc<WrongReturnIsDivisible>, CorrectReduction>);
}
...
void test_pdet_reduce_body_constraints() {
    ...
    static_assert(
            !can_call_imperative_pdet_reduce<CorrectRange, WrongInputOperatorRoundBrackets<CorrectRange>>);
...
void test_pdet_reduce_func_constraints() {
    using namespace test_concepts::parallel_reduce_function;
    static_assert(
            can_call_functional_pdet_reduce<CorrectRange, int, Correct<CorrectRange>, CorrectReduction>);
    static_assert(!can_call_functional_pdet_reduce<CorrectRange, int, NoOperatorRoundBrackets<CorrectRange>,
                                                   CorrectReduction>);
    static_assert(
            !can_call_functional_pdet_reduce<CorrectRange, int, OperatorRoundBracketsNonConst<CorrectRange>,
                                             CorrectReduction>);
    static_assert(!can_call_functional_pdet_reduce<
                  CorrectRange, int, WrongFirstInputOperatorRoundBrackets<CorrectRange>, CorrectReduction>);
    static_assert(!can_call_functional_pdet_reduce<
                  CorrectRange, int, WrongSecondInputOperatorRoundBrackets<CorrectRange>, CorrectReduction>);
    static_assert(!can_call_functional_pdet_reduce<
                  CorrectRange, int, WrongReturnOperatorRoundBrackets<CorrectRange>, CorrectReduction>);
}

void test_pdet_reduce_combine_constraints() {
    using namespace test_concepts::parallel_reduce_combine;
    static_assert(
            can_call_functional_pdet_reduce<CorrectRange, int, CorrectFunc<CorrectRange>, Correct<int>>);
    static_assert(!can_call_functional_pdet_reduce<CorrectRange, int, CorrectFunc<CorrectRange>,
                                                   NoOperatorRoundBrackets<int>>);
    static_assert(!can_call_functional_pdet_reduce<CorrectRange, int, CorrectFunc<CorrectRange>,
                                                   OperatorRoundBracketsNonConst<int>>);
    static_assert(!can_call_functional_pdet_reduce<CorrectRange, int, CorrectFunc<CorrectRange>,
...
class MinimalRange {
    ...
    friend void TestSplitting(std::size_t nthread);

public:
    MinimalRange(MinimalRange& r, tbb::split) : end(r.end) { begin = r.end = (r.begin + r.end) / 2; }
...
public:
    ...
    void operator()(const MinimalRange& r) {
        for (size_t k = r.begin; k < r.end; ++k) ++sum;
        if (is_new) {
            is_new = false;
            begin = r.begin;
        } else
...
namespace unsupported {
    ...
    template <typename Range, typename Value, typename RealBody, typename Reduction>
    Value parallel_deterministic_reduce(const Range&, const Value& identity, const RealBody&,
                                        const Reduction&, const tbb::auto_partitioner&) {
        return identity;
    }
    template <typename Range, typename Value, typename RealBody, typename Reduction>
    Value parallel_deterministic_reduce(const Range&, const Value& identity, const RealBody&,
                                        const Reduction&, tbb::affinity_partitioner&) {
        return identity;
    }
}// namespace unsupported
...
TEST_CASE("Test Unsupported Partitioners") {
    ...
    parallel_deterministic_reduce(
            blocked_range<int>(0, 10), 0, [](const blocked_range<int>&, int init) -> int { return init; },
            [](int x, int y) -> int { return x + y; }, tbb::auto_partitioner());
    parallel_deterministic_reduce(
            blocked_range<int>(0, 10), 0, [](const blocked_range<int>&, int init) -> int { return init; },
            [](int x, int y) -> int { return x + y; }, ap);
}
