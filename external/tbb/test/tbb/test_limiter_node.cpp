...
// +-------------------------------------------------------------+
//
void test_multifunction_to_limiter(int _max, int _nparallel) {
    tbb::flow::graph g;
    emit_count = 0;
    emit_sum = 0;
    receive_count = 0;
...
void test_continue_msg_reception() {
    tbb::flow::graph g;
...
void test_decrementer() {
    ...
    int actual = -1;
    m = 0;
    while (queue.try_get(actual)) CHECK_MESSAGE(actual == expected[m++], "");
    ...
    actual = -1;
    m = 0;
    while (queue.try_get(actual)) CHECK_MESSAGE(actual == expected2[m++], "");
    ...
    actual = -1;
    m = 0;
    while (queue.try_get(actual)) { CHECK_MESSAGE(actual == m++, "Not all messages have been processed."); }
...
void test_try_put_without_successors() {
    ...
    int i = 1;
    for (; i <= try_put_num; i++) bn.try_put(i);

    std::atomic<int> counter {0};
    tbb::flow::function_node<int, int> fn(g, tbb::flow::unlimited, [&](int input) {
        counter += input;
        return int {};
    });
...
void test_follows_and_precedes_api() {
    ...
    follows_and_precedes_testing::test_follows<msg_t, tbb::flow::limiter_node<msg_t, msg_t>>(
            messages_for_follows, 1000);
    follows_and_precedes_testing::test_precedes<msg_t, tbb::flow::limiter_node<msg_t, msg_t>>(
            messages_for_precedes, 1000);
}
#endif
...
void test_decrement_while_try_put_task() {
    ...
    tbb::flow::multifunction_node<int, std::tuple<int>> processing {
            graph, tbb::flow::serial,
            [&](const int &value, typename decltype(processing)::output_ports_type &out) {
                if (value != threshold) std::get<0>(out).try_put(1);
...
void test_try_put_and_wait() {
    ...
    arena.execute([] {
        ...
        tbb::flow::function_node<int, tbb::flow::continue_msg> function(g, tbb::flow::serial, [&](int input) {
        ...
        for (auto item : start_work_items) { limiter.try_put(item); }
...
//! \brief \ref error_guessing
TEST_CASE("Test continue_msg reception") { test_continue_msg_reception(); }
...
TEST_CASE("Message is released if successor does not accept") { test_reserve_release_messages(); }

//! Test decrementer
//! \brief \ref requirement \ref error_guessing
TEST_CASE("Decrementer") { test_decrementer(); }

//! Test try_put() without successor
//! \brief \ref error_guessing
TEST_CASE("Test try_put() without successors") { test_try_put_without_successors(); }
...
#if __TBB_PREVIEW_FLOW_GRAPH_TRY_PUT_AND_WAIT
//! \brief \ref error_guessing
TEST_CASE("test limiter_node try_put_and_wait") { test_try_put_and_wait(); }
#endif
