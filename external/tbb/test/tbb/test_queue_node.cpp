...
struct touches {
    ...
    touches(int num_threads) : my_num_threads(num_threads) {
        ...
        for (int p = 0; p < my_num_threads; ++p) {
            ...
            for (int p2 = 0; p2 < my_num_threads; ++p2) my_last_touch[p][p2] = -1;

            my_touches[p] = new bool[N * my_num_threads];
            for (int n = 0; n < N * my_num_threads; ++n) my_touches[p][n] = false;
        }
    }
    ...
    bool validate_touches() {
        ...
        for (int n = 0; n < N * my_num_threads; ++n) all_touches[n] = false;
...
int test_parallel(int num_threads) {
    ...
    tbb::flow::queue_node<T> q3(g);
    {
        ...
        for (int tid = 0; tid < num_threads; ++tid) { CHECK_MESSAGE(next_value[tid] == T(N), ""); }
        delete[] next_value;
...
void test_queue_node_try_put_and_wait() {
    ...
    // test_buffer_push returns the index from which the items processed during wait_for_all() starts
    {
        ...
        std::size_t after_start = test_buffer_push<tbb::flow::queue_node<int>>(
...

    {
        ...
        std::size_t after_start = test_buffer_pull<tbb::flow::queue_node<int>>(
    ...
    // Test reserve
    {
        ...
        for (int threshold : thresholds) {
            ...
            std::size_t after_start = test_buffer_reserve<tbb::flow::queue_node<int>>(
...
#if __TBB_PREVIEW_FLOW_GRAPH_TRY_PUT_AND_WAIT
//! \brief \ref error_guessing
TEST_CASE("test queue_node try_put_and_wait") { test_queue_node_try_put_and_wait(); }
#endif
