...
void test_buffer_node_try_put_and_wait() {
    ...
    // test_buffer_push returns the index from which the items processed during wait_for_all() starts
    {
        ...
        std::size_t after_start = test_buffer_push<tbb::flow::buffer_node<int>>(
        ...
        CHECK_MESSAGE(processed_items[check_index++] == wait_message,
                      "try_put_and_wait should process only the wait_message");
...

    {
        ...
        std::size_t after_start = test_buffer_pull<tbb::flow::buffer_node<int>>(
        ...
        CHECK_MESSAGE(after_start == 2,
                      "Only wait_message and occupier should be processed by try_put_and_wait");
    ...
    // Test reserve
    {
        ...
        for (int threshold : thresholds) {
            ...
            std::size_t after_start = test_buffer_reserve<tbb::flow::buffer_node<int>>(
...
#if __TBB_PREVIEW_FLOW_GRAPH_TRY_PUT_AND_WAIT
//! \brief \ref error_guessing
TEST_CASE("test buffer_node try_put_and_wait") { test_buffer_node_try_put_and_wait(); }
#endif
