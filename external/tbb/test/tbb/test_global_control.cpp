...
struct task_scheduler_handle_guard {
    ...
    task_scheduler_handle_guard() { m_handle = tbb::task_scheduler_handle {tbb::attach {}}; }

    ~task_scheduler_handle_guard() { m_handle.release(); }

    tbb::task_scheduler_handle& get() { return m_handle; }
};

namespace TestBlockingTerminateNS {

    struct TestAutoInitBody {
        void operator()(int) const { tbb::parallel_for(0, 100, utils::DummyBody()); }
    };
    ...
    public:
        ...
        void operator()(int) const {
            ...
            if (!myAutoInit) { tbb::parallel_for(0, 10, utils::DummyBody()); }
            ...
            if (!myAutoInit && tbb::finalize(init.get(), std::nothrow)) ++gNumSuccesses;
        }
    };
        ...
        public:
            ...
            void operator()(int) const {
                ...
                REQUIRE_MESSAGE(false,
                                "Blocking terminate did not throw the exception inside the parallel region");
            }
        };
        void operator()() {
...
void TestTerminationAndAutoinit(bool autoinit) {
    ...
    if (autoinit) { tbb::parallel_for(0, 10, utils::DummyBody()); }
...
TEST_CASE("prolong lifetime advanced") {
    ...
    utils::NativeParallelFor(1, [&](int) { TestBlockingTerminateNS::TestExceptions(); });
}
#endif
...
//! \brief \ref error_guessing
TEST_CASE("prolong lifetime multiple wait") { TestBlockingTerminateNS::TestMultpleWait(); }
#endif
...
TEST_CASE("test concurrent task_scheduler_handle destruction") {
    ...
    for (int i = 0; i < 1000; ++i) {
        std::thread thr2([] { tbb::parallel_for(0, 1, [](int) {}); });
        thr2.join();
    }
    stop = true;
    thr1.join();
