...
struct task_scheduler_handle_guard {
    ...
    task_scheduler_handle_guard() { m_handle = tbb::task_scheduler_handle::get(); }

    ~task_scheduler_handle_guard() { tbb::task_scheduler_handle::release(m_handle); }
...
namespace TestBlockingTerminateNS {
    ...
    struct TestAutoInitBody {
        void operator()(int) const { tbb::parallel_for(0, 100, EmptyBody()); }
    };
    ...
    public:
        ...
        void operator()(int) const {
            ...
            if (!myAutoInit) { tbb::parallel_for(0, 10, EmptyBody()); }
...
void TestTerminationAndAutoinit(bool autoinit) {
    ...
    if (autoinit) { tbb::parallel_for(0, 10, TestBlockingTerminateNS::EmptyBody()); }
