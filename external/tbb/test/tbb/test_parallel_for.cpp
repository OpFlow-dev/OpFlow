...
struct SSE_Functor {
    ...
    void operator()(tbb::blocked_range<int>& r) const {
        for (int i = r.begin(); i != r.end(); ++i) Dst[i] = Src[i];
    }
};
...
struct TestSimplePartitionerStabilityFunctor {
    ...
    void operator()(tbb::blocked_range<size_t>& r) const { ranges.at(r.begin()) = 1; }
};
void TestSimplePartitionerStability() {
    ...
    for (std::size_t i = 0, grainsize = grainsizeStep; i < repeat_count; i++, grainsize += grainsizeStep) {
        ...
        tbb::parallel_for(tbb::blocked_range<size_t>(0, rangeToSplitSize, grainsize),
                          FunctorType(firstSeries), tbb::simple_partitioner());
        tbb::parallel_for(tbb::blocked_range<size_t>(0, rangeToSplitSize, grainsize),
                          FunctorType(secondSeries), tbb::simple_partitioner());

        CHECK_MESSAGE(firstSeries == secondSeries,
...
namespace test_cancellation {
    ...
    struct FunctorToCancel {
        ...
        void operator()(std::size_t) const {
            ++g_CurExecuted;
            if (need_to_wait) { need_to_wait = Cancellator::WaitUntilReady(); }
        }
    ...
    public:
        ParallelForRunner(tbb::task_group_context& ctx) : my_ctx(ctx) {}
    ...
    struct ParallelForTestRunner<maxParallelForRunnerMode> {
        static void run() { run_parallel_for_cancellation_test<maxParallelForRunnerMode>(); }
    };// struct ParallelForTestRunner<maxParallelForRunnerMode>
...
//! \brief \ref error_guessing
TEST_CASE("Simple partitioner stability") { TestSimplePartitionerStability(); }

//! Testing various range implementations
//! \brief \ref requirement
TEST_CASE("Various range implementations") { various_range_implementations::test(); }
