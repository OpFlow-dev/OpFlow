...
struct TaskInfo {
    ...
    TaskInfo(int priority, int task_index) : my_priority(priority), my_task_index(task_index) {}
    int my_priority;
    int my_task_index;
};
...
namespace PriorityNodesTakePrecedence {
    ...
    void body_func(int priority, utils::SpinBarrier& my_barrier) {
        while (!g_work_submitted.load(std::memory_order_acquire)) tbb::detail::d0::yield();
        int current_task_index = g_task_num++;
        if (priority != no_priority)
            g_task_info[g_priority_task_index++] = TaskInfo(priority, current_task_index);
        const bool all_threads_will_come = unsigned(current_task_index)
                                           < node_num - (node_num % tbb::this_task_arena::max_concurrency());
        if (all_threads_will_come) my_barrier.wait();
    }
    ...
    template <typename T>
    struct Body {
        Body(int priority, utils::SpinBarrier& barrier) : my_priority(priority), my_barrier(barrier) {}
    ...
    void test_node(NodeTypeCreator node_creator) {
        ...
        for (size_t repeat = 0; repeat < repeats; ++repeat) {
            ...
            g_task_info.clear();
            g_task_info.resize(priority_nodes_num);
            ...
            CHECK_MESSAGE((g_priority_task_index == g_task_info.size()),
                          "Incorrect number of tasks with priority.");
            CHECK_MESSAGE((priority_nodes_num == g_task_info.size()),
                          "Incorrect number of tasks with priority executed.");
    ...
    void call_within_arena(tbb::task_arena& arena) {
        arena.execute([&]() { test_node<NodeType>(node_creator_t<NodeType, NodeBody>()); });
    }
...
namespace ThreadsEagerReaction {
    ...
    typedef multifunction_node<data_type, std::tuple<data_type, data_type>> decider_node_type;
    ...
    struct CpuWorkBody {
        ...
        data_type operator()(const data_type& input) {
            tbb::parallel_for(0, my_tasks_count, ParallelForBody(my_barrier, input),
    ...
    struct DeciderBody {
        ...
        void operator()(data_type input, decider_node_type::output_ports_type& ports) {
            if (input < my_limit) std::get<0>(ports).try_put(input + 1);
        }
    };
    ...
    void test(unsigned num_threads) {
        ...
        arena.execute([&]() {
            ...
            function_node<data_type, data_type> cpu_work_node(g, unlimited,
                                                              CpuWorkBody(barrier, nested_cpu_tasks));
            ...
            decider_node_type async_restarter_node(g, unlimited, DeciderBody(async_subgraph_reruns),
                                                   node_priority_t(1));
...
namespace LimitingExecutionToPriorityTask {
    ...
    void do_nested_work(const std::thread::id& tid, const tbb::blocked_range<int>& /*subrange*/) {
        ...
        if (!exec_tracker.prioritized_work_started || exec_tracker.prioritized_work_submitter != tid) return;
        ...
        if (exec_tracker.prioritized_work_finished) return;
    ...
    void create_nodes(nodes_container_t& nodes, graph& g, int num, int body_size) {
        for (int i = 0; i < num; ++i)
            nodes.push_back(std::unique_ptr<continue_node<continue_msg>>(
    ...
    void test(int num_threads) {
        ...
        arena.execute([&]() {
            ...
            nodes.push_back(std::unique_ptr<continue_node<continue_msg>>(new continue_node<continue_msg>(
            ...
            for (int i = 0; i < nodes_num; ++i)
                if (i % nodes_in_lane == 0) make_edge(bn, *nodes[i]);
                else
...
namespace NestedCase {
    ...
    struct InnerBody {
        continue_msg operator()(const continue_msg&) const { return continue_msg(); }
    };
    ...
    void execute_outer_graph(bool same_arena, task_arena& inner_arena, int max_threads, graph& outer_graph,
                             function_node<int, int>& start_node) {
        if (same_arena) {
    ...
    void test_in_arena(int max_threads, task_arena& outer_arena, task_arena& inner_arena, graph& outer_graph,
                       function_node<int, int>& start_node) {
        bool same_arena = &outer_arena == &inner_arena;
        auto threads_range = utils::concurrency_range(max_threads);
        for (auto num_threads : threads_range) {
            INFO("Testing nested nodes with specified priority in "
                 << (same_arena ? "same" : "different") << " arenas, num_threads=" << num_threads << ") - ");
    ...
    void test(int max_threads) {
        task_arena outer_arena;
        task_arena inner_arena;
        ...
        for (unsigned node_index = 0; node_index < num_outer_nodes; ++node_index) {
            ...
            if (node_index == num_outer_nodes / 2) priority = 10;

            outer_nodes.push_back(std::unique_ptr<function_node<int, int>>(new function_node<int, int>(
...
namespace BypassPrioritizedTask {
    ...
    void test_use_case() {
        ...
        CHECK_MESSAGE(g_task_info[1].my_priority == 2,
                      "Bypassed task with higher priority executed in wrong order.");
...
namespace ManySuccessors {
    ...
    struct no_priority_node_body {
        void operator()(continue_msg) {
            CHECK_MESSAGE(barrier == 0,
                          "Non-priority successor has to be executed after all priority successors");
        }
    ...
    struct priority_node_body {
        void operator()(continue_msg) {
            --barrier;
            while (barrier) tbb::detail::d0::yield();
        }
        std::atomic<int>& barrier;
    };
    ...
    void test(int num_threads) {
        ...
        arena.execute([&]() {
            ...
            for (int trial = 0; trial < 10; ++trial) {
                ...
                for (auto& n : nodes) make_edge(bn, *n);
                bn.try_put(continue_msg());
                g.wait_for_all();
                for (auto& n : nodes) remove_edge(bn, *n);
            }
        });
    }
...
namespace Exceptions {
    void test() {
        ...
        continue_node<unsigned> c(
                g, [&msg_count](continue_msg) { return ++msg_count; }, 2);
        function_node<unsigned> f(
                g, unlimited,
                [&throwing_msgs](unsigned v) {
                    for (auto i : throwing_msgs) {
                        if (i == v) throw std::runtime_error("Exception::test");
                    }
                },
                1);
        make_edge(c, f);
        for (int i = 0; i < 10; ++i) {
            ...
            try {
                for (unsigned j = 0; j < num_messages; ++j) { c.try_put(continue_msg()); }
                g.wait_for_all();
...
TEST_CASE("Priority nodes take precedence") {
    for (auto p : utils::concurrency_range()) { PriorityNodesTakePrecedence::test(static_cast<int>(p)); }
}

//! Test thread eager reaction
//! \brief \ref error_guessing
TEST_CASE("Thread eager reaction") {
    for (auto p : utils::concurrency_range()) { ThreadsEagerReaction::test(static_cast<int>(p)); }
}

//! Test prioritization under concurrency limits
//! \brief \ref error_guessing
TEST_CASE("Limiting execution to prioritized work") {
    for (auto p : utils::concurrency_range()) { LimitingExecutionToPriorityTask::test(static_cast<int>(p)); }
}
...
TEST_CASE("Many successors") {
    for (auto p : utils::concurrency_range()) { ManySuccessors::test(static_cast<int>(p)); }
}

#if TBB_USE_EXCEPTIONS
//! Test for exceptions
//! \brief \ref error_guessing
TEST_CASE("Exceptions") { Exceptions::test(); }
#endif
