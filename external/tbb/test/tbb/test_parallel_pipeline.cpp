...
void* fetchNextBuffer() {
    for (size_t icnt = 0; icnt < n_buffers; ++icnt) {
        if (!buf_in_use[icnt].test_and_set()) { return buffers[icnt]; }
    }
...
template <typename T>
bool middle_is_ready(T& /*p*/) {
    return false;
}

template <typename U>
bool middle_is_ready(CheckType<U>& p) {
    return p.is_ready();
}

template <typename T>
bool output_is_ready(T& /*p*/) {
    return true;
}

template <typename U>
bool output_is_ready(CheckType<U>& p) {
    return p.is_ready();
}

template <typename T>
int middle_my_id(T& /*p*/) {
    return 0;
}

template <typename U>
int middle_my_id(CheckType<U>& p) {
    return p.id();
}

template <typename T>
int output_my_id(T& /*p*/) {
    return 1;
}

template <typename U>
int output_my_id(CheckType<U>& p) {
    return p.id();
}

template <typename T>
void my_function(T& p) {
    p = 0;
}

template <typename U>
void my_function(CheckType<U>& p) {
    p.get_ready();
}
...
public:
    int operator()(tbb::flow_control& control) const {
        CHECK(is_alive());
...
void resetCounters() {
    ...
    for (size_t i = 0; i < n_buffers; ++i) buf_in_use[i].clear();
}
...
void checkCounters(final_assert_type my_t) {
    ...
    switch (my_t) {
        case assert_nonpointer:
            CHECK_MESSAGE(pointer_specialized_calls + first_pointer_specialized_calls
                                          + second_pointer_specialized_calls
                                  == 0,
                          "non-pointer filters specialized to pointer, p:" << concurrency);
            CHECK_MESSAGE(non_pointer_specialized_calls == 3 * max_counter,
                          "bad count for non-pointer filters, p:" << concurrency);
            if (check_intbuffer) {
        ...
        case assert_secondpointer: {
            bool check = pointer_specialized_calls == max_counter && first_pointer_specialized_calls == 0
                         && non_pointer_specialized_calls == max_counter &&// input filter
                         second_pointer_specialized_calls == max_counter;
            CHECK_MESSAGE(check, "incorrect specialization for firstpointer, p:" << concurrency);
        } break;
        case assert_allpointer:
            CHECK_MESSAGE(non_pointer_specialized_calls + first_pointer_specialized_calls
                                          + second_pointer_specialized_calls
                                  == 0,
                          "pointer filters specialized to non-pointer, p:" << concurrency);
            CHECK_MESSAGE(pointer_specialized_calls == 3 * max_counter,
                          "bad count for pointer filters, p:" << concurrency);
            break;
...
static const tbb::filter_mode filter_table[] = {tbb::filter_mode::parallel, tbb::filter_mode::serial_in_order,
...
void run_function_spec(Context&... context) {
    ...
    for (unsigned i = 0; i < number_of_filter_types; i++) {
        ...
        tbb::parallel_pipeline(n_tokens,
                               tbb::filter<void, void>(filter_table[i],
                                                       [&counter](tbb::flow_control& control) {
                                                           if (counter-- == 0) control.stop();
                                                       }),
                               context...);
    }
...
void run_filter_set(input_filter<t1>& i_filter, middle_filter<t1, t2>& m_filter, output_filter<t2>& o_filter,
                    mode_array* filter_type, final_assert_type my_t, Context&... context) {
    ...
    resetCounters();
    tbb::parallel_pipeline(n_tokens, tbb::filter<void, t1>(filter_type[0], i_filter),
                           tbb::filter<t1, t2>(filter_type[1], m_filter),
                           tbb::filter<t2, void>(filter_type[2], o_filter), context...);
    checkCounters(my_t);

    // Construct filters, make a copy, destroy the original filters, and run with the copy
    int cnt = filter_node_count;
    {
        tbb::filter<void, void>* p123
                = new tbb::filter<void, void>(tbb::filter<void, t1>(filter_type[0], i_filter)
                                              & tbb::filter<t1, t2>(filter_type[1], m_filter)
...
void run_lambdas_test(mode_array* filter_type, Context&... context) {
    ...
    tbb::parallel_pipeline(
            n_tokens,
            tbb::make_filter<void, t1>(filter_type[0],
                                       [&counter](tbb::flow_control& control) -> t1 {
                                           if (--counter < 0) control.stop();
                                           return t1();
                                       }),
            tbb::make_filter<t1, t2>(filter_type[1], [](t1 /*my_storage*/) -> t2 { return t2(); }),
            tbb::make_filter<t2, void>(filter_type[2], [](t2) -> void { output_counter++; }), context...);
    checkCounters(no_pointer_counts);// don't have to worry about specializations
    counter = max_counter;
    // pointer filters
    resetCounters();
    tbb::parallel_pipeline(n_tokens,
                           tbb::filter<void, t1*>(filter_type[0],
                                                  [&counter](tbb::flow_control& control) -> t1* {
                                                      if (--counter < 0) {
                                                          control.stop();
                                                          return nullptr;
                           ...
                           tbb::filter<t1*, t2*>(filter_type[1],
                                                 [](t1* my_storage) -> t2* {
                                                     my_storage->~t1();
                                                     return new (my_storage) t2();
                                                 }),
                           tbb::filter<t2*, void>(filter_type[2],
                                                  [](t2* my_storage) -> void {
                                                      my_storage->~t2();
    ...
    tbb::parallel_pipeline(
            ...
            tbb::make_filter(filter_type[0],
                    ...
                    & tbb::make_filter(filter_type[1],
                                       [](t1* my_storage) -> t2 {
                                           my_storage->~t1();
                                           freeBuffer(my_storage);
                                           return t2();
                                       }),
            tbb::make_filter(filter_type[2], [](t2 /*my_storage*/) -> void { output_counter++; }),
            context...);
    checkCounters(no_pointer_counts);
    ...
    tbb::parallel_pipeline(
            ...
            tbb::filter<t1, t2*>(filter_type[1],
                                 [](t1 /*my_storage*/) -> t2* { return new (fetchNextBuffer()) t2(); })
                    & tbb::make_filter<t2*, void>(filter_type[2],
                                                  [](t2* my_storage) -> void {
                                                      my_storage->~t2();
                                                      freeBuffer(my_storage);
                                                      output_counter++;
...
void run_function(const char* l1, const char* l2) {
    ...
    for (unsigned i = 0; i < number_of_filters; ++i) limit *= number_of_filter_types;
    ...
    for (unsigned i = 0; i < (unsigned) n_buffers; ++i) { free(buffers[i]); }
}
...
#define RUN_TYPED_TEST_CASE(type1, type2)                                                                    \
    TEST_CASE("Pipeline testing with " #type1 " and " #type2) {                                              \
