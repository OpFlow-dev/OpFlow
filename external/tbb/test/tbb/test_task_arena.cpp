...
class ArenaObserver : public tbb::task_scheduler_observer {
    ...
    void on_scheduler_entry(bool is_worker) override {
        ...
        if (is_worker) { CHECK(current_index >= myNumReservedSlots); }
...
public:
    ArenaObserver(tbb::task_arena& a, int maxConcurrency, int numReservedSlots, int id)
        : tbb::task_scheduler_observer(a), myId(id), myMaxConcurrency(maxConcurrency),
...
void TestConcurrentArenasFunc(int idx) {
    ...
    struct LocalObserver : public tbb::task_scheduler_observer {
        ...
        LocalObserver(tbb::task_arena& a) : tbb::task_scheduler_observer(a) { observe(true); }
        ~LocalObserver() { observe(false); }
    };
    ...
    a3.execute([] { utils::doDummyWork(100); });
...
public:
    ...
    void operator()(int) const { my_a.execute(AsynchronousWork(my_b, /*blocking=*/false)); }
};
...
public:
    MultipleMastersPart3(tbb::task_arena& a, utils::SpinBarrier& b) : my_a(a), my_b(b) {}
    void operator()(int) const {
        wait_context wait(0);
...
void TestMultipleMasters(int p) {
    {
        ...
        utils::SpinBarrier barrier1(p),
                barrier2(2 * p + 1);// each of p threads will submit two tasks signaling the barrier
    ...
    }
    {
        ...
        a.enqueue(AsynchronousWork(barrier,
                                   /*blocking=*/true));// occupy the worker, a regression test for bug 1981
    ...
    }
    {
        ...
        utils::SpinBarrier barrier(p
                                   + 1);// for external threads to avoid endless waiting at least in some runs
...
struct TestArenaEntryBody : FPModeContext {
    ...
    TestArenaEntryBody(std::atomic<int>& s, int idx, int i)// init thread-specific instance
        : FPModeContext(idx + i), my_stage(s), is_caught(false)
    ...
    void operator()() {// inside task_arena::execute()
        ...
        my_id << (stage < 3 ? (tbb::this_task_arena::current_thread_index()
                                       ? "delegated_to_worker"
                                       : stage < 2 ? "direct" : "delegated_to_master")
                            : stage == 3 ? "nested_same_ctx" : "nested_alien_ctx");
        AssertFPMode(arenaFPMode);
        if (is_expected) { TBB_TEST_THROW(std::logic_error(my_id.str())); }
...
public:
    ForEachArenaEntryBody(tbb::task_arena& a, std::atomic<int>& c) : my_a(a), my_stage(c), my_idx(0) {}
    ...
    void operator()(int i) const { test_arena_entry(i); }
...
private:
    void test_arena_entry(int i) const {
        ...
        } catch (std::logic_error& e) { scoped_functor.on_exception(e.what()); } catch (...) {
            CHECK_MESSAGE(false, "Unexpected exception type");
        }
#else
        my_a.execute(scoped_functor);
#endif
...
public:
    TestArenaConcurrencyBody(tbb::task_arena& a, int max_concurrency, int reserved_slots,
                             utils::SpinBarrier* b = nullptr, utils::SpinBarrier* wb = nullptr)
        : my_a(a), my_max_concurrency(max_concurrency), my_reserved_slots(reserved_slots), my_barrier(b),
          my_worker_barrier(wb) {}
    ...
    void operator()() const {
        ...
        if (my_worker_barrier) {
            if (local_id.local() == 1) {
                ...
                CHECK_MESSAGE(idx < my_reserved_slots,
                              "External threads are supposed to use only reserved slots in this test");
            } else {
                // Worker thread
    ...
    for (; reserved <= p; reserved += step) {
        ...
        if (p - reserved < tbb::this_task_arena::max_concurrency()) {
            ...
            for (int i = reserved; i < p; ++i)// requests p-reserved worker threads
                a.enqueue(test);
            if (reserved == 1) test(0);// calls execute()
            else
...
struct TestMandatoryConcurrencyObserver : public tbb::task_scheduler_observer {
    ...
    ~TestMandatoryConcurrencyObserver() { observe(false); }
    void on_scheduler_exit(bool worker) override {
        if (worker) { m_barrier.wait(); }
    }
};
...
void TestMandatoryConcurrency() {
    ...
    a.execute([&a] {
        ...
        for (int j = 0; j < 5; ++j) {
            ...
            do { exit_barrier.wait(); } while (num_tasks < n_threads * 5);
        }
    });
}
...
struct TaskArenaValidator {
    ...
    TaskArenaValidator(const tbb::task_arena& other)
        : my_slot_at_construction(tbb::this_task_arena::current_thread_index()), my_arena(other) {}
...
void ValidateAttachedArena(tbb::task_arena& arena, bool expect_activated, int expect_concurrency,
                           int expect_masters) {
...
namespace TestIsolatedExecuteNS {
    ...
    public:
        ParForBody(bool outer_isolation, tbb::enumerable_thread_specific<int>& ets,
                   std::atomic<bool>& is_stolen)
            : myOuterIsolation(outer_isolation), myEts(ets), myIsStolen(is_stolen) {}
        void operator()(int) const {
            int& e = myEts.local();
            if (e++ > 0) myIsStolen = true;
            if (myOuterIsolation) NestedParFor<NestedPartitioner>()();
            else
    ...
    public:
        OuterParFor(bool outer_isolation, std::atomic<bool>& is_stolen)
            : myOuterIsolation(outer_isolation), myIsStolen(is_stolen) {}
        void operator()() const {
    ...
    void TwoLoopsTest(bool outer_isolation) {
        ...
        if (outer_isolation) {
            ...
            REQUIRE_MESSAGE(!is_stolen,
                            "isolate() on nested levels should prevent stealing from outer levels");
        }
    }
    ...
    class HeavyMixTestBody : utils::NoAssign {
        ...
        static void RunTwoBodies(utils::FastRandom<>& rnd, const Body& body, Partitioner& p,
                                 tbb::task_group_context* ctx = nullptr) {
            if (rnd.get() % 2) {
                if (ctx) tbb::parallel_for(0, 2, body, p, *ctx);
                else
        ...
        void RunNextLevel(utils::FastRandom<>& rnd, int& isolated_level) const {
            ...
            switch (rnd.get() % 2) {
                case 0: {
                    // No features
                    tbb::task_group_context ctx;
                    RunTwoBodies(rnd, HeavyMixTestBody(myRandom, myIsolatedLevel, myNestedLevel + 1), p,
                                 &ctx);
                    break;
                }
                case 1: {
    ...
    public:
        HeavyMixTestBody(tbb::enumerable_thread_specific<utils::FastRandom<>>& random,
            ...
            : myRandom(random), myIsolatedLevel(isolated_level), myNestedLevel(nested_level) {}
        void operator()() const {
            int& isolated_level = myIsolatedLevel.local();
            CHECK_FAST_MESSAGE(myNestedLevel > isolated_level,
                               "The outer-level task should not be stolen on isolated level");
            if (myNestedLevel == 20) return;
        ...
        void operator()(int) const { this->operator()(); }
    };
    ...
    void HeavyMixTest() {
        std::size_t num_threads
                = tbb::this_task_arena::max_concurrency() < 3 ? 3 : tbb::this_task_arena::max_concurrency();
    ...
    public:
        ...
        TestEnqueueTask(tbb::enumerable_thread_specific<bool>& exe, std::atomic<int>& c, wait_context& w,
                        tbb::task_arena& a)
    ...
    public:
        ...
        TestEnqueueIsolateBody(tbb::enumerable_thread_specific<bool>& exe, std::atomic<int>& c,
                               tbb::task_arena& a)
    ...
    void TestEnqueue() {
        ...
        if (!executed.local()) { REPORT("Warning: No one enqueued task has executed by the main thread.\n"); }
...
public:
    TestMultipleWaitsArenaWait(int idx, int bunch_size, int num_tasks, std::vector<wait_context*>& waiters,
                               std::atomic<int>& processed, tbb::task_group_context& tgc)
        : my_idx(idx), my_bunch_size(bunch_size), my_num_tasks(num_tasks), my_waiters(waiters),
          my_processed(processed), my_context(tgc) {}
    void operator()() const {
        ++my_processed;
        // Wait for all tasks
        if (my_idx < my_num_tasks) { tbb::detail::d1::wait(*my_waiters[my_idx], my_context); }
        // Signal waiting tasks
        if (my_idx >= my_bunch_size) { my_waiters[my_idx - my_bunch_size]->release(); }
    }
...
public:
    TestMultipleWaitsThreadBody(int bunch_size, int num_tasks, tbb::task_arena& a,
                                std::vector<wait_context*>& waiters, std::atomic<int>& processed,
                                tbb::task_group_context& tgc)
        : my_bunch_size(bunch_size), my_num_tasks(num_tasks), my_arena(a), my_waiters(waiters),
          my_processed(processed), my_context(tgc) {}
    void operator()(int idx) const {
        my_arena.execute(TestMultipleWaitsArenaWait(idx, my_bunch_size, my_num_tasks, my_waiters,
                                                    my_processed, my_context));
        --my_processed;
    }
    ...
    for (int repeats = 0; repeats < 10; ++repeats) {
        ...
        for (int bunch = 0; bunch < num_bunches - 1; ++bunch) {
            ...
            for (int i = 0; i < bunch_size; ++i) {
                waiters[idx]->reserve();
                std::thread(TestMultipleWaitsThreadBody(bunch_size, num_tasks, a, waiters, processed, tgc),
                            idx++)
                        .detach();
            }
        }
        ...
        for (int i = 0; i < bunch_size; ++i)
            std::thread(TestMultipleWaitsThreadBody(bunch_size, num_tasks, a, waiters, processed, tgc), idx++)
                    .detach();
...
void TestMoveSemantics() { TestMoveSemanticsNS::TestFunctors(); }
...
namespace TestReturnValueNS {
    ...
    public:
        ...
        void check() {
            ...
            for (int i = 0; i < SIZE; ++i) REQUIRE(data[i] == i);
    ...
    template <typename R>
    struct Functor {
        R operator()() const { return function<R>(); }
    };
...
void TestReturnValue() { TestReturnValueNS::Test(); }
...
struct MyObserver : public tbb::task_scheduler_observer {
    ...
    MyObserver(tbb::task_arena& a, tbb::enumerable_thread_specific<tbb::task_arena*>& tls,
               std::atomic<int>& failure_counter, std::atomic<int>& counter, utils::SpinBarrier& barrier)
        : tbb::task_scheduler_observer(a), my_tls(tls), my_arena(a), my_failure_counter(failure_counter),
          my_counter(counter), m_barrier(barrier) {
        observe(true);
    }
    ~MyObserver() { observe(false); }
    void on_scheduler_entry(bool worker) override {
        if (worker) {
            ++my_counter;
            tbb::task_arena*& cur_arena = my_tls.local();
            if (cur_arena != nullptr && cur_arena != &my_arena) { ++my_failure_counter; }
            cur_arena = &my_arena;
            m_barrier.wait();
        }
    }
...
void TestArenaWorkersMigrationWithNumThreads(int n_threads = 0) {
    if (n_threads == 0) { n_threads = tbb::this_task_arena::max_concurrency(); }
    ...
    for (int i = 0; i < n_arenas; ++i) { delete observer[i]; }
...
void TestArenaWorkersMigration() {
    ...
    if (tbb::this_task_arena::max_concurrency() != 4) { TestArenaWorkersMigrationWithNumThreads(); }
}
    ...
    utils::NativeParallelFor(1, [threads](int idx) {
        ...
        for (auto blocked : {false, true}) {
            for (int trail = 0; trail < (blocked ? 10 : 10000); ++trail) {
                tbb::parallel_for(
                        0, threads,
                        [threads, blocked, &barrier](int) {
                            CHECK_FAST_MESSAGE(threads == tbb::this_task_arena::max_concurrency(),
                                               "concurrency level is not equal specified threadnum");
                            CHECK_FAST_MESSAGE(tbb::this_task_arena::current_thread_index()
                                                       < tbb::this_task_arena::max_concurrency(),
                                               "amount of created threads is more than specified by default");
                            local_id.local() = 1;
                            if (blocked) {
                ...
                REQUIRE_MESSAGE(local_id.size() <= size_t(threads),
                                "amount of created threads is not equal to default num");
                if (blocked) {
                    REQUIRE_MESSAGE(local_id.size() == size_t(threads),
                                    "amount of created threads is not equal to default num");
                }
        ...
        for (std::size_t i = 0; i < 1000; ++i) {
            try {
                test_arena.execute([] {
                    ...
                    if (suppress_unreachable_code_warning) { throw - 1; }
                });
                FAIL("An exception should have thrown.");
            } catch (int) { ++canceled_task; } catch (...) {
...
public:
    simple_observer(tbb::task_arena& a, int maxConcurrency, int numReservedSlots)
        : tbb::task_scheduler_observer(a), my_idx(idx_counter++), myMaxConcurrency(maxConcurrency),
          myNumReservedSlots(numReservedSlots) {
        observe(true);
    }

    ~simple_observer() { observe(false); }
...
struct arena_handler {
    enum arena_status { alive, deleting, deleted };
...
void StressTestMixFunctionality() {
            ...
            switch (get_random_operation()) {
                ...
                case delete_arena: {
                    ...
                    for (; curr_arena != arenas_pool.end(); ++curr_arena) {
                        ...
                        if (curr_arena->status.compare_exchange_strong(curr_status,
                                                                       arena_handler::deleting)) {
                            break;
                        }
                    }
    ...
    for (std::size_t i = 0; i < thread_number - 1; ++i) { thread_pool.emplace_back(thread_func); }
...
struct enqueue_test_helper {
    enqueue_test_helper(tbb::task_arena& arena, tbb::enumerable_thread_specific<bool>& ets,
                        std::atomic<std::size_t>& task_counter)
        : my_arena(arena), my_ets(ets), my_task_counter(task_counter) {}

    enqueue_test_helper(const enqueue_test_helper& ef)
        : my_arena(ef.my_arena), my_ets(ef.my_ets), my_task_counter(ef.my_task_counter) {}

    void operator()() const {
        CHECK(my_ets.local());
        if (my_task_counter++ < 100000)
            my_arena.enqueue(enqueue_test_helper(my_arena, my_ets, my_task_counter));
        utils::yield();
    }
...
//! \brief \ref requirement
TEST_CASE("Test for concurrent functionality") { TestConcurrentFunctionality(); }

#if !EMSCRIPTEN
//! For emscripten, FPU control state has not been set correctly
//! Test for arena entry consistency
//! \brief \ref requirement \ref error_guessing
TEST_CASE("Test for task arena entry consistency") { TestArenaEntryConsistency(); }
#endif

//! Test for task arena attach functionality
//! \brief \ref requirement \ref interface
TEST_CASE("Test for the attach functionality") { TestAttach(4); }

//! Test for constant functor requirements
//! \brief \ref requirement \ref interface
TEST_CASE("Test for constant functor requirement") { TestConstantFunctorRequirement(); }

//! Test for move semantics support
//! \brief \ref requirement \ref interface
TEST_CASE("Move semantics support") { TestMoveSemantics(); }

//! Test for different return value types
//! \brief \ref requirement \ref interface
TEST_CASE("Return value test") { TestReturnValue(); }

//! Test for delegated task spawn in case of unsuccessful slot attach
//! \brief \ref error_guessing
TEST_CASE("Delegated spawn wait") { TestDelegatedSpawnWait(); }
...
TEST_CASE("Isolated execute") {
    ...
    if (tbb::this_task_arena::max_concurrency() > 2) { TestIsolatedExecute(); }
}
#endif

//! Test for TBB Workers creation limits
//! \brief \ref requirement
TEST_CASE("Default workers limit") { TestDefaultWorkersLimit(); }

//! Test for workers migration between arenas
//! \brief \ref error_guessing \ref stress
TEST_CASE("Arena workers migration") { TestArenaWorkersMigration(); }

#if !EMSCRIPTEN
//! For emscripten, FPU control state has not been set correctly
//! Test for multiple waits, threads should not block each other
//! \brief \ref requirement
TEST_CASE("Multiple waits") { TestMultipleWaits(); }
#endif

//! Test for small stack size settings and arena initialization
//! \brief \ref error_guessing
TEST_CASE("Small stack size") { TestSmallStackSize(); }

#if TBB_USE_EXCEPTIONS
//! \brief \ref requirement \ref stress
TEST_CASE("Test for exceptions during execute.") { ExceptionInExecute(); }
...
TEST_CASE("Exception thrown during tbb::task_arena::execute call") {
    ...
    REQUIRE_THROWS_AS([&] { arena.execute([] { return throwing_obj {}; }); }(), std::exception);
}
#endif// TBB_USE_EXCEPTIONS

//! \brief \ref stress
TEST_CASE("Stress test with mixing functionality") { StressTestMixFunctionality(); }
...
TEST_CASE("Workers oversubscription") {
    ...
    arena.execute([&] {
        tbb::parallel_for(std::size_t(0), num_threads * 2, [&](const std::size_t&) {
            ets.local() = true;
            barrier.wait();
        });
    });
...
TEST_CASE("Empty task_handle cannot be scheduled"
          * doctest::
                  should_fail()//Test needs to revised as implementation uses assertions instead of exceptions
          * doctest::skip()    //skip the test for now, to not pollute the test log
) {
    tbb::task_arena ta;

    CHECK_THROWS_WITH_AS(ta.enqueue(tbb::task_handle {}), "Attempt to schedule empty task_handle",
                         std::runtime_error);
    CHECK_THROWS_WITH_AS(tbb::this_task_arena::enqueue(tbb::task_handle {}),
                         "Attempt to schedule empty task_handle", std::runtime_error);
}
#endif
...
TEST_CASE("is_inside_task in task_group") {
    ...
    tbb::task_group tg;
    tg.run_and_wait([&] { CHECK(true == tbb::is_inside_task()); });
}
...
TEST_CASE("worker threads occupy slots in correct range") {
    ...
    for (auto& arena : arenas) { arena.initialize(1, 0); }
