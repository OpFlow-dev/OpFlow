...
public:
    ...
    ~ArenaObserver() { CHECK_MESSAGE(!old_id.local(), "inconsistent observer state"); }
};
...
public:
    TestArenaConcurrencyBody(tbb::task_arena& a, int max_concurrency, int reserved_slots,
                             utils::SpinBarrier* b = NULL, utils::SpinBarrier* wb = NULL)
    ...
    void TwoLoopsTest(bool outer_isolation) {
        ...
        if (outer_isolation) {
            ...
            REQUIRE_MESSAGE(!is_stolen,
                            "isolate() on nested levels should prevent stealing from outer leves");
        }
    }
...
struct MyObserver : public tbb::task_scheduler_observer {
    ...
    void on_scheduler_entry(bool worker) override {
        if (worker) {
            ...
            if (cur_arena != 0 && cur_arena != &my_arena) { ++my_failure_counter; }
            cur_arena = &my_arena;
            m_barrier.wait();
        }
    }
