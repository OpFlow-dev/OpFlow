...
template <class Body>
intptr_t TestNumSubrangesCalculation(intptr_t length, intptr_t grain, intptr_t inner_length,
                                     intptr_t inner_grain) {
    ResetGlobals();
    g_ThrowException = false;
    intptr_t outerCalls = NumSubranges(length, grain), innerCalls = NumSubranges(inner_length, inner_grain),
...
void TestParallelLoopAux() {
    ...
    for (int i = 0; i < 2; ++i) {
        ResetGlobals();
        TRY();
        if (i == 0) tbb::parallel_for(range_type(0, FLAT_RANGE, FLAT_GRAIN), ParForBody(), partitioner);
        else {
        ...
        REQUIRE_MESSAGE(g_CurExecuted <= g_ExecutedAtLastCatch + g_NumThreads,
                        "Too many tasks survived exception");
        if (g_SolitaryException) {
            REQUIRE_MESSAGE(g_NumExceptionsCaught == 1, "No try_blocks in any body expected in this test");
            REQUIRE_MESSAGE(
                    g_NumExceptionsCaught
                            == (g_ExceptionInMaster ? g_MasterExecutedThrow : g_NonMasterExecutedThrow),
...
/** Inner algorithms are spawned inside the new bound context by default. Since
    ...
    algorithms. **/
void Test2() { TestParallelLoop<OuterParForBody>(); }// void Test2 ()
...
void Test3() {
    ...
    bool okayNoExceptionsCaught
            = (g_ExceptionInMaster && !g_MasterExecuted) || (!g_ExceptionInMaster && !g_NonMasterExecuted);
    if (g_SolitaryException) {
        ...
        REQUIRE_MESSAGE((g_CurExecuted <= minExecuted + (g_ExecutedAtLastCatch + g_NumThreads)),
                        "Too many tasks survived exception");
        REQUIRE_MESSAGE((g_NumExceptionsCaught == 1 || okayNoExceptionsCaught),
                        "No try_blocks in any body expected in this test");
    } else {
        REQUIRE_MESSAGE((g_CurExecuted <= g_ExecutedAtLastCatch + g_NumThreads),
                        "Too many tasks survived exception");
        REQUIRE_MESSAGE((g_NumExceptionsCaught >= 1 || okayNoExceptionsCaught),
...
void Test4() {
    ...
    if (g_SolitaryException) {
        ...
        REQUIRE_MESSAGE(g_CurExecuted <= minExecuted + (g_NumThreads - 1) * g_NumThreads / 2,
                        "Too many tasks survived exception");
    } else {
        REQUIRE_MESSAGE(((g_NumExceptionsCaught >= 1 && g_NumExceptionsCaught <= outerCalls)
                         || okayNoExceptionsCaught),
                        "Unexpected actual number of exceptions");
        REQUIRE_MESSAGE(g_CurExecuted >= minExecuted, "Too few executed tasks reported");
        REQUIRE_MESSAGE((g_CurExecuted <= g_ExecutedAtLastCatch + g_NumThreads),
                        "Too many tasks survived multiple exceptions");
        REQUIRE_MESSAGE(g_CurExecuted <= outerCalls * (1 + g_NumThreads),
                        "Too many tasks survived exception");
    }
}// void Test4 ()
...
void TestCancelation1() {
    ResetGlobals(false);
    RunCancellationTest<ParForLauncher<ParForBodyToCancel>, Cancellator>(NumSubranges(FLAT_RANGE, FLAT_GRAIN)
                                                                         / 4);
}
...
public:
    void operator()(const range_type&) const {
        ...
        while (!tbb::is_current_task_group_canceling()) utils::yield();
    }
};
...
void TestCancelation2() {
    ...
    REQUIRE_MESSAGE(g_ExecutedAtLastCatch < g_NumThreads,
                    "Somehow worker tasks started their execution before the cancellator task");
    g_TGCCancelled.validate(g_NumThreads, "Too many tasks survived cancellation");
    REQUIRE_MESSAGE(g_CurExecuted <= g_ExecutedAtLastCatch + g_NumThreads,
                    "Some tasks were executed after cancellation");
}
...
public:
    ...
    int Validate(int start_level) {
        ...
        for (int i = start_level + 1; i < max_nesting; ++i) expected *= reduce_range;
        return expected;
    }
};
...
public:
    RecursiveParReduceBodyWithSharedWorker(RecursiveParReduceBodyWithSharedWorker& src, tbb::split)
        : m_SharedWorker(src.m_SharedWorker), m_NestingLevel(src.m_NestingLevel), m_Result(0) {}
    RecursiveParReduceBodyWithSharedWorker(Worker* w, int outer)
        : m_SharedWorker(w), m_NestingLevel(outer), m_Result(0) {}
    ...
    void operator()(const tbb::blocked_range<size_t>& r) {
        if (g_Master == std::this_thread::get_id()) g_MasterExecuted = true;
            ...
            g_NonMasterExecuted = true;
        if (tbb::is_current_task_group_canceling()) g_TGCCancelled.increment();
        for (size_t i = r.begin(); i != r.end(); ++i) { m_Result += m_SharedWorker->DoWork(m_NestingLevel); }
    }
    void join(const RecursiveParReduceBodyWithSharedWorker& x) { m_Result += x.m_Result; }
...
public:
    ...
    void operator()(const tbb::blocked_range<std::size_t>& /*range*/) const {
        ...
        if (my_id >= 12) {
            if (my_exception) ThrowTestException(1);
            else
...
void TestCancelation4() {
...
#endif
    {
        ...
        tbb::parallel_reduce(tbb::blocked_range<std::size_t>(0, 100000000, 100), body_for_cancellation,
                             tbb::simple_partitioner(), tgc1);
        tbb::parallel_reduce(tbb::blocked_range<std::size_t>(0, 100000000, 100), body_for_exception,
                             tbb::simple_partitioner(), tgc2);
    }
#if TBB_USE_EXCEPTIONS
    catch (...) {
    }
#endif
    REQUIRE_MESSAGE(statsObj.my_total_created == statsObj.my_total_deleted,
                    "Not all parallel_reduce body objects created were reclaimed");
}
...
struct adaptive_range {
    ...
    iterator begin() const { return iterator {&my_array.front()}; }
    iterator begin() { return iterator {&my_array.front()}; }
    iterator end() const { return iterator {&my_array.back()}; }
    iterator end() { return iterator {&my_array.back()}; }
};
...
void Test4_parallel_for_each() {
    ...
    CATCH();
    REQUIRE_MESSAGE(!l_ExceptionCaughtAtCurrentLevel,
                    "All exceptions must have been handled in the parallel_for_each body");
    intptr_t innerCalls = get_iter_range_size(), outerCalls = get_iter_range_size() + g_FedTasksCount,
             maxExecuted = outerCalls * innerCalls, minExecuted = 0;
    g_TGCCancelled.validate(g_NumThreads, "Too many tasks survived exception");
    if (g_SolitaryException) {
        ...
        REQUIRE_MESSAGE(g_CurExecuted < g_ExecutedAtLastCatch + g_NumThreads + outerCalls,
                        "Too many tasks survived multiple exceptions");
        REQUIRE_MESSAGE(g_CurExecuted <= outerCalls * (1 + g_NumThreads),
                        "Too many tasks survived exception");
    }
...
public:
    ...
    void operator()(const size_t& value, tbb::feeder<size_t>& feeder) const {
        ...
        if (tbb::is_current_task_group_canceling()) g_TGCCancelled.increment();
        Feed(feeder, 1);
        if (value == 1) ThrowTestException(1);
    }
};// class ParForEachBodyWithThrowingFeederTasks
...
void Test5_parallel_for_each() {
    ...
    if (g_SolitaryException) {
        ...
        REQUIRE_MESSAGE(
                (l_ExceptionCaughtAtCurrentLevel// we saw an exception
                 || (!g_ExceptionInMaster
                     && !g_NonMasterExecutedThrow)// non-external trhead throws but none tried
                 || (g_ExceptionInMaster
                     && !g_MasterExecutedThrow))// external thread throws but external thread didn't try
                ,
...
void TestCancelation1_parallel_for_each() {
    ...
    REQUIRE_MESSAGE(get_iter_range_size() - threshold > g_NumThreads,
                    "Threshold should leave more than max_threads tasks to test the cancellation.");
    tbb::task_group tg;
    ...
    CATCH_AND_FAIL();
    REQUIRE_MESSAGE(g_CurExecuted < g_ExecutedAtLastCatch + g_NumThreads,
                    "Too many tasks were executed after cancellation");
}
...
public:
    InputFilter() : m_Buffer(get_iter_range_size()) {
        m_Item = 0;
        for (size_t i = 0; i < get_iter_range_size(); ++i) m_Buffer[i] = 1;
    }
    InputFilter(const InputFilter& other) : m_Item(other.m_Item.load()), m_Buffer(get_iter_range_size()) {
        for (size_t i = 0; i < get_iter_range_size(); ++i) m_Buffer[i] = other.m_Buffer[i];
    }
...
FilterSet serial_parallel(tbb::filter_mode::serial_in_order, tbb::filter_mode::parallel, /*throw1*/ false,
                          /*throw2*/ true);
...
public:
    CustomPipeline(const FilterSet& filters)
        : filter1(filters.throw1, filters.mode1 != tbb::filter_mode::parallel),
          filter2(filters.throw2, filters.mode2 != tbb::filter_mode::parallel), my_filters(filters) {}

    void run() {
        tbb::parallel_pipeline(g_NumTokens,
...
void Test1_pipeline(const FilterSet& filters) {
    ...
    if (g_CurExecuted == 2 * static_cast<int>(get_iter_range_size())) {
        ...
        if (!g_ExceptionInMaster && g_NonMasterExecutedThrow > 0) {
            ...
            REQUIRE_MESSAGE((filters.mode1 != tbb::filter_mode::parallel
                             && filters.mode2 != tbb::filter_mode::parallel),
                            "Unusual count");
        }
...
void Test2_pipeline(const FilterSet& filters) {
    ...
    testPipeline.run();
    CATCH_AND_ASSERT();
    bool okayNoExceptionCaught = (g_ExceptionInMaster && !g_MasterExecutedThrow)
...
void Test3_pipeline(const FilterSet& filters) {
    for (int nTries = 1; nTries <= 4; ++nTries) {
        ...
        g_NestedPipelines = true;
        g_Master = std::this_thread::get_id();
        intptr_t innerCalls = get_iter_range_size(), minExecuted = (g_NumThreads - 1) * innerCalls;
...
void Test4_pipeline(const FilterSet& filters) {
    ...
    intptr_t innerCalls = 2 * get_iter_range_size(), outerCalls = 2 * get_iter_range_size(),
...
void TestWithDifferentFiltersAndConcurrency() {
    for (auto concurrency_level : utils::concurrency_range()) {
        ...
        if (g_NumThreads > 1) {
            ...
            const tbb::filter_mode modes[] = {tbb::filter_mode::parallel, tbb::filter_mode::serial_in_order,
            ...
            for (size_t j = 0; j < 4; ++j) {
                ...
                for (int i = 0; i < NumFilterTypes; ++i) {
                    for (int n = 0; n < NumFilterTypes; ++n) {
                        for (int k = 0; k < 2; ++k) testFunc(FilterSet(modes[i], modes[n], k == 0, k != 0));
                    }
                }
            }
        }
