...
class SharedGroupBodyImpl : utils::NoCopy, utils::NoAfterlife {
    ...
    struct TaskFunctor {
        ...
        void operator()() const {
            if (m_pOwner->m_sharingMode & ParallelWait) {
                while (utils::ConcurrencyTracker::PeakParallelism() < m_pOwner->m_numThreads) utils::yield();
            }
            ++s_tasksExecuted;
        }
    };
    ...
    void Wait() {
        while (m_threadsReady != m_numThreads) utils::yield();
        ...
        INFO("Max spawning parallelism is " << utils::ConcurrencyTracker::PeakParallelism() << "out of "
                                            << g_MaxConcurrency);
        if (m_sharingMode & ParallelWait) {
            ...
            m_taskGroup->wait();
        CHECK_MESSAGE(m_tasksSpawned == numSpawned,
                      "No tasks should be spawned after wait starts. The test is broken");
...
public:
    SharedGroupBodyImpl(std::uintptr_t numThreads, std::uintptr_t sharingMode = 0)
        : m_numThreads(numThreads), m_sharingMode(sharingMode), m_taskGroup(nullptr), m_barrier(numThreads) {
    ...
    void Run(std::uintptr_t idx) {
        ...
        if (idx == 0) {
            ...
            Spawn(c_numTasks0);
            Wait();
            if (m_sharingMode & VagabondGroup) m_barrier.wait();
            else
                DeleteTaskGroup();
        } else {
            while (m_tasksSpawned == 0) utils::yield();
            CHECK_MESSAGE(m_taskGroup, "Task group is not initialized");
            Spawn(c_numTasks1);
            if (m_sharingMode & ParallelWait) Wait();
...
public:
    SharedGroupBody(std::uintptr_t numThreads, std::uintptr_t sharingMode = 0)
        : utils::NoAssign(), utils::NoAfterlife(), m_bOwner(true),
          m_pImpl(new SharedGroupBodyImpl<task_group_type>(numThreads, sharingMode)) {}
    SharedGroupBody(const SharedGroupBody& src)
        : utils::NoAssign(), utils::NoAfterlife(), m_bOwner(false), m_pImpl(src.m_pImpl) {}
    ~SharedGroupBody() {
        if (m_bOwner) delete m_pImpl;
    }
...
public:
    RunAndWaitSyncronizationTestBody(utils::SpinBarrier& barrier, std::atomic<bool>& completed,
                                     task_group_type& tg)
    ...
    void operator()(int id) const {
        if (id == 0) {
            ...
            m_barrier.wait();
            m_tg.wait();
            CHECK_MESSAGE(m_completed,
                          "A concurrent waiter has left the wait method earlier than work has finished");
        }
    }
};
...
public:
    FibTaskAsymmetricTreeWithFunctor(std::uintptr_t* y, std::uintptr_t n)
        : FibTaskBase<task_group_type>(y, n) {}
...
public:
    FibTaskSymmetricTreeWithFunctor(std::uintptr_t* y, std::uintptr_t n)
        : FibTaskBase<task_group_type>(y, n) {}
    virtual void impl() const override {
        std::uintptr_t x = ~0u, y = ~0u;
        task_group_type tg;
...
void RunFibTest() {
    FIB_TEST_PROLOGUE();
    std::uintptr_t sum = 0;
    for (unsigned i = 0; i < numRepeats; ++i) sum += RunFibTask<fib_task>(N);
...
public:
    ...
    void operator()() const {
        ...
        AssertLive();
        if (g_Throw) {
            if (++m_TaskCount == SKIP_CHORES) TBB_TEST_THROW(test_exception(EXCEPTION_DESCR1));
            utils::yield();
        } else {
            ++g_TaskCount;
...
void LaunchChildrenWithFunctor() {
    ...
    if (g_Rethrow && g_ExceptionCount > SKIP_GROUPS) { throw test_exception(EXCEPTION_DESCR2); }
#else
    g.wait();
#endif
}
...
void TestManualCancellationWithFunctor() {
    ...
    while (g_MaxConcurrency > 1 && g_TaskCount == 0) utils::yield();
    tg.cancel();
    ...
    CHECK_MESSAGE(g_TaskCount < NUM_GROUPS * NUM_CHORES,
                  "No tasks were cancelled. Cancellation model changed?");
    CHECK_MESSAGE(g_TaskCount <= g_ExecutedAtCancellation + utils::ConcurrencyTracker::PeakParallelism(),
                  "Too many tasks survived cancellation");
}
...
void TestExceptionHandling1() {
    ...
    try {
        tg.wait();
    } catch (...) { CHECK_MESSAGE(false, "Unexpected exception"); }
    CHECK_MESSAGE(g_ExceptionCount <= NUM_GROUPS,
                  "Too many exceptions from the child groups. The test is broken");
...
void TestExceptionHandling2() {
    ...
    CHECK_MESSAGE(g_ExceptionCount >= SKIP_GROUPS,
                  "Too few exceptions from the child groups. The test is broken");
    CHECK_MESSAGE(g_ExceptionCount <= NUM_GROUPS - SKIP_GROUPS,
                  "Too many exceptions from the child groups. The test is broken");
...
void TestExceptionHandling3() {
    ...
    } catch (int error) { CHECK(error == 1); } catch (...) {
...
public:
    void Launch(task_group_type& tg) {
        ResetGlobals(false, false);
        for (unsigned i = 0; i < NUM_GROUPS; ++i) { tg.run(LaunchChildrenWithFunctor<task_group_type>); }
        CHECK_MESSAGE(!tbb::is_current_task_group_canceling(), "Unexpected cancellation");
        while (g_MaxConcurrency > 1 && g_TaskCount == 0) utils::yield();
    }
    ...
    void Finish() {
        ...
        CHECK_MESSAGE(g_TaskCount < NUM_GROUPS * NUM_CHORES,
                      "No tasks were cancelled. Cancellation model changed?");
        CHECK_MESSAGE(g_TaskCount <= g_ExecutedAtCancellation + g_MaxConcurrency,
                      "Too many tasks survived cancellation");
    }
...
void TestMissingWait() {
    bool exception_occurred = false, unexpected_exception = false;
    ...
    try {
        ...
        exception_occurred = true;
        unexpected_exception = !Throw;
    } catch (...) { exception_occurred = unexpected_exception = true; }
...
namespace TestIsolationNS {
    ...
    public:
        ParForBody(task_group_type& tg, std::atomic<bool>& preserved,
        ...
        void operator()(int) const {
            ...
            for (int i = 0; i < 1000; ++i) m_tg.run(DummyFunctor());
            m_tg.wait();
    ...
    void CheckIsolation(bool isolation_is_expected) {
        ...
        ASSERT(isolation_is_expected == isolation_is_preserved,
...
TEST_CASE("Fibonacci test for the task group") {
    for (unsigned p = MinThread; p <= MaxThread; ++p) {
        ...
        tbb::task_arena a(p);
        g_MaxConcurrency = p;
        a.execute([] { RunFibonacciTests<tbb::task_group>(); });
    }
}
...
TEST_CASE("Cancellation and exception test for the task group") {
    for (unsigned p = MinThread; p <= MaxThread; ++p) {
        ...
        tbb::task_arena a(p);
        g_MaxConcurrency = p;
        a.execute([] { RunCancellationAndExceptionHandlingTests<tbb::task_group>(); });
    }
}
...
TEST_CASE("Thread safety test for the isolated task group") {
    ...
    for (unsigned p = MinThread; p <= MaxThread; ++p) {
        ...
        g_MaxConcurrency = p;
        tbb::task_arena a(p);
        a.execute([] { TestThreadSafety<tbb::isolated_task_group>(); });
    }
}
#endif
...
TEST_CASE("Fibonacci test for the isolated task group") {
    for (unsigned p = MinThread; p <= MaxThread; ++p) {
        ...
        g_MaxConcurrency = p;
        tbb::task_arena a(p);
        a.execute([] { RunFibonacciTests<tbb::isolated_task_group>(); });
    }
}
...
TEST_CASE("Cancellation and exception test for the isolated task group") {
    for (unsigned p = MinThread; p <= MaxThread; ++p) {
        ...
        g_MaxConcurrency = p;
        tbb::task_arena a(p);
        a.execute([] { RunCancellationAndExceptionHandlingTests<tbb::isolated_task_group>(); });
    }
}
...
TEST_CASE("Constant functor test for the isolated task group") {
    for (unsigned p = MinThread; p <= MaxThread; ++p) {
        ...
        g_MaxConcurrency = p;
        tbb::task_arena a(p);
        a.execute([] { TestConstantFunctorRequirement<tbb::isolated_task_group>(); });
    }
}
...
TEST_CASE("Move semantics test for the isolated task group") {
    for (unsigned p = MinThread; p <= MaxThread; ++p) {
        ...
        g_MaxConcurrency = p;
        tbb::task_arena a(p);
        a.execute([] { TestMoveSemantics<tbb::isolated_task_group>(); });
    }
}
...
void run_deep_stealing(tbb::task_group& tg1, tbb::task_group& tg2, int num_tasks,
...
TEST_CASE("Test for stack overflow avoidance mechanism within arena") {
    ...
    a1.execute([] {
        ...
        tg1.run_and_wait([&a2, &tg1, &tg2, &tasks_executed, second_thread_executed] {
            ...
            a2.execute([&tg1, &tg2, &tasks_executed] { run_deep_stealing(tg1, tg2, 10000, tasks_executed); });
...
struct SelfRunner {
    ...
    void operator()() const {
        ...
        if (previous_count > 1) m_tg.run(*this);
    }
};
...
namespace accept_task_group_context {
    ...
    void run_cancellation_use_case(CancelF&& cancel, WaitF&& wait) {
        ...
        auto outer_tg_task = [&] {
            ...
            utils::try_call([&] { std::forward<CancelF>(cancel)(outer_tg); }).on_completion([&] {
        ...
        auto check = [&] {
            ...
            CHECK_MESSAGE(outer_status == tbb::task_group_status::canceled,
    ...
    void test() {
        run_cancellation_use_case<TaskGroup>([](tbb::task_group& outer) { outer.cancel(); },
        ...
        run_cancellation_use_case<TaskGroup>(
                [](tbb::task_group& /*outer*/) {
                    volatile bool suppress_unreachable_code_warning = true;
                    if (suppress_unreachable_code_warning) { throw int(); }
                },
                [](tbb::task_group& outer) {
                    try {
                        outer.wait();
                        return tbb::task_group_status::complete;
                    } catch (const int&) { return tbb::task_group_status::canceled; }
                });
#endif
    }
...
TEST_CASE("Task handle for scheduler bypass") {
    ...
    tg.run([&] { return tg.defer([&] { run = true; }); });
...
TEST_CASE("Task handle for scheduler bypass via run_and_wait") {
    ...
    tg.run_and_wait([&] { return tg.defer([&] { run = true; }); });
...
TEST_CASE("Empty task_handle cannot be scheduled"
    ...
    CHECK_THROWS_WITH_AS(tg.run(tbb::task_handle {}), "Attempt to schedule empty task_handle",
                         std::runtime_error);
}
...
TEST_CASE("task_handle cannot be scheduled into different task_group"
    ...
    CHECK_THROWS_WITH_AS(tg1.run(tg.defer([] {})),
                         "Attempt to schedule task_handle into different task_group", std::runtime_error);
}

//! The test for error in task_handle being scheduled into task_group different from one it was created from
//! \brief \ref requirement
TEST_CASE(
        "task_handle cannot be scheduled into other task_group of the same context"
        * doctest::
                should_fail()//Implementation is no there yet, as it is not clear that is the expected behavior
        * doctest::skip()    //skip the test for now, to not pollute the test log
) {
