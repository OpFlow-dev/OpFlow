...
bool is_observer_created(const tbb::task_arena::constraints& c) {
    ...
    return (c.numa_id != tbb::task_arena::automatic && numa_nodes.size() > 1)
           || (c.core_type != tbb::task_arena::automatic && core_types.size() > 1)
           || c.max_threads_per_core != tbb::task_arena::automatic;
}

void recursive_arena_binding(constraints_container::iterator current_pos,
                             constraints_container::iterator end_pos) {
    ...
    if (current_pos != end_pos) {
        ...
        current_level_arena.execute(
                [&current_pos, &end_pos]() { recursive_arena_binding(++current_pos, end_pos); });
    }
        ...
        { /* All DTORs must be called before GetMemoryUsage() call*/
            ...
            arena.execute([] {
                ...
                tbb::parallel_for(tbb::blocked_range<size_t>(0, tbb::this_task_arena::max_concurrency()),
                                  [&barrier](const tbb::blocked_range<size_t>&) { barrier.wait(); });
            });
        }
...
TEST_CASE("Test arena constraints setters") {
    ...
    for (const auto& numa_index : tbb::info::numa_nodes()) {
        ...
        constraints assignment_c {};
        assignment_c.numa_id = numa_index;
    ...
    for (const auto& core_type_index : tbb::info::core_types()) {
        ...
        constraints assignment_c {};
        assignment_c.core_type = core_type_index;
    ...
    // Max concurrency setter testing
    {
        ...
        constraints assignment_c {};
        assignment_c.max_concurrency = 1;
    ...
    // Threads per core setter testing
    {
        ...
        constraints assignment_c {};
        assignment_c.max_threads_per_core = 1;
...
void check_concurrency_level(const tbb::task_arena::constraints& c) {
    REQUIRE_MESSAGE(tbb::info::default_concurrency(c) == custom_concurrency_value,
                    "Custom arena concurrency was passed to constraints, but was not respected by "
                    "default_concurrency() call.");
