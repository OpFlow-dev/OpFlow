...
void test_single_dest() {
    ...
    src.activate();
    g.wait_for_all();
    for (int i = 0; i < N; ++i) { CHECK_MESSAGE(dest.get_count(i) == 1, ""); }
    ...
    g.wait_for_all();
    for (int i = 0; i < N; ++i) { CHECK_MESSAGE(dest_c.get_count(i) == 1, ""); }
}
...
TEST_CASE("try_get before activation") {
    tbb::flow::graph g;
    tbb::flow::input_node<int> in(g, [&](tbb::flow_control& fc) {
        fc.stop();
        return 0;
    });
...
concept can_call_input_node_ctor
        = requires(tbb::flow::graph& graph, Body body, tbb::flow::buffer_node<int> f) {
