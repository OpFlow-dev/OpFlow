        ...
        for (int j = 0; j < W; ++j) {
            ...
            for (int w = 0; w < W; ++w) {
                if (w < j) CHECK_MESSAGE(done_flag[w] == true, "");
                    ...
                    CHECK_MESSAGE(done_flag[w] == false, "");
            }
        }
    }
}
...
public:
    ...
    my_int_buffer& operator=(const my_int_buffer& /*other*/) { return *this; }
};
...
void test_iterator() {
    ...
    int count = 0;
    for (tbb::flow::graph::iterator it = g.begin(); it != g.end(); ++it) { count++; }
...
struct nodes_test_functor : utils::NoAssign {
    ...
    void operator()() const {
        ...
        tbb::flow::continue_node<tbb::flow::continue_msg> c_n(
                my_graph, function_body<tbb::flow::continue_msg>(midway_arena));
...
TEST_CASE("Test parallel") {
    for (unsigned int p = utils::MinThread; p <= utils::MaxThread; ++p) {
        tbb::task_arena arena(p);
        arena.execute([&]() { test_parallel(p); });
    }
}

//! Test separate arena isn't used
//! \brief \ref error_guessing
TEST_CASE("Test graph_arena") { test_graph_arena(); }
...
TEST_CASE("graph iterator") {
    ...
    for (auto it = g.cbegin(); it != g.cend(); it++) ++item_count;
    CHECK_MESSAGE((item_count == 1), "Should find 1 item");

    item_count = 0;
    auto jt(g.begin());
    for (; jt != g.end(); jt++) ++item_count;
    CHECK_MESSAGE((item_count == 1), "Should find 1 item");
