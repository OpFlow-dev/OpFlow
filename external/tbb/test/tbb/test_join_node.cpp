...
public:
    ...
    static void test() {
        ...
        for (int maxTag = 1; maxTag < 10; maxTag *= 3) {
            ...
            tbb::flow::function_node<TType, int> recreate_tag(g, tbb::flow::unlimited,
                                                              recirc_output_func_body<TType>());
...
public:
    ...
    static void do_test() { tag_recirculation_test<join_node_type>::test(); }
};
...
namespace multiple_predecessors {
    ...
    void twist_join_connections(buffer_node<continue_msg>& bn1, buffer_node<continue_msg>& bn2,
                                buffer_node<continue_msg>& bn3, join_node_t& jn) {
    ...
    std::unique_ptr<join_node_t> connect_join_via_make_edge(graph& g, buffer_node<continue_msg>& bn1,
                                                            buffer_node<continue_msg>& bn2,
                                                            buffer_node<continue_msg>& bn3,
                                                            queue_node_t& qn) {
    ...
    std::unique_ptr<join_node_t> connect_join_via_follows(graph&, buffer_node<continue_msg>& bn1,
                                                          buffer_node<continue_msg>& bn2,
    ...
    std::unique_ptr<join_node_t> connect_join_via_precedes(graph&, buffer_node<continue_msg>& bn1,
                                                           buffer_node<continue_msg>& bn2,
                                                           buffer_node<continue_msg>& bn3, queue_node_t& qn) {
    ...
    void run_and_check(graph& g, buffer_node<continue_msg>& bn1, buffer_node<continue_msg>& bn2,
        ...
        CHECK_MESSAGE((qn.try_get(msg) == expected),
...
//! \brief \ref error_guessing
TEST_CASE("Tagged buffers test") { TestTaggedBuffers(); }
...
//! \brief \ref error_guessing
TEST_CASE("Recirculation test") { generate_recirc_test<std::tuple<int, float>>::do_test(); }
