...
public:
    tiny_node(tbb::flow::graph &g, bool hidden = false)
        : base_type(g), f1(g, tbb::flow::unlimited, passthru_body()),
          f2(g, tbb::flow::unlimited, passthru_body()) {
        ...
        if (hidden) base_type::add_nodes(f1, f2);
        else
...
int test_tiny(bool hidden = false) {
    ...
    CHECK_MESSAGE((&tbb::flow::input_port<0>(t) == &t.f1),
                  "f1 not bound to input port 0 in composite_node t");
    CHECK_MESSAGE((&tbb::flow::output_port<0>(t) == &t.f2),
                  "f2 not bound to output port 0 in composite_node t");

    tiny_node t1(g, hidden);
    CHECK_MESSAGE((&std::get<0>(t1.input_ports()) == &t1.f1),
                  "f1 not bound to input port 0 in composite_node t1");
    CHECK_MESSAGE((&std::get<0>(t1.output_ports()) == &t1.f2),
                  "f2 not bound to output port 0 in composite_node t1");

    test_input_ports_return_ref(t1);
    test_output_ports_return_ref(t1);

    tiny_node t2(g, hidden);
    CHECK_MESSAGE((&tbb::flow::input_port<0>(t2) == &t2.f1),
                  "f1 not bound to input port 0 in composite_node t2");
    CHECK_MESSAGE((&tbb::flow::output_port<0>(t2) == &t2.f2),
                  "f2 not bound to output port 0 in composite_node t2");
...
private:
    ...
    struct f_body {
        int operator()(const std::tuple<int, int> &t) { return std::get<0>(t) + std::get<1>(t); }
    };

public:
    adder_node(tbb::flow::graph &g, bool hidden = false)
        : base_type(g), j(g), f(g, tbb::flow::unlimited, f_body()) {
        tbb::flow::make_edge(j, f);

        base_type::set_external_ports(
                base_type::input_ports_type(tbb::flow::input_port<0>(j), tbb::flow::input_port<1>(j)),
                base_type::output_ports_type(f));

        if (hidden) base_type::add_nodes(j, f);
        else
...
int adder_sum(int i) { return (int) (pow(3 * pow(i, 3) + pow(i, 2), 2)); }
int test_adder(bool hidden = false) {
    ...
    CHECK_MESSAGE((&tbb::flow::input_port<0>(a0) == &tbb::flow::input_port<0>(a0.j)),
                  "input_port 0 of j not bound to input port 0 in composite_node a0");
    CHECK_MESSAGE((&tbb::flow::input_port<1>(a0) == &tbb::flow::input_port<1>(a0.j)),
                  "input_port 1 of j not bound to input port 1 in composite_node a0");
    CHECK_MESSAGE((&tbb::flow::output_port<0>(a0) == &a0.f),
                  "f not bound to output port 0 in composite_node a0");

    adder_node a1(g, hidden);
    CHECK_MESSAGE((&std::get<0>(a0.input_ports()) == &tbb::flow::input_port<0>(a0.j)),
                  "input_port 0 of j not bound to input port 0 in composite_node a1");
    CHECK_MESSAGE((&std::get<1>(a0.input_ports()) == &tbb::flow::input_port<1>(a0.j)),
                  "input_port1 of j not bound to input port 1 in composite_node a1");
    CHECK_MESSAGE((&std::get<0>(a0.output_ports()) == &a0.f),
                  "f not bound to output port 0 in composite_node a1");

    adder_node a2(g, hidden);
    CHECK_MESSAGE((&tbb::flow::input_port<0>(a2) == &tbb::flow::input_port<0>(a2.j)),
                  "input_port 0 of j not bound to input port 0 in composite_node a2");
    CHECK_MESSAGE((&tbb::flow::input_port<1>(a2) == &tbb::flow::input_port<1>(a2.j)),
                  "input_port 1 of j not bound to input port 1 in composite_node a2");
    CHECK_MESSAGE((&tbb::flow::output_port<0>(a2) == &a2.f),
                  "f not bound to output port 0 in composite_node a2");

    adder_node a3(g, hidden);
    CHECK_MESSAGE((&std::get<0>(a3.input_ports()) == &tbb::flow::input_port<0>(a3.j)),
                  "input_port 0 of j not bound to input port 0 in composite_node a3");
    CHECK_MESSAGE((&std::get<1>(a3.input_ports()) == &tbb::flow::input_port<1>(a3.j)),
                  "input_port1 of j not bound to input port 1 in composite_node a3");
    CHECK_MESSAGE((&std::get<0>(a3.output_ports()) == &a3.f),
                  "f not bound to output port 0 in composite_node a3");
    ...
    CHECK_MESSAGE((result == sum_total),
                  "the new sum after the replacement of the nodes does not match the calculated value");
...
int test_nested_adder(bool hidden = false) {
    ...
    outer_node.set_external_ports(base_type::input_ports_type(tbb::flow::input_port<0>(inner_node1),
                                                              tbb::flow::input_port<1>(inner_node1)),
                                  base_type::output_ports_type(tbb::flow::output_port<0>(inner_node3)));

    CHECK_MESSAGE((&tbb::flow::input_port<0>(outer_node) == &tbb::flow::input_port<0>(inner_node1)),
                  "input port 0 of inner_node1 not bound to input port 0 in outer_node");
    CHECK_MESSAGE((&tbb::flow::input_port<1>(outer_node) == &tbb::flow::input_port<1>(inner_node1)),
                  "input port 1 of inner_node1 not bound to input port 1 in outer_node");
    CHECK_MESSAGE((&tbb::flow::output_port<0>(outer_node) == &tbb::flow::output_port<0>(inner_node3)),
                  "output port 0 of inner_node3 not bound to output port 0 in outer_node");
    ...
    if (hidden) outer_node.add_nodes(inner_node1, inner_node2, inner_node3);
    else
    ...
    for (int i = 1; i < 200000; ++i) {
        ...
        g.wait_for_all();
        output.try_get(out);
        CHECK_MESSAGE((tbb::flow::output_port<0>(outer_node).try_get(out) == output.try_get(out)),
                      "output from outer_node does not match output from graph");
...
private:
    ...
    struct f_body {
        my_tuple_t operator()(const my_tuple_t &t) {
            return my_tuple_t(std::get<0>(t), std::get<0>(t) + std::get<1>(t),
...
public:
    prefix_node(tbb::flow::graph &g, bool hidden = false)
        : base_type(g), j(g), s(g), f(g, tbb::flow::serial, f_body()) {
        tbb::flow::make_edge(j, f);
        tbb::flow::make_edge(f, s);

        typename base_type::input_ports_type input_tuple(
                tbb::flow::input_port<0>(j), tbb::flow::input_port<1>(j), tbb::flow::input_port<2>(j),
                tbb::flow::input_port<3>(j), tbb::flow::input_port<4>(j));

        typename base_type::output_ports_type output_tuple(
                tbb::flow::output_port<0>(s), tbb::flow::output_port<1>(s), tbb::flow::output_port<2>(s),
                tbb::flow::output_port<3>(s), tbb::flow::output_port<4>(s));

        base_type::set_external_ports(input_tuple, output_tuple);

        if (hidden) base_type::add_nodes(j, s, f);
        else
...
int test_prefix(bool hidden = false) {
    ...
    CHECK_MESSAGE((&std::get<0>(p.input_ports()) == &tbb::flow::input_port<0>(p.j)),
                  "input port 0 of j is not bound to input port 0 of composite node p");
    CHECK_MESSAGE((&tbb::flow::input_port<1>(p.j) == &tbb::flow::input_port<1>(p.j)),
                  "input port 1 of j is not bound to input port 1 of composite node p");
    CHECK_MESSAGE((&std::get<2>(p.input_ports()) == &tbb::flow::input_port<2>(p.j)),
                  "input port 2 of j is not bound to input port 2 of composite node p");
    CHECK_MESSAGE((&tbb::flow::input_port<3>(p.j) == &tbb::flow::input_port<3>(p.j)),
                  "input port 3 of j is not bound to input port 3 of composite node p");
    CHECK_MESSAGE((&std::get<4>(p.input_ports()) == &tbb::flow::input_port<4>(p.j)),
                  "input port 4 of j is not bound to input port 4 of composite node p");

    CHECK_MESSAGE((&std::get<0>(p.output_ports()) == &tbb::flow::output_port<0>(p.s)),
                  "output port 0 of s is not bound to output port 0 of composite node p");
    CHECK_MESSAGE((&tbb::flow::output_port<1>(p.s) == &tbb::flow::output_port<1>(p.s)),
                  "output port 1 of s is not bound to output port 1 of composite node p");
    CHECK_MESSAGE((&std::get<2>(p.output_ports()) == &tbb::flow::output_port<2>(p.s)),
                  "output port 2 of s is not bound to output port 2 of composite node p");
    CHECK_MESSAGE((&tbb::flow::output_port<3>(p.s) == &tbb::flow::output_port<3>(p.s)),
                  "output port 3 of s is not bound to output port 3 of composite node p");
    CHECK_MESSAGE((&std::get<4>(p.output_ports()) == &tbb::flow::output_port<4>(p.s)),
                  "output port 4 of s is not bound to output port 4 of composite node p");
...
void input_only_output_only_composite(bool hidden) {
    ...
    CHECK_MESSAGE((&std::get<0>(a_in.input_ports()) == &f),
                  "f not bound to input port 0 in composite_node a_in");

    std::tuple<src_type &> output_tuple(src);
    a_out.set_external_ports(output_tuple);
    CHECK_MESSAGE((&std::get<0>(a_out.output_ports()) == &src),
                  "src not bound to output port 0 in composite_node a_out");
