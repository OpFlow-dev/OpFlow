...
struct numa {
    ...
    DWORD maxProcessors;
    numa()
        : processorGroupCount(GetMaximumProcessorGroupCount()),
          maxProcessors(GetActiveProcessorCount(ALL_PROCESSOR_GROUPS)) {
...
int TestNumaDistribution(std::vector<DWORD>& validateProcgrp, int additionalParallelism, bool allThreads) {
    ...
    if (allThreads) requested_parallelism = additionalParallelism;
    else
    ...
    limited.execute([&]() {
        tbb::parallel_for(
                ...
                s);
        for (const auto& it : tls) { validateProcgrp[it.first]++; }
    });
