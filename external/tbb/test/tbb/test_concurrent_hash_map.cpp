...
void TestRangeBasedFor() {
    ...
    for (int i = 1; i <= sequence_length; ++i) { a_ch_map.insert(ch_map::value_type(i, i)); }

    REQUIRE_MESSAGE((range_based_for_accumulate(a_ch_map, pair_second_summer(), 0)
...
template <bool default_construction_present>
struct do_default_construction_test {
    template <typename FuncType>
    void operator()(FuncType func) const {
        func();
    }
};

template <>
struct do_default_construction_test<false> {
    template <typename FuncType>
...
public:
    test_range(Table& c, const std::list<value_type>& lst, std::vector<detail::atomic_type<bool>>& marks)
        : my_c(c), my_lst(lst), my_marks(marks) {
    ...
    void do_test_range(Iterator i, Iterator j) const {
        for (Iterator it = i; it != j;) {
            Iterator it_prev = it++;
            typename std::list<value_type>::const_iterator it2
                    = std::search(my_lst.begin(), my_lst.end(), it_prev, it, utils::IsEqual());
...
void TypeTester(const std::list<Value>& lst) {
    ...
    using ch_map_debug_alloc
            = tbb::concurrent_hash_map<key_type, second_type, tbb::detail::d1::tbb_hash_compare<key_type>,
    ...
    using ch_map_debug_hash = tbb::concurrent_hash_map<key_type, second_type, debug_hash_compare<key_type>,
...
void TestSpecificTypes() {
    ...
    for (int i = 0; i < NUMBER; ++i) { arr_pair_int.push_back(pair_int_t(pair_key_type {i, i}, i)); }
...
void TestScopedAllocator() {
    ...
    using allocator_type = std::scoped_allocator_adaptor<
            tbb::tbb_allocator<std::pair<const allocator_data_type, allocator_data_type>>>;
...
void TestInternalFastFind() {
    ...
    for (int i = 0; i != sz; ++i) { m.insert(std::make_pair(i, i * i)); }
...
struct hash_map_traits : default_container_traits {
    ...
    struct apply {
        using type = tbb::concurrent_hash_map<element_type, element_type, hash_compare<element_type>,
                                              allocator_type>;
    };
...
void test_heterogeneous_insert() {
    ...
    using chmap_type
            = tbb::concurrent_hash_map<key_type, DefaultConstructibleValue, HeterogeneousHashCompare>;
...
public:
    ...
    public:
        ...
        scoped_lock(MinimalisticMutex& m, bool = true) : my_mutex_ptr(&m) { my_mutex_ptr->my_mutex.lock(); }
...
void test_with_minimalistic_mutex() {
    ...
    using chmap_type = tbb::concurrent_hash_map<int, int, tbb::tbb_hash_compare<int>,
                                                tbb::tbb_allocator<std::pair<const int, int>>, mutex_type>;
        ...
        for (std::size_t item = range.begin(); item != range.end(); ++item) {
            switch (item % 4) {
                ...
                case 3:
                    ...
                    for (int i = 10000; i < 10005; ++i) { chmap.erase(i); }
                    break;
            }
        }
    };// thread_body
...
TEST_CASE("testing range based for support") { TestRangeBasedFor(); }

//! Test concurrent_hash_map with specific key/mapped types
//! \brief \ref regression \ref error_guessing
TEST_CASE("testing concurrent_hash_map with specific key/mapped types") { TestSpecificTypes(); }

//! Test work with scoped allocator
//! \brief \ref regression
TEST_CASE("testing work with scoped allocator") { TestScopedAllocator(); }

//! Test internal fast find for concurrent_hash_map
//! \brief \ref regression
TEST_CASE("testing internal fast find for concurrent_hash_map") { TestInternalFastFind(); }
...
TEST_CASE("Test exception in constructors") {
    ...
    auto init_list = {std::pair<const int, int>(1, 42), std::pair<const int, int>(2, 42),
                      std::pair<const int, int>(3, 42), std::pair<const int, int>(4, 42),
                      std::pair<const int, int>(5, 42), std::pair<const int, int>(6, 42)};
    ...
    map_type big_map {};
    for (int i = 0; i < 1000; ++i) { big_map.insert(std::pair<const int, int>(i, 42)); }
...
//! \brief \ref error_guessing
TEST_CASE("test concurrent_hash_map mutex customization") { test_mutex_customization(); }

#if __TBB_CPP20_CONCEPTS_PRESENT
template <bool ExpectSatisfies, typename Key, typename Mapped, typename... HCTypes>
requires(...
         && (utils::well_formed_instantiation<
                     tbb::concurrent_hash_map, Key, Mapped,
                     HCTypes> == ExpectSatisfies)) void test_chmap_hash_compare_constraints() {}
...
TEST_CASE("tbb::concurrent_hash_map hash_compare constraints") {
    ...
    test_chmap_hash_compare_constraints<
            /*Expected = */ false, /*key = */ key_type, /*mapped = */ mapped_type, NonCopyable<key_type>,
            NonDestructible<key_type>, NoHash<key_type>, HashNonConst<key_type>, WrongInputHash<key_type>,
            WrongReturnHash<key_type>, NoEqual<key_type>, EqualNonConst<key_type>,
...
requires(...
         && (utils::well_formed_instantiation<
                     tbb::concurrent_hash_map, Key, Mapped, tbb::tbb_hash_compare<Key>,
                     tbb::tbb_allocator<std::pair<const Key, Mapped>>,
                     RWMutexes> == ExpectSatisfies)) void test_chmap_mutex_constraints() {}
...
TEST_CASE("tbb::concurrent_hash_map rw_mutex constraints") {
    ...
    test_chmap_mutex_constraints</*Expected = */ true, key_type, mapped_type, Correct>();

    test_chmap_mutex_constraints<
            /*Expected = */ false, key_type, mapped_type, NoScopedLock, ScopedLockNoDefaultCtor,
            ScopedLockNoMutexCtor, ScopedLockNoDtor, ScopedLockNoAcquire, ScopedLockWrongFirstInputAcquire,
            ScopedLockWrongSecondInputAcquire, ScopedLockNoTryAcquire, ScopedLockWrongFirstInputTryAcquire,
            ScopedLockWrongSecondInputTryAcquire, ScopedLockWrongReturnTryAcquire, ScopedLockNoRelease,
            ScopedLockNoUpgrade, ScopedLockWrongReturnUpgrade, ScopedLockNoDowngrade,
            ScopedLockWrongReturnDowngrade, ScopedLockNoIsWriter, ScopedLockIsWriterNonConst,
            ScopedLockWrongReturnIsWriter>();
}
