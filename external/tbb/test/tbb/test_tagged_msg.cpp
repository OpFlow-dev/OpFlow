...
typedef tbb::flow::tagged_msg<size_t, int, char, double, odd_array_type, odder_array, d_vector,
                              CheckType<int>, counted_array_type, i_vector_array>
        tagged_msg_type;
...
void TestWrapper() {
    ...
    odd_array_type ww;
    for (int ii = 0; ii < 15; ++ii) { ww[ii] = char('0' + ii); }
    ww[14] = 0;
    ...
    for (int j = 0; j < 12; ++j) CHECK_MESSAGE((1 + j == cat.value()[j]), "Error in cat array");
...
void RunTests() {
    ...
    } catch (...) { INFO("cast of non-array to int disallowed (okay)\n"); }
#endif
    z = cast_to<int>(i);
    ...
    CHECK_MESSAGE((CheckType<int>::check_type_counter == 1),
                  "Incorrect number of CheckType<int>s after reassignment");
    ...
    CHECK_MESSAGE((CheckType<int>::check_type_counter == 13),
                  "Incorrect number of CheckType<int>s after counted_array construction");
    tagged_msg_type counted_array_tagged_msg(5, counted_array);
    // the is_a<>() should return exact type matches.
    CHECK_MESSAGE((!is_a<CheckType<int> *>(counted_array_tagged_msg)),
                  "Test of is_a for counted_array_tagged_msg fails");
#if TBB_USE_EXCEPTIONS
    try {
    ...
    } catch (std::runtime_error &re) { INFO("attempt to cast to invalid type caught " << re.what() << "\n"); }
    ...
    CHECK_MESSAGE((CheckType<int>::check_type_counter == 25),
                  "Incorrect number of CheckType<int>s after counted_array_tagged_msg construction");
    counted_array_tagged_msg = default_tagged_msg;
    CHECK_MESSAGE((CheckType<int>::check_type_counter == 13),
                  "Incorrect number of CheckType<int>s after counted_array_tagged_msg destruction");
    CHECK_MESSAGE((counted_array_tagged_msg.is_default_constructed()),
                  "Assigned counted_array_type is not default-constructed");
    ...
    {
        odd_array_type my_b;
        for (size_t ii = 0; ii < 14; ++ii) { my_b[ii] = (char) ('0' + ii); }
        my_b[14] = 0;
        {
            ...
            try {
                ...
                CHECK_MESSAGE((!strncmp(my_b, my_copy2, 14)),
                              "char array from default tagged_msg assign not correct value");
            } catch (...) { CHECK_MESSAGE((false), "Bad cast"); }
        }
    }
    ...
    } catch (...) { CHECK_MESSAGE((false & cast_to<int>(i)), "Error: improper exception thrown"); }
...
TEST_CASE("Test base of tagged_msg") {
    TestWrapper();
    CHECK_MESSAGE((CheckType<int>::check_type_counter == 0),
                  "After TestWrapper return not all CheckType<int>s were destroyed");
}
...
TEST_CASE("Test tagged_msg") {
    RunTests();
    CHECK_MESSAGE((CheckType<int>::check_type_counter == 0),
                  "After RunTests return not all CheckType<int>s were destroyed");
}
