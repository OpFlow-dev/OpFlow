...
void run_buffered_levels(int c) {
    typedef typename tbb::flow::multifunction_node<InputType, OutputTuple>::output_ports_type
            output_ports_type;
    buffered_levels<InputType, OutputTuple>(c, [](InputType i, output_ports_type& p) {
        harness_graph_multifunction_executor<InputType, OutputTuple>::func(i, p);
    });
    buffered_levels<InputType, OutputTuple>(
            c, &harness_graph_multifunction_executor<InputType, OutputTuple>::func);
    buffered_levels<InputType, OutputTuple>(
            c, typename harness_graph_multifunction_executor<InputType, OutputTuple>::functor());
            ...
            for (size_t num_senders = 1; num_senders <= MAX_NODES; ++num_senders) {
                {
                    ...
                    tbb::spin_rw_mutex::scoped_lock l(
                            harness_graph_multifunction_executor<InputType, OutputTuple>::
                                    template mutex_holder<tbb::spin_rw_mutex>::mutex);
...
void run_concurrency_levels(int c) {
    typedef typename tbb::flow::multifunction_node<InputType, OutputTuple>::output_ports_type
            output_ports_type;
    concurrency_levels<InputType, OutputTuple>(c, [](InputType i, output_ports_type& p) {
        harness_graph_multifunction_executor<InputType, OutputTuple>::template tfunc<tbb::spin_rw_mutex>(i,
                                                                                                         p);
    });
    concurrency_levels<InputType, OutputTuple>(
            c, &harness_graph_multifunction_executor<InputType,
                                                     OutputTuple>::template tfunc<tbb::spin_rw_mutex>);
    concurrency_levels<InputType, OutputTuple>(
            c, typename harness_graph_multifunction_executor<InputType, OutputTuple>::template tfunctor<
                       tbb::spin_rw_mutex>());
}
...
void unlimited_concurrency(Body body) {
    ...
    for (unsigned int p = 1; p < 2 * utils::MaxThread; ++p) {
        tbb::flow::graph g;
        tbb::flow::multifunction_node<InputType, OutputTuple, tbb::flow::rejecting> exe_node(
...
void run_unlimited_concurrency() {
    ...
    typedef typename tbb::flow::multifunction_node<InputType, OutputTuple>::output_ports_type
            output_ports_type;
    unlimited_concurrency<InputType, OutputTuple>([](InputType i, output_ports_type& p) {
        harness_graph_multifunction_executor<InputType, OutputTuple>::func(i, p);
    });
    unlimited_concurrency<InputType, OutputTuple>(
            &harness_graph_multifunction_executor<InputType, OutputTuple>::func);
    unlimited_concurrency<InputType, OutputTuple>(
            typename harness_graph_multifunction_executor<InputType, OutputTuple>::functor());
}
...
void run_multiport_test(int num_threads) {
    ...
    arena.execute([&]() {
        ...
        for (InputType i = 0; i < N; ++i) { mo_node.try_put(i); }
...
void test_precedes() {
    ...
    int storage;
    CHECK_MESSAGE(
            (b1.try_get(storage) && !b1.try_get(storage) && b2.try_get(storage) && !b2.try_get(storage)),
...
void test_follows_and_precedes_api() {
    ...
    follows_and_precedes_testing::test_follows<int,
                                               tbb::flow::multifunction_node<int, std::tuple<int, int, int>>>(
            messages_for_follows, tbb::flow::unlimited,
            [](const int& i, multinode::output_ports_type& op) -> void { std::get<0>(op).try_put(i); });
...
//! \brief \ref error_guessing
TEST_CASE("Lightweight testing") { lightweight_testing::test<tbb::flow::multifunction_node>(10); }

#if __TBB_PREVIEW_FLOW_GRAPH_NODE_SET
//! Test follows and precedes API
//! \brief \ref error_guessing
TEST_CASE("Test follows-precedes API") { test_follows_and_precedes_api(); }
...
TEST_CASE("Test priority with follows and precedes") {
    ...
    multinode node(
            precedes(b1, b2), unlimited,
            [](const int& i, multinode::output_ports_type& op) -> void {
                if (i % 2) std::get<0>(op).try_put(i);
                else
                    std::get<1>(op).try_put(i);
            },
...
TEST_CASE("constraints for multifunction_node input") {
    ...
    static_assert(!utils::well_formed_instantiation<tbb::flow::multifunction_node, test_concepts::NonCopyable,
                                                    int>);
    static_assert(!utils::well_formed_instantiation<tbb::flow::multifunction_node,
                                                    test_concepts::NonDefaultInitializable, int>);
}

template <typename Input, typename Output, typename Body>
concept can_call_multifunction_node_ctor
...
TEST_CASE("constraints for multifunction_node body") {
    ...
    static_assert(
            can_call_multifunction_node_ctor<input_type, output_type, Correct<input_type, output_type>>);
    static_assert(
            !can_call_multifunction_node_ctor<input_type, output_type, NonCopyable<input_type, output_type>>);
    static_assert(!can_call_multifunction_node_ctor<input_type, output_type,
                                                    NonDestructible<input_type, output_type>>);
    static_assert(!can_call_multifunction_node_ctor<input_type, output_type,
                                                    NoOperatorRoundBrackets<input_type, output_type>>);
    static_assert(
            !can_call_multifunction_node_ctor<input_type, output_type,
                                              WrongFirstInputOperatorRoundBrackets<input_type, output_type>>);
    static_assert(!can_call_multifunction_node_ctor<
                  input_type, output_type, WrongSecondInputOperatorRoundBrackets<input_type, output_type>>);
}
