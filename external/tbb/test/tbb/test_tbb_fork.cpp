...
int main() {
    ...
    if (sigaction(SIGCHLD, &sa, nullptr)) ASSERT(0, "sigaction failed");
    if (sigaction(SIGALRM, &sa, nullptr)) ASSERT(0, "sigaction failed");
    // block SIGCHLD and SIGALRM, the mask is inherited by worker threads
    sigemptyset(&sig_set);
    sigaddset(&sig_set, SIGCHLD);
    sigaddset(&sig_set, SIGALRM);
    if (pthread_sigmask(SIG_BLOCK, &sig_set, nullptr)) ASSERT(0, "pthread_sigmask failed");
#endif
    ...
    for (int threads = MinThread; threads <= MaxThread; threads += MinThread) {
        for (int i = 0; i < 20; i++) {
            ...
            for (TidTableType::const_iterator it = tidTable.begin(); it != tidTable.end(); ++it) {
                if (masterTid != it->first) { ASSERT(threadTerminated(it->second.h), nullptr); }
            }
            tidTable.clear();
#else // _WIN32||_WIN64
            if (child) exit(0);
