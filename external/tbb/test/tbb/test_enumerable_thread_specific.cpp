...
public:
    minimalN() : utils::NoAssign(), my_value(0) {
        ++construction_counter;
        is_constructed = true;
    }
    minimalN(const minimalN& m) : utils::NoAssign(), my_value(m.my_value) {
        ++construction_counter;
        is_constructed = true;
    }
    ~minimalN() {
...
void do_std_threads(int max_threads, flogged_ets a[]) {
    ...
    for (int p = 0; p < max_threads; ++p) { threads.push_back(new std::thread(set_body(a))); }

    for (int p = 0; p < max_threads; ++p) { threads[p]->join(); }

    for (int p = 0; p < max_threads; ++p) { delete threads[p]; }
}
...
void flog_segmented_interator() {
    ...
    for (int i = 0; i < maxval; i++) {
        ...
        for (int j = 0; j < maxval; j++) { my_vec.at(i).push_back((T)(maxval * i + j)); }
    }
    ...
    for (my_si = my_vec.begin(), ii = 0; my_si != my_vec.end(); ++my_si, ++ii) {
        if ((*my_si) != ii) { found_error = true; }
    }

    // outer level empty
    my_vec.clear();
    for (my_si = my_vec.begin(); my_si != my_vec.end(); ++my_si) { found_error = true; }

    // inner levels empty
    my_vec.clear();
    for (int i = 0; i < maxval; ++i) { my_vec.push_back(my_inner_container); }
    for (my_si = my_vec.begin(); my_si != my_vec.end(); ++my_si) { found_error = true; }
    ...
    for (int i = 0; i < maxval; ++i) {
        my_vec.push_back(my_inner_container);
        if (i % 2) {
            for (int j = 0; j < maxval; ++j) { my_vec.at(i).push_back((T)(maxval * (i / 2) + j)); }
        }
    }
    for (my_si = my_vec.begin(), ii = 0; my_si != my_vec.end(); ++my_si, ++ii) {
        if ((*my_si) != ii) { found_error = true; }
    ...
    for (my_csi = my_vec.begin(), ii = 0; my_csi != my_vec.end(); ++my_csi, ++ii) {
        if ((*my_csi) != ii) { found_error = true; }
    }

    // outer level empty
    my_vec.clear();
    for (my_csi = my_vec.begin(); my_csi != my_vec.end(); ++my_csi) { found_error = true; }
...
void flog_segmented_iterator_map() {
    ...
    Key ii;
    for (my_si = my_vec.begin(), ii = 0; my_si != my_vec.end(); ++my_si, ++ii) {
        if (((*my_si).first != ii) || ((*my_si).second != 2 * ii)) { found_error = true; }
    }
...
void allocate_ets_element_on_stack(const char* /* name */) {
    ...
    REQUIRE_MESSAGE(tbb::detail::is_aligned(my_stack_element.value(), my_align),
                    "Error in first stack alignment");
    REQUIRE_MESSAGE(tbb::detail::is_aligned(my_stack_element2.value(), my_align),
                    "Error in second stack alignment");
    REQUIRE_MESSAGE(tbb::detail::is_aligned(mystruct1.my_struct_element.value(), my_align),
                    "Error in struct element alignment");
}
