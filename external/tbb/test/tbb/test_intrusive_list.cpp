...
using intrusive_list2
        = tbb::detail::r1::memptr_intrusive_list<DataItemWithMemberNodes, DataItemWithMemberNodeBase,
                                                 &DataItemWithMemberNodeBase::m_BaseNode>;

using intrusive_list3
        = tbb::detail::r1::memptr_intrusive_list<DataItemWithMemberNodes, DataItemWithMemberNodes,
...
void check_list_nodes(List& il, int value_step) {
    ...
    REQUIRE_MESSAGE(i == NumElements + value_step - 1,
                    "Wrong number of list elements while iterating forward");
    it = il.end();
...
void test_list_operations() {
    ...
    for (int i = NumElements - 1; i >= 0; --i) { il.push_front(*new Item(i)); }
