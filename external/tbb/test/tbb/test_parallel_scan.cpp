...
inline int TriangularSum(int i) { return i & 1 ? ((i >> 1) + 1) * i : (i >> 1) * (i + 1); }
...
void VerifySum(int init, long finish_index, int sum, int line) {
    ...
    CHECK_MESSAGE(expected == sum, "line " << line << ": sum[0.." << finish_index << "] should be = "
                                           << expected << ", but was computed as " << sum << "\n");
}

const int MAXN = 20000;

enum AddendFlag { UNUSED = 0, USED_NONFINAL = 1, USED_FINAL = 2 };
...
struct Storage {
    T my_total;
    Range my_range;
    Storage(T init) : my_total(init), my_range(-1, -1, 1) { ++NumberOfLiveStorage; }
    ~Storage() { --NumberOfLiveStorage; }
    Storage(const Storage& strg) : my_total(strg.my_total), my_range(strg.my_range) { ++NumberOfLiveStorage; }
...
template <typename T>
void Scan(const Range& r, bool is_final, Storage<T>& storage, std::vector<T>& sum,
          const std::vector<T>& addend) {
    CHECK((!is_final || (storage.my_range.begin() == 0 && storage.my_range.end() == r.begin())
           || (storage.my_range.empty() && r.begin() == 0)));
    ...
    if (storage.my_range.empty()) storage.my_range = r;
    else
        storage.my_range = Range(storage.my_range.begin(), r.end(), 1);
}

template <typename T>
Storage<T> ScanWithInit(const Range& r, T init, bool is_final, Storage<T>& storage, std::vector<T>& sum,
                        const std::vector<T>& addend) {
    if (r.begin() == 0) storage.my_total = init;
...
public:
    ...
    Accumulator(Accumulator& a, tbb::split)
        ...
        if (!(a.my_state == partial)) CHECK(a.my_state == full);
        if (!(a.my_state == full)) CHECK(a.my_state == partial);
        CHECK(ScanIsRunning);
        // Set self as last action of constructor, to indicate that object is fully constructed.
        self = this;
    }
    ...
    void operator()(const Range& r, Tag /*tag*/) {
        if (Tag::is_final_scan()) CHECK(my_state == full);
        else
    ...
    T get_total() { return storage.my_total; }
};

template <typename T, typename Scan, typename ReverseJoin>
T ParallelScanFunctionalInvoker(const Range& range, T idx, const Scan& scan, const ReverseJoin& reverse_join,
                                int mode) {
    switch (mode % 3) {
        case 0:
...
public:
    ScanBody(T init, const std::vector<T>& addend, std::vector<T>& sum)
        : my_addend(addend), my_sum(sum), my_init(init) {}
...
struct ParallelScanLambda {
    ...
    T operator()(Range range, T init, const std::vector<T>& addend, std::vector<T>& sum, int mode) {
        for (long i = 0; i < MAXN; ++i) { AddendHistory[i] = UNUSED; }
        ScanIsRunning = true;
        Storage<T> res = ParallelScanFunctionalInvoker(
                range, Storage<T>(0),
                [&addend, &sum, init](const Range& r, Storage<T> storage, bool is_final_scan /*tag*/)
                        -> Storage<T> { return ScanWithInit(r, init, is_final_scan, storage, sum, addend); },
...
void TestAccumulator(int mode) {
    ...
    for (int n = 0; n <= MAXN; n = n <= 128 ? n + 1 : n * 3) {
        for (int gs : {1, 2, 100, 511, 12345, n / 111, n / 17, n - 1, n}) {
            ...
            for (long i = 0; i < n; ++i)
                CHECK_MESSAGE((AddendHistory[i] & USED_FINAL),
                              "failed to use addend["
                                      << i << "] "
                                      << (AddendHistory[i] & USED_NONFINAL ? "(but used nonfinal)\n" : "\n"));
            for (long i = 0; i < n; ++i) { VerifySum(42, i, sum[i], __LINE__); }
            if (n) CHECK(acc.get_total() == sum[n - 1]);
            else
...
struct ParallelScanGenericLambda {
    ...
    T operator()(Range range, T init, const std::vector<T>& addend, std::vector<T>& sum, int mode) {
        ...
        Storage<T> res = ParallelScanFunctionalInvoker(
                ...
                [](const auto& left, const auto& right) { return JoinStorages(left, right); }, mode);
        ScanIsRunning = false;
        if (range.empty()) res.my_total = init;
        return res.my_total;
    }
...
concept can_call_functional_pscan
        = can_call_parallel_scan_basic<const Range&, const Value&, const Func&, const Combine&>&&
                can_call_parallel_scan_basic<const Range&, const Value&, const Func&, const Combine&,
...
void test_pscan_range_constraints() {
    ...
    static_assert(
            !can_call_functional_pscan<NonCopyable, int, CorrectFunc<NonCopyable, int>, CorrectCombine<int>>);
    static_assert(!can_call_functional_pscan<NonDestructible, int, CorrectFunc<NonDestructible, int>,
                                             CorrectCombine<int>>);
    static_assert(!can_call_functional_pscan<NonSplittable, int, CorrectFunc<NonSplittable, int>,
                                             CorrectCombine<int>>);
    static_assert(!can_call_functional_pscan<NoEmpty, int, CorrectFunc<NoEmpty, int>, CorrectCombine<int>>);
    static_assert(!can_call_functional_pscan<EmptyNonConst, int, CorrectFunc<EmptyNonConst, int>,
                                             CorrectCombine<int>>);
    static_assert(!can_call_functional_pscan<WrongReturnEmpty, int, CorrectFunc<WrongReturnEmpty, int>,
                                             CorrectCombine<int>>);
    static_assert(!can_call_functional_pscan<NoIsDivisible, int, CorrectFunc<NoIsDivisible, int>,
                                             CorrectCombine<int>>);
    static_assert(!can_call_functional_pscan<IsDivisibleNonConst, int, CorrectFunc<IsDivisibleNonConst, int>,
                                             CorrectCombine<int>>);
    static_assert(!can_call_functional_pscan<WrongReturnIsDivisible, int,
                                             CorrectFunc<WrongReturnIsDivisible, int>, CorrectCombine<int>>);
}
...
void test_pscan_body_constraints() {
    ...
    static_assert(!can_call_imperative_pscan<CorrectRange,
                                             WrongFirstInputPreScanOperatorRoundBrackets<CorrectRange>>);
    static_assert(!can_call_imperative_pscan<CorrectRange,
                                             WrongSecondInputPreScanOperatorRoundBrackets<CorrectRange>>);
    static_assert(!can_call_imperative_pscan<CorrectRange, NoFinalScanOperatorRoundBrackets<CorrectRange>>);
    static_assert(!can_call_imperative_pscan<CorrectRange,
                                             WrongFirstInputFinalScanOperatorRoundBrackets<CorrectRange>>);
    static_assert(!can_call_imperative_pscan<CorrectRange,
                                             WrongSecondInputFinalScanOperatorRoundBrackets<CorrectRange>>);
...
void test_pscan_func_constraints() {
    ...
    static_assert(
            can_call_functional_pscan<CorrectRange, int, Correct<CorrectRange, int>, CorrectCombine<int>>);
    static_assert(!can_call_functional_pscan<CorrectRange, int, NoOperatorRoundBrackets<CorrectRange, int>,
                                             CorrectCombine<int>>);
    static_assert(
            !can_call_functional_pscan<CorrectRange, int, OperatorRoundBracketsNonConst<CorrectRange, int>,
                                       CorrectCombine<int>>);
    static_assert(!can_call_functional_pscan<CorrectRange, int,
                                             WrongFirstInputOperatorRoundBrackets<CorrectRange, int>,
                                             CorrectCombine<int>>);
    static_assert(!can_call_functional_pscan<CorrectRange, int,
                                             WrongSecondInputOperatorRoundBrackets<CorrectRange, int>,
                                             CorrectCombine<int>>);
    static_assert(
            !can_call_functional_pscan<CorrectRange, int, WrongReturnOperatorRoundBrackets<CorrectRange, int>,
                                       CorrectCombine<int>>);
}
...
void test_pscan_combine_constraints() {
    ...
    static_assert(!can_call_functional_pscan<CorrectRange, int, CorrectFunc<CorrectRange, int>,
