...
void TestPrimitiveTypes(std::size_t nthread, T exemplar) {
    CQ queue;
    for (std::size_t i = 0; i < 100; ++i) { queue.push(exemplar); }
...
struct throw_element {
    ...
    throw_element(const throw_element&) {
        if (global_counter++ == rnd_elem) { throw std::exception {}; }
    }
...
void CopyWithThrowElement() {
    ...
    for (std::size_t i = 0; i < queue_size; ++i) { source.emplace(); }
...
TEST_CASE("testing work with different fypes") { TestQueueWorksWithPrimitiveTypes(); }

//! Test work with vector types
//! \brief \ref error_guessing
TEST_CASE("testing vector types") { TestQueueWorksWithSSE(); }
...
TEST_CASE("Test exception in allocation") {
    ...
    for (int i = 0; i < 100000; ++i) { src_queue.push(i); }
...
void fill_and_catch(Container& q, std::size_t elements_count) {
    ...
    for (std::size_t i = 0; i < elements_count; ++i) {
        ...
        try {
            ...
            q.emplace();
#if TBB_USE_EXCEPTIONS
        } catch (int exception) { CHECK(exception == 1); }
#endif
    }
#if TBB_USE_EXCEPTIONS
