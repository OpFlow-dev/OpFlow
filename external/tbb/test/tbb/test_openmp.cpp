...
void SerialConvolve(data_type c[], const data_type a[], int m, const data_type b[], int n) {
    for (int i = 0; i < m + n - 1; ++i) {
        ...
        data_type sum = 0;
        for (int j = start; j < finish; ++j) sum += a[j] * b[i - j];
        c[i] = sum;
    }
}
...
public:
    ...
    InnerBody(InnerBody& x, tbb::split) : my_a(x.my_a), my_b(x.my_b), i(x.i), sum(0) {}
    void join(InnerBody& x) { sum += x.sum; }
    void operator()(const tbb::blocked_range<int>& range) {
        for (int j = range.begin(); j != range.end(); ++j) sum += my_a[j] * my_b[i - j];
    }
};
...
#pragma omp parallel for reduction(+ : sum) num_threads(p)
            for (int j = start; j < finish; ++j) sum += my_a[j] * my_b[i - j];
            my_c[i] = sum;
        }
    }
};
