...
public:
    counting_array_receiver(tbb::flow::graph& g) : my_graph(g) {
        for (int i = 0; i < N; ++i) my_counters[i] = 0;
    }
    ...
    tbb::flow::graph& graph_reference() const override { return my_graph; }
};
...
void test_serial_broadcasts() {
    ...
    for (int num_receivers = 1; num_receivers < R; ++num_receivers) {
        ...
        for (int r = 0; r < num_receivers; ++r) { tbb::flow::make_edge(b, *receivers[r]); }

        for (int n = 0; n < N; ++n) { CHECK_MESSAGE(b.try_put((T) n), ""); }

        for (int r = 0; r < num_receivers; ++r) {
            for (int n = 0; n < N; ++n) { CHECK_MESSAGE((*receivers[r])[n] == 1, ""); }
            tbb::flow::remove_edge(b, *receivers[r]);
        }
        CHECK_MESSAGE(b.try_put((T) 0), "");
        for (int r = 0; r < num_receivers; ++r) CHECK_MESSAGE((*receivers[0])[0] == 1, "");
    }
}
...
public:
    ...
    void operator()(int) const {
        for (int n = 0; n < N; ++n) { CHECK_MESSAGE(my_b.try_put((T) n), ""); }
    }
};
...
void run_parallel_broadcasts(tbb::flow::graph& g, int p, tbb::flow::broadcast_node<T>& b) {
    for (int num_receivers = 1; num_receivers < R; ++num_receivers) {
        ...
        for (int r = 0; r < num_receivers; ++r) {
            for (int n = 0; n < N; ++n) { CHECK_MESSAGE((int) (*receivers[r])[n] == p, ""); }
            tbb::flow::remove_edge(b, *receivers[r]);
        }
        CHECK_MESSAGE(b.try_put((T) 0), "");
        for (int r = 0; r < num_receivers; ++r) CHECK_MESSAGE((int) (*receivers[r])[0] == p, "");
    }
}
...
void test_resets() {
    ...
    for (int testNo = 0; testNo < 2; ++testNo) {
        for (T i = 0; i <= 3; i += 1) { b0.try_put(i); }
        g.wait_for_all();
        for (T i = 0; i <= 3; i += 1) { CHECK_MESSAGE((q0.try_get(j) && j == i), "Bad value in queue"); }
    ...
    for (T i = 0; i <= 3; i += 1) { b1.try_put(i); }
    g.wait_for_all();
    CHECK_MESSAGE((!q0.try_get(j)), "edge between nodes not removed");
}
...
void test_follows_and_precedes_api() {
    ...
    follows_and_precedes_testing::test_precedes<msg_t, tbb::flow::broadcast_node<msg_t>>(
...
void test_try_put_and_wait_spawning_and_serial_receiver() {
    ...
    arena.execute([&] {
        ...
        tbb::flow::function_node<int, int, tbb::flow::queueing> unlimited(
                g, tbb::flow::unlimited, [&](int input) noexcept {
                    if (input == wait_message) {
                        for (auto item : new_work_items) { broadcast.try_put(item); }
                    }
        ...
        tbb::flow::function_node<int, int, tbb::flow::queueing> serial(
                g, tbb::flow::serial, [&](int input) noexcept {
                    processed_items_serial.emplace_back(input);
                    return 0;
                });
        tbb::flow::make_edge(broadcast, serial);

        for (int i = 0; i < wait_message; ++i) { broadcast.try_put(i); }
        ...
        CHECK_MESSAGE(processed_items_unlimited[unlimited_check_index++] == wait_message,
                      "Unexpected items processing");
        for (int i = int(new_work_items.size()) - 1; i >= 0; --i) {
            CHECK_MESSAGE(processed_items_unlimited[unlimited_check_index++] == new_work_items[i],
                          "Unexpected items processing");
        }
        for (int i = int(start_work_items.size()) - 1; i >= 1; --i) {
            CHECK_MESSAGE(processed_items_unlimited[unlimited_check_index++] == start_work_items[i],
                          "Unexpected items processing");
        }
        ...
        for (auto item : start_work_items) {
            CHECK_MESSAGE(processed_items_serial[serial_check_index++] == item,
                          "Unexpected items processing");
        }
        CHECK_MESSAGE(processed_items_serial[serial_check_index++] == wait_message,
                      "Unexpected items processing");

        g.wait_for_all();

        CHECK_MESSAGE(processed_items_unlimited[unlimited_check_index++] == start_work_items[0],
                      "Unexpected items processing");
...
void test_try_put_and_wait_spawning_receivers() {
    ...
    arena.execute([&] {
        ...
        for (int i = 0; i < num_successors; ++i) {
            successors.emplace_back(g, tbb::flow::unlimited, [&, i](int input) noexcept {
                if (input == wait_message) { broadcast.try_put(new_work_items[i]); }
        ...
        for (int i = num_successors - 1; i >= 0; --i) {
            ...
            for (int j = num_successors - 1; j != i; --j) {
                CHECK_MESSAGE(processed_items[i][check_index++] == new_work_items[j],
                              "Unexpected items processing");
            }
        ...
        for (auto& processed_item : processed_items) {
            ...
            for (int i = int(start_work_items.size()) - 1; i >= 0; --i) {
                CHECK_MESSAGE(processed_item[check_index++] == start_work_items[i],
                              "Unexpected items processing");
            }
        }
    });
...
#if __TBB_PREVIEW_FLOW_GRAPH_TRY_PUT_AND_WAIT
//! \brief \ref error_guessing
TEST_CASE("test broadcast_node try_put_and_wait") { test_try_put_and_wait(); }
#endif
