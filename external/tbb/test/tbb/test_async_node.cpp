...
public:
    ...
    minimal_type& operator=(const minimal_type& m) {
        value = m.value;
        return *this;
    }
};
...
struct place_wrapper {
    ...
    template <typename Q>
    place_wrapper(const place_wrapper<Q>& v) : value(v.value), thread_id(v.thread_id) {}
...
struct wrapper_helper<place_wrapper<T1>, place_wrapper<T2>> {
    ...
    static void copy_value(const place_wrapper<T1>& in, place_wrapper<T2>& out) { out.value = in.value; }
};
...
struct counting_async_serial_body : counting_async_unlimited_body {
    ...
    counting_async_serial_body(tbb::task_group_context& tgc) : base_type(tgc), my_async_body_exec_count(0) {}
...
void test_reset() {
    ...
    for (size_t i = 0; i < R; ++i) { r.push_back(std::make_shared<harness_counting_receiver<int>>(g)); }

    for (int i = 0; i < R; ++i) { tbb::flow::make_edge(a, *r[i]); }

    INFO("One body execution\n");
    a.try_put(-1);
    for (int i = 0; i < N; ++i) { a.try_put(i); }
    g.wait_for_all();
    ...
    CHECK_MESSAGE((int(async_body_exec_count) == int(b1.my_async_body_exec_count)),
                  "body and global body counts are different");
...
public:
    ...
    void stop() { my_quit = true; }

    void activate() { is_active = true; }

    bool should_reserve_each_time() {
        if (my_expected_items == UNKNOWN_NUMBER_OF_ITEMS) return true;
        else
            return false;
    }
...
struct basic_test {
    ...
    static int run(int async_expected_items = UNKNOWN_NUMBER_OF_ITEMS) {
        ...
        tbb::flow::function_node<int, input_type> start_node(g, tbb::flow::unlimited,
                                                             [](int input) { return input_type(input); });
        async_node_type offload_node(g, tbb::flow::unlimited,
        ...
        tbb::flow::function_node<output_type> end_node(
                g, tbb::flow::unlimited, [&](const output_type& input) {
                    ++end_body_exec_count;
                    output_type output;
        ...
        if (async_expected_items != UNKNOWN_NUMBER_OF_ITEMS) { offload_node.gateway().reserve_wait(); }
        for (int i = 0; i < NUMBER_OF_MSGS; ++i) { start_node.try_put(i); }
        g.wait_for_all();
        CHECK_MESSAGE((async_body_exec_count == NUMBER_OF_MSGS),
                      "AsyncBody processed wrong number of signals");
        CHECK_MESSAGE((async_activity_processed_msg_count == NUMBER_OF_MSGS),
...
int test_copy_ctor() {
    ...
    CHECK_MESSAGE((int(async_body_exec_count) == NUMBER_OF_MSGS),
                  "AsyncBody processed wrong number of signals");
    ...
    for (int i = 0; i < N; ++i) { b.try_put(i); }
    g.wait_for_all();

    INFO("async_body_exec_count = " << int(async_body_exec_count) << "\n");
    INFO("r1.my_count == " << int(r1.my_count) << " and r2.my_count = " << int(r2.my_count) << "\n");
    CHECK_MESSAGE((int(async_body_exec_count) == 2 * NUMBER_OF_MSGS),
                  "AsyncBody processed wrong number of signals");
    CHECK_MESSAGE((int(r1.my_count) == N), "counting receiver r1 has not received N items");
...
struct spin_test {
    ...
    public:
        ...
        void operator()(const output_type&) {
            ++end_body_exec_count;
            if (std::this_thread::get_id() == my_main_tid) { ++main_tid_count; }
            my_barrier->wait();
        }
    };
    ...
    static int run(int nthreads, int async_expected_items = UNKNOWN_NUMBER_OF_ITEMS) {
        ...
        if (async_expected_items != UNKNOWN_NUMBER_OF_ITEMS) { offload_node.gateway().reserve_wait(); }
        for (int i = 0; i < overall_message_count; ++i) { start_node.try_put(i); }
        g.wait_for_all();
...
void test_for_spin_avoidance() {
    ...
    a.execute([&] { spin_test<int, int>::run(nthreads); });
}
...
public:
    ...
    AsyncActivity(size_t limit)
        : stop_limit(limit), c(0), thr([this]() {
              while (!end_of_work()) {
                  work_type w;
...
void test_follows() {
    ...
    std::array<broadcast_node<input_t>, 3> preds
            = {{broadcast_node<input_t>(g), broadcast_node<input_t>(g), broadcast_node<input_t>(g)}};

    node_t node(
            follows(preds[0], preds[1], preds[2]), unlimited,
            [&](int input, node_t::gateway_type& gtw) { async_activity.submit(input, &gtw); }, no_priority);

    buffer_node<output_t> buf(g);
    make_edge(node, buf);

    for (auto& pred : preds) { pred.try_put(1); }
    ...
    output_t storage;
    CHECK_MESSAGE(
            (buf.try_get(storage) && buf.try_get(storage) && buf.try_get(storage) && !buf.try_get(storage)),
...
void test_precedes() {
    ...
    node_t node(
            precedes(successors[0]), unlimited,
            [&](int input, node_t::gateway_type& gtw) { async_activity.submit(input, &gtw); }, no_priority);
...
//! \brief \ref requirement \ref error_guessing
TEST_CASE("Lightweight tests") { lightweight_testing::test<tbb::flow::async_node>(NUMBER_OF_MSGS); }

//! Test reset and cancellation
//! \brief \ref error_guessing
TEST_CASE("Reset test") { test_reset(); }

//! Test
//! \brief \ref requirement \ref error_guessing
TEST_CASE("Copy constructor test") { test_copy_ctor(); }

//! Test if main thread spins
//! \brief \ref stress
TEST_CASE("Spin avoidance test") { test_for_spin_avoidance(); }

//! Test nested enqueuing
//! \brief \ref error_guessing
TEST_CASE("Inner enqueuing test") { run_test_enqueueing_on_inner_level(); }

#if __TBB_PREVIEW_FLOW_GRAPH_NODE_SET
//! Test deprecated follows and preceedes API
//! \brief \ref error_guessing
TEST_CASE("Test follows and preceedes API") { test_follows_and_precedes_api(); }
#endif
...
TEST_CASE("constraints for async_node input") {
    ...
    static_assert(!utils::well_formed_instantiation<tbb::flow::async_node,
                                                    test_concepts::NonDefaultInitializable, int>);
}

template <typename Input, typename Output, typename Body>
concept can_call_async_node_ctor
...
TEST_CASE("constraints for async_node body") {
    ...
    static_assert(
            !can_call_async_node_ctor<input_type, output_type, NonDestructible<input_type, output_type>>);
    static_assert(!can_call_async_node_ctor<input_type, output_type,
                                            NoOperatorRoundBrackets<input_type, output_type>>);
    static_assert(!can_call_async_node_ctor<input_type, output_type,
                                            WrongFirstInputOperatorRoundBrackets<input_type, output_type>>);
    static_assert(!can_call_async_node_ctor<input_type, output_type,
                                            WrongSecondInputOperatorRoundBrackets<input_type, output_type>>);
}
