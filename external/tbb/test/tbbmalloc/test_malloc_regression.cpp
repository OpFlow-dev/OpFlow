...
bool TestBootstrapLeak() {
    ...
    */
    ...
    for (int i = 0; i < take_out_count; ++i) array[i] = a.allocate(alloc_size);
    ...
    for (;;) {
        memory_in_use = utils::GetMemoryUsage();
        for (int i = 0; i < num_thread_runs; ++i) RunThread(minimalAllocFree(), alloc_size);
    ...
    for (int i = 0; i < take_out_count; ++i) a.deallocate(array[i], alloc_size);
...
bool TestReallocMsize(size_t startSz) {
    ...
    REQUIRE_MESSAGE((scalable_msize(buf1) >= 2 * realSz),
    ...
    for (int a = 0; align[a]; a++)
        for (int s = 0; allocSz[s]; s++) {
            ...
            for (int i = 0; i < NUM; i++) {
                ...
                CHECK_FAST_MESSAGE(objSizes[i] >= allocSz[s],
            ...
            for (int i = 0; i < NUM; i++) scalable_free(p[i]);
        }
}
...
TEST_CASE("testing leaks") {
    ...
    if (utils::GetMemoryUsage()) { REQUIRE_MESSAGE(TestBootstrapLeak(), "Test failed"); }
}
...
TEST_CASE("testing realloc mem size") {
    ...
    for (size_t a = 2; a <= 64 * 1024; a *= 2) { passed &= TestReallocMsize(a); }
    REQUIRE_MESSAGE(passed, "Test failed");
}

//! \brief \ref error_guessing
TEST_CASE("testing memory align") { TestAlignedMsize(); }
