...
extern "C" void scalable_free(void *) { ASSERT(0, globalCallMsg); }
extern "C" void safer_scalable_free(void *, void (*)(void *)) { ASSERT(0, globalCallMsg); }
...
extern "C" void scalable_aligned_free(void *) { ASSERT(0, globalCallMsg); }
...
#if _WIN32 || _WIN64
        ASSERT(GetModuleHandle(actual_name), "allocator library must not be unloaded");
#else
        ASSERT(dlsym(RTLD_DEFAULT, "scalable_malloc"), "allocator library must not be unloaded");
#endif
    }
};
...
TEST_CASE("test unload lib") {
...

    {
        ...
        for (;;) {
            std::size_t memory_in_use = utils::GetMemoryUsage();
            for (int j = 0; j < 10; j++) utils::NativeParallelFor(1, Run());
            memory_leak = utils::GetMemoryUsage() - memory_in_use;
            if (memory_leak == 0) return;
        }
    }
}
