...
public:
    ...
    void check() const {
        for (int k = 0; k < size; k++) ASSERT(p[k] == val, nullptr);
    }
...
void *getMem(intptr_t /*pool_id*/, size_t &bytes) {
    ...
    if (pos + bytes > BUF_SIZE) return nullptr;
...
public:
    ...
    rml::internal::LargeMemoryBlock *next() {
        if (numOps--) {
            if (lmbArray.empty() || rand() / (RAND_MAX >> 1) == 0) return nullptr;
...
namespace FunctionReplacement {
    ...
    void LogCleanup() {
        ...
        for (unsigned i = 0; i < Log::RECORDS_COUNT + 1; i++) { Log::records[i] = nullptr; }
...
private:
    bool cacheBinEmpty(int idx) {
        return (loc->hugeCache.bin[idx].cachedSize.load(std::memory_order_relaxed) == 0
                && loc->hugeCache.bin[idx].get() == nullptr);
    }
