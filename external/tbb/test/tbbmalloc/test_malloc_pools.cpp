...
struct PoolSpace : utils::NoCopy {
    ...
    PoolSpace(size_t bufSz = BUF_SIZE) : pos(0), regions(0), bufSize(bufSz), space(new char[bufSize]) {
        memset(space, 0, bufSize);
    }
    ~PoolSpace() { delete[] space; }
};
...
class SharedPoolRun : utils::NoAssign {
    ...
    static utils::SpinBarrier startB, mallocDone;
    static rml::MemoryPool *pool;
    static void **crossThread, **afterTerm;
...
public:
    ...
    void operator()(int id) const {
        ...
        for (int i = myVictim * OBJ_CNT; i < (myVictim + 1) * OBJ_CNT; i++) pool_free(pool, crossThread[i]);
        for (int i = 0; i < ITERS; i++) pool_free(pool, local[i]);
    }
};

long SharedPoolRun::threadNum;
utils::SpinBarrier SharedPoolRun::startB, SharedPoolRun::mallocDone;
rml::MemoryPool *SharedPoolRun::pool;
void **SharedPoolRun::crossThread, **SharedPoolRun::afterTerm;
...
void TestSharedPool() {
    ...
    for (int p = utils::MinThread; p <= utils::MaxThread; p++) {
        ...
        for (int i = 0; i < p * SharedPoolRun::OBJ_CNT; i++) pool_free(pool, afterTerm[i]);
    }
    delete[] afterTerm;
    delete[] crossThread;
...
void *CrossThreadGetMem(intptr_t pool_id, size_t &bytes) {
    if (poolSpace[pool_id].pos + bytes > poolSpace[pool_id].bufSize) return nullptr;
...
public:
    ...
    void operator()(int id) const {
        ...
        for (int i = 0; i < 10 * myPool; i++) REQUIRE(myPool == obj[myPool][i]);
...
void TestCrossThreadPools() {
    for (int p = utils::MinThread; p <= utils::MaxThread; p++) {
        ...
        for (int i = 0; i < p; i++) REQUIRE_MESSAGE(!poolSpace[i].regions, "Region leak detected");
...
public:
    FixedPoolHeadBase(size_t s) : size(s), used(false) { data = new char[size]; }
...
public:
    FixedPoolUse(unsigned threads, rml::MemoryPool *p, size_t sz, int it) : pool(p), reqSize(sz), iters(it) {
...
public:
    FixedPoolNomem(utils::SpinBarrier *b, rml::MemoryPool *p) : startB(b), pool(p) {}
    void operator()(int id) const {
        startB->wait();
...
public:
    FixedPoolSomeMem(utils::SpinBarrier *b, rml::MemoryPool *p) : barrier(b), pool(p) {}
    void operator()(int id) const {
        barrier->wait();
        utils::Sleep(2 * id);
        void *o = pool_malloc(pool, id % 2 ? 64 : 128 * 1024);
...
void TestFixedBufferPool() {
    ...
    rml::MemoryPool *pool;
    {
        ...
        pool_create_v1((intptr_t) &head, &pol, &pool);
        {
            ...
            for (int i = 0; i < ITERS; i++) pool_free(pool, ptrs[i]);
        ...
        }
        {
            ...
            for (l = MAX_OBJECT / 2, r = MAX_OBJECT + 1024 * 1024; l < r - 1;) {
                size_t mid = (l + r) / 2;
                if (haveEnoughSpace(pool, mid)) l = mid;
                else
                    r = mid;
            }
            maxSz = l;
...
void TestPoolKeepTillDestroy() {
    ...
    for (int keep = 0; keep < 2; keep++) {
        ...
        REQUIRE(!putMemAll);
        for (int i = 0; i < 2 * ITERS; i++) pool_free(pool, ptrs[i]);
        pool_free(pool, ptrs[2 * ITERS]);
        size_t totalPutMemCalls = putMemAll;
        if (keep) REQUIRE(!putMemAll);
        else {
            REQUIRE(putMemAll);
            putMemAll = 0;
        }
        ...
        REQUIRE(p);
        if (keep) REQUIRE_MESSAGE(getCallsBefore == getMemAll, "Must not lead to new getMem call");
...
rml::MemoryPool *CreateUsablePool(size_t size) {
    ...
    REQUIRE_MESSAGE((getMemSuccessful == 1 || getMemSuccessful == 5 || getMemAll > getMemSuccessful),
                    "Multiple requests are allowed when unsuccessful request occurred or cannot search in "
                    "bootstrap memory. ");
    REQUIRE(!putMemAll);
    pool_free(pool, o);
...
void TestPoolCreation() {
    ...
    rml::MemPoolPolicy nullPolicy(nullptr, putMemFree), emptyFreePolicy(getMemMalloc, nullptr),
...
void TestPoolDetection() {
    ...
    for (int i = 0; i < POOLS; i++) pool_create_v1((intptr_t)(head + i), &pol, &pools[i]);
    ...
    for (int k = 0; k < 10; k++) {
        ...
        if (k & 1) utils::NativeParallelFor(POOLS, check);
        else
...
public:
    ...
    void operator()(int id) const {
        ...
        if (!id) {
            bool ok = pool_destroy(pool);
            REQUIRE(ok);
            REQUIRE_MESSAGE(!liveRegions.load(std::memory_order_relaxed),
                            "Expected all regions were released.");
        }
...
TEST_CASE("Too small buffer") { TestTooSmallBuffer(); }

//! \brief \ref error_guessing
TEST_CASE("Pool reset") { TestPoolReset(); }
TEST_CASE("Shared pool") { TestSharedPool(); }

//! \brief \ref error_guessing
TEST_CASE("Cross thread pools") { TestCrossThreadPools(); }

//! \brief \ref interface
TEST_CASE("Fixed buffer pool") { TestFixedBufferPool(); }

//! \brief \ref interface
TEST_CASE("Pool granularity") { TestPoolGranularity(); }

//! \brief \ref error_guessing
TEST_CASE("Keep pool till destroy") { TestPoolKeepTillDestroy(); }

//! \brief \ref error_guessing
TEST_CASE("Entries") { TestEntries(); }

//! \brief \ref interface
TEST_CASE("Pool creation") { TestPoolCreation(); }

//! \brief \ref error_guessing
TEST_CASE("Pool detection") { TestPoolDetection(); }

//! \brief \ref error_guessing
TEST_CASE("Lazy bootstrap") { TestLazyBootstrap(); }

//! \brief \ref error_guessing
TEST_CASE("No leak on destroy") { TestNoLeakOnDestroy(); }

//! \brief \ref error_guessing
TEST_CASE("Destroy failed") { TestDestroyFailed(); }

//! \brief \ref interface
TEST_CASE("Pool msize") { TestPoolMSize(); }
