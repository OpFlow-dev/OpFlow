...
struct GlobalObject {
    ~GlobalObject() { utils::Sleep(rand() % MAX_DELAY); }
} go;
...
void allocatorRandomThrashing() {
    ...
    for (int i = 0; i < rand() % MAX_ITER; ++i) {
        ...
        for (int j = 0; j < rand() % ARRAY_SIZE; ++j) { arr[j] = scalable_malloc(rand() % MAX_ALLOC); }
...
void hangOnExitReproducer() {
    ...
    for (int i = 0; i < P - 1; i++) { test_arena.enqueue(allocatorRandomThrashing); }
}

#if (_WIN32 || _WIN64) && !__TBB_WIN8UI_SUPPORT
#include <process.h>// _spawnl
void processSpawn(const char* self) { _spawnl(_P_WAIT, self, self, "1", nullptr); }
...
int main(int argc, char* argv[]) {
    ...
    if (argc == 2 && strcmp(argv[1], "1") == 0) { return doctest::Context(argc, argv).run(); }

    // The number of executions is a tradeoff
    // between execution time and NBTS statistics
    const int EXEC_TIMES = 100;
    const char* self = argv[0];
    for (int i = 0; i < EXEC_TIMES; i++) { processSpawn(self); }
