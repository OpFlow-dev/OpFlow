...
extern "C" void callDll() {
    ...
    for (int i = 0; i < NUM; i++) scalable_free(ptrs[i]);
}
...
public:
    ...
    void operator()(std::size_t /*id*/) const { (*run)(); }
};

void LoadThreadsUnload() {
    utils::LIBRARY_HANDLE lib = utils::OpenLibrary(TEST_LIBRARY_NAME("_test_malloc_used_by_lib"));
...
struct UnloadCallback {
    ...
    void operator()() const { utils::CloseLibrary(lib); }
};
