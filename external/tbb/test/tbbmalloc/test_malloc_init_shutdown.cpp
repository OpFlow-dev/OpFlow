...
struct TestThread : utils::NoAssign {
    ...
    void operator()(std::size_t /*id*/) const {
        ...
        REQUIRE_MESSAGE((!prevLarge || intersectingObjects(currLarge, prevLarge, 32 * 1024)),
...
void TestKeyDtor() {
    ...
    for (int i = 0; i < 4; i++) utils::NativeParallelFor(1, TestThread(1));
...
//! \brief \ref error_guessing
TEST_CASE("test key dtor") { TestKeyDtor(); }
