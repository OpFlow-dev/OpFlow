...
void limitMem(size_t limit) {
    ...
    if (0 == SetInformationJobObject(hJob, JobObjectExtendedLimitInformation, &jobInfo, sizeof(jobInfo))) {
...
void limitMem(size_t limit) {
    ...
    if (rlim.rlim_max == (rlim_t) RLIM_INFINITY) rlim.rlim_cur = (limit > 0) ? limit * MByte : rlim.rlim_max;
    else
        rlim.rlim_cur
                = (limit > 0 && static_cast<rlim_t>(limit) < rlim.rlim_max) ? limit * MByte : rlim.rlim_max;
...
struct RoundRobin : utils::NoAssign {
    ...
    void operator()(int /*id*/) const { test.RunAllTests(number_of_threads); }
};

bool CMemTest::firstTime = true;

inline size_t choose_random_alignment() { return sizeof(void *) << (rand() % POWERS_OF_2); }
...
void ReallocParam() {
    ...
    */
    ...
    for (int j = 0; j < i; j++) Trealloc(bufs[j], 0);
}
...
void InvariantDataRealloc(bool aligned, size_t maxAllocSize, bool checkData) {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        char *ptrNew = aligned ? (char *) Taligned_realloc(ptr, sizeNew, choose_random_alignment())
    ...
    if (aligned) Taligned_realloc(ptr, 0, choose_random_alignment());
    else
        Trealloc(ptr, 0);
    Tfree(base);
}
...
void CheckReallocLeak() {
    ...
    // do bootstrap
    for (int k = 0; k < 3; k++) InvariantDataRealloc(/*aligned=*/false, 128 * MByte, /*checkData=*/false);
    ...
    for (size_t i = 0; i < words; i++)
        if (buf[i]) {
            for (unsigned b = 0; b < sizeof(intptr_t); b++)
                if (((char *) (buf + i))[b]) return sizeof(intptr_t) * i + b + 1;
        }
    for (size_t i = 0; i < tailSz; i++)
        if (bufTail[i]) { return words * sizeof(intptr_t) + i + 1; }
    return 0;
}
...
void *Taligned_malloc(size_t size, size_t alignment) {
    ...
    if (0 != ret)
        CHECK_FAST_MESSAGE(0 == ((uintptr_t) ret & (alignment - 1)), "allocation result should be aligned");
    return ret;
}
void *Taligned_realloc(void *memblock, size_t size, size_t alignment) {
...
void CMemTest::AddrArifm() {
    ...
    for (int i = 0; i < COUNT_ELEM; i++) { Tfree(arr[i].ptr); }
    //-------------------------------------------
...
void CMemTest::NULLReturn(UINT MinSize, UINT MaxSize, int total_threads) {
    ...
    */
     ...
     */
    if (perProcessLimits) limitBarrier->wait(limit_total);
    else
    ...
    */
    ...
    for (int i = 0; i < num_1024; i++) Tfree(buf_1024[i]);
    Tfree(buf_1024);
            ...
            if (tmp == NULL) {
                ...
                if (CHECK_ERRNO(errno != ENOMEM + j + 1) && !known_issue) {
                    ...
                    if (ShouldReportError())
                        REPORT("error: errno changed to %d though valid pointer was returned\n", errno);
                }
                myMemset(tmp, 0, Size);
                PointerList.push_back(MemStruct(tmp, Size));
            }
    ...
    if (CountErrors) REPORT("%s\n", strError);
        ...
        REPORT("%s\n", strOk);
    if (CountErrors) error_occurred = true;
    for (UINT i = 0; i < PointerList.size(); i++) { Tfree(PointerList[i].Pointer); }

    if (perProcessLimits) limitBarrier->wait(no_limit);
    else
        limitMem(0);
}
#endif /* #if !__APPLE__ */
...
void CMemTest::UniquePointer() {
    ...
    for (int i = 0; i < COUNT_ELEM; i++) {
        ...
        if (NULL == MasPointer[i]) MasCountElem[i] = 0;
    ...
    //----------------------------------------------------------
    //calloc
    for (int i = 0; i < COUNT_ELEM; i++) Tfree(MasPointer[i]);
    ...
    if (FullLog) REPORT("realloc....");
    for (int i = 0; i < COUNT_ELEM - 1; i++) {
        if (NonZero(MasPointer[i], sizeof(int) * MasCountElem[i])) CountErrors++;
        memset(MasPointer[i], 1, sizeof(int) * MasCountElem[i]);
    }
    if (CountErrors) REPORT("%s\n", strError);
    else if (FullLog)
...
void CMemTest::TestAlignedParameters() {
    ...
    if (Rposix_memalign) {
        ...
        REQUIRE_MESSAGE(memptr == &ret, "memptr should not be changed after unsuccessful call");
        REQUIRE(EINVAL == ret);
...
void CMemTest::RunAllTests(int total_threads) {
    ...
    if (Raligned_realloc) InvariantDataRealloc(/*aligned=*/true, 8 * MByte, /*checkData=*/true);
...
TEST_CASE("MAIN TEST") {
     ...
     */
    if (utils::LinuxKernelVersion() < 2 * 1000000 + 6 * 1000 + 10) perProcessLimits = false;
#endif
        //-------------------------------------
...
#define __TBBMALLOCDLL "tbbmalloc.dll"
    ...
    if (version_info_block_size && ((version_info_block = (char *) malloc(version_info_block_size)) != NULL)
        && GetFileVersionInfo(__TBBMALLOCDLL, NULL, version_info_block_size, version_info_block)
        && VerQueryValue(version_info_block, "\\StringFileInfo\\000004b0\\Comments", &comments_block,
                         &comments_block_size)
