...
static void *fixedBufGetMem(intptr_t pool_id, size_t &bytes) {
    if (((FixedPool *) pool_id)->used) return nullptr;
...
*/
        ...
        } catch (...) { REQUIRE_MESSAGE(false, "wrong exception type;"); }
#else
...
*/
        ...
        if (ret == rml::POOL_OK) {
            ...
            REQUIRE_MESSAGE(ret == rml::NO_MEMORY,
                            "Expected that pool either valid or have no memory to be created");
#endif
        free(buf);
    }
    ...
    } catch (...) { REQUIRE_MESSAGE(false, "wrong exception type; expected invalid_argument"); }
#endif
}
...
TEST_CASE("Exceptions") { TestAllocator<tbb::scalable_allocator<void>>(Exceptions); }
#endif /* TBB_USE_EXCEPTIONS */

//! Testing allocators thread safety (should not introduce data races)
//! \brief \ref requirements
TEST_CASE("Thread safety") { TestAllocator<tbb::scalable_allocator<void>>(ThreadSafety); }

//! Test that pools in small space are either usable or not created (i.e., exception raised)
//! \brief \ref error_guessing
TEST_CASE("Small fixed pool") { TestSmallFixedSizePool(); }

//! Test that allocator with no memory must not allocate anything.
//! \brief \ref error_guessing
TEST_CASE("Zero space pool") { TestZeroSpaceMemoryPool(); }
...
TEST_CASE("polymorphic_allocator test") {
    ...
    REQUIRE_MESSAGE(tbb::scalable_memory_resource()->is_equal(*tbb::scalable_memory_resource()),
                    "Memory that was allocated by one scalable resource should be deallocated by any other "
                    "instance.");
