...
std::tuple<size_t, double> solve(const Matrix &A, const boost::property_tree::ptree &prm,
    ...
    typedef amgcl::make_solver<Precond, amgcl::runtime::solver::wrapper<Backend>> Solver;
...
#define CALL_BLOCK_SOLVER(z, data, B)                                                                        \
    case B: {                                                                                                \
        ...
        std::tie(iters, error) = solve<amgcl::runtime::preconditioner<Backend>>(                             \
                amgcl::adapter::block_matrix<value_type>(std::tie(rows, ptr, col, val)), prm, rhs, x);       \
    } break;
