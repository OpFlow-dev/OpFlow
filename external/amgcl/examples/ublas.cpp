...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < n; ++i)
        for (int j = ptr[i], e = ptr[i + 1]; j < e; ++j) A.push_back(i, col[j], val[j]);
    prof.toc("assemble");

    prof.tic("build");
    amgcl::make_solver<amgcl::amg<amgcl::backend::builtin<double>, amgcl::coarsening::smoothed_aggregation,
