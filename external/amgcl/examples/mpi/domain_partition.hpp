...
public:
    ...
    domain_partition(point lo, point hi, int num_processes) {
        ...
        for (int i = 0; i < num_processes; ++i) rtree.insert(std::make_pair(subdomains[i], i));
    }
    ...
    size_t size(size_t process) const {
        ...
        for (int i = 0; i < NDIM; ++i) v *= hi[i] - lo[i] + 1;

        return v;
    }

    box domain(size_t process) const {
        if (process < subdomains.size()) return subdomains[process];
        else {
            point lo;
            point hi;
...
private:
    ...
    boost::geometry::index::rtree<process, boost::geometry::index::quadratic<16>> rtree;
