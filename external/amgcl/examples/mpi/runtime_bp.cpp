...
std::tuple<size_t, double> solve(const amgcl::mpi::communicator &comm, const boost::property_tree::ptree &prm,
                                 const Matrix &A) {
    typedef amgcl::backend::builtin<double> Backend;

    typedef amgcl::mpi::make_solver<amgcl::mpi::block_preconditioner<Precond<Backend>>,
...
int main(int argc, char *argv[]) {
    ...
    desc.add_options()("help,h", "Show this help.")("prm-file,P", po::value<string>(),
            ...
            "size,n", po::value<int>()->default_value(1024),
    ...
    if (single_level) prm.put("precond.class", "relaxation");

    std::tie(iters, error)
            = solve<amgcl::runtime::preconditioner>(world, prm, std::tie(chunk, ptr, col, val));
