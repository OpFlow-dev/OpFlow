...
struct renumbering {
    ...
    renumbering(const domain_partition<3> &p, const std::vector<ptrdiff_t> &d) : part(p), dom(d) {}
...
int main(int argc, char *argv[]) {
    ...
    desc.add_options()("help,h", "show help")("symbc",
            ...
            ", pastix"
#endif
            )("cd", po::bool_switch(&constant_deflation),
    ...
    try {
        if (just_relax) {
            ...
            prof.tic("setup");
            typedef amgcl::mpi::subdomain_deflation<amgcl::amg<Backend, amgcl::runtime::coarsening::wrapper,
    ...
    if (world.rank == 0) {
        ...
        log << n3 << "\t" << nt << "\t" << world.size << "\t" << tm_setup << "\t" << tm_solve << "\t" << iters
            << "\t" << std::endl;
    }
}
