...
//---------------------------------------------------------------------------
std::vector<ptrdiff_t> read_problem(const amgcl::mpi::communicator &world, const std::string &A_file,
                                    const std::string &rhs_file, const std::string &part_file,
    ...
    for (ptrdiff_t i = 0; i < n; ++i) order[i] = domain[part[i]]++;
    ...
    // Read matrix chunk
    {
        ...
        for (ptrdiff_t i = 0; i < n; ++i)
            if (part[i] == world.rank) ptr.push_back(gptr[i + 1] - gptr[i]);
...
int main(int argc, char *argv[]) {
    ...
    desc.add_options()("help,h", "show help")("matrix,A", po::value<string>()->required(),
                                              "The system matrix in binary format")(
            "rhs,f", po::value<string>(), "The right-hand side in binary format")(
            "part,s", po::value<string>()->required(), "Partitioning of the problem in MatrixMarket format")(
            "pmask,m", po::value<string>(),
            ...
            "parameter file in json format")("prm,p", po::value<std::vector<string>>()->multitoken(),
    ...
    if (vm.count("help")) {
        if (world.rank == 0) std::cout << desc << std::endl;
        return 0;
    }
    ...
    if (vm.count("prm")) {
        for (const string &v : vm["prm"].as<std::vector<string>>()) { amgcl::put(prm, v); }
    }
    ...
    std::vector<ptrdiff_t> domain = read_problem(world, vm["matrix"].as<string>(), vm["rhs"].as<string>(),
                                                 vm["part"].as<string>(), ptr, col, val, rhs);
    ...
    typedef amgcl::mpi::make_solver<
            amgcl::mpi::schur_pressure_correction<
                    amgcl::mpi::make_solver<
                            amgcl::mpi::block_preconditioner<amgcl::relaxation::as_preconditioner<
                                    Backend, amgcl::runtime::relaxation::wrapper>>,
                            amgcl::runtime::mpi::solver::wrapper<Backend>>,
                    amgcl::mpi::subdomain_deflation<amgcl::amg<Backend, amgcl::runtime::coarsening::wrapper,
    ...
    if (world.rank == 0) {
        ...
        log << domain.back() << "\t" << nt << "\t" << world.size << "\t" << tm_setup << "\t" << tm_solve
            << "\t" << iters << "\t" << std::endl;
    }
}
