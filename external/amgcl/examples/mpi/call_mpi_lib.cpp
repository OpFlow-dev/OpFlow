...
double STDCALL constant_deflation(int, ptrdiff_t, void *) { return 1; }
...
int main(int argc, char *argv[]) {
    ...
    if (rank == 0) std::cout << "World size: " << size << std::endl;
    ...
    MPI_Allgather(&chunk, 1, amgcl::mpi::datatype<ptrdiff_t>(), &domain[1], 1,
                  amgcl::mpi::datatype<ptrdiff_t>(), MPI_COMM_WORLD);
    ...
    amgclHandle solver = amgcl_mpi_create(MPI_COMM_WORLD, chunk, ptr.data(), col.data(), val.data(), 1,
                                          constant_deflation, NULL, prm);
    ...
    if (n <= 4096) {
        if (rank == 0) {
            ...
            for (ptrdiff_t i = 0; i < n2; ++i) f.write((char *) &X[renum[i]], sizeof(double));
        } else {
