...
std::vector<ptrdiff_t> read_problem(const amgcl::mpi::communicator &world, const std::string &A_file,
                                    const std::string &rhs_file, const std::string &part_file, int block_size,
...
int main(int argc, char *argv[]) {
    ...
    desc.add_options()("help,h", "show help")(
            ...
            ", pastix"
#endif
            )("params,p", po::value<std::string>(&parameter_file), "parameter file in json format")(
            "matrix,A", po::value<std::string>(&A_file)->default_value(A_file),
            "The system matrix in MatrixMarket format")(
            "rhs,b", po::value<std::string>(&rhs_file)->default_value(rhs_file),
            "The right-hand side in MatrixMarket format")(
            "part,s", po::value<std::string>(&part_file)->default_value(part_file),
            "Partitioning of the problem in MatrixMarket format")(
            "output,o", po::value<std::string>(&out_file), "The output file (saved in MatrixMarket format)");
    ...
    std::vector<ptrdiff_t> domain
            = read_problem(world, A_file, rhs_file, part_file, block_size, ptr, col, val, rhs);
    ...
    typedef amgcl::mpi::subdomain_deflation<
            amgcl::amg<amgcl::backend::builtin<double>, amgcl::runtime::coarsening::wrapper,
    ...
    if (vm.count("output")) {
        ...
        for (int r = 0; r < world.size; ++r) {
            if (r == world.rank) {
                ...
                if (r == 0) { f << "%%MatrixMarket matrix array real general\n" << domain.back() << " 1\n"; }
