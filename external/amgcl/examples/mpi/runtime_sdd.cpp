...
struct partitioned_deflation {
    ...
    partitioned_deflation(boost::array<ptrdiff_t, 2> LO, boost::array<ptrdiff_t, 2> HI, unsigned nparts)
        : nparts(nparts) {
        ...
        for (unsigned p = 0; p < nparts; ++p) {
            ...
            for (int j = lo[1]; j <= hi[1]; ++j) {
                for (int i = lo[0]; i <= hi[0]; ++i) { domain[(j - LO[1]) * nx + (i - LO[0])] = p; }
            }
        }
    }
    ...
    double operator()(ptrdiff_t i, unsigned j) const { return domain[i] == j; }
};
...
struct linear_deflation {
    ...
    linear_deflation(ptrdiff_t chunk, boost::array<ptrdiff_t, 2> lo, boost::array<ptrdiff_t, 2> hi) {
...
struct bilinear_deflation {
    ...
    bilinear_deflation(ptrdiff_t n, ptrdiff_t chunk, boost::array<ptrdiff_t, 2> lo,
                       boost::array<ptrdiff_t, 2> hi)
        : nv(0), chunk(chunk) {
        // See which neighbors we have.
        int neib[2][2] = {{lo[0] > 0 || lo[1] > 0, hi[0] + 1 < n || lo[1] > 0},
    ...
    double operator()(ptrdiff_t i, unsigned j) const { return v[j * chunk + i]; }
};
...
struct mba_deflation {
    ...
    mba_deflation(ptrdiff_t n, ptrdiff_t chunk, boost::array<ptrdiff_t, 2> lo, boost::array<ptrdiff_t, 2> hi)
        : chunk(chunk), nv(1) {
        // See which neighbors we have.
        int neib[2][2] = {{lo[0] > 0 || lo[1] > 0, hi[0] + 1 < n || lo[1] > 0},
                          {lo[0] > 0 || hi[1] + 1 < n, hi[0] + 1 < n || hi[1] + 1 < n}};
    ...
    static double zero(const std::array<double, 2> &) { return 0; }
};
#endif
...
struct harmonic_deflation {
    ...
    harmonic_deflation(ptrdiff_t n, ptrdiff_t chunk, boost::array<ptrdiff_t, 2> lo,
                       boost::array<ptrdiff_t, 2> hi)
        : nv(0), chunk(chunk) {
        // See which neighbors we have.
        int neib[2][2] = {{lo[0] > 0 || lo[1] > 0, hi[0] + 1 < n || lo[1] > 0},
...
struct renumbering {
    ...
    renumbering(const domain_partition<2> &p, const std::vector<ptrdiff_t> &d) : part(p), dom(d) {}
...
int main(int argc, char *argv[]) {
    ...
    desc.add_options()("help,h", "show help")(
            ...
            "laplace2d, recirc2d")("symbc", po::value<bool>(&symm_dirichlet)->default_value(symm_dirichlet),
                                   "Use symmetric Dirichlet conditions in laplace2d")(
            "size,n", po::value<ptrdiff_t>(&n)->default_value(n), "domain size")(
            "coarsening,c",
            ...
            ", pastix"
#endif
            )("deflation,v", po::value<std::string>(&deflation_type)->default_value(deflation_type),
              "constant, partitioned, linear, bilinear, mba, harmonic")(
            "subparts", po::value<int>()->default_value(16),
            "number of partitions for partitioned deflation")(
            "params,P", po::value<std::string>(&parameter_file),
            "parameter file in json format")("prm,p", po::value<std::vector<std::string>>()->multitoken(),
                                             "Parameters specified as name=value pairs. "
                                             "May be provided multiple times. Examples:\n"
                                             "  -p solver.tol=1e-3\n"
                                             "  -p precond.coarse_enough=300")(
            "just-relax,0", po::bool_switch(&just_relax),
            "Do not create AMG hierarchy, use relaxation as preconditioner")(
            "out,o", po::value<std::string>(&out_file), "out file");
    ...
    MPI_Allgather(&chunk, 1, amgcl::mpi::datatype<ptrdiff_t>(), &domain[1], 1,
                  amgcl::mpi::datatype<ptrdiff_t>(), world);
        ...
        for (ptrdiff_t j = lo[1]; j <= hi[1]; ++j) {
            ...
            for (ptrdiff_t i = lo[0]; i <= hi[0]; ++i) {
                ...
                if (i == 0 || j == 0 || i + 1 == n || j + 1 == n) {
                    ...
                    val.push_back(1);
                    rhs.push_back(sin(M_PI * x) + sin(M_PI * y) + sin(13 * M_PI * x) + sin(13 * M_PI * y));
                } else {
    ...
    prof.tic("setup");
    typedef amgcl::mpi::subdomain_deflation<amgcl::runtime::preconditioner<Backend>,
    ...
    if (world.rank == 0) {
        ...
        log << n2 << "\t" << nt << "\t" << world.size << "\t" << tm_setup << "\t" << tm_solve << "\t" << iters
            << "\t" << std::endl;
    }
    ...
    if (!out_file.empty()) {
        ...
        if (world.rank == 0) {
            for (int i = 1; i < world.size; ++i)
                MPI_Recv(&X[domain[i]], domain[i + 1] - domain[i], MPI_DOUBLE, i, 42, MPI_COMM_WORLD,
                         MPI_STATUS_IGNORE);
