...
partition(amgcl::mpi::communicator comm, const Matrix &Astrip, std::vector<double> &rhs,
...
int main(int argc, char *argv[]) {
    ...
    desc.add_options()("help,h",
            ...
            "block-size,b", po::value<int>()->default_value(1), "The block size of the system matrix. ")(
            "partitioner,r",
            ...
            "Repartition the system matrix")("prm-file,P", po::value<std::string>(),
    ...
    if (vm["binary"].as<bool>()) {
        n = read_binary(comm, vm["matrix"].as<std::string>(), vm["rhs"].as<std::string>(), block_size, ptr,
                        col, val, rhs);
    } else {
        n = read_matrix_market(comm, vm["matrix"].as<std::string>(), vm["rhs"].as<std::string>(), block_size,
                               ptr, col, val, rhs);
    }
    prof.toc("read");
    ...
    auto A = partition<Backend>(comm, std::tie(n, ptr, col, val), rhs, Backend::params(),
                                vm["partitioner"].as<amgcl::runtime::mpi::partition::type>(), block_size);

    prof.tic("setup");
    typedef amgcl::mpi::make_solver<
            amgcl::mpi::cpr<amgcl::mpi::amg<Backend, amgcl::runtime::mpi::coarsening::wrapper<Backend>,
