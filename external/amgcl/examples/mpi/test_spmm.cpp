...
template <class Val>
void assemble(int n, int beg, int end, std::vector<int> &ptr, std::vector<int> &col, std::vector<Val> &val) {
    int chunk = end - beg;
...
void test() {
    ...
    for (int i = 1; i < comm.size; ++i) displ[i] = displ[i - 1] + chunks[i - 1];
    ...
    MPI_Gatherv(&x[0], chunk, amgcl::mpi::datatype<Rhs>(), &X[0], &chunks[0], &displ[0],
                amgcl::mpi::datatype<Rhs>(), 0, comm);
    MPI_Gatherv(&y[0], chunk, amgcl::mpi::datatype<Rhs>(), &R[0], &chunks[0], &displ[0],
                amgcl::mpi::datatype<Rhs>(), 0, comm);
