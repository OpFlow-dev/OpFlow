...
template <class T>
using Backend = amgcl::backend::vexcl<T>;
#elif defined(SOLVER_BACKEND_VIENNACL)
#include <amgcl/backend/viennacl.hpp>
template <class T>
using Backend = amgcl::backend::viennacl<viennacl::compressed_matrix<T>>;
#elif defined(SOLVER_BACKEND_CUDA)
#include <amgcl/backend/cuda.hpp>
#include <amgcl/relaxation/cusparse_ilu0.hpp>
template <class T>
using Backend = amgcl::backend::cuda<T>;
#else
...
void solve_schur(const Matrix &K, const std::vector<double> &rhs, boost::property_tree::ptree &prm) {
    ...
    prof.tic("setup");
    amgcl::make_solver<amgcl::preconditioner::schur_pressure_correction<USolver, PSolver>,
...
#define AMGCL_BLOCK_PSOLVER(z, data, B)                                                                      \
    case B: {                                                                                                \
        ...
        typedef amgcl::make_block_solver<amgcl::runtime::preconditioner<BBackend>,                           \
...
void solve_schur(int pb, const Matrix &K, const std::vector<double> &rhs, boost::property_tree::ptree &prm) {
    switch (pb) {
        case 1: {
            typedef amgcl::make_solver<amgcl::runtime::preconditioner<Backend<double>>,
                                       amgcl::runtime::solver::wrapper<Backend<double>>>
                    PSolver;
            solve_schur<USolver, PSolver>(K, rhs, prm);
        } break;
...
void solve_schur(int ub, int pb, const Matrix &K, const std::vector<double> &rhs,
...
int main(int argc, char *argv[]) {
    ...
    desc.add_options()("help,h", "show help")(
            ...
            "ub", po::value<int>()->default_value(1),
            "Block-size of the 'flow'/'non-pressure' part of the matrix")(
            "pb", po::value<int>()->default_value(1), "Block-size of the 'pressure' part of the matrix")(
            "params,P", po::value<string>(),
            "parameter file in json format")("prm,p", po::value<vector<string>>()->multitoken(),
    ...
    solve_schur(vm["ub"].as<int>(), vm["pb"].as<int>(), std::tie(rows, ptr, col, val), rhs, prm);
