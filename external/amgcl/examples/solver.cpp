...
//---------------------------------------------------------------------------
template <int B>
std::tuple<size_t, double> block_solve(const boost::property_tree::ptree &prm, size_t rows,
                                       std::vector<ptrdiff_t> const &ptr, std::vector<ptrdiff_t> const &col,
                                       std::vector<double> const &val, std::vector<double> const &rhs,
                                       std::vector<double> &x, bool reorder) {
    ...
    typedef amgcl::make_solver<amgcl::runtime::preconditioner<BBackend>,
...
//---------------------------------------------------------------------------
std::tuple<size_t, double> scalar_solve(const boost::property_tree::ptree &prm, size_t rows,
                                        std::vector<ptrdiff_t> const &ptr, std::vector<ptrdiff_t> const &col,
                                        std::vector<double> const &val, std::vector<double> const &rhs,
                                        std::vector<double> &x, bool reorder) {
    Backend::params bprm;
    ...
    bprm.q = ctx;
#elif defined(SOLVER_BACKEND_VIENNACL)
    std::cout << viennacl::ocl::current_device().name() << " (" << viennacl::ocl::current_device().vendor()
              << ")\n\n";
#elif defined(SOLVER_BACKEND_CUDA)
...
//---------------------------------------------------------------------------
std::tuple<size_t, double> solve(const boost::property_tree::ptree &prm, size_t rows,
                                 std::vector<ptrdiff_t> const &ptr, std::vector<ptrdiff_t> const &col,
                                 std::vector<double> const &val, std::vector<double> const &rhs,
                                 std::vector<double> &x, int block_size, bool reorder) {
    switch (block_size) {
        case 1:
...
int main(int argc, char *argv[]) {
    ...
    desc.add_options()("help,h", "Show this help.")("prm-file,P", po::value<string>(),
            ...
            "f0", po::bool_switch()->default_value(false),
            "Use zero RHS vector. Implies --random-initial and solver.ns_search=true")(
            "f1", po::bool_switch()->default_value(false),
            "Set RHS = Ax where x = 1")("null,N", po::value<string>(),
            ...
            "size,n", po::value<int>()->default_value(32),
            ...
            "anisotropy,a", po::value<double>()->default_value(1.0),
            ...
            "reorder,r", po::bool_switch()->default_value(false),
            "When specified, the matrix will be reordered to improve cache-locality")(
            "initial,x", po::value<double>()->default_value(0), "Value to use as initial approximation. ")(
            "random-initial", po::bool_switch()->default_value(false),
            "Use random initial approximation. ")("output,o", po::value<string>(),
    ...
    if (vm.count("matrix")) {
        ...
        if (vm.count("rhs")) {
            ...
            for (size_t i = 0; i < rows; ++i) {
                double s = 0;
                for (ptrdiff_t j = ptr[i], e = ptr[i + 1]; j < e; ++j) s += val[j];
                rhs[i] = s;
            }
        } else {
    ...
    if (vm["f0"].as<bool>()) { prm.put("solver.ns_search", true); }
    ...
    std::tie(iters, error) = solve(prm, rows, ptr, col, val, rhs, x, block_size, vm["reorder"].as<bool>());
