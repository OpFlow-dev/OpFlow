 ...
 */
    ...
    public:
        ...
        make_block_solver(const Matrix &A, const params &prm = params(),
        ...
        std::tuple<size_t, scalar_type> operator()(const Matrix &A, const Vec1 &rhs, Vec2 &&x) const {
            typedef typename math::scalar_of<
                    typename backend::value_type<typename std::decay<Vec1>::type>::type>::type fs;
            typedef typename math::scalar_of<
                    typename backend::value_type<typename std::decay<Vec2>::type>::type>::type xs;
        ...
        std::shared_ptr<typename Precond::matrix> system_matrix_ptr() const { return S->system_matrix_ptr(); }

        typename Precond::matrix const &system_matrix() const { return S->system_matrix(); }

        friend std::ostream &operator<<(std::ostream &os, const make_block_solver &p) {
            return os << *p.S << std::endl;
        }

        size_t bytes() const { return backend::bytes(*S); }
