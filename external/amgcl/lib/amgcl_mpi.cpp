...
typedef amgcl::mpi::subdomain_deflation<
        ...
        amgcl::runtime::mpi::solver::wrapper<Backend>, amgcl::runtime::mpi::direct::solver<double>>
        Solver;
...
struct deflation_vectors {
    ...
    double operator()(int i, ptrdiff_t j) const { return user_func(i, j, user_data); }
};

//---------------------------------------------------------------------------
amgclHandle STDCALL amgcl_mpi_create(MPI_Comm comm, ptrdiff_t n, const ptrdiff_t *ptr, const ptrdiff_t *col,
                                     const double *val, int n_def_vec, amgclDefVecFunction def_vec_func,
                                     void *def_vec_data, amgclHandle params) {
    ...
    return static_cast<amgclHandle>(
            new Solver(comm,
                       std::make_tuple(n, boost::make_iterator_range(ptr, ptr + n + 1),
...
//---------------------------------------------------------------------------
conv_info STDCALL amgcl_mpi_solve(amgclHandle handle, double const *rhs, double *x) {
...
//---------------------------------------------------------------------------
void STDCALL amgcl_mpi_destroy(amgclHandle handle) { delete static_cast<Solver *>(handle); }
