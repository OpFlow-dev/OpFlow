...
BOOST_AUTO_TEST_CASE(complex_matrix_adapter) {
    ...
    boost::iterator_range<const double*> f_range = amgcl::adapter::complex_range(rhs);

    boost::iterator_range<double*> x_range = amgcl::adapter::complex_range(x);
    ...
    std::cout << "iters: " << iters << std::endl << "resid: " << resid << std::endl;
}
