...
void test_problem(size_t n, std::vector<ptrdiff_t> ptr, std::vector<ptrdiff_t> col,
                  std::vector<value_type> val, std::vector<rhs_type> rhs) {
    ...
    for (amgcl::runtime::solver::type s : solver) {
        ...
        try {
            test_solver<Backend>(amgcl::adapter::zero_copy(n, ptr.data(), col.data(), val.data()), y, x, s,
                                 relaxation[0], coarsening[0]);
    ...
    for (amgcl::runtime::relaxation::type r : relaxation) {
        ...
        try {
            test_solver<Backend>(amgcl::adapter::zero_copy(n, ptr.data(), col.data(), val.data()), y, x,
                                 solver[0], r, coarsening[0]);
        } catch (const std::logic_error &) {}

        try {
            std::cout << "Relaxation as preconditioner: " << r << std::endl;

            test_rap<Backend>(amgcl::adapter::zero_copy(n, ptr.data(), col.data(), val.data()), y, x,
                              solver[0], r);
        } catch (const std::logic_error &) {}
    }
    ...
    for (amgcl::runtime::coarsening::type c : coarsening) {
        ...
        switch (c) {
            ...
            case amgcl::runtime::coarsening::smoothed_aggr_emin:
                test_solver<Backend>(amgcl::adapter::zero_copy(n, ptr.data(), col.data(), val.data()), y, x,
                                     solver[0], relaxation[0], c, /*test_null_space*/ true);
                break;
            default:
                break;
        }
