...
py::array mkarray_via_buffer(size_t n) {
    return py::array(
            py::buffer_info(nullptr, sizeof(T), py::format_descriptor<T>::format(), 1, {n}, {sizeof(T)}));
}
...
py::array_t<S, 0> create_recarray(size_t n) {
    ...
    for (size_t i = 0; i < n; i++) { SET_TEST_VALS(ptr[i], i); }
    return arr;
}
...
TEST_SUBMODULE(numpy_dtypes, m) {
    ...
    py::class_<SimpleStruct>(m, "SimpleStruct")
            ...
            .def_static("fromtuple", [](const py::tuple tup) {
                if (py::len(tup) != 4) { throw py::cast_error("Invalid size"); }
    ...
    py::detail::npy_format_descriptor<B>::register_dtype(std::vector<py::detail::field_descriptor> {});
    ...
    m.def("create_rec_partial_nested", [](size_t n) {
        ...
        for (size_t i = 0; i < n; i++) { SET_TEST_VALS(ptr[i].a, i); }
        return arr;
    });
    ...
    m.def("print_dtypes", []() {
        ...
        for (const py::handle& d :
             {py::dtype::of<SimpleStruct>(), py::dtype::of<PackedStruct>(), py::dtype::of<NestedStruct>(),
              py::dtype::of<PartialStruct>(), py::dtype::of<PartialNestedStruct>(),
              py::dtype::of<StringStruct>(), py::dtype::of<ArrayStruct>(), py::dtype::of<EnumStruct>(),
    ...
    m.def("create_string_array", [](bool non_empty) {
        ...
        if (non_empty) {
            ...
            for (py::ssize_t i = 0; i < req.size * req.itemsize; i++) static_cast<char*>(req.ptr)[i] = 0;
            ptr[1].a[0] = 'a';
            ptr[1].b[0] = 'a';
            ptr[2].a[0] = 'a';
            ptr[2].b[0] = 'a';
    ...
    m.def("create_array_array", [](size_t n) {
        ...
        for (size_t i = 0; i < n; i++) {
            for (size_t j = 0; j < 3; j++)
                for (size_t k = 0; k < 4; k++) ptr[i].a[j][k] = char('A' + (i * 100 + j * 10 + k) % 26);
            for (size_t j = 0; j < 2; j++) ptr[i].b[j] = int32_t(i * 1000 + j);
            for (size_t j = 0; j < 3; j++) ptr[i].c[j] = uint8_t(i * 10 + j);
