...
TEST_SUBMODULE(pytypes, m) {
    ...
    m.def("print_list", [](py::list list) {
        int index = 0;
        for (auto item : list) py::print("list item {}: {}"_s.format(index++, item));
    });
    // test_none
    m.def("get_none", [] { return py::none(); });
    m.def("print_none", [](py::none none) { py::print("none: {}"_s.format(none)); });
    ...
    m.def("print_set", [](py::set set) {
        for (auto item : set) py::print("key:", item);
    });
    ...
    m.def("print_dict", [](py::dict dict) {
        for (auto item : dict) py::print("key: {}, value={}"_s.format(item.first, item.second));
    });
    ...
    m.def("return_capsule_with_destructor_2", []() {
        py::print("creating capsule");
        return py::capsule((void*) 1234,
                           [](void* ptr) { py::print("destructing capsule: {}"_s.format((size_t) ptr)); });
    });
    ...
    m.def("return_capsule_with_name_and_destructor", []() {
        auto capsule = py::capsule((void*) 12345, "pointer type description", [](PyObject* ptr) {
            if (ptr) {
                ...
                py::print("destructing capsule ({}, '{}')"_s.format((size_t) PyCapsule_GetPointer(ptr, name),
                                                                    name));
            }
        });
    ...
    m.def("accessor_api", [](py::object o) {
        ...
        auto l = py::list();
        for (auto item : o.attr("begin_end")) { l.append(item); }
        d["begin_end"] = l;
        ...
        } catch (const py::error_already_set&) { d["missing_attr_ptr"] = "raised"_s; }
        try {
            o.attr("missing").attr("doesn't matter");
        } catch (const py::error_already_set&) { d["missing_attr_chain"] = "raised"_s; }
    ...
    m.def("tuple_accessor", [](py::tuple existing_t) {
        try {
            ...
            for (size_t i = 0; i < new_t.size(); ++i) { new_t[i] = i; }
            return new_t;
        }
        return py::tuple();
    });
    ...
    m.def("default_constructors", []() {
        return py::dict("bytes"_a = py::bytes(), "str"_a = py::str(), "bool"_a = py::bool_(),
    ...
    m.def("converting_constructors", [](py::dict d) {
        return py::dict("bytes"_a = py::bytes(d["bytes"]), "str"_a = py::str(d["str"]),
    ...
    m.def("cast_functions", [](py::dict d) {
        ...
        return py::dict("bytes"_a = d["bytes"].cast<py::bytes>(), "str"_a = d["str"].cast<py::str>(),
    ...
    m.def("test_memoryview_from_buffer", [](bool is_unsigned) {
        ...
        if (is_unsigned) return py::memoryview::from_buffer(ui16, {4}, {sizeof(uint16_t)});
        else
