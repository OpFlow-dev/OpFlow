...
py::list test_random_access_iterator(PythonType x) {
    if (x.size() < 5) throw py::value_error("Please provide at least 5 elements for testing.");
...
TEST_SUBMODULE(sequences_and_iterators, m) {
    ...
    public:
        ...
        Sequence reversed() const {
            ...
            for (size_t i = 0; i < m_size; ++i) result[m_size - i - 1] = m_data[i];
            return result;
        }
    ...
    py::class_<StringMap>(m, "StringMap")
            ...
            .def(
                    ...
                    py::keep_alive<0, 1>())
            .def(
                    "items", [](const StringMap &map) { return py::make_iterator(map.begin(), map.end()); },
                    py::keep_alive<0, 1>());
    ...
    m.def("count_nonzeros", [](py::dict d) {
        return std::count_if(d.begin(), d.end(),
                             [](std::pair<py::handle, py::handle> p) { return p.second.cast<int>() != 0; });
    });
    ...
    m.def("iterator_passthrough",
          [](py::iterator s) -> py::iterator { return py::make_iterator(std::begin(s), std::end(s)); });
