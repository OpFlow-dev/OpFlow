...
Eigen::MatrixXd adjust_matrix(MatrixArgType m) {
    ...
    for (int c = 0; c < m.cols(); c++)
        for (int r = 0; r < m.rows(); r++) ret(r, c) += 10 * r + 100 * c;
    return ret;
}
...
TEST_SUBMODULE(eigen, m) {
    ...
    m.def("cholesky2",
          [](const Eigen::Ref<const MatrixXdR> &x) -> Eigen::MatrixXd { return x.llt().matrixL(); });
    ...
    m.def(
            ...
            [](py::EigenDRef<Eigen::MatrixXd> m) {
                return py::EigenDMap<Eigen::MatrixXd>(m.data(), (m.rows() + 1) / 2, m.cols(),
    ...
    m.def(
            "even_cols",
            [](py::EigenDRef<Eigen::MatrixXd> m) {
                return py::EigenDMap<Eigen::MatrixXd>(m.data(), m.rows(), (m.cols() + 1) / 2,
    ...
    m.def("block",
          [](const Eigen::Ref<const Eigen::MatrixXd> &x, int start_row, int start_col, int block_rows,
    ...
    m.def("symmetric_lower", [](const Eigen::MatrixXi &m) { return m.selfadjointView<Eigen::Lower>(); });
    // Returns a SelfAdjointView referencing the lower triangle of m
    ...
    m.def(
            "get_elem_nocopy", [](Eigen::Ref<const Eigen::MatrixXd> m) -> double { return get_elem(m); },
            py::arg().noconvert());
