...
TEST_SUBMODULE(stl, m) {
    ...
    m.def("load_bool_vector", [](const std::vector<bool> &v) { return v.at(0) == true && v.at(1) == false; });
    ...
    m.def("load_valarray",
          [](const std::valarray<int> &v) { return v.size() == 3 && v[0] == 1 && v[1] == 4 && v[2] == 9; });
    ...
    m.def("load_variant_2pass",
          [](variant<double, int> v) { return py::detail::visit_helper<variant>::call(visitor(), v); });
    m.def("cast_variant", []() {
    ...
    m.def("return_vec_of_reference_wrapper", [](std::reference_wrapper<UserType> p4) {
        ...
        return std::vector<std::reference_wrapper<UserType>> {std::ref(p1), std::ref(p2), std::ref(p3), p4};
    });
