...
struct PythonAlreadySetInDestructor {
    ...
    ~PythonAlreadySetInDestructor() {
        ...
        try {
            ...
            py::object o = foo["bar"];
        } catch (py::error_already_set &ex) { ex.discard_as_unraisable(s); }
    }
...
TEST_SUBMODULE(exceptions, m) {
    ...
    py::register_exception_translator([](std::exception_ptr p) {
        try {
            if (p) std::rethrow_exception(p);
        } catch (const MyException4 &e) { throw MyException(e.what()); }
    });
    ...
    m.def("throw_already_set", [](bool err) {
        if (err) PyErr_SetString(PyExc_ValueError, "foo");
        try {
    ...
    m.def("python_call_in_destructor", [](py::dict d) {
        try {
            ...
            throw py::error_already_set();
        } catch (const py::error_already_set &) { return true; }
        return false;
    });
    ...
    m.def("try_catch", [m](py::object exc_type, py::function f, py::args args) {
        try {
            ...
            if (ex.matches(exc_type)) py::print(ex.what());
            else
                throw;
        }
    });
