...
#define PYBIND11_BASEN(N)                                                                                    \
    py::class_<BaseN<N>>(m, "BaseN" #N).def(py::init<int>()).def("f" #N, [](BaseN<N> &b) { return b.i + N; })
    PYBIND11_BASEN(1);
    PYBIND11_BASEN(2);
    PYBIND11_BASEN(3);
    PYBIND11_BASEN(4);
    ...
    py::class_<Base1a, std::shared_ptr<Base1a>>(m, "Base1a").def(py::init<int>()).def("foo", &Base1a::foo);
    ...
    py::class_<Base12a, /* Base1 missing */ Base2a, std::shared_ptr<Base12a>>(m, "Base12a",
