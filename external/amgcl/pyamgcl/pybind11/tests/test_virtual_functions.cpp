...
#endif
    Movable get_movable(int a, int b) override { PYBIND11_OVERRIDE_PURE(Movable, NCVirt, get_movable, a, b); }
};
...
TEST_SUBMODULE(virtual_functions, m) {
    ...
    py::class_<Base, DispatchIssue>(m, "DispatchIssue").def(py::init<>()).def("dispatch", &Base::dispatch);
    ...
    py::class_<OverrideTest::A>(m, "OverrideTest_A").def_readwrite("value", &OverrideTest::A::value);
...
public:                                                                                                      \
    ...
    virtual std::string say_something(unsigned times) {                                                      \
        std::string s = "";                                                                                  \
        for (unsigned i = 0; i < times; ++i) s += "hi";                                                      \
        return s;                                                                                            \
    }                                                                                                        \
    std::string say_everything() { return say_something(1) + " " + std::to_string(unlucky_number()); }
    A_METHODS
    A_Repeat() = default;
