...
boost::property_tree::ptree make_ptree(const py::dict& args) {
    ...
    for (auto p : args)
        prm.put(static_cast<std::string>(py::str(p.first)), static_cast<std::string>(py::str(p.second)));
    return prm;
}
...
boost::iterator_range<T*> make_range(py::array_t<T> a) {
    ...
    amgcl::precondition(i.ndim == 1, "Got multidimensional array for a vector parameter");

    return boost::make_iterator_range(static_cast<T*>(i.ptr), static_cast<T*>(i.ptr) + i.shape[0]);
}
...
//---------------------------------------------------------------------------
class solver : public amgcl::runtime::solver::wrapper<amgcl::backend::builtin<double>> {
public:
    solver(const precond& P, py::dict prm)
        : S(amgcl::backend::rows(P.system_matrix()), make_ptree(prm)), P(P),
          x(amgcl::backend::rows(P.system_matrix())), f(amgcl::backend::rows(P.system_matrix())) {}

    py::array_t<double> solve(py::array_t<int> _ptr, py::array_t<int> _col, py::array_t<double> _val,
...
public:
    amg_precond(py::array_t<int> _ptr, py::array_t<int> _col, py::array_t<double> _val, py::dict prm) {
        auto ptr = make_range(_ptr);
        auto col = make_range(_col);
        auto val = make_range(_val);

        P = std::make_shared<Precond>(std::make_tuple(boost::size(ptr) - 1, ptr, col, val), make_ptree(prm));
    }

    void apply(const precond::vector& rhs, precond::vector& x) const { P->apply(rhs, x); }

    const matrix& system_matrix() const { return P->system_matrix(); }
...
PYBIND11_MODULE(pyamgcl_ext, m) {
    ...
    Precond.def("__repr__", &precond::repr)
    ...
    py::class_<solver>(m, "solver")
            .def(py::init<const precond&, py::dict>())
            .def("__call__", (py::array_t<double>(solver::*)(py::array_t<double>) const) & solver::solve)
            .def("__call__", (py::array_t<double>(solver::*)(py::array_t<int>, py::array_t<int>,
                                                             py::array_t<double>, py::array_t<double>) const)
                                     & solver::solve)
