...
#define zypre_newBoxLoopInit(ndim, loop_size)                                                                \
    ...
    for (HYPRE_Int d = 0; d < ndim; d++) hypre__tot *= loop_size[d];
...
#define zypre_newBoxLoop1Begin(ndim, loop_size, dbox1, start1, stride1, i1)                                  \
    {                                                                                                        \
        zypre_newBoxLoopInit(ndim, loop_size);                                                               \
...
#define zypre_newBoxLoop2Begin(ndim, loop_size, dbox1, start1, stride1, i1, dbox2, start2, stride2, i2)      \
    {                                                                                                        \
        zypre_newBoxLoopInit(ndim, loop_size);                                                               \
        zypre_BoxLoopDataDeclareK(1, ndim, loop_size, dbox1, start1, stride1);                               \
...
#define zypre_newBoxLoop3Begin(ndim, loop_size, dbox1, start1, stride1, i1, dbox2, start2, stride2, i2,      \
...
#define zypre_newBoxLoop4Begin(ndim, loop_size, dbox1, start1, stride1, i1, dbox2, start2, stride2, i2,      \
                               dbox3, start3, stride3, i3, dbox4, start4, stride4, i4)                       \
    {                                                                                                        \
        zypre_newBoxLoopInit(ndim, loop_size);                                                               \
        zypre_BoxLoopDataDeclareK(1, ndim, loop_size, dbox1, start1, stride1);                               \
        zypre_BoxLoopDataDeclareK(2, ndim, loop_size, dbox2, start2, stride2);                               \
...
#define hypre_BoxLoop1ReductionEnd(i1, reducesum) hypre_BoxLoop1End(i1)

#define hypre_BoxLoop2ReductionBegin(ndim, loop_size, dbox1, start1, stride1, i1, dbox2, start2, stride2,    \
                                     i2, reducesum)                                                          \
    hypre_BoxLoop2Begin(ndim, loop_size, dbox1, start1, stride1, i1, dbox2, start2, stride2, i2)

#define hypre_BoxLoop2ReductionEnd(i1, i2, reducesum) hypre_BoxLoop2End(i1, i2)
...
#define hypre_newBoxLoop2Begin(ndim, loop_size, dbox1, start1, stride1, i1, dbox2, start2, stride2, i2)      \
    {                                                                                                        \
        hypre_newBoxLoopInit(ndim, loop_size);                                                               \
        hypre_BoxLoopDataDeclareK(1, ndim, loop_size, dbox1, start1, stride1);                               \
...
#define hypre_newBoxLoop3Begin(ndim, loop_size, dbox1, start1, stride1, i1, dbox2, start2, stride2, i2,      \
                               dbox3, start3, stride3, i3)                                                   \
    {                                                                                                        \
        hypre_newBoxLoopInit(ndim, loop_size);                                                               \
        hypre_BoxLoopDataDeclareK(1, ndim, loop_size, dbox1, start1, stride1);                               \
...
#define hypre_newBoxLoop4Begin(ndim, loop_size, dbox1, start1, stride1, i1, dbox2, start2, stride2, i2,      \
                               dbox3, start3, stride3, i3, dbox4, start4, stride4, i4)                       \
    {                                                                                                        \
        hypre_newBoxLoopInit(ndim, loop_size);                                                               \
        hypre_BoxLoopDataDeclareK(1, ndim, loop_size, dbox1, start1, stride1);                               \
...
#define hypre_BoxLoop1ReductionBegin(ndim, loop_size, dbox1, start1, stride1, i1, HYPRE_BOX_REDUCTION)       \
    {                                                                                                        \
...
#if (defined(HYPRE_USING_CUDA) || defined(HYPRE_USING_HIP)) && !defined(HYPRE_USING_RAJA)                    \
        && !defined(HYPRE_USING_KOKKOS)
...
__global__ void forall_kernel(hypre_DeviceItem &item, LOOP_BODY loop_body, HYPRE_Int length) {
    ...
    if (idx < length) { loop_body(idx); }
}

template <typename LOOP_BODY>
void BoxLoopforall(HYPRE_Int length, LOOP_BODY loop_body) {
...
#endif
        for (HYPRE_Int idx = 0; idx < length; idx++) { loop_body(idx); }
...
__global__ void reductionforall_kernel(hypre_DeviceItem &item, HYPRE_Int length, REDUCER reducer,
                                       LOOP_BODY loop_body) {
    ...
    for (HYPRE_Int idx = thread_id; idx < length; idx += n_threads) { loop_body(idx, reducer); }
...
template <typename LOOP_BODY, typename REDUCER>
void ReductionBoxLoopforall(HYPRE_Int length, REDUCER &reducer, LOOP_BODY loop_body) {
    if (length <= 0) { return; }
    ...
    if (exec_policy == HYPRE_EXEC_HOST) {
        for (HYPRE_Int idx = 0; idx < length; idx++) { loop_body(idx, reducer); }
    } else if (exec_policy == HYPRE_EXEC_DEVICE) {
...
#define hypre_newBoxLoopInit(ndim, loop_size)                                                                \
    HYPRE_Int hypre__tot = 1;                                                                                \
    for (HYPRE_Int hypre_d = 0; hypre_d < ndim; hypre_d++) { hypre__tot *= loop_size[hypre_d]; }
...
/* BoxLoop 2 */
#define hypre_BoxLoop2BeginCUDA(ndim, loop_size, dbox1, start1, stride1, i1, dbox2, start2, stride2, i2)     \
    {                                                                                                        \
        hypre_newBoxLoopInit(ndim, loop_size);                                                               \
        hypre_BoxLoopDataDeclareK(1, ndim, loop_size, dbox1, start1, stride1);                               \
        hypre_BoxLoopDataDeclareK(2, ndim, loop_size, dbox2, start2, stride2);                               \
...
/* BoxLoop 3 */
#define hypre_BoxLoop3BeginCUDA(ndim, loop_size, dbox1, start1, stride1, i1, dbox2, start2, stride2, i2,     \
                                dbox3, start3, stride3, i3)                                                  \
    {                                                                                                        \
        hypre_newBoxLoopInit(ndim, loop_size);                                                               \
        hypre_BoxLoopDataDeclareK(1, ndim, loop_size, dbox1, start1, stride1);                               \
...
/* BoxLoop 4 */
#define hypre_BoxLoop4BeginCUDA(ndim, loop_size, dbox1, start1, stride1, i1, dbox2, start2, stride2, i2,     \
                                dbox3, start3, stride3, i3, dbox4, start4, stride4, i4)                      \
    {                                                                                                        \
        hypre_newBoxLoopInit(ndim, loop_size);                                                               \
        hypre_BoxLoopDataDeclareK(1, ndim, loop_size, dbox1, start1, stride1);                               \
...
/* Reduction BoxLoop2 */
#define hypre_BoxLoop2ReductionBeginCUDA(ndim, loop_size, dbox1, start1, stride1, i1, dbox2, start2,          \
                                         stride2, i2, reducesum)                                              \
    {                                                                                                         \
        hypre_newBoxLoopInit(ndim, loop_size);                                                                \
        hypre_BoxLoopDataDeclareK(1, ndim, loop_size, dbox1, start1, stride1);                                \
        hypre_BoxLoopDataDeclareK(2, ndim, loop_size, dbox2, start2, stride2);                                \
...
template <typename LOOP_BODY>
void ReductionBoxLoopforall(LOOP_BODY loop_body, HYPRE_Int length, HYPRE_Real *shared_sum_var) {
    if (length <= 0) { return; }
    dim3 bDim = hypre_GetDefaultDeviceBlockDimension();
    dim3 gDim = hypre_GetDefaultDeviceGridDimension(length, "thread", bDim);
    ...
    hypre_HandleComputeStream(hypre_handle())
            ->submit([&](sycl::handler &cgh) {
                cgh.parallel_for(sycl::nd_range<3>(gDim * bDim, bDim),
                                 sycl::reduction(shared_sum_var, std::plus<>()), loop_body);
            })
            .wait_and_throw();
}
...
/* Reduction BoxLoop2 */
#define hypre_newBoxLoop2ReductionBegin(ndim, loop_size, dbox1, start1, stride1, i1, dbox2, start2, stride2, \
                                        i2, sum_var)                                                         \
    {                                                                                                        \
        hypre_newBoxLoopInit(ndim, loop_size);                                                               \
        hypre_BoxLoopDataDeclareK(1, ndim, loop_size, dbox1, start1, stride1);                               \
...
#define hypre_BoxLoop1ReductionEnd(i1, reducesum) hypre_newBoxLoop1ReductionEnd(i1, reducesum)

#define hypre_BoxLoop2ReductionBegin(ndim, loop_size, dbox1, start1, stride1, i1, dbox2, start2, stride2,    \
                                     i2, reducesum)                                                          \
    hypre_newBoxLoop2ReductionBegin(ndim, loop_size, dbox1, start1, stride1, i1, dbox2, start2, stride2, i2, \
                                    reducesum)

#define hypre_BoxLoop2ReductionEnd(i1, i2, reducesum) hypre_newBoxLoop2ReductionEnd(i1, i2, reducesum)
