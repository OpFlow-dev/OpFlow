...
template <class Iter1, class Iter2, class Iter3, class UnaryOperation, class Pred>
Iter3 hypreSycl_transform_if(Iter1 first, Iter1 last, Iter2 mask, Iter3 result, UnaryOperation unary_op,
                             Pred pred) {
    static_assert(std::is_same<typename std::iterator_traits<Iter1>::iterator_category,
    ...
    HYPRE_ONEDPL_CALL(std::for_each, begin_for_each, begin_for_each + n,
...
struct hypreSycl_predicate_key_fun {
    hypreSycl_predicate_key_fun(Predicate _pred) : pred(_pred) {}

    template <typename _T1>
    bool operator()(_T1 &&a) const {
...
Iter3 hypreSycl_copy_if(Iter1 first, Iter1 last, Iter2 mask, Iter3 result, Pred pred) {
    static_assert(std::is_same<typename std::iterator_traits<Iter1>::iterator_category,
                               std::random_access_iterator_tag>::value
                          && std::is_same<typename std::iterator_traits<Iter2>::iterator_category,
                                          std::random_access_iterator_tag>::value
                          && std::is_same<typename std::iterator_traits<Iter3>::iterator_category,
                                          std::random_access_iterator_tag>::value,
                  "Iterators passed to algorithms must be random-access iterators.");
    auto ret_val = HYPRE_ONEDPL_CALL(std::copy_if, oneapi::dpl::make_zip_iterator(first, mask),
...
Iter1 hypreSycl_remove_if(Iter1 first, Iter1 last, Iter2 mask, Pred pred) {
    ...
    hypre_TMemcpy(mask_cpy, mask, ValueType, std::distance(first, last), HYPRE_MEMORY_DEVICE,
                  HYPRE_MEMORY_DEVICE);
    auto ret_val
            = HYPRE_ONEDPL_CALL(std::remove_if, oneapi::dpl::make_zip_iterator(first, mask_cpy),
...
Iter3 hypreSycl_remove_copy_if(Iter1 first, Iter1 last, Iter2 mask, Iter3 result, Pred pred) {
    static_assert(std::is_same<typename std::iterator_traits<Iter1>::iterator_category,
                          ...
                          && std::is_same<typename std::iterator_traits<Iter2>::iterator_category,
                                          std::random_access_iterator_tag>::value,
                  "Iterators passed to algorithms must be random-access iterators.");
    auto ret_val = HYPRE_ONEDPL_CALL(std::remove_copy_if, oneapi::dpl::make_zip_iterator(first, mask),
                                     oneapi::dpl::make_zip_iterator(last, mask + std::distance(first, last)),
                                     oneapi::dpl::make_zip_iterator(result, oneapi::dpl::discard_iterator()),
                                     hypreSycl_predicate_key_fun<Pred>(pred));
    return std::get<0>(ret_val.base());
...
template <typename InputIter1, typename InputIter2, typename InputIter3, typename OutputIter,
          typename Predicate>
void hypreSycl_scatter_if(InputIter1 first, InputIter1 last, InputIter2 map, InputIter3 mask,
                          OutputIter result, Predicate pred) {
    static_assert(std::is_same<typename std::iterator_traits<InputIter1>::iterator_category,
                               std::random_access_iterator_tag>::value
                          && std::is_same<typename std::iterator_traits<InputIter2>::iterator_category,
                                          std::random_access_iterator_tag>::value
                          && std::is_same<typename std::iterator_traits<InputIter3>::iterator_category,
                                          std::random_access_iterator_tag>::value
                          && std::is_same<typename std::iterator_traits<OutputIter>::iterator_category,
                                          std::random_access_iterator_tag>::value,
                  "Iterators passed to algorithms must be random-access iterators.");
    hypreSycl_transform_if(
            first, last, mask, oneapi::dpl::make_permutation_iterator(result, map),
            [=](auto &&v) { return v; }, [=](auto &&m) { return pred(m); });
}

// Equivalent of thrust::scatter
template <typename InputIter1, typename InputIter2, typename OutputIter>
void hypreSycl_scatter(InputIter1 first, InputIter1 last, InputIter2 map, OutputIter result) {
    static_assert(std::is_same<typename std::iterator_traits<InputIter1>::iterator_category,
                               std::random_access_iterator_tag>::value
                          && std::is_same<typename std::iterator_traits<InputIter2>::iterator_category,
                                          std::random_access_iterator_tag>::value
                          && std::is_same<typename std::iterator_traits<OutputIter>::iterator_category,
                                          std::random_access_iterator_tag>::value,
                  "Iterators passed to algorithms must be random-access iterators.");
    auto perm_result = oneapi::dpl::make_permutation_iterator(result, map);
...
template <typename InputIter1, typename InputIter2, typename InputIter3, typename OutputIter,
          typename Predicate>
void hypreSycl_gather_if(InputIter1 map_first, InputIter1 map_last, InputIter2 mask, InputIter3 input_first,
                         OutputIter result, Predicate pred) {
    static_assert(std::is_same<typename std::iterator_traits<InputIter1>::iterator_category,
                               std::random_access_iterator_tag>::value
                          && std::is_same<typename std::iterator_traits<InputIter2>::iterator_category,
    ...
    hypreSycl_copy_if(perm_begin, perm_begin + n, mask, result, [=](auto &&m) { return pred(m); });
}

// Equivalent of thrust::gather
template <typename InputIter1, typename InputIter2, typename OutputIter>
OutputIter hypreSycl_gather(InputIter1 map_first, InputIter1 map_last, InputIter2 input_first,
                            OutputIter result) {
    static_assert(std::is_same<typename std::iterator_traits<InputIter1>::iterator_category,
                               std::random_access_iterator_tag>::value
                          && std::is_same<typename std::iterator_traits<InputIter2>::iterator_category,
...
void hypreSycl_sequence(Iter first, Iter last, T init = 0) {
    static_assert(std::is_same<typename std::iterator_traits<Iter>::iterator_category,
                               std::random_access_iterator_tag>::value,
                  "Iterators passed to algorithms must be random-access iterators.");
    using DiffType = typename std::iterator_traits<Iter>::difference_type;
    HYPRE_ONEDPL_CALL(std::transform, oneapi::dpl::counting_iterator<DiffType>(init),
                      oneapi::dpl::counting_iterator<DiffType>(init + std::distance(first, last)), first,
                      [](auto i) { return i; });
}
