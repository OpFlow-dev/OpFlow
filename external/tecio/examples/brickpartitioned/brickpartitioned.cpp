...
static INTEGER4 initializeFile(
        ...
        MPI_Comm mpiComm,
#endif
        int fOffset, int numSolutionFiles, int numPartitions, double solTime) {
    ...
    if (returnValue == 0) {
        ...
        if (numSolutionFiles == 0) {
            ...
            if (numSolutionFiles == 1) snprintf(fileName, fileNameSize, "%s_solution.szplt", baseFileName);
            else
                snprintf(fileName, fileNameSize, "%s_solution_%04d.szplt", baseFileName,
                         static_cast<int>(std::ceil(solTime)));
            varNames = "p";
        }

        returnValue = TECINI142((char*) "SIMPLE DATASET", (char*) varNames, (char*) fileName, (char*) ".",
                                &fileFormat, &dataFileType, &debug, &vIsDouble);
    }
...
/**
 */
static INTEGER4 createZones(int fOffset, INTEGER4 numPartitions, INTEGER4 const* partitionOwners,
                            double solTime) {
    INTEGER4 returnValue = 0;
    ...
    if (returnValue == 0)
        returnValue = TECZNE142(zoneTitle, &zoneType, &nNodes, &nCells, &nFaces, &iCellMax, &jCellMax,
                                &kCellMax, &solTime, &strandID, &parentZn, &isBlock, &nFConns, &fNMode,
                                0,                                    /* TotalNumFaceNodes */
                                0,                                    /* NumConnectedBoundaryFaces */
                                0,                                    /* TotalNumBoundaryConnections */
                                NULL,                                 /* PassiveVarList */
                                &valueLocations[baseVarOffset], NULL, /* SharVarFromZone */
                                &shrConn);
...
/**
 */
static void appendGhostItems(INTEGER4* nGhosts, INTEGER4** ghosts, INTEGER4** gPartitions,
                             INTEGER4** gPGhosts, INTEGER4 ptn, INTEGER4 gIDim, INTEGER4 gJDim,
                             INTEGER4 gIStart, INTEGER4 gIEnd, INTEGER4 gJStart, INTEGER4 gJEnd,
                             INTEGER4 gKStart, INTEGER4 gKEnd, INTEGER4 oIDim, INTEGER4 oJDim,
                             INTEGER4 oIStart, INTEGER4 oIEnd, INTEGER4 oJStart, INTEGER4 oJEnd,
                             INTEGER4 oKStart, INTEGER4 oKEnd) {
    ...
    *ghosts = (INTEGER4*) realloc(*ghosts, *nGhosts * sizeof(INTEGER4));
    if (gPartitions) *gPartitions = (INTEGER4*) realloc(*gPartitions, *nGhosts * sizeof(INTEGER4));
    if (gPGhosts) *gPGhosts = (INTEGER4*) realloc(*gPGhosts, *nGhosts * sizeof(INTEGER4));
...
/**
 */
static void GatherGhostNodesAndCells(INTEGER4* nGNodes, INTEGER4** ghostNodes, INTEGER4** gNPartitions,
                                     INTEGER4** gNPNodes, INTEGER4* nGCells, INTEGER4** ghostCells, int* iDim,
                                     int* jDim, int* kDim, int* jMin, int* jMax) {
    /*
...
static INTEGER4 createData(
        ...
        int commRank,
#endif
        int fOffset, int numSolutionFiles, INTEGER4 numPartitions, INTEGER4 const* partitionOwners,
        double solTime) {
    INTEGER4 returnValue = 0;
        ...
        for (int k = 0; k < kDim[ptn] - 1; ++k)
            for (int j = 0; j < jDim[ptn] - 1; ++j)
                for (int i = 0; i < iDim[ptn] - 1; ++i) {
                    ...
                    if (fOffset == 0) {
                        connectivity[ptn][8 * index]
                                = (k * jDim[ptn] + j) * iDim[ptn] + i + 1; /* One-based to feed TECIO */
    ...
    for (INTEGER4 ptn = 1; ptn <= effectiveNumPartitions; ++ptn) {
...
#endif
        {
            ...
            if (fOffset != 0 || numSolutionFiles == 0) {
                ...
                if (returnValue == 0) returnValue = TECDAT142(&pNCells[ptn - 1], p[ptn - 1], &dIsDouble);
            }
...
/**
 */
static INTEGER4 finalizeFile() { return TECEND142(); }
...
int main(int argc, char** argv) {
    ...
    if (argc == 3
        && (strcmp(argv[1], NUM_SOLUTION_FILES_FLAG) == 0
    ...
    if (numPartitions > 0) partitionOwners = (INTEGER4*) malloc(numPartitions * sizeof(INTEGER4));
    ...
    for (int fOffset = 0; returnValue == 0 && fOffset < numOutputFiles; ++fOffset) {
         ...
         * Create zones.
         */
        if (returnValue == 0) returnValue = createZones(fOffset, numPartitions, partitionOwners, solTime);
        ...
        if (returnValue == 0) returnValue = finalizeFile();
    }
