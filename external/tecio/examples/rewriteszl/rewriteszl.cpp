...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (numZones > 0) {
            ...
            if (type == FieldDataType_Double) isDouble = 1;
        }
        ...
        i = tecFileWriterOpen(argv[2], dataSetTitle, outputStream.str().c_str(), fileFormat, fileType, 1,
                              NULL, &outputFileHandle);
        ...
        for (int32_t inputZone = 1; inputZone <= numZones; ++inputZone) {
            ...
            if (zoneType == 6 || zoneType == 7) throw std::runtime_error("Unsupported zone type.");
            ...
            if (zoneType == 0)
                i = tecZoneCreateIJK(outputFileHandle, zoneTitle, iMax, jMax, kMax, &varTypes[0],
                                     ...
                                     shareConnectivityFromZone, numFaceConnections, faceNeighborMode,
                                     &outputZone);
            else
            ...
            if (parentZone != 0) i = tecZoneSetParentZone(outputFileHandle, outputZone, parentZone);
                ...
                if (passiveVarList[var - 1] == 0 && shareVarFromZone[var - 1] == 0) {
                    ...
                    switch ((FieldDataType_e) varTypes[var - 1]) {
                        case FieldDataType_Float: {
                            ...
                            i = tecZoneVarGetFloatValues(inputFileHandle, inputZone, var, 1, numValues,
                                                         &values[0]);
                            i = tecZoneVarWriteFloatValues(outputFileHandle, outputZone, var, 0, numValues,
                                                           &values[0]);
                        } break;
                        case FieldDataType_Double: {
                            std::vector<double> values(numValues);
                            i = tecZoneVarGetDoubleValues(inputFileHandle, inputZone, var, 1, numValues,
                                                          &values[0]);
                            i = tecZoneVarWriteDoubleValues(outputFileHandle, outputZone, var, 0, numValues,
                                                            &values[0]);
                        } break;
                        case FieldDataType_Int32: {
                            std::vector<int32_t> values(numValues);
                            i = tecZoneVarGetInt32Values(inputFileHandle, inputZone, var, 1, numValues,
                                                         &values[0]);
                            i = tecZoneVarWriteInt32Values(outputFileHandle, outputZone, var, 0, numValues,
                                                           &values[0]);
                        } break;
                        case FieldDataType_Int16: {
                            std::vector<int16_t> values(numValues);
                            i = tecZoneVarGetInt16Values(inputFileHandle, inputZone, var, 1, numValues,
                                                         &values[0]);
                            i = tecZoneVarWriteInt16Values(outputFileHandle, outputZone, var, 0, numValues,
                                                           &values[0]);
                        } break;
                        case FieldDataType_Byte: {
                            std::vector<uint8_t> values(numValues);
                            i = tecZoneVarGetUInt8Values(inputFileHandle, inputZone, var, 1, numValues,
                                                         &values[0]);
                            i = tecZoneVarWriteUInt8Values(outputFileHandle, outputZone, var, 0, numValues,
                                                           &values[0]);
                        } break;
                        default:
                            i = -1;
        ...
        for (int32_t whichSet = 1; whichSet <= numSets; ++whichSet) {
            char* labelSet = NULL;
            i = tecCustomLabelsGetSet(inputFileHandle, whichSet,
                                      &labelSet);// Labels returned as "\"Mon\",\"Tues\",\"Wed\""
        ...
        for (int32_t geom = 1; geom <= numGeoms; ++geom) {
            ...
            switch (geomType) {
                ...
                case GeomType_LineSegs3D:
                    ...
                    for (int32_t segment = 1; segment <= numSegments; ++segment) {
                        i = tecGeomLineSegmentGetPointCount(inputFileHandle, geom, segment,
                                                            &numSegPts[segment - 1]);
                        for (int32_t index = 1; index <= numSegPts[segment - 1]; ++index) {
                            double geomX, geomY, geomZ;
                            i = tecGeomLineGetPoint(inputFileHandle, geom, segment, index, &geomX, &geomY,
                                                    &geomZ);
                    ...
                    if (geomType == GeomType_LineSegs) {
                        if (numSegments == 1)
                            i = tecGeom2DLineSegmentsBegin(outputFileHandle, x, y, numSegPts[0],
                                                           &xGeomData[0], &yGeomData[0], coordMode);
                        else
                            i = tecGeom2DMultiLineSegmentsBegin(outputFileHandle, x, y, numSegments,
                                                                &numSegPts[0], &xGeomData[0], &yGeomData[0],
                                                                coordMode);
                    } else {
                        if (numSegments == 1)
                            i = tecGeom3DLineSegmentsBegin(outputFileHandle, x, y, z, numSegPts[0],
                                                           &xGeomData[0], &yGeomData[0], &zGeomData[0]);
                        else
                            i = tecGeom3DMultiLineSegmentsBegin(outputFileHandle, x, y, z, numSegments,
                                                                &numSegPts[0], &xGeomData[0], &yGeomData[0],
                                                                &zGeomData[0]);
                    }
                    ...
                    if (arrowheadAttachment != 0)
                        i = tecGeomArrowheadSetInfo(outputFileHandle, arrowheadAngle, arrowheadAttachment,
            ...
            if (strlen(macroFunctionCmd)) i = tecGeomSetMacroFunctionCmd(outputFileHandle, macroFunctionCmd);
        ...
        for (int32_t text = 1; text <= numTexts; ++text) {
            ...
            if (boxType != 0) {
                ...
                tecTextBoxSetInfo(outputFileHandle, boxType, boxColor, boxFillColor, boxLineThickness,
                                  boxMargin);
            }
