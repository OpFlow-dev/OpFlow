...
int32_t initializeFile(
        ...
        MPI_Comm mpiComm,
#endif
        int numPartitions, void** fileHandle);

int32_t createZone(int timeStep, int32_t numPartitions, int32_t const* partitionOwners, double solTime,
...
int32_t createData(
        ...
        int commRank,
#endif
        int timeStep, int32_t numPartitions, int32_t const* partitionOwners, double solTime, void* fileHandle,
        int32_t zone);
...
int main(int argc, char** argv) {
    ...
    assert(numPartitions == 0
           || numPartitions == 3); /* ...because this example handles only on or the other */
    int32_t* partitionOwners = NULL;
    if (numPartitions > 0) partitionOwners = (int32_t*) malloc(numPartitions * sizeof(int32_t));
    ...
    if (returnValue == 0) {
        ...
        if (commSize == 0) {
            ...
            for (int ptn = 0; ptn < numPartitions; ++ptn) partitionOwners[ptn] = (ptn % commSize);
        }
    }
#endif
...
// Add custom labels to the file
...
#else
            returnValue = tecCustomLabelsAddSet(fileHandle,
    ...
    if (returnValue == 0) returnValue = finalizeFile(&fileHandle);
...
int32_t initializeFile(
    ...
    int32_t debug = 1;
    int32_t vIsDouble = 1;
    int32_t returnValue = TECINI142("Flush Example Data Set", (char*) varNames, fileName, ".", &fileFormat,
                                    &fileType, &debug, &vIsDouble);
#else
    int32_t returnValue = tecFileWriterOpen(fileName, "Flush Example Data Set", varNames, FILEFORMAT_SZL,
                                            FILETYPE_FULL, FieldDataType_Double, NULL, fileHandle);
    if (returnValue == 0) returnValue = tecFileSetDiagnosticsLevel(*fileHandle, 1);
#endif
...
int32_t createZone(int timeStep, int32_t numPartitions, int32_t const* partitionOwners, double solTime,
    ...
    int32_t fNMode = 0;
    int32_t zero = 0;
    int32_t returnValue = TECZNE142(zoneTitle, &zoneType, &nNodes, &nCells, &nFaces, &iCellMax, &jCellMax,
                                    &kCellMax, &solTime, &strandID, &parentZn, &isBlock, &nFConns, &fNMode,
                                    &zero, &zero, &zero, NULL, valueLocations, shareVarPtr, &shrConn);
    ++*zone;
#else
    // Note: There are 64-bit versions of this and related routines for zones that overflow 32-bit limits
    int32_t returnValue = tecZoneCreateFE(fileHandle, zoneTitle, ZONETYPE_FEBRICK, nNodes, nCells, NULL,
                                          shareVarPtr, valueLocations, NULL, shrConn, 0, 0, zone);
    if (returnValue == 0) returnValue = tecZoneSetUnsteadyOptions(fileHandle, *zone, solTime, strandID);
#endif
...
/**
 */
static void appendGhostItems(int32_t* nGhosts, int32_t** ghosts, int32_t** gPartitions, int32_t** gPGhosts,
                             int32_t ptn, int32_t gIDim, int32_t gJDim, int32_t gIStart, int32_t gIEnd,
                             int32_t gJStart, int32_t gJEnd, int32_t gKStart, int32_t gKEnd, int32_t oIDim,
                             int32_t oJDim, int32_t oIStart, int32_t oIEnd, int32_t oJStart, int32_t oJEnd,
                             int32_t oKStart, int32_t oKEnd) {
    ...
    *ghosts = (int32_t*) realloc(*ghosts, *nGhosts * sizeof(int32_t));
    if (gPartitions) *gPartitions = (int32_t*) realloc(*gPartitions, *nGhosts * sizeof(int32_t));
    if (gPGhosts) *gPGhosts = (int32_t*) realloc(*gPGhosts, *nGhosts * sizeof(int32_t));
    ...
    for (int32_t i = gIStart; i < gIEnd; ++i)
        for (int32_t j = gJStart; j < gJEnd; ++j)
            for (int32_t k = gKStart; k < gKEnd; ++k) {
                ...
                if (gPartitions) (*gPartitions)[gOffset] = ptn;
                if (gPGhosts) {
...
/**
 */
static void GatherGhostNodesAndCells(int32_t* nGNodes, int32_t** ghostNodes, int32_t** gNPartitions,
                                     int32_t** gNPNodes, int32_t* nGCells, int32_t** ghostCells,
                                     int32_t* iDim, int32_t* jDim, int32_t* kDim, int32_t* jMin,
                                     int32_t* jMax) {
    /*
    ...
    appendGhostItems(&nGNodes[0], &ghostNodes[0], &gNPartitions[0], &gNPNodes[0], 2, iDim[0],
                     jDim[0],                                          /* I- and J-dimensions */
    ...
    appendGhostItems(&nGNodes[0], &ghostNodes[0], &gNPartitions[0], &gNPNodes[0], 3, iDim[0],
                     jDim[0],                                                /* I- and J-dimensions */
    ...
    appendGhostItems(&nGCells[0], &ghostCells[0], NULL, NULL, 2, iDim[0] - 1,
                     jDim[0] - 1,                                              /* I- and J-dimensions */
    ...
    appendGhostItems(
            &nGCells[0], &ghostCells[0], NULL, NULL, 3, iDim[0] - 1, jDim[0] - 1, /* I- and J-dimensions */
    ...
    appendGhostItems(&nGNodes[1], &ghostNodes[1], &gNPartitions[1], &gNPNodes[1], 1, iDim[1],
                     jDim[1],                                           /* I- and J-dimensions */
    ...
    /* Nodes owned by the third partition. */
    appendGhostItems(&nGNodes[1], &ghostNodes[1], &gNPartitions[1], &gNPNodes[1], 3, iDim[1],
                     jDim[1],                                      /* I- and J-dimensions */
    ...
    appendGhostItems(&nGCells[1], &ghostCells[1], NULL, NULL, 1, iDim[1] - 1,
                     jDim[1] - 1,                                               /* I- and J-dimensions */
    ...
    /* Cells owned by the third partition. */
    appendGhostItems(&nGCells[1], &ghostCells[1], NULL, NULL, 3, iDim[1] - 1,
                     jDim[1] - 1,                                              /* I- and J-dimensions */
    ...
    ghostCells[2] = NULL;
    /* Nodes owned by the first partition */
    appendGhostItems(&nGNodes[2], &ghostNodes[2], &gNPartitions[2], &gNPNodes[2], 1, iDim[2],
                     jDim[2],                                                 /* I- and J-dimensions */
                     0, 2, 0, jDim[2], 0, kDim[2],                            /* local index ranges */
                     iDim[0], jDim[0],                                        /* I- and J-dimensions */
                     iDim[0] - 3, iDim[0] - 1, jMin[2], jMax[2], 0, kDim[0]); /* local index ranges */
    /* Nodes owned by the second partition. */
    appendGhostItems(&nGNodes[2], &ghostNodes[2], &gNPartitions[2], &gNPNodes[2], 2, iDim[2],
                     jDim[2],                                           /* I- and J-dimensions */
                     2, iDim[2], 0, 2, 0, kDim[2],                      /* local index ranges */
                     iDim[1], jDim[1],                                  /* I- and J-dimensions */
                     2, iDim[1], jDim[1] - 3, jDim[1] - 1, 0, kDim[1]); /* local index ranges */
    /* Cells owned by the first partition */
    appendGhostItems(&nGCells[2], &ghostCells[2], NULL, NULL, 1, iDim[2] - 1,
                     jDim[2] - 1,                          /* I- and J-dimensions */
                     0, 1, 0, jDim[2] - 1, 0, kDim[2] - 1, /* local index ranges */
                     iDim[0] - 1, jDim[0] - 1,             /* I- and J-dimensions */
                     iDim[0] - 2, iDim[0] - 1, jMin[2], jMax[2] - 1, 0, kDim[0] - 1); /* local index ranges */
    /* Nodes owned by the second partition. */
    appendGhostItems(&nGCells[2], &ghostCells[2], NULL, NULL, 2, iDim[2] - 1,
                     jDim[2] - 1,                                               /* I- and J-dimensions */
...
int32_t createData(
...
#endif
    {
        ...
        returnValue
                = TECTXT142(&xPos, &yPos, &zPos, &posCoordMode, &attachToZone, &zone, &font, &fontHeightUnits,
                            &fontHeight, &boxType, &boxMargin, &boxLineThickness, &boxColor, &boxFillColor,
                            &angle, &anchor, &lineSpacing, &textColor, &scope, &clipping, timeStepLabel, MFC);
#else
        if (returnValue == 0) returnValue = tecText3DBegin(fileHandle, timeStepLabel, 5.0, 0.0, 8.0, 18.0, 2);
        if (returnValue == 0) returnValue = tecTextAttachToZone(fileHandle, zone);
        if (returnValue == 0) returnValue = tecTextEnd(fileHandle);
#endif
    }
...
/*
...
#endif
    {
        ...
        char const* MFC = "";
        returnValue = TECGEO142(&xPos, &yPos, &zPos, &posCoordMode, &attachToZone, &zone, &color, &fillColor,
                                &isFilled, &geomType, &linePattern, &patternLength, &lineThicknessness,
                                &numEllipsePts, &arrowheadStyle, &arrowheadAttachment, &arrowheadSize,
                                &arrowheadAngle, &scope, &clipping, &numSegments, numSegPts, xOrThetaGeomData,
                                yOrRGeomData, zGeomData, MFC);
#else
        if (returnValue == 0)
            returnValue = tecGeomCircleBegin(fileHandle, 3.0, 2.0 + 0.1 * timeStep, 1.0, 0 /* grid-mode */);
        if (returnValue == 0) returnValue = tecGeomAttachToZone(fileHandle, zone);
        if (returnValue == 0) returnValue = tecGeomEnd(fileHandle);
#endif
    }
    ...
    assert(numPartitions == 0
           || numPartitions == 3); /* ...because this example only handles on or the other */
    if (numPartitions > 0) {
        ...
        for (int32_t k = 0; k < kDim[ptn] - 1; ++k)
            for (int32_t j = 0; j < jDim[ptn] - 1; ++j)
                for (int32_t i = 0; i < iDim[ptn] - 1; ++i) {
                    ...
                    p[ptn][index]
                            = (float) (solTime
                                       + (i + iMin[ptn] + 1) * (j + jMin[ptn] + 1) * (k + kMin[ptn] + 1));

                    if (timeStep == 0) {
                        connectivity[ptn][8 * index] = (k * jDim[ptn] + j) * iDim[ptn] + i
                                                       + 1; /* One-based to feed TECIO old API */
                        ...
                        connectivity[ptn][8 * index + 4]
                                = connectivity[ptn][8 * index] + iDim[ptn] * jDim[ptn];
                        connectivity[ptn][8 * index + 5]
                                = connectivity[ptn][8 * index + 1] + iDim[ptn] * jDim[ptn];
                        connectivity[ptn][8 * index + 6]
                                = connectivity[ptn][8 * index + 2] + iDim[ptn] * jDim[ptn];
                        connectivity[ptn][8 * index + 7]
                                = connectivity[ptn][8 * index + 3] + iDim[ptn] * jDim[ptn];
                    }
                }
    }
    ...
    if (numPartitions > 0) {
        ...
        GatherGhostNodesAndCells(nGNodes, ghostNodes, gNPartitions, gNPNodes, nGCells, ghostCells, iDim, jDim,
                                 kDim, jMin, jMax);
    }
    ...
    for (int32_t ptn = 1; returnValue == 0 && ptn <= effectiveNumPartitions; ++ptn) {
...
#endif
        {
            if (numPartitions > 0) {
                if (returnValue == 0)
#if defined OLD_API
                    returnValue = TECFEPTN142(&ptn, &pNNodes[ptn - 1], &pNCells[ptn - 1], &nGNodes[ptn - 1],
                                              ghostNodes[ptn - 1], gNPartitions[ptn - 1], gNPNodes[ptn - 1],
                                              &nGCells[ptn - 1], ghostCells[ptn - 1]);
#else
                    returnValue = tecFEPartitionCreate32(
                            fileHandle, zone, ptn, pNNodes[ptn - 1], pNCells[ptn - 1], nGNodes[ptn - 1],
                            ghostNodes[ptn - 1], gNPartitions[ptn - 1], gNPNodes[ptn - 1], nGCells[ptn - 1],
                            ghostCells[ptn - 1]);
#endif
...
#if defined OLD_API
                if (returnValue == 0) returnValue = TECDAT142(&pNNodes[ptn - 1], x[ptn - 1], &dIsDouble);
                if (returnValue == 0) returnValue = TECDAT142(&pNNodes[ptn - 1], y[ptn - 1], &dIsDouble);
                if (returnValue == 0) returnValue = TECDAT142(&pNNodes[ptn - 1], z[ptn - 1], &dIsDouble);
#else
                if (returnValue == 0)
                    returnValue = tecZoneVarWriteFloatValues(fileHandle, zone, 1, ptn, pNNodes[ptn - 1],
                                                             x[ptn - 1]);
                if (returnValue == 0)
                    returnValue = tecZoneVarWriteFloatValues(fileHandle, zone, 2, ptn, pNNodes[ptn - 1],
                                                             y[ptn - 1]);
                if (returnValue == 0)
                    returnValue = tecZoneVarWriteFloatValues(fileHandle, zone, 3, ptn, pNNodes[ptn - 1],
                                                             z[ptn - 1]);
#endif
            }
...
#else
                returnValue
                        = tecZoneVarWriteFloatValues(fileHandle, zone, 4, ptn, pNCells[ptn - 1], p[ptn - 1]);
#endif
...
/* write out the connectivity */
                ...
                if (returnValue == 0) returnValue = TECNODE142(&connectivityCount, connectivity[ptn - 1]);
#else
                int32_t connectivityCount = 8 * pNCells[ptn - 1];
                if (returnValue == 0)
                    returnValue = tecZoneNodeMapWrite32(fileHandle, zone, ptn, 1, connectivityCount,
                                                        connectivity[ptn - 1]);
#endif
            }
