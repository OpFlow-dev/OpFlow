...
int main(int argc, const char *argv[]) {
    ...
    if (argc == 2 && strncmp(argv[1], "--szl", 5) == 0) fileFormat = 1;
    else
        fileFormat = 0;
    ...
    I = TECINI142((char *) "DATASET WITH ONE ORDERED ZONE AND ONE FE-QUAD ZONE OVER 2 TIME STEPS",
                  (char *) "X Y P", (char *) "comtestcpp-field.plt", (char *) ".", &fileFormat,
                  &FieldFileType, &Debug, &VIsDouble);
    /*
     * Open line.plt and write the header information.
     */
    VIsDouble = 1;
    I = TECINI142((char *) "DATASET WITH ONE I-ORDERED ZONE", (char *) "X Y", (char *) "comtestcpp-line.plt",
                  (char *) ".", &fileFormat, &LineFileType, &Debug, &VIsDouble);
    ...
    I = TECZNE142((char *) "Ordered Zone 1", &ZoneType, &IMax, &JMax, &KMax, &ICellMax, &JCellMax, &KCellMax,
                  &SolTime, &StrandID, &ParentZn, &IsBlock, &NumFaceConnections, &FaceNeighborMode,
                  NULL, /* PassiveVarList */
                  NULL, /* ValueLocation */
    ...
    I = TECZNE142((char *) "XY Line plot", &ZoneType, &IMax, &JMax, &KMax, &ICellMax, &JCellMax, &KCellMax,
                  &SolTime, &StrandID, &ParentZn, &IsBlock, &NumFaceConnections, &FaceNeighborMode,
                  0,    /* TotalNumFaceNodes */
                  0,    /* NumConnectedBoundaryFaces */
                  0,    /* TotalNumBoundaryConnections */
    ...
    SolTime = 10.0;
    StrandID = 2;
    I = TECZNE142((char *) "Finite Zone 1", &ZoneType, &NPts, &NElm, &KMax, &ICellMax, &JCellMax, &KCellMax,
                  &SolTime, &StrandID, &ParentZn, &IsBlock, &NumFaceConnections, &FaceNeighborMode,
                  0,    /* TotalNumFaceNodes */
                  0,    /* NumConnectedBoundaryFaces */
                  0,    /* TotalNumBoundaryConnections */
     ...
     */
    for (J = 0; J < 5; J++)
        for (I = 0; I < 4; I++) { P[J][I] = (float) (2.0 * (I + 1) * (J + 1)); }
    ...
    I = TECZNE142((char *) "Ordered Zone 2", &ZoneType, &IMax, &JMax, &KMax, &ICellMax, &JCellMax, &KCellMax,
                  &SolTime, &StrandID, &ParentZn, &IsBlock, &NumFaceConnections, &FaceNeighborMode,
                  0, /* TotalNumFaceNodes */
                  0, /* NumConnectedBoundaryFaces */
                  0, /* TotalNumBoundaryConnections */
                  NULL, NULL, SharingZone, &ShareConnectivityFromZone);
     ...
     */
    for (J = 0; J < 5; J++)
        for (I = 0; I < 4; I++) { P[J][I] = (float) (3.0 * (I + 1) * (J + 1)); }
    ...
    I = TECZNE142((char *) "Finite Zone 2", &ZoneType, &NPts, &NElm, &KMax, &ICellMax, &JCellMax, &KCellMax,
                  &SolTime, &StrandID, &ParentZn, &IsBlock, &NumFaceConnections, &FaceNeighborMode,
                  0,    /* TotalNumFaceNodes */
                  0,    /* NumConnectedBoundaryFaces */
                  0,    /* TotalNumBoundaryConnections */
                  NULL, /* PassiveVarList */
                  NULL, /* ValueLocation */
                  SharingZone, &ShareConnectivityFromZone);
    ...
    III = TECTXT142(&XP, &YP, &ZP, &PositionCoordSys, &AttachToZone, &Zone, &FontType, &HeightUnits, &FH,
                    &BoxType, &BoxMargin, &BoxLineThickness, &BoxColor, &BoxFillColor, &TextAngle, &Anchor,
                    &LineSpacing, &TextColor, &Scope, &Clipping, (char *) "Hi Mom", (char *) "");
    ...
    III = TECGEO142(&XP, &YP, &ZP, &PositionCoordSys, &AttachToZone, &Zone, &Color, &FillColor, &IsFilled,
                    &GeomType, &LinePattern, &PatternLength, &LineThickness, &NumEllipsePts, &ArrowheadStyle,
                    &ArrowheadAttachment, &ArrowheadSize, &ArrowheadAngle, &Scope, &Clipping, &NumSegments,
                    NumSegPts, &XGeomData[0], &YGeomData[0], &ZGeomData[0], (char *) "");
