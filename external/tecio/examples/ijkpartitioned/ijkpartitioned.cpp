...
INTEGER4 outputVarData(VAR_TYPE* var, INTEGER4 iDim, INTEGER4 jDim, INTEGER4 kDim, INTEGER4 iMin,
                       INTEGER4 jMin, INTEGER4 kMin, INTEGER4 iMax, INTEGER4 jMax, INTEGER4 kMax);
...
int main(int argc, char** argv) {
     ...
     * header information
     */
    I = TECINI142((char*) "IJK Ordered Zone", (char*) "X Y Z P", (char*) "ijkpartitioned.szplt", (char*) ".",
    ...
    INTEGER4 ZoneType = 0;
    I = TECZNE142((char*) "Ordered Zone", &ZoneType, &IMax, &JMax, &KMax, &ICellMax, &JCellMax, &KCellMax,
                  &SolTime, &StrandID, &ParentZn, &IsBlock, &NFConns, &FNMode, &TotalNumFaceNodes,
                  &TotalNumBndryFaces, &TotalNumBndryConnections,
                  NULL,          // PassiveVarList
     ...
     */
    INTEGER4 partitionIndices[4][6] = {{1, 1, 1, XDIM / 3 + 1, YDIM, ZDIM},
...
// Output partitions
    ...
    for (INTEGER4 ptn = 0; ptn < numPartitions; ++ptn) partitionOwners.push_back(ptn % commSize);
    ...
    for (INTEGER4 partition = 1; partition <= 4; ++partition) {
        ...
        if (partitionOwners[partition - 1] == commRank) {
            ...
            I = TECIJKPTN142(&partition, &partitionIMin, &partitionJMin, &partitionKMin, &partitionIMax,
                             &partitionJMax, &partitionKMax);
            I = outputVarData(x, XDIM, YDIM, ZDIM, partitionIMin, partitionJMin, partitionKMin, partitionIMax,
                              partitionJMax, partitionKMax);
            I = outputVarData(y, XDIM, YDIM, ZDIM, partitionIMin, partitionJMin, partitionKMin, partitionIMax,
                              partitionJMax, partitionKMax);
            I = outputVarData(z, XDIM, YDIM, ZDIM, partitionIMin, partitionJMin, partitionKMin, partitionIMax,
                              partitionJMax, partitionKMax);
            I = outputVarData(p, XDIM - 1, YDIM - 1, ZDIM - 1, partitionIMin, partitionJMin, partitionKMin,
                              partitionIMax - 1, partitionJMax - 1, partitionKMax - 1);
#if defined TECIOMPI
        }
#endif
    }
    ...
    for (INTEGER4 partition = 2; partition <= 4; ++partition) {
        ...
        if (commRank == mainRank || partitionOwners[partition - 1] == commRank) {
            ...
            I = TECZNE142((char*) "Ordered Zone", &ZoneType, &IMax, &JMax, &KMax, &ICellMax, &JCellMax,
...
#endif
            I = outputVarData(x, XDIM, YDIM, ZDIM, XDIM, surfaceZoneJMin, surfaceZoneKMin, XDIM,
                              surfaceZoneJMax, surfaceZoneKMax);
            I = outputVarData(y, XDIM, YDIM, ZDIM, XDIM, surfaceZoneJMin, surfaceZoneKMin, XDIM,
                              surfaceZoneJMax, surfaceZoneKMax);
            I = outputVarData(z, XDIM, YDIM, ZDIM, XDIM, surfaceZoneJMin, surfaceZoneKMin, XDIM,
                              surfaceZoneJMax, surfaceZoneKMax);
            // Output the cell-centered values for the iMax - .5 plane of cells
            I = outputVarData(p, XDIM - 1, YDIM - 1, ZDIM - 1, XDIM - 1, surfaceZoneJMin, surfaceZoneKMin,
                              XDIM - 1, surfaceZoneJMax - 1, surfaceZoneKMax - 1);
#if defined TECIOMPI
        }
#endif
    }
