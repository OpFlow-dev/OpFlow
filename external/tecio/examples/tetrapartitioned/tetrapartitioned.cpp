...
void GatherGhostNodesAndCells(INTEGER4* nGNodes, INTEGER4** ghostNodes, INTEGER4** gNPartitions,
...
int main(int argc, char** argv) {
...
#endif
    returnValue = TECINI142((char*) "SIMPLE DATASET", (char*) "x y z p", (char*) "tetrapartitioned.szplt",
                            (char*) ".", &fileFormat, &fileType, &debug, &vIsDouble);
#if defined TECIOMPI
    if (returnValue == 0) returnValue = TECMPIINIT142(&mpiComm, &mainRank);
#endif
    ...
    if (returnValue == 0)
        returnValue
                = TECZNE142((char*) "Partitioned Zone", &zoneType, &nNodes, &nCells, &nFaces, &iCellMax,
        ...
        for (int k = 0; k < kDim[ptn] - 1; ++k)
            for (int j = 0; j < jDim[ptn] - 1; ++j)
                for (int i = 0; i < iDim[ptn] - 1; ++i) {
                    ...
                    int tetraCorners[2][5][4] = {{/* "Even" bricks */
                                                  {0, 1, 2, 5},
                                                  {0, 2, 3, 7},
                                                  {4, 7, 5, 0},
                                                  {5, 7, 6, 2},
                    ...
                    for (whichTet = 0; whichTet < 5; ++whichTet) {
                        ...
                        p[ptn][index]
                                = (float) ((i + iMin[ptn] + 1) * (j + jMin[ptn] + 1) * (k + kMin[ptn] + 1));
                        for (whichCorner = 0; whichCorner < 4; ++whichCorner) {
                            connectivity[ptn][index * 4 + whichCorner]
                                    = brickNodes[tetraCorners[evenOdd][whichTet][whichCorner]];
                        }
                    }
                }
    }
    ...
    // Output partitions
#if defined TECIOMPI
    for (int ptn = 0; ptn < 3; ++ptn) partitionOwners[ptn] = (ptn % commSize);
    if (returnValue == 0) returnValue = TECZNEMAP142(&numPartitions, &partitionOwners[0]);
#endif
    ...
    for (INTEGER4 ptn = 1; ptn <= 3; ++ptn) {
        ...
        if (partitionOwners[ptn - 1] == commRank) {
            ...
            if (ghostNodes[ptn - 1]) free(ghostNodes[ptn - 1]);
            if (gNPartitions[ptn - 1]) free(gNPartitions[ptn - 1]);
            if (gNPNodes[ptn - 1]) free(gNPNodes[ptn - 1]);
            if (ghostCells[ptn - 1]) free(ghostCells[ptn - 1]);
            /*
...
void appendGhostNodes(INTEGER4* nGhosts, INTEGER4** ghosts, INTEGER4** gPartitions, INTEGER4** gPGhosts,
                      INTEGER4 ptn, INTEGER4 gIDim, INTEGER4 gJDim, INTEGER4 gIStart, INTEGER4 gIEnd,
                      INTEGER4 gJStart, INTEGER4 gJEnd, INTEGER4 gKStart, INTEGER4 gKEnd, INTEGER4 oIDim,
                      INTEGER4 oJDim, INTEGER4 oIStart, INTEGER4 oIEnd, INTEGER4 oJStart, INTEGER4 oJEnd,
...
void appendGhostCells(INTEGER4* nGhosts, INTEGER4** ghosts, INTEGER4 gIDim, INTEGER4 gJDim, INTEGER4 gIStart,
                      INTEGER4 gIEnd, INTEGER4 gJStart, INTEGER4 gJEnd, INTEGER4 gKStart, INTEGER4 gKEnd) {
...
void GatherGhostNodesAndCells(INTEGER4* nGNodes, INTEGER4** ghostNodes, INTEGER4** gNPartitions,
    ...
    appendGhostNodes(&nGNodes[0], &ghostNodes[0], &gNPartitions[0], &gNPNodes[0], 2, iDim[0],
    ...
    appendGhostNodes(&nGNodes[0], &ghostNodes[0], &gNPartitions[0], &gNPNodes[0], 3, iDim[0],
                     jDim[0],                                               // I- and J-dimensions
    ...
    appendGhostCells(&nGCells[0], &ghostCells[0], iDim[0] - 1, jDim[0] - 1,    // I- and J-dimensions
    ...
    appendGhostNodes(&nGNodes[1], &ghostNodes[1], &gNPartitions[1], &gNPNodes[1], 1, iDim[1],
    ...
    // Nodes owned by the third partition.
    appendGhostNodes(&nGNodes[1], &ghostNodes[1], &gNPartitions[1], &gNPNodes[1], 3, iDim[1],
                     jDim[1],                                     // I- and J-dimensions
    ...
    appendGhostCells(&nGCells[1], &ghostCells[1], iDim[1] - 1, jDim[1] - 1,// I- and J-dimensions
    ...
    ghostCells[2] = NULL;
    // Nodes owned by the first partition
    appendGhostNodes(&nGNodes[2], &ghostNodes[2], &gNPartitions[2], &gNPNodes[2], 1, iDim[2],
    ...
    // Nodes owned by the second partition.
    appendGhostNodes(&nGNodes[2], &ghostNodes[2], &gNPartitions[2], &gNPNodes[2], 2, iDim[2],
                     jDim[2],                                          // I- and J-dimensions
                     2, iDim[2], 0, 2, 0, kDim[2],                     // local index ranges
                     iDim[1], jDim[1],                                 // I- and J-dimensions
                     2, iDim[1], jDim[1] - 3, jDim[1] - 1, 0, kDim[1]);// local index ranges
    // Cells owned by the first partition
    appendGhostCells(&nGCells[2], &ghostCells[2], iDim[2] - 1, jDim[2] - 1,// I- and J-dimensions
