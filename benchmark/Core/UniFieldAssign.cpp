...
#pragma omp tile sizes(8, 8)
        ...
        for (auto j = 0; j < n - 1; ++j)
            for (auto i = 0; i < n - 1; ++i) u(DS::MDIndex < 2 >
        {
            i, j
        }
        )
...
BENCHMARK(UniFieldAssignRaw_2d) -> Apply(UniFieldAssign_2d_Params)
->
UseRealTime();

BENCHMARK(UniFieldAssign_2d) -> Apply(UniFieldAssign_2d_Params)
->
UseRealTime();
