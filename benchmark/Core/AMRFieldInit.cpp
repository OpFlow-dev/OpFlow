...
class AMRInit2D : public benchmark::Fixture {
public:
...
BENCHMARK_DEFINE_F(AMRInit2D, InitByFunctor)(benchmark::State& state) {
    omp_set_num_threads(state.range(0));
    for (auto _ : state) {
        u->initBy([](auto&& x) { return x[0] * x[1]; });
    }
}
...
BENCHMARK_DEFINE_F(AMRInit2D, InitByConst)(benchmark::State& state) {
    omp_set_num_threads(state.range(0));
    for (auto _ : state) { u->initBy(0.123456789); }
}

BENCHMARK_REGISTER_F(AMRInit2D, InitByFunctor)
        ->RangeMultiplier(2)
        ->Range(1, omp_get_max_threads())
        ->UseRealTime()
        ->Unit(benchmark::kMicrosecond);
BENCHMARK_REGISTER_F(AMRInit2D, InitByConstFunctor)
        ->RangeMultiplier(2)
        ->Range(1, omp_get_max_threads())
        ->UseRealTime()
        ->Unit(benchmark::kMicrosecond);
BENCHMARK_REGISTER_F(AMRInit2D, InitByConst)
        ->RangeMultiplier(2)
        ->Range(1, omp_get_max_threads())
        ->UseRealTime()
        ->Unit(benchmark::kMicrosecond);

class AMRInit3D : public benchmark::Fixture {
public:
...
BENCHMARK_DEFINE_F(AMRInit3D, InitByFunctor)(benchmark::State& state) {
    omp_set_num_threads(state.range(0));
    for (auto _ : state) {
        u->initBy([](auto&& x) { return x[0] * x[1] * x[2]; });
    }
}
...
BENCHMARK_DEFINE_F(AMRInit3D, InitByConst)(benchmark::State& state) {
    omp_set_num_threads(state.range(0));
    for (auto _ : state) { u->initBy(0.123456789); }
}

BENCHMARK_REGISTER_F(AMRInit3D, InitByFunctor)
        ->RangeMultiplier(2)
        ->Range(1, omp_get_max_threads())
        ->UseRealTime()
        ->Unit(benchmark::kMillisecond);
BENCHMARK_REGISTER_F(AMRInit3D, InitByConstFunctor)
        ->RangeMultiplier(2)
        ->Range(1, omp_get_max_threads())
        ->UseRealTime()
        ->Unit(benchmark::kMillisecond);
BENCHMARK_REGISTER_F(AMRInit3D, InitByConst)
        ->RangeMultiplier(2)
        ->Range(1, omp_get_max_threads())
        ->UseRealTime()
        ->Unit(benchmark::kMillisecond);
