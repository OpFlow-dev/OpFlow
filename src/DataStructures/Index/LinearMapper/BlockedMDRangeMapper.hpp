...
namespace OpFlow::DS
{
    ...
    struct BlockedMDRangeMapper
    {
        ...
        std::array<int, d> getBlockIndex(const MDIndex<d>& idx) const
        {
            if (!checkIndexInBlock(idx, getBlockRank(last_block)))
            {
                ...
                {
                    for (std::size_t j = 0; j < _split[i].size() - 1; ++j)
                    {
        ...
        int getBlockRank(const std::array<int, d>& block_idx) const
        {
            ...
            OP_ASSERT_MSG(static_cast<std::size_t>(block_rank) < _ranges.size(), "Block rank {} out of range {}",
                          block_rank,
                          _ranges.size());
            return block_rank;
        }
        ...
        int getLocalRank(const MDIndex<d>& idx) const
        {
            ...
            OP_ASSERT_MSG(static_cast<std::size_t>(block_rank) < _ranges.size(),
    ...
    private:
        void gatherSplits()
        {
            ...
            for (std::size_t i = 0; i < d; ++i)
            {
                for (const auto& _r : _ranges) { segments[i].emplace_back(_r.start[i], _r.end[i]); }
