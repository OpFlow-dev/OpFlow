...
namespace OpFlow {
    ...
    struct DecayOp {
        ...
        OPFLOW_STRONG_INLINE static auto eval_safe(const T1& t1, const T2& t2, auto&& i) {
            return (DS::inRange(t1.accessibleRange, i) ? t1.evalSafeAt(OP_PERFECT_FOWD(i))
                                                       : t2.evalSafeAt(OP_PERFECT_FOWD(i)));
        }

        template <ExprType T1, ExprType T2>
        OPFLOW_STRONG_INLINE static auto eval(const T1& t1, const T2& t2, auto&& i) {
            return DS::inRange(t1.accessibleRange, i) ? t1.evalAt(OP_PERFECT_FOWD(i))
    ...
    struct ResultType<DecayOp<bc_width>, Arg1, Arg2> {
        using type = typename internal::ExprTrait<Arg2>::template
                twin_type<Expression<DecayOp<bc_width>, Arg1, Arg2>>;
    ...
    OPFLOW_STRONG_INLINE static auto decay(T1&& t1, T2&& t2, T3&& t3, Ts&&... ts) {
        return decay<bc_width>(OP_PERFECT_FOWD(t1),
                     decay<bc_width>(OP_PERFECT_FOWD(t2), OP_PERFECT_FOWD(t3), OP_PERFECT_FOWD(ts)...));
    }
}// namespace OpFlow
