        ...
        resplitWithStrategy(AbstractSplitStrategy<T>* strategy)
        {
            ...
            for (int i = 0; i < intersections.size(); ++i)
            {
                if (!intersections[i].empty())
                {
                    ...
                    if constexpr ((std::is_trivially_default_constructible_v<D> && std::is_trivially_copyable_v<D>) &&
                        std::is_standard_layout_v<D>)
                    {
                        MPI_Request request;
                    ...
                    if constexpr (std::same_as < Meta::RealType < T >, CartesianField >)
                        ...
                        for (int i = 0; i < dim; ++i)
                        {
                            this->bc[i].start = other.bc[i].start
                                                    ...
                                                    : nullptr;
                            this->bc[i].end = other.bc[i].end
                                                  ? genProxyBC<CartesianField>(*other.bc[i].end)
                                                  : nullptr;
                        }
                }
                else
