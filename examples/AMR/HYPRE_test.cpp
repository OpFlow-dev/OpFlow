...
HYPRE_Int GetVariableBox(Index cell_ilower, Index cell_iupper, HYPRE_Int vartype, Index var_ilower,
                         Index var_iupper) {
    HYPRE_Int ierr = 0;
...
HYPRE_Int SScanIntArray(char *sdata_ptr, char **sdata_ptr_ptr, HYPRE_Int size, HYPRE_Int *array) {
    ...
    for (i = 0; i < size; i++) { array[i] = strtol(sdata_ptr, &sdata_ptr, 10); }
...
HYPRE_Int SScanProblemIndex(char *sdata_ptr, char **sdata_ptr_ptr, HYPRE_Int ndim, ProblemIndex index) {
    ...
    for (i = 0; i < 9; i++) { index[i] = 0; }

    sdata_ptr += strspn(sdata_ptr, " \t\n(");
    switch (ndim) {
        case 1:
            hypre_sscanf(sdata_ptr, "%d%c", &index[0], &sign[0]);
            break;

        case 2:
            hypre_sscanf(sdata_ptr, "%d%c%d%c", &index[0], &sign[0], &index[1], &sign[1]);
            break;

        case 3:
            hypre_sscanf(sdata_ptr, "%d%c%d%c%d%c", &index[0], &sign[0], &index[1], &sign[1], &index[2],
                         &sign[2]);
            break;
    }
    ...
    if (*sdata_ptr == ':') {
        ...
        /* pre-shift the index */
        for (i = 0; i < ndim; i++) { index[i] += index[i + 6]; }
    }
    sdata_ptr += strcspn(sdata_ptr, ")") + 1;

    for (i = 0; i < ndim; i++) {
        if (sign[i] == '+') { index[i + 3] = 1; }
    }
...
HYPRE_Int ReadData(char *filename, ProblemData *data_ptr) {
        ...
        if ((hypre_sscanf(sdata_ptr, "%s", key) > 0) && (sdata_ptr[0] != '#')) {
            ...
            if (strcmp(key, "GridCreate:") == 0) {
                ...
                if ((pdata.nboxes % 10) == 0) {
                    ...
                    pdata.ilowers = hypre_TReAlloc(pdata.ilowers, ProblemIndex, size, HYPRE_MEMORY_HOST);
                    pdata.iuppers = hypre_TReAlloc(pdata.iuppers, ProblemIndex, size, HYPRE_MEMORY_HOST);
                    pdata.boxsizes = hypre_TReAlloc(pdata.boxsizes, HYPRE_Int, size, HYPRE_MEMORY_HOST);
                }
                ...
                if ((pdata.ilowers[pdata.nboxes][3] * pdata.ilowers[pdata.nboxes][4]
                             * pdata.ilowers[pdata.nboxes][5]
                     != 0)
                    || (pdata.iuppers[pdata.nboxes][3] * pdata.iuppers[pdata.nboxes][4]
                                * pdata.iuppers[pdata.nboxes][5]
                        != 1)) {
                ...
                for (i = 0; i < 3; i++) {
                    pdata.boxsizes[pdata.nboxes]
                            *= (pdata.iuppers[pdata.nboxes][i] - pdata.ilowers[pdata.nboxes][i] + 2);
                }
                pdata.max_boxsize = hypre_max(pdata.max_boxsize, pdata.boxsizes[pdata.nboxes]);
                pdata.nboxes++;
                ...
                for (s = 0; s < data.nstencils; s++) {
                    data.stencil_offsets[s] = hypre_CTAlloc(Index, data.stencil_sizes[s], HYPRE_MEMORY_HOST);
                    data.stencil_vars[s] = hypre_CTAlloc(HYPRE_Int, data.stencil_sizes[s], HYPRE_MEMORY_HOST);
                ...
                if ((pdata.graph_nentries % 10) == 0) {
                    ...
                    pdata.graph_strides = hypre_TReAlloc(pdata.graph_strides, Index, size, HYPRE_MEMORY_HOST);
                    pdata.graph_vars = hypre_TReAlloc(pdata.graph_vars, HYPRE_Int, size, HYPRE_MEMORY_HOST);
                ...
                for (i = data.ndim; i < 3; i++) { pdata.graph_strides[pdata.graph_nentries][i] = 1; }
                pdata.graph_vars[pdata.graph_nentries] = strtol(sdata_ptr, &sdata_ptr, 10);
                pdata.graph_to_parts[pdata.graph_nentries] = strtol(sdata_ptr, &sdata_ptr, 10);
                SScanProblemIndex(sdata_ptr, &sdata_ptr, data.ndim,
                ...
                for (i = data.ndim; i < 3; i++) { pdata.graph_to_strides[pdata.graph_nentries][i] = 1; }
                pdata.graph_to_vars[pdata.graph_nentries] = strtol(sdata_ptr, &sdata_ptr, 10);
                SScanIntArray(sdata_ptr, &sdata_ptr, data.ndim, pdata.graph_index_maps[pdata.graph_nentries]);
                for (i = data.ndim; i < 3; i++) { pdata.graph_index_maps[pdata.graph_nentries][i] = i; }
                for (i = 0; i < 3; i++) {
                ...
                pdata.graph_entries[pdata.graph_nentries] = strtol(sdata_ptr, &sdata_ptr, 10);
                pdata.graph_values[pdata.graph_nentries] = strtod(sdata_ptr, &sdata_ptr);
                ...
                if ((pdata.matrix_nentries % 10) == 0) {
                    ...
                    pdata.matrix_vars = hypre_TReAlloc(pdata.matrix_vars, HYPRE_Int, size, HYPRE_MEMORY_HOST);
                ...
                SScanIntArray(sdata_ptr, &sdata_ptr, data.ndim, pdata.matrix_strides[pdata.matrix_nentries]);
                for (i = data.ndim; i < 3; i++) { pdata.matrix_strides[pdata.matrix_nentries][i] = 1; }
                pdata.matrix_vars[pdata.matrix_nentries] = strtol(sdata_ptr, &sdata_ptr, 10);
                pdata.matrix_entries[pdata.matrix_nentries] = strtol(sdata_ptr, &sdata_ptr, 10);
                pdata.matrix_values[pdata.matrix_nentries] = strtod(sdata_ptr, &sdata_ptr);
    ...
    for (part = 0; part < data.nparts; part++) {
        data.max_boxsize = hypre_max(data.max_boxsize, data.pdata[part].max_boxsize);
    }
...
HYPRE_Int IntersectBoxes(ProblemIndex ilower1, ProblemIndex iupper1, ProblemIndex ilower2,
                         ProblemIndex iupper2, ProblemIndex int_ilower, ProblemIndex int_iupper) {
    HYPRE_Int d, size;
...
HYPRE_Int DistributeData(ProblemData global_data, Index *refine, Index *distribute, Index *block,
                         HYPRE_Int num_procs, HYPRE_Int myid, ProblemData *data_ptr) {
        ...
        if ((pid < 0) || (pid >= np)) {
            ...
            if ((m[0] * m[1] * m[2]) > 1) {
                ...
                for (entry = 0; entry < pdata.graph_nentries; entry++) {
                    ...
                    for (box = 0; box < pdata.nboxes; box++) {
                        GetVariableBox(pdata.ilowers[box], pdata.iuppers[box],
                                       pdata.vartypes[pdata.graph_vars[entry]], ilower, iupper);
                        size = IntersectBoxes(pdata.graph_ilowers[entry], pdata.graph_iuppers[entry], ilower,
                        ...
                        if (size > 0) {
                            for (d = 0; d < 3; d++) {
                                ...
                                if (mod_graph[d] >= mod_lower) {
                                    ...
                                    int_ilower[d]
                                            += pdata.graph_strides[entry][d] + (mod_graph[d] - mod_lower);
                                }
                            ...
                            for (d = 0; d < 3; d++) {
                                ...
                                pdata.graph_to_iuppers[i][dmap]
                                        = pdata.graph_to_ilowers[entry][dmap]
                                          + sign * size * pdata.graph_to_strides[entry][d];
                                ...
                                pdata.graph_strides[i][d] = pdata.graph_strides[entry][d];
                                pdata.graph_to_strides[i][d] = pdata.graph_to_strides[entry][d];
                                pdata.graph_index_maps[i][d] = dmap;
                                pdata.graph_index_signs[i][d] = sign;
                            }
                            for (d = 3; d < 9; d++) {
                                pdata.graph_ilowers[i][d] = pdata.graph_ilowers[entry][d];
                                pdata.graph_iuppers[i][d] = pdata.graph_iuppers[entry][d];
                                pdata.graph_to_ilowers[i][d] = pdata.graph_to_ilowers[entry][d];
                                pdata.graph_to_iuppers[i][d] = pdata.graph_to_iuppers[entry][d];
                            }
                ...
                for (entry = 0; entry < pdata.matrix_nentries; entry++) {
                    ...
                    for (box = 0; box < pdata.nboxes; box++) {
                        GetVariableBox(pdata.ilowers[box], pdata.iuppers[box],
                                       pdata.vartypes[pdata.matrix_vars[entry]], ilower, iupper);
                        size = IntersectBoxes(pdata.matrix_ilowers[entry], pdata.matrix_iuppers[entry],
                        ...
                        if (size > 0) {
                            ...
                            for (d = 0; d < 3; d++) {
                                ...
                                pdata.matrix_strides[i][d] = pdata.matrix_strides[entry][d];
                            }
                            for (d = 3; d < 9; d++) {
                                pdata.matrix_ilowers[i][d] = pdata.matrix_ilowers[entry][d];
                                pdata.matrix_iuppers[i][d] = pdata.matrix_iuppers[entry][d];
                            }
            ...
            m[2] = block[part][2];
            if ((m[0] * m[1] * m[2]) > 1) {
                pdata.ilowers = hypre_TReAlloc(pdata.ilowers, ProblemIndex, m[0] * m[1] * m[2] * pdata.nboxes,
                                               HYPRE_MEMORY_HOST);
                pdata.iuppers = hypre_TReAlloc(pdata.iuppers, ProblemIndex, m[0] * m[1] * m[2] * pdata.nboxes,
                                               HYPRE_MEMORY_HOST);
                pdata.boxsizes = hypre_TReAlloc(pdata.boxsizes, HYPRE_Int, m[0] * m[1] * m[2] * pdata.nboxes,
                                                HYPRE_MEMORY_HOST);
                for (box = 0; box < pdata.nboxes; box++) {
                    n[0] = pdata.iuppers[box][0] - pdata.ilowers[box][0] + 1;
                    n[1] = pdata.iuppers[box][1] - pdata.ilowers[box][1] + 1;
            ...
            for (box = 0; box < pdata.nboxes; box++) {
                ...
                pdata.max_boxsize = hypre_max(pdata.max_boxsize, pdata.boxsizes[box]);
            }
            for (box = 0; box < pdata.graph_nentries; box++) {
                pdata.graph_boxsizes[box] = 1;
                for (i = 0; i < 3; i++) {
                    pdata.graph_boxsizes[box]
                            *= (pdata.graph_iuppers[box][i] - pdata.graph_ilowers[box][i] + 1);
                }
            }
        }
...
HYPRE_Int DestroyData(ProblemData data) {
    ...
    for (part = 0; part < data.nparts; part++) {
        ...
        if (pdata.nvars > 0) { hypre_TFree(pdata.vartypes, HYPRE_MEMORY_HOST); }
        ...
        if (pdata.nvars > 0) { hypre_TFree(pdata.stencil_num, HYPRE_MEMORY_HOST); }
...
hypre_int hypre_test(hypre_int argc, char *argv[]) {
    char *infile;
    ...
    while (arg_index < argc) {
        if (strcmp(argv[arg_index], "-pt") == 0) {
            arg_index++;
            nparts = 0;
            while (strncmp(argv[arg_index], "-", 1) != 0) { parts[nparts++] = atoi(argv[arg_index++]); }
        } else if (strcmp(argv[arg_index], "-r") == 0) {
            arg_index++;
            for (i = 0; i < nparts; i++) {
                part = parts[i];
                k = arg_index;
                for (j = 0; j < 3; j++) { refine[part][j] = atoi(argv[k++]); }
            }
            arg_index += 3;
            ...
            for (i = 0; i < nparts; i++) {
                part = parts[i];
                k = arg_index;
                for (j = 0; j < 3; j++) { distribute[part][j] = atoi(argv[k++]); }
            }
            arg_index += 3;
            ...
            for (i = 0; i < nparts; i++) {
                part = parts[i];
                k = arg_index;
                for (j = 0; j < 3; j++) { block[part][j] = atoi(argv[k++]); }
            }
            arg_index += 3;
    ...
    for (s = 0; s < data.nstencils; s++) {
        ...
        for (i = 0; i < data.stencil_sizes[s]; i++) {
            HYPRE_SStructStencilSetEntry(stencils[s], i, data.stencil_offsets[s][i], data.stencil_vars[s][i]);
        }
    }
     ...
     *-----------------------------------------------------------*/
    plevels = hypre_TAlloc(HYPRE_Int, data.nparts, HYPRE_MEMORY_HOST);
    prefinements = hypre_TAlloc(Index, data.nparts, HYPRE_MEMORY_HOST);
    for (part = 0; part < data.nparts; part++) {
        pdata = data.pdata[part];
        plevels[part] = pdata.fac_plevel;
        for (i = 0; i < data.ndim; i++) { prefinements[part][i] = pdata.fac_prefinement[i]; }
    }
    ...
    if (((solver_id >= 20) && (solver_id <= 30)) || ((solver_id >= 40) && (solver_id < 60))) {
    ...
    for (part = 0; part < data.nparts; part++) {
        ...
        /* add entries */
        for (entry = 0; entry < pdata.graph_nentries; entry++) {
            for (index[2] = pdata.graph_ilowers[entry][2]; index[2] <= pdata.graph_iuppers[entry][2];
                 index[2] += pdata.graph_strides[entry][2]) {
                for (index[1] = pdata.graph_ilowers[entry][1]; index[1] <= pdata.graph_iuppers[entry][1];
                     index[1] += pdata.graph_strides[entry][1]) {
                    for (index[0] = pdata.graph_ilowers[entry][0]; index[0] <= pdata.graph_iuppers[entry][0];
                        ...
                        for (i = 0; i < 3; i++) {
                            ...
                            to_index[j]
                                    = pdata.graph_to_ilowers[entry][j] + k * pdata.graph_to_strides[entry][j];
                        }
                        HYPRE_SStructGraphAddEntries(graph, part, index, pdata.graph_vars[entry],
    ...
    for (part = 0; part < data.nparts; part++) {
        ...
        for (var = 0; var < pdata.nvars; var++) {
            ...
            for (i = 0; i < data.stencil_sizes[s]; i++) {
                for (j = 0; j < pdata.max_boxsize; j++) { values[j] = data.stencil_values[s][i]; }
                for (box = 0; box < pdata.nboxes; box++) {
                    GetVariableBox(pdata.ilowers[box], pdata.iuppers[box], pdata.vartypes[var], ilower,
                                   iupper);
        ...
        for (entry = 0; entry < pdata.graph_nentries; entry++) {
            for (index[2] = pdata.graph_ilowers[entry][2]; index[2] <= pdata.graph_iuppers[entry][2];
                ...
                for (index[1] = pdata.graph_ilowers[entry][1]; index[1] <= pdata.graph_iuppers[entry][1];
                    ...
                    for (index[0] = pdata.graph_ilowers[entry][0]; index[0] <= pdata.graph_iuppers[entry][0];
                         index[0] += pdata.graph_strides[entry][0]) {
                        HYPRE_SStructMatrixSetValues(A, part, index, pdata.graph_vars[entry], 1,
                                                     &pdata.graph_entries[entry], &pdata.graph_values[entry]);
                    }
                }
            }
        }
                ...
                for (index[2] = pdata.graph_ilowers[entry][2]; index[2] <= pdata.graph_iuppers[entry][2];
                    ...
                    for (index[1] = pdata.graph_ilowers[entry][1]; index[1] <= pdata.graph_iuppers[entry][1];
                        ...
                        for (index[0] = pdata.graph_ilowers[entry][0];
                             ...
                             index[0] += pdata.graph_strides[entry][0]) {
                            HYPRE_SStructMatrixSetValues(A_amg, part, index, pdata.graph_vars[entry], 1,
    ...
    for (part = 0; part < data.nparts; part++) {
        pdata = data.pdata[part];
        for (entry = 0; entry < pdata.matrix_nentries; entry++) {
            for (index[2] = pdata.matrix_ilowers[entry][2]; index[2] <= pdata.matrix_iuppers[entry][2];
                 index[2] += pdata.matrix_strides[entry][2]) {
                for (index[1] = pdata.matrix_ilowers[entry][1]; index[1] <= pdata.matrix_iuppers[entry][1];
                    ...
                    for (index[0] = pdata.matrix_ilowers[entry][0];
                        ...
                        HYPRE_SStructMatrixSetValues(A, part, index, pdata.matrix_vars[entry], 1,
                                                     &pdata.matrix_entries[entry],
    ...
    for (part = data.nparts - 1; part > 0; part--) {
        ...
        HYPRE_SStructFACZeroFCSten(A, grid, part);
    ...
    if (((solver_id >= 20) && (solver_id <= 30)) || ((solver_id >= 40) && (solver_id < 60))) {
        for (part = data.nparts - 1; part >= 0; part--) {
            ...
            for (var = 0; var < pdata.nvars; var++) {
                sA = hypre_SStructPMatrixSMatrix(hypre_SStructMatrixPMatrix(A, part), var, var);
                s = pdata.stencil_num[var];
                for (i = 0; i < data.stencil_sizes[s]; i++) {
                    for (box = 0; box < pdata.nboxes; box++) {
                        ...
                        GetVariableBox(pdata.ilowers[box], pdata.iuppers[box], pdata.vartypes[var], ilower,
                                       iupper);
                        HYPRE_StructMatrixGetBoxValues(sA, ilower, iupper, 1, &i, box_values);

                        HYPRE_SStructMatrixSetBoxValues(A_amg, part, ilower, iupper, var, 1, &i, box_values);
    ...
    for (j = 0; j < data.max_boxsize; j++) { values[j] = 1.0; }
    for (part = 0; part < data.nparts; part++) {
        pdata = data.pdata[part];
        for (var = 0; var < pdata.nvars; var++) {
            for (box = 0; box < pdata.nboxes; box++) {
    ...
    if (((solver_id >= 20) && (solver_id <= 30)) || ((solver_id >= 40) && (solver_id < 60))) {
        for (part = 0; part < data.nparts; part++) {
            pdata = data.pdata[part];
            for (var = 0; var < pdata.nvars; var++) {
                sx = hypre_SStructPVectorSVector(hypre_SStructVectorPVector(b, part), var);
                for (box = 0; box < pdata.nboxes; box++) {
                    box_values = hypre_TAlloc(HYPRE_Real, data.max_boxsize, HYPRE_MEMORY_HOST);
                    GetVariableBox(pdata.ilowers[box], pdata.iuppers[box], var, ilower, iupper);
                    HYPRE_StructVectorGetBoxValues(sx, ilower, iupper, box_values);
                    HYPRE_SStructVectorSetBoxValues(b_amg, part, ilower, iupper, var, values);
                    hypre_TFree(box_values, HYPRE_MEMORY_HOST);
                }
            }
        }
    ...
    if (((solver_id >= 20) && (solver_id <= 30)) || ((solver_id >= 40) && (solver_id < 60))) {
        for (part = 0; part < data.nparts; part++) {
            pdata = data.pdata[part];
            for (var = 0; var < pdata.nvars; var++) {
                sx = hypre_SStructPVectorSVector(hypre_SStructVectorPVector(x, part), var);
                for (box = 0; box < pdata.nboxes; box++) {
                    box_values = hypre_TAlloc(HYPRE_Real, data.max_boxsize, HYPRE_MEMORY_HOST);
                    GetVariableBox(pdata.ilowers[box], pdata.iuppers[box], var, ilower, iupper);
                    HYPRE_StructVectorGetBoxValues(sx, ilower, iupper, box_values);
                    HYPRE_SStructVectorSetBoxValues(x_amg, part, ilower, iupper, var, values);
                    hypre_TFree(box_values, HYPRE_MEMORY_HOST);
                }
            }
        }
        ...
        for (part = 0; part < data.nparts; part++) {
            ...
            for (var = 0; var < pdata.nvars; var++) {
                for (box = 0; box < pdata.nboxes; box++) {
                    ...
                    hypre_fprintf(file, "\nPart %d, var %d, box %d:\n", part, var, box);
                    for (i = 0; i < pdata.boxsizes[box]; i++) { hypre_fprintf(file, "%e\n", values[i]); }
                }
            }
        }
        fclose(file);
    ...
    if ((solver_id >= 20) && (solver_id < 30)) {
        ...
        if (solver_id == 20) { HYPRE_BoomerAMGDestroy(par_precond); }
    }
    ...
    if ((solver_id >= 40) && (solver_id < 50)) {
        ...
        if (solver_id == 40) {
            ...
            HYPRE_BoomerAMGSetPrintFileName(par_precond, "sstruct.out.log");
            HYPRE_BoomerAMGSetMaxIter(par_precond, 1);
            HYPRE_GMRESSetPrecond(par_solver, (HYPRE_PtrToSolverFcn) HYPRE_BoomerAMGSolve,
                                  (HYPRE_PtrToSolverFcn) HYPRE_BoomerAMGSetup, par_precond);
        }
    ...
    for (s = 0; s < data.nstencils; s++) { HYPRE_SStructStencilDestroy(stencils[s]); }
    ...
    hypre_TFree(plevels, HYPRE_MEMORY_HOST);
    hypre_TFree(prefinements, HYPRE_MEMORY_HOST);
