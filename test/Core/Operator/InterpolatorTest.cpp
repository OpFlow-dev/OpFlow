...
protected:
    void SetUp() override {
        m = MeshBuilder<Mesh>().newMesh(10, 10)
        .setMeshOfDim(0, 0., 1.).setMeshOfDim(1, 0., 1.)
        .build();
        u = ExprBuilder<Field>().setMesh(m)
...
TEST_F(Intp1DCenterToCornerTest, NeumRangeCheck) {
    auto t = d1IntpCenterToCorner<1>(u); t.prepare();
...
TEST_F(Intp1DCenterToCornerTest, ProductRangeCheck) {
    auto t = d1IntpCenterToCorner<1>(u) * d1IntpCenterToCorner<0>(v); t.prepare();
