...
TEST_F(Intp1DCenterToCornerTest, DircCheck) {
    ...
    t.prepare();
    ASSERT_EQ(t.accessibleRange.start[0], m.getRange().start[0] + 1);
    ASSERT_EQ(t.accessibleRange.end[0], m.getRange().end[0] - 1);
    ASSERT_EQ(t.logicalRange.start[0], m.getRange().start[0]);
    ASSERT_EQ(t.logicalRange.end[0], m.getRange().end[0]);
