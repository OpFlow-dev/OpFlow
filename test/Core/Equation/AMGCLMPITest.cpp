...
class AMGCLManualSplitMPITest : public virtual testing::Test {
protected:
    void SetUp() override {
        if (getWorkerCount() != 4) GTEST_SKIP();
        m = MeshBuilder<Mesh>().newMesh(65, 65).setMeshOfDim(0, 0., 1.).setMeshOfDim(1, 0., 1.).build();
        auto s = std::make_shared<ManualSplitStrategy<Field>>();
        s->splitMap.emplace_back(std::array {0, 0}, std::array {16, 64});
        s->splitMap.emplace_back(std::array {48, 0}, std::array {64, 64});
        s->splitMap.emplace_back(std::array {16, 32}, std::array {48, 64});
        s->splitMap.emplace_back(std::array {16, 0}, std::array {48, 32});
        strategy = s;
...
TEST_F(AMGCLManualSplitMPITest, AMGCLUnifiedSolveGMRES) {
    this->reset_case(0.5, 0.5);
    using DBackend = amgcl::backend::builtin<double>;
    using Solver = amgcl::mpi::make_solver<
            amgcl::mpi::amg<DBackend, amgcl::mpi::coarsening::smoothed_aggregation<DBackend>,
    ...
    params.verbose = true;
    Solve<Solver>(poisson_eqn(), p,
                  DS::GeneralBlockedMDRangeMapper<2> {
                          strategy->getSplitMap(p.getMesh().getRange(), getGlobalParallelPlan())},
                  params);
    ASSERT_TRUE(check_solution(2e-8));
}

TEST_F(AMGCLManualSplitMPITest, AMGCLHandlerSolveGMRES) {
    this->reset_case(0.5, 0.5);
    using DBackend = amgcl::backend::builtin<double>;
    using Solver = amgcl::mpi::make_solver<
            amgcl::mpi::amg<DBackend, amgcl::mpi::coarsening::smoothed_aggregation<DBackend>,
    ...
    params.verbose = true;
    auto handler = makeEqnSolveHandler<Solver>(poisson_eqn(), p,
                                               DS::GeneralBlockedMDRangeMapper<2> {strategy->getSplitMap(
                                                       p.getMesh().getRange(), getGlobalParallelPlan())},
                                               params);
    handler->solve();
    ASSERT_TRUE(check_solution(2e-8));
