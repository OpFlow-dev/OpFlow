...
TEST_F(AMGCLTest, TrimmedCSR) {
    ...
    err = std::max({std::abs(4 * x[0] - x[1] - x[3] - 1), std::abs(4 * x[4] - x[1] - x[7] - x[3] - x[5] - 1),
                    std::abs(4 * x[8] - x[5] - x[7] - 1)});
    ASSERT_TRUE(std::abs(err) < 1e-10);
}

TEST_F(AMGCLTest, RepeatCSR) {
    typedef amgcl::backend::builtin<double> SBackend;
#ifdef MIXED_PRECISION
    typedef amgcl::backend::builtin<float> PBackend;
#else
    ...
    val.assign(45, 0.);
    // on macOS the non-trimmed version will cause segfault
    auto trim = [](int i, int b) { return (0 <= i && i < 8) ? i : b; };
    ...
    for (int j = 0; j < 3; ++j) {
        for (int i = 0; i < 3; ++i) {
            ...
            row[rank] = 5 * rank;
            col[5 * rank] = rank;
            col[5 * rank + 1] = trim(rank - 1, rank);
            col[5 * rank + 2] = trim(rank + 1, rank);
            col[5 * rank + 3] = trim(rank - 3, rank);
            col[5 * rank + 4] = trim(rank + 3, rank);
            val[5 * rank] = 4.;
            if (i != 0) val[5 * rank + 1] = -1.;
            if (i != 2) val[5 * rank + 2] = -1.;
            if (j != 0) val[5 * rank + 3] = -1.;
